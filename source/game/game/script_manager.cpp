...
namespace Game {
	...
	ScriptManagerMessage::ScriptManagerMessage(const string & textIn,
		const string & headerIn,
		int factionIndex,
		int teamIndex,
		...
		this->
			teamIndex = teamIndex;
		this->
			messageNotTranslated = messageNotTranslated;
	}

	void
		ScriptManagerMessage::saveGame(XmlNode * rootNode) {
		std::map < string, string > mapTagReplacements;
		XmlNode *
			scriptManagerMessageNode =
			rootNode->addChild("ScriptManagerMessage");

		//string text;
		scriptManagerMessageNode->addAttribute("text", text,
			mapTagReplacements);
		//string header;
		scriptManagerMessageNode->addAttribute("header", header,
			mapTagReplacements);
		scriptManagerMessageNode->addAttribute("factionIndex",
			intToStr(factionIndex),
			mapTagReplacements);
		scriptManagerMessageNode->addAttribute("teamIndex",
			intToStr(teamIndex),
			mapTagReplacements);
		scriptManagerMessageNode->addAttribute("messageNotTranslated",
			intToStr(messageNotTranslated),
			mapTagReplacements);
	}

	void
		ScriptManagerMessage::loadGame(const XmlNode * rootNode) {
		const XmlNode *
			scriptManagerMessageNode = rootNode;

		text = scriptManagerMessageNode->getAttribute("text")->getValue();
		header = scriptManagerMessageNode->getAttribute("header")->getValue();
		factionIndex =
			scriptManagerMessageNode->getAttribute("factionIndex")->
			getIntValue();
		teamIndex =
			scriptManagerMessageNode->getAttribute("teamIndex")->getIntValue();

		messageNotTranslated = true;
		if (scriptManagerMessageNode->hasAttribute("messageNotTranslated") ==
			true) {
			messageNotTranslated =
				(scriptManagerMessageNode->getAttribute("messageNotTranslated")->
					getIntValue() != 0);
		}
	}
	...
	void
		PlayerModifiers::saveGame(XmlNode * rootNode) {
		std::map < string, string > mapTagReplacements;
		XmlNode *
			playerModifiersNode = rootNode->addChild("PlayerModifiers");

		//bool winner;
		playerModifiersNode->addAttribute("winner", intToStr(winner),
			mapTagReplacements);
		//bool aiEnabled;
		playerModifiersNode->addAttribute("aiEnabled", intToStr(aiEnabled),
			mapTagReplacements);
		//bool consumeEnabled;
		playerModifiersNode->addAttribute("consumeEnabled",
			intToStr(consumeEnabled),
			mapTagReplacements);
	}

	void
		PlayerModifiers::loadGame(const XmlNode * rootNode) {
		const XmlNode *
			playerModifiersNode = rootNode;

		winner =
			playerModifiersNode->getAttribute("winner")->getIntValue() != 0;
		aiEnabled =
			playerModifiersNode->getAttribute("aiEnabled")->getIntValue() != 0;
		consumeEnabled =
			playerModifiersNode->getAttribute("consumeEnabled")->
			getIntValue() != 0;
	}
	...
	void
		CellTriggerEvent::saveGame(XmlNode * rootNode) {
		std::map < string, string > mapTagReplacements;
		XmlNode *
			cellTriggerEventNode = rootNode->addChild("CellTriggerEvent");

		//      CellTriggerEventType type;
		cellTriggerEventNode->addAttribute("type", intToStr(type),
			mapTagReplacements);
		//      int sourceId;
		cellTriggerEventNode->addAttribute("sourceId", intToStr(sourceId),
			mapTagReplacements);
		//      int destId;
		cellTriggerEventNode->addAttribute("destId", intToStr(destId),
			mapTagReplacements);
		//      Vec2i destPos;
		cellTriggerEventNode->addAttribute("destPos", destPos.getString(),
			mapTagReplacements);
		//      int triggerCount;
		cellTriggerEventNode->addAttribute("triggerCount",
			intToStr(triggerCount),
			mapTagReplacements);

		//      Vec2i destPosEnd;
		cellTriggerEventNode->addAttribute("destPosEnd",
			destPosEnd.getString(),
			mapTagReplacements);
	}

	void
		CellTriggerEvent::loadGame(const XmlNode * rootNode) {
		const XmlNode *
			cellTriggerEventNode = rootNode->getChild("CellTriggerEvent");

		type =
			static_cast <CellTriggerEventType>
			(cellTriggerEventNode->getAttribute("type")->getIntValue());
		sourceId =
			cellTriggerEventNode->getAttribute("sourceId")->getIntValue();
		destId = cellTriggerEventNode->getAttribute("destId")->getIntValue();
		destPos =
			Vec2i::strToVec2(cellTriggerEventNode->getAttribute("destPos")->
				getValue());
		triggerCount =
			cellTriggerEventNode->getAttribute("triggerCount")->getIntValue();

		if (cellTriggerEventNode->hasAttribute("destPosEnd") == true) {
			destPosEnd =
				Vec2i::strToVec2(cellTriggerEventNode->
					getAttribute("destPosEnd")->getValue());
		}
	}
	...
	void
		TimerTriggerEvent::saveGame(XmlNode * rootNode) {
		std::map < string, string > mapTagReplacements;
		XmlNode *
			timerTriggerEventNode = rootNode->addChild("TimerTriggerEvent");

		//      bool running;
		timerTriggerEventNode->addAttribute("running", intToStr(running),
			mapTagReplacements);
		//      //time_t startTime;
		//      //time_t endTime;
		//      int startFrame;
		timerTriggerEventNode->addAttribute("startFrame",
			intToStr(startFrame),
			mapTagReplacements);
		//      int endFrame;
		timerTriggerEventNode->addAttribute("endFrame", intToStr(endFrame),
			mapTagReplacements);

		if (triggerSecondsElapsed > 0) {
			timerTriggerEventNode->addAttribute("triggerSecondsElapsed",
				intToStr
	...
	void
		TimerTriggerEvent::loadGame(const XmlNode * rootNode) {
		const XmlNode *
			timerTriggerEventNode = rootNode->getChild("TimerTriggerEvent");

		running =
			timerTriggerEventNode->getAttribute("running")->getIntValue() != 0;
		startFrame =
			timerTriggerEventNode->getAttribute("startFrame")->getIntValue();
		endFrame =
			timerTriggerEventNode->getAttribute("endFrame")->getIntValue();
		if (timerTriggerEventNode->hasAttribute("triggerSecondsElapsed") ==
			true) {
			triggerSecondsElapsed =
				timerTriggerEventNode->getAttribute("triggerSecondsElapsed")->
				getIntValue();
		}
	}
	...
	// =====================================================
	ScriptManager *
		ScriptManager::thisScriptManager = NULL;
	const int
		ScriptManager::messageWrapCount = 35;
	const int
		ScriptManager::displayTextWrapCount = 64;
	...
	void
		ScriptManager::init(World * world, GameCamera * gameCamera,
		...
		//register functions
		luaScript.registerFunction(networkShowMessageForFaction,
			"networkShowMessageForFaction");
		luaScript.registerFunction(networkShowMessageForTeam,
			"networkShowMessageForTeam");

		luaScript.registerFunction(networkSetCameraPositionForFaction,
			"networkSetCameraPositionForFaction");
		luaScript.registerFunction(networkSetCameraPositionForTeam,
			"networkSetCameraPositionForTeam");
		...
		luaScript.registerFunction(DisplayFormattedText,
			"displayFormattedText");
		luaScript.registerFunction(DisplayFormattedLangText,
		...
		luaScript.registerFunction(giveProductionCommand,
			"giveProductionCommand");
		...
		luaScript.registerFunction(giveAttackStoppedCommand,
			"giveAttackStoppedCommand");
		...
		luaScript.registerFunction(startPerformanceTimer,
			"startPerformanceTimer");
		luaScript.registerFunction(endPerformanceTimer, "endPerformanceTimer");
		luaScript.registerFunction(getPerformanceTimerResults,
			"getPerformanceTimerResults");

		luaScript.registerFunction(registerCellTriggerEventForUnitToUnit,
			"registerCellTriggerEventForUnitToUnit");
		luaScript.registerFunction(registerCellTriggerEventForUnitToLocation,
			"registerCellTriggerEventForUnitToLocation");
		luaScript.registerFunction(registerCellTriggerEventForFactionToUnit,
			"registerCellTriggerEventForFactionToUnit");
		luaScript.
			registerFunction(registerCellTriggerEventForFactionToLocation,
				"registerCellTriggerEventForFactionToLocation");

		luaScript.
			registerFunction(registerCellAreaTriggerEventForUnitToLocation,
				"registerCellAreaTriggerEventForUnitToLocation");
		luaScript.
			registerFunction(registerCellAreaTriggerEventForFactionToLocation,
				"registerCellAreaTriggerEventForFactionToLocation");
		luaScript.registerFunction(registerCellAreaTriggerEvent,
			"registerCellAreaTriggerEvent");

		luaScript.registerFunction(getCellTriggerEventCount,
			"getCellTriggerEventCount");
		luaScript.registerFunction(unregisterCellTriggerEvent,
			"unregisterCellTriggerEvent");
		luaScript.registerFunction(startTimerEvent, "startTimerEvent");
		luaScript.registerFunction(startEfficientTimerEvent,
			"startEfficientTimerEvent");
		luaScript.registerFunction(resetTimerEvent, "resetTimerEvent");
		luaScript.registerFunction(stopTimerEvent, "stopTimerEvent");
		luaScript.registerFunction(getTimerEventSecondsElapsed,
			"timerEventSecondsElapsed");
		luaScript.registerFunction(getCellTriggeredEventId,
			"triggeredCellEventId");
		luaScript.registerFunction(getTimerTriggeredEventId,
			"triggeredTimerEventId");

		luaScript.registerFunction(getCellTriggeredEventAreaEntryUnitId,
			"triggeredEventAreaEntryUnitId");
		luaScript.registerFunction(getCellTriggeredEventAreaExitUnitId,
			"triggeredEventAreaExitUnitId");

		luaScript.registerFunction(getCellTriggeredEventUnitId,
		...
		luaScript.registerFunction(getLastDeadUnitCauseOfDeath,
			"lastDeadUnitCauseOfDeath");
		luaScript.registerFunction(getLastDeadUnitKillerName,
			"lastDeadUnitKillerName");
		luaScript.registerFunction(getLastDeadUnitKillerId,
			"lastDeadUnitKiller");

		luaScript.registerFunction(getLastAttackedUnitName,
			"lastAttackedUnitName");
		luaScript.registerFunction(getLastAttackedUnitId, "lastAttackedUnit");

		luaScript.registerFunction(getLastAttackingUnitName,
			"lastAttackingUnitName");
		luaScript.registerFunction(getLastAttackingUnitId,
			"lastAttackingUnit");
		...
		luaScript.registerFunction(addUnitToGroupSelection,
			"addUnitToGroupSelection");
		luaScript.registerFunction(recallGroupSelection,
		...
		luaScript.registerFunction(registerDayNightEvent,
			"registerDayNightEvent");
		luaScript.registerFunction(unregisterDayNightEvent,
			"unregisterDayNightEvent");

		luaScript.registerFunction(registerUnitTriggerEvent,
			"registerUnitTriggerEvent");
		luaScript.registerFunction(unregisterUnitTriggerEvent,
		...
		luaScript.registerFunction(getSpeedChangeEnabled,
		...
		try {
			...
			if (this->rootNode == NULL) {
				...
				if (LuaScript::getDebugModeEnabled() == true)
					printf("Calling onLoad luaSavedGameData.size() = %d\n",
	...
	void
		ScriptManager::onMessageBoxOk(bool popFront) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		for (int index = 0; messageQueue.empty() == false; ++index) {
			...
			if (messageQueue.empty() == false) {
				...
				if ((messageQueue.front().getFactionIndex() < 0
					&& messageQueue.front().getTeamIndex() < 0)
					|| messageQueue.front().getFactionIndex() ==
					this->world->getThisFactionIndex()
					|| messageQueue.front().getTeamIndex() ==
					this->world->getThisTeamIndex()) {

					messageBox.setEnabled(true);

					string
						msgText = messageQueue.front().getText();
					string
						msgHeader = messageQueue.front().getHeader();
					if (messageQueue.front().getMessageNotTranslated() ==
						false) {

						msgText =
							lang.getScenarioString(messageQueue.front().
								getText());
						msgHeader =
							lang.getScenarioString(messageQueue.front().
								getHeader());
	...
	void
		ScriptManager::onUnitCreated(const Unit * unit) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		if (this->rootNode == NULL) {
			...
			luaScript.endCall();
			luaScript.beginCall("unitCreatedOfType_" +
	...
	void
		ScriptManager::onUnitDied(const Unit * unit) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		if (this->rootNode == NULL) {
			if (unit->getLastAttackerUnitId() >= 0) {
				Unit *
					killer = world->findUnitById(unit->getLastAttackerUnitId());
	...
	void
		ScriptManager::onUnitAttacked(const Unit * unit) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::onUnitAttacking(const Unit * unit) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::onGameOver(bool won) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] TimerTriggerEventList.size() = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				TimerTriggerEventList.size());

		for (std::map < int, TimerTriggerEvent >::iterator iterMap =
			TimerTriggerEventList.begin();
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).
				enabled)
				SystemFlags::OutputDebug(SystemFlags::debugLUA,
					"In [%s::%s Line: %d] event.running = %d, event.startTime = %lld, event.endTime = %lld, diff = %f\n",
					__FILE__, __FUNCTION__, __LINE__,
					event.running,
					(long long int) event.startFrame,
					(long long int) event.endFrame,
			...
			if (event.running == true) {
				...
				if (event.triggerSecondsElapsed > 0) {
					int
						elapsed =
						(world->getFrameCount() -
							event.startFrame) / GameConstants::updateFps;
	...
	void
		ScriptManager::onCellTriggerEvent(Unit * movingUnit) {
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] movingUnit = %p, CellTriggerEventList.size() = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				movingUnit, CellTriggerEventList.size());
		...
		if (movingUnit != NULL) {
			...
			for (std::map < int, CellTriggerEvent >::iterator iterMap =
				CellTriggerEventList.begin();
				...
				if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).
					...
					SystemFlags::OutputDebug(SystemFlags::debugLUA,
						...
						__FILE__, __FUNCTION__, __LINE__,
						movingUnit->getId(), event.type,
						movingUnit->getPos().getString().
						c_str(), event.sourceId,
						event.destId,
						event.destPos.getString().
						c_str());
				...
				switch (event.type) {
					...
					{
						Unit *
							destUnit = world->findUnitById(event.destId);
						if (destUnit != NULL) {
							if (movingUnit->getId() == event.sourceId) {
								bool
									srcInDst =
									world->getMap()->isInUnitTypeCells(destUnit->
										getType(),
										destUnit->
										getPos(),
										movingUnit->
										getPos());
								if (SystemFlags::
									getSystemSettingType(SystemFlags::debugLUA).
									enabled)
									SystemFlags::OutputDebug(SystemFlags::debugLUA,
										"In [%s::%s Line: %d] movingUnit = %d, event.type = %d, movingUnit->getPos() = %s, event.sourceId = %d, event.destId = %d, event.destPos = %s, destUnit->getPos() = %s, srcInDst = %d\n",
										__FILE__,
										__FUNCTION__,
										__LINE__,
										movingUnit->getId(),
								...
								if (srcInDst == true) {
									...
									srcInDst =
										world->getMap()->
										isNextToUnitTypeCells(destUnit->getType(),
											destUnit->getPos(),
											movingUnit->
											getPos());
									if (SystemFlags::
					...
					case ctet_UnitPos:
					{
						if (movingUnit->getId() == event.sourceId) {
							bool
								srcInDst =
								world->getMap()->isInUnitTypeCells(movingUnit->
									getType(),
									event.destPos,
									movingUnit->
									getPos());
							if (SystemFlags::
								...
								enabled)
								SystemFlags::OutputDebug(SystemFlags::debugLUA,
									"In [%s::%s Line: %d] movingUnit = %d, event.type = %d, movingUnit->getPos() = %s, event.sourceId = %d, event.destId = %d, event.destPos = %s, srcInDst = %d\n",
					...
					case ctet_UnitAreaPos:
					{
						if (movingUnit->getId() == event.sourceId) {
							...
							std::map < int,
								...
								bool > >::iterator
								iterFind1 =
								event.eventLookupCache.find(movingUnit->
									getType()->
									getSize());
							if (iterFind1 != event.eventLookupCache.end()) {
								std::map < Vec2i, bool >::iterator iterFind2 =
									iterFind1->second.find(movingUnit->getPos());
							...
							if (foundInCache == false) {
								for (int x = event.destPos.x;
									srcInDst == false && x <= event.destPosEnd.x;
									++x) {
									for (int y = event.destPos.y;
										srcInDst == false
										&& y <= event.destPosEnd.y; ++y) {
										srcInDst =
											world->getMap()->
											isInUnitTypeCells(movingUnit->
												getType(), Vec2i(x,
													y),
												movingUnit->
												getPos());
								...
								event.eventLookupCache[movingUnit->getType()->
									getSize()][movingUnit->
									getPos()] =
									srcInDst;
							}
					...
					case ctet_AreaPos:
					{
						...
						if (event.eventStateInfo.find(movingUnit->getId()) ==
							event.eventStateInfo.end()) {
							//printf("ctet_FactionPos event.destPos = [%s], movingUnit->getPos() [%s]\n",event.destPos.getString().c_str(),movingUnit->getPos().getString().c_str());
							...
							for (int x = event.destPos.x;
								...
								++x) {
								for (int y = event.destPos.y;
									srcInDst == false && y <= event.destPosEnd.y;
									++y) {

									srcInDst =
										world->getMap()->
										isInUnitTypeCells(movingUnit->getType(),
											Vec2i(x, y),
											movingUnit->getPos());
									if (srcInDst == true) {
										if (SystemFlags::
										...
										currentCellTriggeredEventAreaEntryUnitId =
											movingUnit->getId();
										event.eventStateInfo[movingUnit->
											getId()] =
											Vec2i(x, y).getString();
									}
								}
	...
	string
		ScriptManager::wrapString(const string & str, int wrapCount) {
		string
			returnString;
	...
	void
		ScriptManager::networkShowMessageForFaction(const string & text,
			const string & header,
			int factionIndex) {
		messageQueue.
			push_back(ScriptManagerMessage(text, header, factionIndex));
		thisScriptManager->onMessageBoxOk(false);
	}
	void
		ScriptManager::networkShowMessageForTeam(const string & text,
			const string & header,
			int teamIndex) {
		// Team indexes are 0 based internally (but 1 based in the lua script) so convert
		teamIndex--;
		messageQueue.
			push_back(ScriptManagerMessage(text, header, -1, teamIndex));
		thisScriptManager->onMessageBoxOk(false);
	}

	void
		ScriptManager::networkSetCameraPositionForFaction(int factionIndex,
			const Vec2i & pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	void
		ScriptManager::networkSetCameraPositionForTeam(int teamIndex,
			const Vec2i & pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	void
		ScriptManager::showMessage(const string & text, const string & header) {
		messageQueue.push_back(ScriptManagerMessage(text, header));
		thisScriptManager->onMessageBoxOk(false);
	}

	void
		ScriptManager::clearDisplayText() {
		displayText = "";
	}

	void
		ScriptManager::setDisplayText(const string & text) {
		displayText =
			wrapString(Lang::getInstance().getScenarioString(text),
	...
	void
		...
		const int
			max_debug_buffer_size = 8096;
		char
			szBuf[max_debug_buffer_size] = "";
	...
	void
		ScriptManager::DisplayFormattedText(const char *fmt, ...) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::DisplayFormattedLangText(const char *fmt, ...) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::setCameraPosition(const Vec2i & pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::shakeCamera(int shakeIntensity, int shakeDuration,
		...
		if (cameraDistanceAffected) {
			...
			if (unit) {
				gameCamera->shake(shakeDuration, shakeIntensity,
					cameraDistanceAffected,
					unit->getCurrMidHeightVector());
			}
		} else {
			gameCamera->shake(shakeDuration, shakeIntensity,
				cameraDistanceAffected, Vec3f(0.f, 0.f, 0.f));
		}
	}
	...
	void
		ScriptManager::destroyUnit(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__, unitId);
		Unit *
			unit = world->findUnitById(unitId);
		if (unit != NULL) {
			unit->kill();
	...
	void
		ScriptManager::giveKills(int unitId, int amount) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::playStaticSound(const string & playSound) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playSound [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				playSound.c_str());
	...
	void
		ScriptManager::playStreamingSound(const string & playSound) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playSound [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::stopStreamingSound(const string & playSound) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playSound [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::playStaticVideo(const string & playVideo) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playVideo [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				playVideo.c_str());
	...
	void
		ScriptManager::playStreamingVideo(const string & playVideo) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playVideo [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::stopStreamingVideo(const string & playVideo) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] playVideo [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::togglePauseGame(int pauseStatus) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] pauseStatus = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				pauseStatus);
	...
	void
		ScriptManager::morphToUnit(int unitId, const string & morphName,
			int ignoreRequirements) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d] morphName [%s] forceUpgradesIfRequired = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__, unitId,
				morphName.c_str(), ignoreRequirements);

		world->morphToUnit(unitId, morphName, (ignoreRequirements == 1));
	}

	void
		ScriptManager::moveToUnit(int unitId, int destUnitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d] destUnitId [%d]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__, unitId,
				destUnitId);
	...
	void
		ScriptManager::giveResource(const string & resourceName,
			int factionIndex, int amount) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::givePositionCommand(int unitId,
	...
	void
		ScriptManager::giveAttackCommand(int unitId, int unitToAttackId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::giveProductionCommand(int unitId,
			const string & producedName) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::giveUpgradeCommand(int unitId,
			const string & producedName) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	void
		ScriptManager::giveAttackStoppedCommand(int unitId,
	...
	void
		ScriptManager::disableAi(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::enableAi(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	bool
		ScriptManager::getAiEnabled(int factionIndex) {
	...
	void
		ScriptManager::disableConsume(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::enableConsume(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	bool
		ScriptManager::getConsumeEnabled(int factionIndex) {
	...
	int
		ScriptManager::registerCellTriggerEventForUnitToUnit(int sourceUnitId,
			int destUnitId) {
		CellTriggerEvent
			trigger;
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] Unit: %d will trigger cell event when reaching unit: %d, eventId = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				sourceUnitId, destUnitId, eventId);
	...
	int
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] Unit: %d will trigger cell event when reaching pos: %s, eventId = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				sourceUnitId, pos.getString().c_str(),
				eventId);
	...
	int
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] Faction: %d will trigger cell event when reaching unit: %d, eventId = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				sourceFactionId, destUnitId, eventId);
	...
	int
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]Faction: %d will trigger cell event when reaching pos: %s, eventId = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				sourceFactionId, pos.getString().c_str(),
				eventId);
	...
	int
		ScriptManager::registerCellAreaTriggerEvent(const Vec4i & pos) {
		CellTriggerEvent
			trigger;
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] trigger cell event when reaching pos: %s, eventId = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				pos.getString().c_str(), eventId);

		return eventId;
	}

	int
		ScriptManager::getCellTriggerEventCount(int eventId) {
		int
			result = 0;
	...
	void
		ScriptManager::unregisterCellTriggerEvent(int eventId) {
		if (CellTriggerEventList.find(eventId) != CellTriggerEventList.end()) {
		...
		if (inCellTriggerEvent == false) {
			if (unRegisterCellTriggerEventList.empty() == false) {
				for (int i = 0;
					i < (int) unRegisterCellTriggerEventList.size(); ++i) {
					int
						delayedEventId = unRegisterCellTriggerEventList[i];
	...
	int
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] TimerTriggerEventList.size() = %d, eventId = %d, trigger.startTime = %lld, trigger.endTime = %lld\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				TimerTriggerEventList.size(), eventId,
				(long long int) trigger.startFrame,
				(long long int) trigger.endFrame);

		return eventId;
	}

	int
		ScriptManager::startEfficientTimerEvent(int triggerSecondsElapsed) {
		TimerTriggerEvent
			trigger;
		trigger.running = true;
		//trigger.startTime = time(NULL);
	...
	int
		ScriptManager::resetTimerEvent(int eventId) {
		int
			result = 0;
		if (TimerTriggerEventList.find(eventId) !=
			TimerTriggerEventList.end()) {
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).
				enabled)
				SystemFlags::OutputDebug(SystemFlags::debugLUA,
					"In [%s::%s Line: %d] TimerTriggerEventList.size() = %d, eventId = %d, trigger.startTime = %lld, trigger.endTime = %lld, result = %d\n",
					extractFileFromDirectoryPath(__FILE__).
					c_str(), __FUNCTION__, __LINE__,
					TimerTriggerEventList.size(), eventId,
					(long long int) trigger.startFrame,
	...
	int
		ScriptManager::stopTimerEvent(int eventId) {
		int
			result = 0;
		if (TimerTriggerEventList.find(eventId) !=
			TimerTriggerEventList.end()) {
	...
	int
		ScriptManager::getTimerEventSecondsElapsed(int eventId) {
		int
			result = 0;
		if (TimerTriggerEventList.find(eventId) !=
			TimerTriggerEventList.end()) {
			...
			if (trigger.running) {
				...
				result =
					(world->getFrameCount() -
						trigger.startFrame) / GameConstants::updateFps;
			} else {
	...
	void
		ScriptManager::setPlayerAsWinner(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::startPerformanceTimer() {
	...
	void
		ScriptManager::endPerformanceTimer() {
	...
	Vec2i
		ScriptManager::getPerformanceTimerResults() {
	...
	Vec2i
		ScriptManager::getStartLocation(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	Vec2i
		...
		Vec2i
			result = world->getUnitPosition(unitId);
	...
	void
		ScriptManager::setUnitPosition(int unitId, Vec2i pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::addCellMarker(Vec2i pos, int factionIndex,
			const string & note,
	...
	void
		ScriptManager::removeCellMarker(Vec2i pos, int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getIsUnitAlive(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getUnitFaction(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const string
		ScriptManager::getUnitName(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const string
		ScriptManager::getUnitDisplayName(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getResourceAmount(const string & resourceName,
			int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	const
		string &
		ScriptManager::getLastCreatedUnitName() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getCellTriggeredEventId() {

		return currentCellTriggeredEventId;
	}

	int
		ScriptManager::getTimerTriggeredEventId() {

		return currentTimerTriggeredEventId;
	}

	int
		ScriptManager::getCellTriggeredEventAreaEntryUnitId() {

		return currentCellTriggeredEventAreaEntryUnitId;
	}

	int
		ScriptManager::getCellTriggeredEventAreaExitUnitId() {

		return currentCellTriggeredEventAreaExitUnitId;
	}

	int
		ScriptManager::getCellTriggeredEventUnitId() {

		return currentCellTriggeredEventUnitId;
	}

	void
		ScriptManager::setRandomGenInit(int seed) {

		random.init(seed);
	}

	int
		ScriptManager::getRandomGen(int minVal, int maxVal) {

		return random.randRange(minVal, maxVal);
	}

	int
		ScriptManager::getWorldFrameCount() {

		return world->getFrameCount();
	}

	bool
		ScriptManager::getGameWon() const {
	...
	bool
		ScriptManager::getIsGameOver() const {
	...
	int
		ScriptManager::getLastCreatedUnitId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const
		string &
		ScriptManager::getLastDeadUnitName() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getLastDeadUnitId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getLastDeadUnitCauseOfDeath() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const
		string &
		ScriptManager::getLastDeadUnitKillerName() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getLastDeadUnitKillerId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const
		string &
		ScriptManager::getLastAttackedUnitName() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getLastAttackedUnitId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	const
		string &
		ScriptManager::getLastAttackingUnitName() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getLastAttackingUnitId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getUnitCount(int factionIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::getUnitCountOfType(int factionIndex,
	...
	const string
		ScriptManager::getSystemMacroValue(const string & key) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::loadScenario(const string & name, bool keepFactions) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	vector < int >
		ScriptManager::getUnitsForFaction(int factionIndex,
			const string & commandTypeName,
			int field) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	int
		ScriptManager::getUnitCurrentField(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::isFreeCellsOrHasUnit(int field, int unitId, Vec2i pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		int
			result =
			world->getMap()->isFreeCellsOrHasUnit(pos,
				unit->getType()->getSize(),
				static_cast <Field> (field),
				unit);

		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s] unitId = %d, [%s] pos [%s] field = %d result = %d\n",
				__FUNCTION__, unitId,
				unit->getType()->getName(false).c_str(),
				pos.getString().c_str(), field, result);

		return result;
	}

	int
		ScriptManager::isFreeCells(int unitSize, int field, Vec2i pos) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		int
			result =
			world->getMap()->isFreeCellsOrHasUnit(pos, unitSize,
				static_cast <Field> (field),
				NULL);

		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s] unitSize = %d, pos [%s] field = %d result = %d\n",
				__FUNCTION__, unitSize,
				pos.getString().c_str(), field, result);
	...
	int
		ScriptManager::getHumanFactionId() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::highlightUnit(int unitId, float radius, float thickness,
			Vec4f color) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
	...
	void
		ScriptManager::unhighlightUnit(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::giveStopCommand(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	bool
		ScriptManager::selectUnit(int unitId) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::addUnitToGroupSelection(int unitId, int groupIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::recallGroupSelection(int groupIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::removeUnitFromGroupSelection(int unitId, int groupIndex) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	bool
		ScriptManager::getAttackWarningsEnabled() {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	void
		ScriptManager::registerUnitTriggerEvent(int unitId) {
		UnitTriggerEventList[unitId] = utet_None;
	}

	void
		ScriptManager::unregisterUnitTriggerEvent(int unitId) {
		UnitTriggerEventList.erase(unitId);
	}

	int
		ScriptManager::getLastUnitTriggerEventUnitId() {
		return lastUnitTriggerEventUnitId;
	}
	UnitTriggerEventType
		ScriptManager::getLastUnitTriggerEventType() {
		return lastUnitTriggerEventType;
	}

	int
		ScriptManager::getUnitProperty(int unitId, UnitTriggerEventType type) {
		int
			result = -1;
	...
	void
		ScriptManager::onUnitTriggerEvent(const Unit * unit,
			UnitTriggerEventType event) {
		...
		if (UnitTriggerEventList.empty() == false) {
			//printf("File: %s line: %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__);
			std::map < int,
				UnitTriggerEventType >::iterator
				iterFind = UnitTriggerEventList.find(unit->getId());
	...
	void
		ScriptManager::registerDayNightEvent() {
		registeredDayNightEvent = true;
	}

	void
		ScriptManager::unregisterDayNightEvent() {
		registeredDayNightEvent = false;
	}
	...
	void
		ScriptManager::addMessageToQueue(ScriptManagerMessage msg) {
		messageQueue.push_back(msg);
	}

	void
		ScriptManager::storeSaveGameData(string name, string value) {

		if (LuaScript::getDebugModeEnabled() == true)
			printf("storeSaveGameData name [%s] value [%s]\n", name.c_str(),
				value.c_str());
	...
	string
		ScriptManager::loadSaveGameData(string name) {
		string
			value = luaSavedGameData[name];

		if (LuaScript::getDebugModeEnabled() == true)
			printf("loadSaveGameData result name [%s] value [%s]\n",
	...
	int
		ScriptManager::showMessage(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		thisScriptManager->showMessage(luaArguments.getString(-2),
			luaArguments.getString(-1));
		return luaArguments.getReturnCount();
	}

	int
		ScriptManager::networkShowMessageForFaction(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			thisScriptManager->networkShowMessageForFaction(luaArguments.
				getString(-3),
				luaArguments.
				getString(-2),
				luaArguments.
	...
	int
		ScriptManager::networkShowMessageForTeam(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			thisScriptManager->networkShowMessageForTeam(luaArguments.
				getString(-3),
				luaArguments.
				getString(-2),
				luaArguments.
	...
	int
		ScriptManager::networkSetCameraPositionForFaction(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			thisScriptManager->networkSetCameraPositionForFaction(luaArguments.
				getInt(-2),
				luaArguments.
				getVec2i(-1));
		} catch (const game_runtime_error & ex) {
	...
	int
		ScriptManager::networkSetCameraPositionForTeam(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			thisScriptManager->networkSetCameraPositionForTeam(luaArguments.
				getInt(-2),
				luaArguments.
				getVec2i(-1));
		} catch (const game_runtime_error & ex) {
	...
	int
		ScriptManager::setDisplayText(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::addConsoleText(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::clearDisplayText(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::setCameraPosition(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::shakeCamera(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::shakeCameraOnUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::createUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				...
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				luaArguments.getString(-3).c_str(),
	...
	void
		...
		snprintf(szErrBuf, 8096, "in %s::%s %d ",
			extractFileFromDirectoryPath(file).c_str(), function, line);
		snprintf(szErrBuf2, 8096, "Lua: tag=<%s> line=%d ", luaSource,
			luaLine - 1);
		string
		...
		thisScriptManager->
			...
			(sErrBuf.c_str(),
				"error " +
				intToStr(thisScriptManager->errorCount), -1, -1,
				true));
	...
	int
		ScriptManager::createUnitNoSpacing(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::destroyUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::setLockedUnitForFaction(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::giveKills(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::morphToUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d] morphName [%s] forceUpgrade = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				luaArguments.getInt(-3),
				luaArguments.getString(-2).c_str(),
				luaArguments.getInt(-1));
	...
	int
		ScriptManager::moveToUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] unit [%d] dest unit [%d]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				luaArguments.getInt(-2),
				luaArguments.getInt(-1));
	...
	int
		ScriptManager::playStaticSound(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d] sound [%s]\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, __LINE__,
				luaArguments.getString(-1).c_str());
	...
	int
		ScriptManager::playStreamingSound(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::stopStreamingSound(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::stopAllSound(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::playStaticVideo(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::playStreamingVideo(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::stopStreamingVideo(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::stopAllVideo(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
	...
	int
		ScriptManager::togglePauseGame(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
	...
	int
		ScriptManager::giveResource(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::givePositionCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::giveAttackCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::giveProductionCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::giveUpgradeCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::giveAttackStoppedCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			thisScriptManager->giveAttackStoppedCommand(luaArguments.getInt(-3),
				luaArguments.
				getString(-2),
				luaArguments.
				getInt(-1));
	...
	int
		ScriptManager::disableAi(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::enableAi(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getAiEnabled(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			bool
	...
	int
		ScriptManager::disableConsume(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::enableConsume(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getConsumeEnabled(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			bool
	...
	int
		ScriptManager::registerCellTriggerEventForUnitToUnit(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::registerCellTriggerEventForUnitToLocation(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
				result =
				thisScriptManager->
				registerCellTriggerEventForUnitToLocation(luaArguments.getInt(-2),
					luaArguments.
					getVec2i(-1));
			luaArguments.returnInt(result);
		} catch (const game_runtime_error & ex) {
	...
	int
		ScriptManager::registerCellAreaTriggerEventForUnitToLocation(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
				result =
				thisScriptManager->
				registerCellAreaTriggerEventForUnitToLocation(luaArguments.
					getInt(-2),
					luaArguments.
					getVec4i(-1));
			luaArguments.returnInt(result);
	...
	int
		ScriptManager::registerCellTriggerEventForFactionToUnit(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::registerCellTriggerEventForFactionToLocation(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
				result =
				thisScriptManager->
				registerCellTriggerEventForFactionToLocation(luaArguments.
					getInt(-2),
					luaArguments.
					getVec2i(-1));
			luaArguments.returnInt(result);
	...
	int
		ScriptManager::
		registerCellAreaTriggerEventForFactionToLocation(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
				result =
				thisScriptManager->
				registerCellAreaTriggerEventForFactionToLocation(luaArguments.
					getInt(-2),
					luaArguments.
					getVec4i(-1));
			luaArguments.returnInt(result);
	...
	int
		ScriptManager::registerCellAreaTriggerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::getCellTriggerEventCount(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::unregisterCellTriggerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::startTimerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::startEfficientTimerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::stopTimerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::resetTimerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::getTimerEventSecondsElapsed(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::setPlayerAsWinner(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::endGame(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::startPerformanceTimer(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::endPerformanceTimer(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getPerformanceTimerResults(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			Vec2i
	...
	int
		ScriptManager::getStartLocation(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			Vec2i
	...
	int
		ScriptManager::getUnitPosition(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			Vec2i
	...
	int
		ScriptManager::setUnitPosition(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		...
		try {
			...
			int
				factionIndex = luaArguments.getInt(-4);
			...
			string
				note = luaArguments.getString(-3);

			//printf("LUA addCellMarker --> START 3\n");

			string
				texture = luaArguments.getString(-2);
	...
	int
		ScriptManager::removeCellMarker(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			int
				factionIndex = luaArguments.getInt(-2);
			Vec2i
				pos = luaArguments.getVec2i(-1);
	...
	int
		ScriptManager::showMarker(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			int
				flashCount = luaArguments.getInt(-5);
			//printf("LUA addCellMarker --> START\n");
	...
	int
		ScriptManager::getUnitFaction(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
	...
	int
		ScriptManager::getUnitName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			const string
	...
	int
		ScriptManager::getUnitDisplayName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			const string
	...
	int
		ScriptManager::getResourceAmount(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastCreatedUnitName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastCreatedUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getCellTriggeredEventId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getTimerTriggeredEventId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getCellTriggeredEventAreaEntryUnitId(LuaHandle *
			luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getCellTriggeredEventAreaExitUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getCellTriggeredEventUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::setRandomGenInit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getRandomGen(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getWorldFrameCount(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastDeadUnitName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastDeadUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastDeadUnitCauseOfDeath(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getLastDeadUnitKillerName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::getLastDeadUnitKillerId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getLastAttackedUnitName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::getLastAttackedUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastAttackingUnitName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::getLastAttackingUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getSystemMacroValue(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::scenarioDir(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::getPlayerName(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	int
		ScriptManager::getUnitCount(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getUnitCountOfType(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::DisplayFormattedText(LuaHandle * luaHandle) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		try {
			...
			int
				args = lua_gettop(luaHandle);
			if (lua_checkstack(luaHandle, args + 1)) {
				LuaArguments
					luaArguments(luaHandle);
				string
					fmt = luaArguments.getString(-args);
				//va_list argList;
	...
	int
		ScriptManager::addConsoleLangText(LuaHandle * luaHandle) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		try {
			...
			if (lua_checkstack(luaHandle, args + 1)) {
				...
				const int
					max_args_allowed = 8;
				if (args == 1) {
					thisScriptManager->addConsoleLangText(Lang::getInstance().
						getScenarioString
						(fmt).c_str());
				} else if (args == 2) {
					thisScriptManager->addConsoleLangText(Lang::getInstance().
	...
	int
		ScriptManager::DisplayFormattedLangText(LuaHandle * luaHandle) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
		...
		try {
			...
			if (lua_checkstack(luaHandle, args + 1)) {
				...
				if (args == 1) {
					thisScriptManager->
						DisplayFormattedLangText(Lang::getInstance().
							getScenarioString(fmt).c_str());
				} else if (args == 2) {
					thisScriptManager->
						DisplayFormattedLangText(Lang::getInstance().
							getScenarioString(fmt).c_str(),
							luaArguments.
							getGenericData(-args + 1));
				} else if (args == 3) {
					thisScriptManager->
						DisplayFormattedLangText(Lang::getInstance().
							getScenarioString(fmt).c_str(),
	...
	int
		ScriptManager::getGameWon(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getIsGameOver(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::loadScenario(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getUnitsForFaction(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			vector < int >
				units =
				thisScriptManager->getUnitsForFaction(luaArguments.getInt(-3),
					luaArguments.getString(-2),
					luaArguments.getInt(-1));
	...
	int
		ScriptManager::getUnitCurrentField(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getIsUnitAlive(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::isFreeCellsOrHasUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		...
		try {
			int
				result =
				thisScriptManager->isFreeCellsOrHasUnit(luaArguments.getInt(-3),
					luaArguments.getInt(-2),
					luaArguments.
					getVec2i(-1));
	...
	int
		ScriptManager::isFreeCells(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);

		try {
			int
				result = thisScriptManager->isFreeCells(luaArguments.getInt(-3),
					luaArguments.getInt(-2),
					luaArguments.
					getVec2i(-1));
	...
	int
		ScriptManager::getHumanFactionId(LuaHandle * luaHandle) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugLUA,
				"In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).
	...
	int
		ScriptManager::highlightUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			thisScriptManager->highlightUnit(luaArguments.getInt(-4),
	...
	int
		ScriptManager::unhighlightUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::giveStopCommand(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::unselectUnit(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::addUnitToGroupSelection(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::recallGroupSelection(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::removeUnitFromGroupSelection(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			thisScriptManager->removeUnitFromGroupSelection(luaArguments.
				getInt(-2),
				luaArguments.
				getInt(-1));
		} catch (const game_runtime_error & ex) {
	...
	int
		ScriptManager::getAttackWarningsEnabled(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getIsDayTime(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getIsNightTime(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getTimeOfDay(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::registerDayNightEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::unregisterDayNightEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::registerUnitTriggerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::unregisterUnitTriggerEvent(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getLastUnitTriggerEventUnitId(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnInt(thisScriptManager->
	...
	int
		ScriptManager::getLastUnitTriggerEventType(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getUnitProperty(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			int
				value =
				thisScriptManager->getUnitProperty(luaArguments.getInt(-2),
					static_cast <
					UnitTriggerEventType>
					(luaArguments.getInt(-1)));
	...
	int
		...
		try {
			const string
				unitname =
				thisScriptManager->getUnitPropertyName(luaArguments.getInt(-2),
					static_cast <
					UnitTriggerEventType>
					(luaArguments.getInt(-1)));
			luaArguments.returnString(unitname);
	...
	int
		ScriptManager::disableSpeedChange(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::enableSpeedChange(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::getSpeedChangeEnabled(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::storeSaveGameData(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
	...
	int
		ScriptManager::loadSaveGameData(LuaHandle * luaHandle) {
		LuaArguments
			luaArguments(luaHandle);
		try {
			luaArguments.returnString(thisScriptManager->
	...
	void
		ScriptManager::saveGame(XmlNode * rootNode) {
		std::map < string, string > mapTagReplacements;
		XmlNode *
			scriptManagerNode = rootNode->addChild("ScriptManager");
	...
	//      string code;
		...
		if (LuaScript::getDebugModeEnabled() == true)
			printf("After onSave luaSavedGameData.size() = %d\n",
			(int) luaSavedGameData.size());
		for (std::map < string, string >::iterator iterMap =
			luaSavedGameData.begin(); iterMap != luaSavedGameData.end();
			++iterMap) {

			XmlNode *
				savedGameDataItemNode =
				scriptManagerNode->addChild("SavedGameDataItem");

			savedGameDataItemNode->addAttribute("key", iterMap->first,
				mapTagReplacements);
			savedGameDataItemNode->addAttribute("value", iterMap->second,
				mapTagReplacements);
		}
		...
		for (std::list < ScriptManagerMessage >::iterator it =
			messageQueue.begin(); it != messageQueue.end(); ++it) {
	...
	//      GraphicMessageBox messageBox;
		scriptManagerNode->addAttribute("messageBox_enabled",
			intToStr(messageBox.getEnabled()),
			mapTagReplacements);
		scriptManagerNode->addAttribute("messageBox_text",
			messageBox.getText(),
			mapTagReplacements);
		scriptManagerNode->addAttribute("messageBox_header",
			messageBox.getHeader(),
			mapTagReplacements);

		//      string displayText;
		scriptManagerNode->addAttribute("displayText", displayText,
			mapTagReplacements);
		//
		//      //last created unit
		//      string lastCreatedUnitName;
		scriptManagerNode->addAttribute("lastCreatedUnitName",
			lastCreatedUnitName,
			mapTagReplacements);
		//      int lastCreatedUnitId;
		scriptManagerNode->addAttribute("lastCreatedUnitId",
			intToStr(lastCreatedUnitId),
			mapTagReplacements);
		//
		//      //last dead unit
		//      string lastDeadUnitName;
		scriptManagerNode->addAttribute("lastDeadUnitName", lastDeadUnitName,
			mapTagReplacements);
		//      int lastDeadUnitId;
		scriptManagerNode->addAttribute("lastDeadUnitId",
			intToStr(lastDeadUnitId),
			mapTagReplacements);
		//      int lastDeadUnitCauseOfDeath;
		scriptManagerNode->addAttribute("lastDeadUnitCauseOfDeath",
			intToStr(lastDeadUnitCauseOfDeath),
			mapTagReplacements);
		//
		//      //last dead unit's killer
		//      string lastDeadUnitKillerName;
		scriptManagerNode->addAttribute("lastDeadUnitKillerName",
			lastDeadUnitKillerName,
			mapTagReplacements);
		//      int lastDeadUnitKillerId;
		scriptManagerNode->addAttribute("lastDeadUnitKillerId",
			intToStr(lastDeadUnitKillerId),
			mapTagReplacements);
		//
		//      //last attacked unit
		//      string lastAttackedUnitName;
		scriptManagerNode->addAttribute("lastAttackedUnitName",
			lastAttackedUnitName,
			mapTagReplacements);
		//      int lastAttackedUnitId;
		scriptManagerNode->addAttribute("lastAttackedUnitId",
			intToStr(lastAttackedUnitId),
			mapTagReplacements);
		//
		//      //last attacking unit
		//      string lastAttackingUnitName;
		scriptManagerNode->addAttribute("lastAttackingUnitName",
			lastAttackingUnitName,
			mapTagReplacements);
		//      int lastAttackingUnitId;
		scriptManagerNode->addAttribute("lastAttackingUnitId",
			intToStr(lastAttackingUnitId),
			mapTagReplacements);
		//
		//      // end game state
		//      bool gameOver;
		scriptManagerNode->addAttribute("gameOver", intToStr(gameOver),
			mapTagReplacements);
		//      bool gameWon;
		scriptManagerNode->addAttribute("gameWon", intToStr(gameWon),
			mapTagReplacements);
		//      PlayerModifiers playerModifiers[GameConstants::maxPlayers];
		for (unsigned int i = 0; i < (unsigned int) GameConstants::maxPlayers;
			++i) {
			PlayerModifiers & player = playerModifiers[i];
			player.saveGame(scriptManagerNode);
		}
		//      int currentTimerTriggeredEventId;
		scriptManagerNode->addAttribute("currentTimerTriggeredEventId",
			intToStr
			(currentTimerTriggeredEventId),
			mapTagReplacements);
		//      int currentCellTriggeredEventId;
		scriptManagerNode->addAttribute("currentCellTriggeredEventId",
			intToStr(currentCellTriggeredEventId),
			mapTagReplacements);
		//      int currentEventId;
		scriptManagerNode->addAttribute("currentEventId",
			intToStr(currentEventId),
			mapTagReplacements);
		...
		for (std::map < int, CellTriggerEvent >::iterator iterMap =
			...
			iterMap != CellTriggerEventList.end(); ++iterMap) {
			XmlNode *
				cellTriggerEventListNode =
				scriptManagerNode->addChild("CellTriggerEventList");

			cellTriggerEventListNode->addAttribute("key",
				intToStr(iterMap->first),
				mapTagReplacements);
		...
		for (std::map < int, TimerTriggerEvent >::iterator iterMap =
			...
			iterMap != TimerTriggerEventList.end(); ++iterMap) {
			XmlNode *
				timerTriggerEventListNode =
				scriptManagerNode->addChild("TimerTriggerEventList");

			timerTriggerEventListNode->addAttribute("key",
				intToStr(iterMap->first),
				mapTagReplacements);
		...
		scriptManagerNode->addAttribute("inCellTriggerEvent",
			intToStr(inCellTriggerEvent),
			mapTagReplacements);
		//      std::vector<int> unRegisterCellTriggerEventList;
		for (unsigned int i = 0; i < unRegisterCellTriggerEventList.size();
			++i) {
			XmlNode *
				unRegisterCellTriggerEventListNode =
				scriptManagerNode->addChild("unRegisterCellTriggerEventList");
			unRegisterCellTriggerEventListNode->addAttribute("eventId",
				intToStr
		...
		scriptManagerNode->addAttribute("registeredDayNightEvent",
			intToStr(registeredDayNightEvent),
			mapTagReplacements);
		scriptManagerNode->addAttribute("lastDayNightTriggerStatus",
			intToStr(lastDayNightTriggerStatus),
			mapTagReplacements);

		for (std::map < int, UnitTriggerEventType >::iterator iterMap =
			UnitTriggerEventList.begin();
			iterMap != UnitTriggerEventList.end(); ++iterMap) {
			XmlNode *
				unitTriggerEventListNode =
				scriptManagerNode->addChild("UnitTriggerEventList");

			unitTriggerEventListNode->addAttribute("unitId",
				intToStr(iterMap->first),
				mapTagReplacements);
			unitTriggerEventListNode->addAttribute("eventType",
				intToStr(iterMap->second),
				mapTagReplacements);
		}
		scriptManagerNode->addAttribute("lastUnitTriggerEventUnitId",
			intToStr(lastUnitTriggerEventUnitId),
			mapTagReplacements);
		scriptManagerNode->addAttribute("lastUnitTriggerEventType",
			intToStr(lastUnitTriggerEventType),
			mapTagReplacements);
	...
	void
		ScriptManager::loadGame(const XmlNode * rootNode) {
		const XmlNode *
			scriptManagerNode = rootNode->getChild("ScriptManager");

		//      string code;
		code = scriptManagerNode->getAttribute("code")->getValue();
		//      LuaScript luaScript;

		vector < XmlNode * >savedGameDataItemNodeList =
			scriptManagerNode->getChildList("SavedGameDataItem");

		if (LuaScript::getDebugModeEnabled() == true)
			printf("In loadGame savedGameDataItemNodeList.size() = %d\n",
			(int) savedGameDataItemNodeList.size());

		for (unsigned int i = 0; i < savedGameDataItemNodeList.size(); ++i) {
			XmlNode *
				node = savedGameDataItemNodeList[i];
			string
				key = node->getAttribute("key")->getValue();
			string
				value = node->getAttribute("value")->getValue();
		...
		//      MessageQueue messageQueue;
		messageQueue.clear();
		vector < XmlNode * >messageQueueNodeList =
			scriptManagerNode->getChildList("ScriptManagerMessage");
		for (unsigned int i = 0; i < messageQueueNodeList.size(); ++i) {
			XmlNode *
				node = messageQueueNodeList[i];
		...
		//      GraphicMessageBox messageBox;
		messageBox.setEnabled(scriptManagerNode->
			getAttribute("messageBox_enabled")->
			getIntValue() != 0);
		messageBox.setText(scriptManagerNode->
			getAttribute("messageBox_text")->getValue());
		messageBox.setHeader(scriptManagerNode->
			getAttribute("messageBox_header")->getValue());

		//      string displayText;
		displayText =
			scriptManagerNode->getAttribute("displayText")->getValue();
		//
		//      //last created unit
		//      string lastCreatedUnitName;
		lastCreatedUnitName =
			scriptManagerNode->getAttribute("lastCreatedUnitName")->getValue();
		//      int lastCreatedUnitId;
		lastCreatedUnitId =
			scriptManagerNode->getAttribute("lastCreatedUnitId")->getIntValue();
		//
		//      //last dead unit
		//      string lastDeadUnitName;
		lastDeadUnitName =
			scriptManagerNode->getAttribute("lastDeadUnitName")->getValue();
		//      int lastDeadUnitId;
		lastDeadUnitId =
			scriptManagerNode->getAttribute("lastDeadUnitId")->getIntValue();
		//      int lastDeadUnitCauseOfDeath;
		lastDeadUnitCauseOfDeath =
			scriptManagerNode->getAttribute("lastDeadUnitCauseOfDeath")->
			getIntValue();
		//
		//      //last dead unit's killer
		//      string lastDeadUnitKillerName;
		lastDeadUnitKillerName =
			scriptManagerNode->getAttribute("lastDeadUnitKillerName")->
			getValue();
		//      int lastDeadUnitKillerId;
		lastDeadUnitKillerId =
			scriptManagerNode->getAttribute("lastDeadUnitKillerId")->
			getIntValue();
		//
		//      //last attacked unit
		//      string lastAttackedUnitName;
		lastAttackedUnitName =
			scriptManagerNode->getAttribute("lastAttackedUnitName")->getValue();
		//      int lastAttackedUnitId;
		lastAttackedUnitId =
			scriptManagerNode->getAttribute("lastAttackedUnitId")->
			getIntValue();
		//
		//      //last attacking unit
		//      string lastAttackingUnitName;
		lastAttackingUnitName =
			scriptManagerNode->getAttribute("lastAttackingUnitName")->
			getValue();
		//      int lastAttackingUnitId;
		lastAttackingUnitId =
			scriptManagerNode->getAttribute("lastAttackingUnitId")->
			getIntValue();
		//
		//      // end game state
		//      bool gameOver;
		gameOver =
			scriptManagerNode->getAttribute("gameOver")->getIntValue() != 0;
		//      bool gameWon;
		gameWon =
			scriptManagerNode->getAttribute("gameWon")->getIntValue() != 0;
		//      PlayerModifiers playerModifiers[GameConstants::maxPlayers];
		vector < XmlNode * >playerModifiersNodeList =
			scriptManagerNode->getChildList("PlayerModifiers");
		for (unsigned int i = 0; i < playerModifiersNodeList.size(); ++i) {
			XmlNode *
				node = playerModifiersNodeList[i];
			playerModifiers[i].loadGame(node);
		}
		//      int currentTimerTriggeredEventId;
		currentTimerTriggeredEventId =
			scriptManagerNode->getAttribute("currentTimerTriggeredEventId")->
			getIntValue();
		//      int currentCellTriggeredEventId;
		currentCellTriggeredEventId =
			scriptManagerNode->getAttribute("currentCellTriggeredEventId")->
			getIntValue();
		//      int currentEventId;
		currentEventId =
			scriptManagerNode->getAttribute("currentEventId")->getIntValue();
		//      std::map<int,CellTriggerEvent> CellTriggerEventList;
		vector < XmlNode * >cellTriggerEventListNodeList =
			scriptManagerNode->getChildList("CellTriggerEventList");
		for (unsigned int i = 0; i < cellTriggerEventListNodeList.size(); ++i) {
			XmlNode *
				node = cellTriggerEventListNodeList[i];
			CellTriggerEvent
				event;
			event.loadGame(node);
			CellTriggerEventList[node->getAttribute("key")->getIntValue()] =
				event;
		}

		//      std::map<int,TimerTriggerEvent> TimerTriggerEventList;
		vector < XmlNode * >timerTriggerEventListNodeList =
			scriptManagerNode->getChildList("TimerTriggerEventList");
		for (unsigned int i = 0; i < timerTriggerEventListNodeList.size(); ++i) {
			XmlNode *
				node = timerTriggerEventListNodeList[i];

			TimerTriggerEvent
				event;
			event.loadGame(node);
			TimerTriggerEventList[node->getAttribute("key")->getIntValue()] =
				event;
		}

		//      bool inCellTriggerEvent;
		inCellTriggerEvent =
			scriptManagerNode->getAttribute("inCellTriggerEvent")->
			getIntValue() != 0;
		//      std::vector<int> unRegisterCellTriggerEventList;
		vector < XmlNode * >unRegisterCellTriggerEventListNodeList =
			scriptManagerNode->getChildList("unRegisterCellTriggerEventList");
		for (unsigned int i = 0;
			i < unRegisterCellTriggerEventListNodeList.size(); ++i) {
			XmlNode *
				node = unRegisterCellTriggerEventListNodeList[i];
			unRegisterCellTriggerEventList.push_back(node->
		...
		if (scriptManagerNode->hasAttribute("registeredDayNightEvent") == true) {
			registeredDayNightEvent =
				scriptManagerNode->getAttribute("registeredDayNightEvent")->
				getIntValue() != 0;
		}
		if (scriptManagerNode->hasAttribute("lastDayNightTriggerStatus") ==
			true) {
			lastDayNightTriggerStatus =
				scriptManagerNode->getAttribute("lastDayNightTriggerStatus")->
				getIntValue();
		}

		vector < XmlNode * >unitTriggerEventListNodeList =
			scriptManagerNode->getChildList("UnitTriggerEventList");
		for (unsigned int i = 0; i < unitTriggerEventListNodeList.size(); ++i) {
			XmlNode *
				node = unitTriggerEventListNodeList[i];

			UnitTriggerEventType
				eventType = utet_None;
			int
				unitId = node->getAttribute("unitId")->getIntValue();
			if (node->hasAttribute("eventType") == true) {
				eventType =
					static_cast <UnitTriggerEventType>
					(node->getAttribute("eventType")->getIntValue());
			} else if (node->hasAttribute("evenType") == true) {
				eventType =
					static_cast <UnitTriggerEventType>
					(node->getAttribute("evenType")->getIntValue());
			}
			UnitTriggerEventList[unitId] = eventType;
		}
		if (scriptManagerNode->hasAttribute("lastUnitTriggerEventUnitId") ==
			true) {
			lastUnitTriggerEventUnitId =
				scriptManagerNode->getAttribute("lastUnitTriggerEventUnitId")->
				getIntValue();
		}
		if (scriptManagerNode->hasAttribute("lastUnitTriggerEventType") ==
			true) {
			lastUnitTriggerEventType =
				static_cast <UnitTriggerEventType>
				(scriptManagerNode->getAttribute("lastUnitTriggerEventType")->
					getIntValue());
		}

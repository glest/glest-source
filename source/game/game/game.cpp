...
namespace Game {
	...
	Game::Game(Program * program, const GameSettings * gameSettings,
		bool masterserverMode) :ProgramState(program),
		lastMousePos(0), isFirstRender(true) {
		if (SystemFlags::VERBOSE_MODE_ENABLED)
			printf("In [%s::%s Line: %d]\n",
				extractFileFromDirectoryPath(__FILE__).c_str(),
				__FUNCTION__, __LINE__);
		...
		Lang::getInstance().setAllowNativeLanguageTechtree(this->
	...
	void Game::endGame() {
		...
		if (originalDisplayMsgCallback != NULL) {
			NetworkInterface::setDisplayMessageFunction
		...
		FileCRCPreCacheThread *&preCacheCRCThreadPtr =
			...
			if (loadScreenList.empty() == false) {
				...
				if (loadScreenList.size() > 1
					...
					for (unsigned int logoIndex = 0;
						...
						string
							scenarioLogo = scenarioDir + loadScreenList[bestLogoIndex];
						...
						if (fileExists(scenarioLogo) == true) {
							if (SystemFlags::getSystemSettingType
							(SystemFlags::debugSystem).enabled)
								SystemFlags::OutputDebug(SystemFlags::debugSystem,
							...
							Texture2D *checkLogo = Renderer::preloadTexture(scenarioLogo);
							if (checkLogo != NULL) {
								...
								if (SystemFlags::VERBOSE_MODE_ENABLED)
									printf("Logo info: %d x %d (%d,%d)\n",
										checkLogo->getPixmapConst()->getW(),
										checkLogo->getPixmapConst()->getH(),
										minWidthDifference, minHeightDifference);
	...
	string
		Game::extractFactionLogoFile(bool & loadingImageUsed,
			...
			string scenarioDir,
			const string & techName, Logger * logger,
			string factionLogoFilter) {
		string result = "";
		if (SystemFlags::
			getSystemSettingType(SystemFlags::debugSystem).enabled)
				...
				if (isdir(path.c_str()) == true) {
					...
					if (loadScreenList.empty() == false) {
						...
						if (loadScreenList.size() > 1
							...
							for (unsigned int logoIndex = 0;
								logoIndex <
								(unsigned int) loadScreenList.size(); ++logoIndex) {
					...
					// Check if this is a linked faction
						...
						if (fileExists(factionXMLFile) == true) {
							...
							if (rootNode->getName() == "link"
								...
								string
									linkedTechTreeName =
									techtreeNode->getAttribute("name")->getValue();
								...
								if (linkedTechTreeName != "") {
									...
									string
										linkedTechTreePath =
										TechTree::findPath(linkedTechTreeName, pathList);
	...
	string
		Game::findFactionLogoFile(const GameSettings * settings,
			Logger * logger,
			const string & factionLogoFilter) {
		string result = "";
		if (settings == NULL) {
			...
			if (logger != NULL) {
				...
				if (scenarioName.empty()) {
					...
					TechTree
						techTree(Config::
							getInstance().getPathListForType(ptTechs,
								scenarioDir));

					logger->setSubtitle(formatString(mapName) + " - " +
						formatString(tilesetName) + " - " +
						formatString(techTree.getTranslatedName
						(techName)));
				} else {
			...
			for (int idx = 0; idx < (int) techPaths.size(); idx++) {
				...
				if (factions.empty() == false) {
					for (unsigned int factionIdx = 0;
						factionIdx < (unsigned int) factions.size(); ++factionIdx) {
						bool loadingImageUsed = false;
	...
	void Game::load(int loadTypes) {
		...
		string
			...
			getGameCustomCoreDataPath(data_path,
				...
				Lang::
				getInstance().getDefaultLanguage() +
				".lng");
		string languageFile =
			getGameCustomCoreDataPath(data_path,
				"data/lang/hint/hint_" +
		...
		if ((loadTypes & lgt_Scenario) == lgt_Scenario) {
			...
			if (scenarioName.empty() == false) {
				...
				Lang::getInstance().
					loadScenarioStrings(gameSettings.getScenarioDir(), scenarioName,
						isTutorial);
	...
	void Game::init(bool initForPreviewOnly) {
		...
		if (initForPreviewOnly == false) {
			...
			for (int i = 0; i < world.getFactionCount(); ++i) {
				...
				if (faction->getCpuControl(enableServerControlledAI, isNetworkGame,
					role) == true) {
			...
			if (Config::
				getInstance().getBool("EnableNewThreadManager",
					"false") == true) {
		...
		if (initForPreviewOnly == false) {
			...
			GameNetworkInterface* networkInterface = networkManager.getGameNetworkInterface();
			if (networkInterface != NULL)
				networkInterface->waitUntilReady(&checksum);
	...
	void Game::initCamera(Map * map) {
		...
		if (world.getThisFaction() != NULL) {
			const
				Vec2i &
				v =
				map->getStartLocation(world.
					getThisFaction()->getStartLocationIndex());
			// This args are set in map.cpp - Map::getStartLocation()
			gameCamera.setPos(Vec2f
			(v.x,
				v.y + gameCamera.getCalculatedDefault() / 2));
			//
	...
	void Game::setupPopupMenus(bool checkClientAdminOverrideOnly) {
			...
			if (disconnectPlayerPopupMenuIndex == -1) {
				...
				if (serverInterface != NULL && checkClientAdminOverrideOnly == true) {
					for (int i = 0; i < world.getFactionCount(); ++i) {
						...
						MutexSafeWrapper
							safeMutex(serverInterface->getSlotMutex
							(faction->getStartLocationIndex()), CODE_AT_LINE);
						ConnectionSlot *slot =
							serverInterface->getSlot(faction->getStartLocationIndex(),
								false);
						if (slot != NULL
		...
		if (checkClientAdminOverrideOnly == false ||
			...
			if (checkClientAdminOverrideOnly == true && clientInterface != NULL) {
				gameSettings.
					setMasterserver_admin(clientInterface->getSessionKey());
			...
			if (allowAdminMenuItems == true) {
				...
				if (gameSettings.isNetworkGame() == true) {
					menuItems.push_back("  " +
						lang.getString("DisconnectNetorkPlayer") +
						"  ");
	...
	void Game::update() {
		try {
			...
			if (this->masterserverMode == false) {
				if (world.getFactionCount() > 0
					...
					const SwitchTeamVote *vote =
						world.getThisFaction()->getFirstSwitchTeamVote();
					GameSettings *settings = world.getGameSettingsPtr();
					...
					if (lang.hasString("AllowPlayerJoinTeam") == true) {
						snprintf(szBuf, 8096,
							...
							settings->
							getNetworkPlayerName(vote->factionIndex).c_str(),
							vote->oldTeam, vote->newTeam);
					} else {
						snprintf(szBuf, 8096,
					...
					world.
						getThisFactionPtr()->setCurrentSwitchTeamVoteFactionIndex
						(vote->factionIndex);
				}
			}
			...
			if (updateLoops > 0) {
				...
				if (framesToSlowDownAsClient > 0) {                     // slowdown still the hard way.
					updateLoops = 0;
						...
						if (commander.hasReplayCommandListForFrame() == false) {
							...
							if (newThreadManager == true) {
								...
								for (int j = 0; j < world.getFactionCount(); ++j) {
									...
									if (faction->getCpuControl(enableServerControlledAI,
										isNetworkGame,
										role) == true
										&& scriptManager.
										getPlayerModifiers(j)->getAiEnabled() == true) {

										if (SystemFlags::getSystemSettingType
										(SystemFlags::debugPerformance).enabled
											&& chrono.getMillis() > 0)
											SystemFlags::
											OutputDebug(SystemFlags::debugPerformance,
							...
							if (lastReplaySecond < chronoReplay.getSeconds()) {
								...
								if (Renderer::renderText3DEnabled) {
									...
									int
										renderX =
										(w / 2) -
										(font->getMetrics()->getTextWidth(text) / 2);
									int h = metrics.getVirtualH();
									int
										renderY =
										(h / 2) + (font->getMetrics()->getHeight(text) / 2);
			...
			if (world.getQueuedScenario() != "") {
				...
				const
					vector <
					string > &
					dirList = Config::getInstance().getPathListForType(ptScenarios);
				...
				try {
					...
					if (keepFactions == false) {
						...
						if (map->getCameraHeight() > 0
							...
							map->getMaxMapHeight() + 13.0f) {
							gameCamera.setCalculatedDefault(map->getMaxMapHeight() +
								13.0f);
						}
				...
				if (gameSettings.getFactionControl(i) == ctNetwork ||
					...
					if (switchSetupRequests[i]->getToSlotIndex() != -1) {
						...
						if (serverInterface->switchSlot(switchSlotIdx, newSlotIdx)) {
							...
							int
								oldFactionIndex =
								gameSettings.getFactionIndexForStartLocation
								(switchSlotIdx);
							int newFactionIndex =
								gameSettings.getFactionIndexForStartLocation(newSlotIdx);
							...
							try {
								...
								if (switchSetupRequests[i]->getNetworkPlayerName() != "") {
									...
									gameSettings.setNetworkPlayerName(newFactionIndex,
										switchSetupRequests
			...
			if (gameNetworkInterface != NULL &&
				...
				for (int idx = 0; idx < (int) chatList.size(); idx++) {
					...
					if (mc.getFactionIndex() >= 0) {
						mc.
							setFaction((const Faction *)
								world.getFaction(mc.getFactionIndex()));
					}
		...
		if (role == nrServer && isNetworkGame == true &&
			...
			if (newAIPlayerCreated == true
				...
				for (int i = 0; i < world.getFactionCount(); ++i) {
					Faction *faction = world.getFaction(i);
					if (faction->getCpuControl
					(enableServerControlledAI, isNetworkGame, role) == true) {
						slaveThreadList.push_back(aiInterfaces[i]->getWorkerThread());
					}
				}
				masterController.setSlaves(slaveThreadList);
	...
	void Game::render() {
		...
		if (this->masterserverMode == false) {
			...
			for (int i = 0; i < world.getFactionCount(); ++i) {
				Faction *faction = world.getFaction(i);
				if (server->isClientConnected(faction->getStartLocationIndex()) ==
					true) {
					connectedClients++;
				}
			}
	...
	void Game::showMarker(Vec2i cellPos, MarkedCell cellData) {
		...
		if (gameNetworkInterface != NULL) {
			gameNetworkInterface->
				sendHighlightCellMessage(cellData.getTargetPos(),
					cellData.getFactionIndex());
		}
	}
	...
	void Game::mouseDownLeft(int x, int y) {
		...
		try {
			...
			if (popupMenu.mouseClick(x, y)) {
				std::pair < int,
					string > result = popupMenu.mouseClickedMenuItem(x, y);
				...
				if (result.first == exitGamePopupMenuIndex) {
					showMessageBox(Lang::
						getInstance().getString("ExitBattleQuestion"),
						"", true);
				...
				switch (factionIndex) {
					...
					default:
						...
						disconnectPlayerConfirmMessageBox.init(lang.getString("Yes"),
							lang.getString("No"));
						...
						if (gameNetworkInterface != NULL) {
							...
							const
								vector <
								string >
								languageList = settings->getUniqueNetworkPlayerLanguages();
							for (unsigned int i = 0;
								i < (unsigned int) languageList.size(); ++i) {
								char szMsg[8096] = "";
								if (lang.hasString("DisconnectNetorkPlayerIndexConfirmed",
									languageList[i]) == true) {
									snprintf(szMsg, 8096,
										lang.getString
			...
			if (switchTeamConfirmMessageBox.getEnabled() == true) {
				...
				if (switchTeamConfirmMessageBox.mouseClick(x, y, button)) {
					...
					SwitchTeamVote *vote =
						...
						getSwitchTeamVote(world.getThisFaction
			...
			if (messageBoxClick == false) {
				if (metrics.isInMinimap(x, y)) {
					...
					if (map->isInside(xCell, yCell)
						&& map->isInsideSurface(map->toSurfCoords(Vec2i
						(xCell, yCell)))) {
						...
						if (setMarker) {
							Vec2i
								surfaceCellPos = map->toSurfCoords(Vec2i(xCell, yCell));
							SurfaceCell *sc = map->getSurfaceCell(surfaceCellPos);
							Vec3f vertex = sc->getVertex();
							Vec2i targetPos(vertex.x, vertex.z);
							...
							GameNetworkInterface *gameNetworkInterface =
								NetworkManager::getInstance().getGameNetworkInterface();
							if (gameNetworkInterface != NULL)
								gameNetworkInterface->sendHighlightCellMessage(mc.getTargetPos(),
									mc.getFaction()->getIndex());
						}
			...
			if (ms->get(mbCenter)) {
				...
				if (currentCameraFollowUnit == NULL) {
					...
					int
						oldy =
						((metrics.getScreenH() -
							oldPos.y) * metrics.getVirtualH() / metrics.getScreenH());
					lastMousePos.x = oldx;
					lastMousePos.y = oldy;
	...
	void Game::startCameraFollowUnit() {
		...
		if (selection->getCount() == 1) {
			...
			if (currentUnit != NULL) {
				...
				getGameCameraPtr()->
					setPos(currentCameraFollowUnit->getCurrMidHeightVector());
	...
	Stats Game::quitGame() {
		...
		if (SystemFlags::
			getSystemSettingType(SystemFlags::debugSystem).enabled == true) {
	...
	void Game::DumpCRCWorldLogIfRequired(string fileSuffix) {
		...
		if (isNetworkGame == true) {
			...
			if (settings != NULL &&
				...
				logFile << "============================" << std::endl;
				logFile << "Software version: " << GameVersionString << "-" <<
				...
				for (unsigned int worldFrameIndex = 0;
					...
					for (int i = 0; i < world.getFactionCount(); ++i) {
						...
						std::pair < int,
							...
							details =
							world.
							getFaction(i)->getCRC_DetailsForWorldFrameIndex
							(worldFrameIndex);

						logFile << string("** world frame: ") << details.first << std::
							endl;
		...
		if (steamInstance != NULL) {
					...
					if (factionIndex == game->getWorld()->getThisFactionIndex()) {
						...
						if (endStats.getVictory(factionIndex)) {
							steamInstance->setStatAsInt(EnumParser <
								SteamStatName >::getString
								(stat_online_wins).c_str(),
						...
						(stat_online_kills_enemy).c_str(),
							...
							(stat_online_kills_enemy).c_str
							()) +
							endStats.getEnemyKills
							(factionIndex));
						steamInstance->setStatAsInt(EnumParser <
							SteamStatName
						>::getString
						(stat_online_deaths).c_str(),
							steamInstance->getStatAsInt
							(EnumParser <
								SteamStatName
							>::getString
						...
						steamInstance->setStatAsInt(EnumParser <
							SteamStatName
						>::getString
						(stat_online_units).c_str(),
							steamInstance->getStatAsInt
							(EnumParser <
								SteamStatName
							>::getString
							(stat_online_units).c_str()) +
							endStats.getUnitsProduced
							(factionIndex));
						steamInstance->setStatAsInt(EnumParser <
							SteamStatName
						>::getString
						(stat_online_resources_harvested).c_str
						(),
							steamInstance->getStatAsInt
							(EnumParser <
								SteamStatName
						...
						if (endStats.getPlayerLeftBeforeEnd(factionIndex)) {
							steamInstance->setStatAsInt(EnumParser <
								SteamStatName >::getString
								(stat_online_quit_before_end).c_str
								(),
								steamInstance->getStatAsInt
								(EnumParser <
									SteamStatName
			...
			for (int factionIndex = 0;
				...
				if (factionIndex == game->getWorld()->getThisFactionIndex()) {
					...
					if (endStats.getVictory(factionIndex)) {
						if (steamInstance->isUnlocked(EnumParser <
							SteamAchievementName
						>::getString
						(ACH_WIN_ONE_GAME).c_str()) ==
							false) {
							steamInstance->unlock(EnumParser <
								SteamAchievementName
							>::getString(ACH_WIN_ONE_GAME).c_str
							());
						}
						if (NetworkManager::getInstance().isNetworkGame()) {
							if (steamInstance->isUnlocked(EnumParser <
								SteamAchievementName
							>::getString
							(ACH_WIN_ONE_GAME_ONLINE).c_str
							()) == false) {
								steamInstance->unlock(EnumParser <
									SteamAchievementName >::getString
									(ACH_WIN_ONE_GAME_ONLINE).c_str());
							}
						}
					}
		...
		if ((renderExtraTeamColor & renderTeamColorCircleBit) > 0) {
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).
				...
				SystemFlags::OutputDebug(SystemFlags::debugPerformance,
					...
					extractFileFromDirectoryPath
					(__FILE__).c_str(), __FUNCTION__,
					__LINE__, renderFps, chrono.getMillis());
			if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).
				enabled && chrono.getMillis() > 0)
				chrono.start();
		}
	...
	void Game::updateWorldStats() {
		world.getStats()->setWorldTimeElapsed(world.
			getTimeFlow()->getTime());
		...
		if (difftime((long int) time(NULL), lastMaxUnitCalcTime) >= 1) {
			...
			world.
				getStats()->setTotalEndGameConcurrentUnitCount(totalUnitcount);
	...
	string Game::getDebugStats(std::map < int, string > &factionDebugInfo) {
		...
		str +=
			"Time: " + floatToStr(world.getTimeFlow()->getTime(),
				2) + " [" +
			floatToStr((double) world.
				getStats()->getFramesToCalculatePlaytime() /
		...
		const string
			selectionType =
			toLower(Config::
				getInstance().getString("SelectionType",
					Config::colorPicking));
	...
	void Game::render2d() {
		...
		if (SystemFlags::
			getSystemSettingType(SystemFlags::debugPerformance).enabled == true
		...
		if (renderer.getShowDebugUI() == true) {
			...
			if (Renderer::renderText3DEnabled == true) {
				...
				renderer.renderTextShadow(str, coreData.getMenuFontNormal(),
					fontColor, 10,
					metrics.getVirtualH() - mh - 60, false);
			}
			...
			int
				...
				(Renderer::renderText3DEnabled ==
					true ? coreData.getMenuFontNormal3D()->
					getMetrics()->getHeight("W") : coreData.getMenuFontNormal()->
					getMetrics()->getHeight("W"));
			int fontHeightBig =
				(Renderer::renderText3DEnabled ==
					true ? coreData.getMenuFontBig3D()->
					getMetrics()->getHeight("W") : coreData.getMenuFontBig()->
					getMetrics()->getHeight("W"));
			...
			for (int i = 0; i < world.getFactionCount(); ++i) {
				...
				const Texture2D* texture = world.getFaction(i)->getTexture();
				Vec4f playerColor = Vec4f(0.f, 0.f, 0.f, 0.f);
				if (texture != NULL)
					playerColor = texture->getPixmapConst()->getPixel4f(0, 0, true);
			...
			if (NetworkManager::getInstance().getGameNetworkInterface() != NULL) {
				...
				if (Renderer::renderText3DEnabled == true) {
					renderer.renderTextShadow3D(NetworkManager::
							getInstance().getGameNetworkInterface()->getNetworkStatus(),
							coreData.getMenuFontNormal3D(), fontColor,
							mx + mw + 5,
							metrics.getVirtualH() - 30 - 20, false);
				} else {
					renderer.
						renderTextShadow(NetworkManager::
							getInstance().getGameNetworkInterface
							()->getNetworkStatus(),
							coreData.getMenuFontNormal(), fontColor,
							mx + mw + 5, metrics.getVirtualH() - 30 - 20,
							false);
				}
			}
		}
	...
	void Game::setEndGameTeamWinnersAndLosers() {
		...
		bool
			checkTeamIndex = !(this->masterserverMode == false
				&& world.
				getThisFaction()->getPersonalityType() !=
				fpt_Observer);
				...
				if (factionIndex != world.getThisFactionIndex() &&
					...
					if (this->masterserverMode == true) {
						...
						printf("Player: %s is on the winning team #: %d\n",
							this->gameSettings.getNetworkPlayerName(factionIndex).
							c_str(), faction->getTeam());
					}
				}
			}
		...
		if (factionLostGame(world.getThisFaction()) == true) {
			...
			if (isFlagType1BitEnabled(ft1_allow_shared_team_units) == true) {
				...
				for (int factionIndex = 0;
					...
					if (world.getFaction(factionIndex)->getPersonalityType() !=
						fpt_Observer) {
						if (world.
							getFaction(factionIndex)->isAlly(world.
								getThisFaction()) ==
							true
							&& factionLostGame(world.getFaction(factionIndex)) ==
							false) {
			...
			if (win) {
				for (int factionIndex = 0;
					...
					if (world.getFaction(factionIndex)->getPersonalityType() !=
						fpt_Observer) {
						if (world.
							getFaction(factionIndex)->isAlly(world.getThisFaction())) {
		...
		if (scriptManager.getIsGameOver()) {
			...
			if (wasGameOverAlready == false) {
				...
				if (this->masterserverMode == true ||
					...
					scriptManager.
						onGameOver(scriptManager.getPlayerModifiers
						(world.getThisFactionIndex())->getWinner());
		...
		if (faction != NULL) {
			for (int factionIndex = 0; factionIndex < faction->getUnitCount();
				...
				if (ut->getCountInVictoryConditions() == ucvcNotSet) {
					if (faction->getUnit(factionIndex)->
						getType()->hasSkillClass(scBeBuilt)) {
						return false;
					}
	...
	void
		...
		if (forceAllowPauseStateChange == true || speedChangesAllowed == true) {
				...
				for (int i = 0; i < world.getFactionCount(); ++i) {
					...
					if (slot != NULL && slot->isConnected() == true
						...
						for (unsigned int i = 0; i < languageList.size(); ++i) {
							...
							char szMsg[8096] = "";
							if (lang.hasString("JoinPlayerToCurrentGameLaunch",
								languageList[i]) == true) {
								snprintf(szMsg, 8096,
									lang.getString
			...
			if (value == false) {
				...
				if (clearCaches == true) {
					if (SystemFlags::getSystemSettingType
					(SystemFlags::debugWorldSynch).enabled)
						SystemFlags::OutputDebug(SystemFlags::debugWorldSynch,
	...
	string Game::saveGame(string name, const string & path) {
		...
		if (config.getBool("SaveCommandsForReplay", "false") == true) {
			...
			rootNodeReplay->addAttribute("version", GameVersionString,
				mapTagReplacements);
		...
		rootNode->addAttribute("version", GameVersionString,
			mapTagReplacements);
	...
	//    int mouse2d;
		...
		XmlNode *unitHighlightListNode =
			gameNode->addChild("unitHighlightList");
	...
	void
		...
		if (joinGameSettings == NULL
			...
			if (rootNode->hasChild("glest-saved-game") == true) {
				rootNode = rootNode->getChild("glest-saved-game");
			}
			...
			if (newGameSettingsReplay.getScenarioDir() != ""
				&& fileExists(newGameSettingsReplay.getScenarioDir()) == false) {
				newGameSettingsReplay.setScenarioDir(Scenario::getScenarioPath
				(Config::
					getInstance
					().getPathListForType
					(ptScenarios),
					newGameSettingsReplay.getScenario
					()));
		...
		/*if (gameVer != GameVersionString
			...
			(gameVer, LastCompatibleVersionString) < 0
				|| compareMajorMinorVersion(GameVersionString, gameVer) < 0)) {
			char szBuf[8096] = "";
			snprintf(szBuf, 8096,
				lang.getString("SavedGameBadVersion").c_str(),
				gameVer.c_str(), GameVersionString.c_str());
		...
		if (joinGameSettings != NULL) {
			...
			for (int i = 0; i < newGameSettings.getFactionCount(); ++i) {
				...
				if (newGameSettings.getStartLocationIndex(i) ==
					...
					worldNode->
						getAttribute("thisFactionIndex")->setValue(intToStr(i));
					...
					factionNode->
						getAttribute("thisFaction")->setValue(intToStr(i));
					factionNode->
						getAttribute("control")->setValue(intToStr(ctNetwork));

					selectionNode->
						getAttribute("factionIndex")->setValue(intToStr(i));
					...
					XmlNode *factionNode = worldNode->getChild("Faction", i);
					factionNode->
						getAttribute("thisFaction")->setValue(intToStr(0));
				}
			}
		} else {
			newGameSettings.loadGame(gameNode);
			...
			if (newGameSettings.getScenarioDir() != ""
				...
				newGameSettings.setScenarioDir(Scenario::getScenarioPath
				(Config::
					getInstance().getPathListForType
					(ptScenarios),
					newGameSettings.getScenario()));
		...
		Game *newGame =
			new Game(programPtr, &newGameSettings, isMasterserverMode);
	...
	//      gameNode->addAttribute("renderFpsAvgTest",intToStr(renderFpsAvgTest), mapTagReplacements);
		...
		if (gameNode->hasAttribute("disableSpeedChange") == true) {
			newGame->disableSpeedChange =
				gameNode->getAttribute("disableSpeedChange")->getIntValue() != 0;
		}

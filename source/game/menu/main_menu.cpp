...
namespace Game {
	...
	void
		MainMenu::initBackgroundVideo() {
		if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false
			&& ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true
			&& CoreData::getInstance().hasMainMenuVideoFilename() == true) {
			string
				introVideoFile =
				CoreData::getInstance().getMainMenuVideoFilename();
			string
				introVideoFileFallback =
				CoreData::getInstance().getMainMenuVideoFilenameFallback();
	...
	void
		MainMenu::mouseMove(int x, int y, const MouseState * ms) {
		mouseX = x;
		mouseY = y;
	...
	//returns if exiting
	void
		MainMenu::mouseDownLeft(int x, int y) {
		if (GraphicComponent::getFade() < 0.2f)
			return;
		state->mouseClick(x, y, mbLeft);
	}

	void
		MainMenu::mouseDownRight(int x, int y) {
		if (GraphicComponent::getFade() < 0.2f)
			return;
	...
	void
		MainMenu::mouseUpLeft(int x, int y) {
		if (GraphicComponent::getFade() < 0.2f)
			return;
	...
	bool
		MainMenu::textInput(std::string text) {
		return state->textInput(text);
	}
	void
		MainMenu::keyDown(SDL_KeyboardEvent key) {
		state->keyDown(key);
	}

	void
		MainMenu::keyUp(SDL_KeyboardEvent key) {
		state->keyUp(key);
	}

	void
		MainMenu::keyPress(SDL_KeyboardEvent c) {
		state->keyPress(c);
	}

	void
		MainMenu::setState(MenuState * newstate) {
	...
	void
		MainMenu::consoleAddLine(string line) {
		if (state != NULL) {
	...
	MenuState::MenuState(Program * program, MainMenu * mainMenu,
		...
		//position
		const XmlNode *
			positionNode = cameraNode->getChild(stateName + "-position");
		Vec3f
			startPosition;
		...
		//rotation
		const XmlNode *
			rotationNode = cameraNode->getChild(stateName + "-rotation");
		Vec3f
			startRotation;
	...
	void
		MenuState::consoleAddLine(string line) {
		bool
			onlyWantChatMsgs = console.getOnlyChatMessagesInStoredLines();
	...
	void
		MenuState::setActiveInputLabel(GraphicLabel * newLabel,
			GraphicLabel ** activeInputLabelPtr) {
		GraphicLabel *
			activeInputLabelEdit = *activeInputLabelPtr;
		if (newLabel != NULL) {
			if (newLabel == activeInputLabelEdit) {
				return;
			}
			string
				text = newLabel->getText();
			size_t
			...
			newLabel->
				setTextCharLengthList(extractKeyPressedUnicodeLength(text));
		}
		if (activeInputLabelEdit != NULL
			&& activeInputLabelEdit->getText().empty() == false) {
			string
				text = activeInputLabelEdit->getText();
			size_t
				found = text.find_last_of("_");
			if (found != string::npos && found == text.length() - 1) {
	...
	bool
		MenuState::textInputEditLabel(string input,
			GraphicLabel ** activeInputLabelPtr) {
		bool
			eventHandled = false;
		GraphicLabel *
		...
		if (activeInputLabel != NULL) {
			int
				maxTextSize = activeInputLabel->getMaxEditWidth();

			//if((c>='0' && c<='9') || (c>='a' && c<='z') || (c>='A' && c<='Z') ||
			//   (c=='-') || (c=='(') || (c==')')) {
			//if(isAllowedInputTextKey(key))
			{
				if ((int) activeInputLabel->getTextCharLengthList().size() <
					maxTextSize) {
					string
	...
	bool
		MenuState::keyDownEditLabel(SDL_KeyboardEvent c,
			GraphicLabel ** activeInputLabelPtr) {
		bool
			eventHandled = false;
		GraphicLabel *
		...
		if (activeInputLabel != NULL) {
			...
			if (isKey(SDLK_BACKSPACE, c) == true && text.length() > 0) {
				...
				if (found == string::npos || found != text.length() - 1) {
					...
					if (activeInputLabel->
						getTextCharLengthList()[activeInputLabel->
						getTextCharLengthList().size() -
						1] >= 1) {
						delChar = true;
					}
				} else {
					...
					hasUnderscore = true;
					if (activeInputLabel->getTextCharLengthList().size() >= 2
						&& activeInputLabel->
						getTextCharLengthList()[activeInputLabel->
						getTextCharLengthList().size() -
						2] >= 1) {
					...
					if (hasUnderscore) {
						...
						if (activeInputLabel->getTextCharLengthList().size() > 1) {
							...
							for (unsigned int i = 0;
								i <
								(unsigned int) activeInputLabel->
								getTextCharLengthList()[activeInputLabel->
								getTextCharLengthList().
								size() - 2]; ++i) {

...
namespace Game {
	...
	MenuStateLoadGame::MenuStateLoadGame(Program * program,
		MainMenu *
		...
		lines[1].init(0,
			slotLinesYBase - (slotsToRender - 1) * slotsLineHeight -
			5);
		...
		slotsScrollBar.init(500 - 20,
			slotLinesYBase - slotsLineHeight * (slotsToRender -
				1), false,
			slotWidth, 20);
	...
	void MenuStateLoadGame::mouseClick(int x, int y, MouseButton mouseButton) {
		...
		if (abortButton.mouseClick(x, y)) {
			...
			if (selectedButton == NULL) {
				...
				string jpgfilename =
					saveGameDir + selectedButton->getText() + ".xml.jpg";
				string replayfilename =
					saveGameDir + selectedButton->getText() + ".xml.replay";
			...
			if (slotsScrollBar.getElementCount() != 0) {
				for (int i = slotsScrollBar.getVisibleStart();
					i <= slotsScrollBar.getVisibleEnd(); ++i) {
					if (slots[i]->mouseClick(x, y) && selectedButton != slots[i]) {
						...
						string filename =
							saveGameDir + selectedButton->getText() + ".xml";
						string screenShotFilename = filename + ".jpg";
						if (fileExists(screenShotFilename) == true) {
							try {
								previewTexture =
									GraphicsInterface::getInstance().getFactory()->
									newTexture2D();
								if (previewTexture) {
								...
								cleanupTexture(&previewTexture);
								showMessageBox(ex.what(), lang.getString("Notice"),
									false);
							}
						} else {
							previewTexture = NULL;
						...
						if (fileExists(filename) == true) {
							...
							if (Config::getInstance().
								getBool("ForceXMLLoadGameUsingXerces", "false") == true) {
							...
							try {
								xmlTree.load(filename,
									Properties::
									getTagReplacementValues
									(&mapExtraTagReplacementValues), true, false,
									true);
								...
								if (rootNode != NULL
									&& rootNode->hasChild("glest-saved-game") == true) {
								...
								char szBuf[8096] = "";
								snprintf(szBuf, 8096,
									lang.getString("LoadSavedGameInfo").c_str(),
									...
									(newGameSettings.getThisFactionIndex() >= 0
										&& newGameSettings.getThisFactionIndex() <
										newGameSettings.
										getFactionCount() ? newGameSettings.
										getFactionTypeName(newGameSettings.
											getThisFactionIndex()).
										c_str() : ""));
								infoTextLabel.setText(szBuf);
							} catch (const game_runtime_error & ex) {
	...
	void MenuStateLoadGame::mouseUp(int x, int y,
	...
	void MenuStateLoadGame::showMessageBox(const string & text,
		const string & header,
		bool toggle) {
		if (toggle == false) {
			mainMessageBox.setEnabled(false);

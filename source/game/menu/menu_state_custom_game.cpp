...
namespace Game {
	static const char *SAVED_GAME_FILENAME = "lastCustomGameSettings.sav";
	static const char *DEFAULT_GAME_FILENAME = "data/defaultGameSetup.sav";
	static const char *DEFAULT_NETWORKGAME_FILENAME =
		"data/defaultNetworkGameSetup.sav";
			...
			if (b1) {
				...
				if (bFound == false) {
					...
					if (menu != NULL
						...
						&& (type == ctCpuEasy || type == ctCpu || type == ctCpuUltra || type == ctCpuGlest || type == ctNetwork)) {
						if (selectedItemIndex == 0)
							selectedItemIndex = items.size() - 1;
						else
							selectedItemIndex--;
				...
				if (bFound == false) {
					...
					if (menu != NULL
						&& faction == formatString(GameConstants::OBSERVER_SLOTNAME)
						&& (type == ctCpuEasy || type == ctCpu || type == ctCpuUltra || type == ctCpuGlest)) {
						selectedItemIndex = (selectedItemIndex + 1) % items.size();
					}
				}
			}
	...
	MenuStateCustomGame::MenuStateCustomGame(Program * program,
		MainMenu * mainMenu,
		...
		string
		autoloadScenarioName) :MenuState
		(program, mainMenu, "new-game") {
		try {
			...
			enableFactionTexturePreview =
				config.getBool("FactionPreview", "true");
			...
			buttonPlayNow.registerGraphicComponent(containerName,
				"buttonPlayNow");
			buttonPlayNow.init(buttonx, buttony, 125);

			labelLocalGameVersion.registerGraphicComponent(containerName,
			...
			string serverPort = config.getString("PortServer",
				intToStr
				(GameConstants::
					serverPort).c_str());
			string externalPort =
				config.getString("PortExternal", serverPort.c_str());
			labelLocalIP.setText(lang.getString("LanIP") + ipText + "  ( " +
			...
			// put them all in a set, to weed out duplicates (gbm, and mgm with same name)
			...
			listBoxTileset.setSelectedItemIndex(rand() %
				listBoxTileset.getItemCount());
			...
			labelTechTree.registerGraphicComponent(containerName,
				"labelTechTree");
			...
			// fog - o - war
			// @350 ? 300 ?
			labelFogOfWar.registerGraphicComponent(containerName,
				"labelFogOfWar");
			...
			labelEnableSwitchTeamMode.setText(lang.getString
			...
			labelAISwitchTeamAcceptPercent.init(xoffset + 500, aHeadPos + 45,
				80);
			...
			int scenarioY = aPos;
			labelScenario.registerGraphicComponent(containerName,
				"labelScenario");
			...
			labelGameName.registerGraphicComponent(containerName,
				"labelGameName");
			labelGameName.init(20 + checkBoxPublishServer.getW() + 5,
				networkPos, 200);
			...
			bool allowInProgressJoin =
				Config::getInstance().getBool("EnableJoinInProgressGame", "true");
			...
			labelAllowTeamUnitSharing.setText(lang.getString
			("AllowTeamUnitSharing"));
			...
			labelAllowTeamResourceSharing.setText(lang.getString
			...
			labelNetworkPauseGameForLaggedClients.init
			(labelAllowTeamResourceSharing.getX(), networkHeadPos, 80);
			labelNetworkPauseGameForLaggedClients.setText(lang.getString
			...
			checkBoxNetworkPauseGameForLaggedClients.init
			...
			for (int i = 0; i < GameConstants::maxPlayers; ++i) {
				...
				listBoxControls[i].registerGraphicComponent(containerName,
					"listBoxControls" +
					intToStr(i));
				listBoxControls[i].init(xoffset + 160,
					setupPos - 30 - i * rowHeight, 174);

				buttonBlockPlayers[i].registerGraphicComponent(containerName,
					"buttonBlockPlayers"
					+ intToStr(i));
				//buttonBlockPlayers[i].init(xoffset+355, setupPos-30-i*rowHeight, 70);
				buttonBlockPlayers[i].init(xoffset + 185,
					setupPos - 30 - i * rowHeight, 124);
				buttonBlockPlayers[i].setText(lang.getString("BlockPlayer"));
				buttonBlockPlayers[i].
			...
			//texts
			buttonClearBlockedPlayers.
				setText(lang.getString("BlockPlayerClear"));
			...
			for (int i = 0; i < GameConstants::maxPlayers; ++i) {
				...
				labelPlayerStatus[i].
					setTexture(CoreData::getInstance().getStatusReadyTexture());
			...
			loadMapInfo(Config::getMapPath(getCurrentMapFile()), &mapInfo,
				true);
			...
			if (serverInitError == false) {
				...
				if (openNetworkSlots == true) {
					...
					listBoxControls[1].setSelectedItemIndex(ctCpuGlest);
				}
				updateControllers();
				updateNetworkSlots();
			...
			publishToMasterserverThread =
				new SimpleTaskThread(this, 0, 300, false,
				(void *) tnt_MASTERSERVER);
			...
			publishToClientsThread =
				new SimpleTaskThread(this, 0, 200, false, (void *) tnt_CLIENTS,
					false);
		...
		if (thread != NULL && *thread != NULL) {
			...
			if (forceWaitForShutdown == true) {
				...
				if (threadPtr->canShutdown(true) == true &&
					...
					char szBuf[8096] = "";
					snprintf(szBuf, 8096,
						"In [%s::%s %d] Error cannot shutdown thread\n",
						extractFileFromDirectoryPath(__FILE__).c_str(),
						__FUNCTION__, __LINE__);
					//SystemFlags::OutputDebug(SystemFlags::debugError,szBuf);
					if (SystemFlags::VERBOSE_MODE_ENABLED)
	...
	void MenuStateCustomGame::mouseClick(int x, int y,
		...
		try {
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (::Shared::Platform::Window::isKeyStateModPressed(KMOD_SHIFT) ==
					true) {
					const wchar_t lastKey = ::Shared::Platform::
						Window::extractLastKeyPressed();
					//                              string helpString = "";
					//                              helpString = lastKey;
					//                              printf("lastKey = '%s'\n",helpString.c_str());
				...
				if (mapPreviewTexture != NULL) {
					...
					if (x >= this->render_mapPreviewTexture_X
						...
						if (this->render_mapPreviewTexture_X ==
							mapPreviewTexture_X
							&& this->render_mapPreviewTexture_Y ==
							mapPreviewTexture_Y
							&& this->render_mapPreviewTexture_W ==
				...
				if (activeInputLabel != NULL
					&& !(activeInputLabel->mouseClick(x, y))) {
				...
				if (buttonReturn.mouseClick(x, y) || serverInitError == true) {
					...
					if (listBoxTileset.mouseClick(x, y, advanceToItemStartingWith)) {
						MutexSafeWrapper
							safeMutex((publishToMasterserverThread !=
								NULL ?
								publishToMasterserverThread->getMutexThreadObjectAccessor
						...
						if (listBoxTechTree.mouseClick(x, y, advanceToItemStartingWith)) {
							reloadFactions(listBoxTechTree.getItemCount() <= 1,
								(checkBoxScenario.getValue() ==
									true ?
									scenarioFiles
									[listBoxScenario.getSelectedItemIndex()] : ""));
							...
							&& checkBoxNetworkPauseGameForLaggedClients.mouseClick(x,
								y)) {
							MutexSafeWrapper
								safeMutex((publishToMasterserverThread !=
									NULL ?
							...
							for (int i = 0; i < mapInfo.players; ++i) {
								...
								if (selectedControlItemIndex != ctHuman &&
									...
									if (::Shared::Platform::
										Window::isKeyStateModPressed(KMOD_CTRL) == true
									...
									for (int index = slotsToChangeStart;
										...
										if (humanIndex1 == -1 && humanIndex2 == -1) {
											...
											(SystemFlags::debugSystem).enabled)
												SystemFlags::OutputDebug(SystemFlags::debugSystem,
													"In [%s::%s Line %d] i = %d, labelPlayerNames[i].getText() [%s]\n",
													extractFileFromDirectoryPath
													(__FILE__).c_str(),
										...
										//2 humans
											...
											int humanSlotIndex =
												(closeSlotIndex ==
													humanIndex1 ? humanIndex2 : humanIndex1);
											...
											(SystemFlags::debugSystem).enabled)
												SystemFlags::OutputDebug(SystemFlags::debugSystem,
													"In [%s::%s Line %d] closeSlotIndex = %d, origPlayName [%s]\n",
													extractFileFromDirectoryPath
													(__FILE__).c_str(),
													__FUNCTION__, __LINE__,
													closeSlotIndex,
											...
											listBoxControls[closeSlotIndex].setSelectedItemIndex
											(ctClosed);
									...
									if (serverInterface != NULL) {
										ServerSocket *serverSocket =
											serverInterface->getServerSocket();
											...
											if (serverSocket != NULL) {
												...
												for (unsigned int j = 0; j < languageList.size(); ++j) {
													...
													if (lang.hasString("BlockPlayerServerMsg",
														...
														snprintf(szMsg, 8096,
															lang.getString
															("BlockPlayerServerMsg",
																languageList[j]).c_str(),
															serverInterface->getSlot(i,
																true)->getIpAddress
																().c_str());
													} else {
														snprintf(szMsg, 8096,
															"The server has temporarily blocked IP Address [%s] from this game.",
															serverInterface->getSlot(i,
																true)->getIpAddress
																().c_str());
													}
	...
	void MenuStateCustomGame::PlayNow(bool saveGame) {
				...
				if (listBoxFactions[i].getSelectedItem() ==
					...
					for (int findRandomFaction = 1; findRandomFaction < 1000;
						...
						int selectedFactionIndex =
							rand() % listBoxFactions[i].getItemCount();
						...
						(SystemFlags::debugSystem).enabled)
							SystemFlags::OutputDebug(SystemFlags::debugSystem,
								"In [%s::%s Line %d] selectedFactionName [%s] selectedFactionIndex = %d, findRandomFaction = %d\n",
								extractFileFromDirectoryPath
								(__FILE__).c_str(), __FUNCTION__,
								__LINE__,
								selectedFactionName.c_str(),
					...
					if (listBoxFactions[i].getSelectedItem() ==
						...
						for (int useIdx = 0;
							useIdx < listBoxFactions[i].getItemCount(); useIdx++) {
							string selectedFactionName =
								listBoxFactions[i].getItem(useIdx);
							if (selectedFactionName !=
								formatString(GameConstants::RANDOMFACTION_SLOTNAME)
								&& selectedFactionName !=
								formatString(GameConstants::OBSERVER_SLOTNAME)) {
						...
						randomFactionSelectionList.push_back(listBoxFactions[i].getItem
						(factionIndexToUse));
					}
					...
					if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
						enabled)
						SystemFlags::OutputDebug(SystemFlags::debugSystem,
							"In [%s::%s Line %d] i = %d, listBoxFactions[i].getSelectedItem() [%s]\n",
							extractFileFromDirectoryPath
							(__FILE__).c_str(), __FUNCTION__,
							__LINE__, i,
		...
		for (int i = 0; i < GameConstants::maxPlayers; ++i) {
			if (listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
				...
				for (unsigned int j = 0; j < languageList.size(); ++j) {
					char szMsg[8096] = "";
					if (lang.hasString("NetworkSlotUnassignedError",
						languageList[j]) == true) {
						string msg_string =
							lang.getString("NetworkSlotUnassignedError");
#ifdef WIN32
						...
						strcpy(szMsg,
							"Cannot start game, some player(s) are not in a network game slot!");
					}
		...
		try {
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (scenarioLogoTexture != NULL) {
					renderer.renderTextureQuad(300, 350, 400, 300,
						scenarioLogoTexture, NULL);
				...
				for (int i = 0; i < GameConstants::maxPlayers; ++i) {
					...
					if (crcPlayerTextureCache[i] != NULL) {
						...
						Vec4f playerColor =
							crcPlayerTextureCache[i]->getPixmap()->getPixel4f(0,
								0);
				...
				if (checkBoxPublishServer.getEditable()) {
					...
					if (checkBoxAdvanced.getValue() == 1) {
						...
						(&checkBoxNetworkPauseGameForLaggedClients);
					}
				}
	...
	void MenuStateCustomGame::switchSetupForSlots(SwitchSetupRequest **
		switchSetupRequests,
		ServerInterface *
		&serverInterface,
		int startIndex,
			...
			if (switchSetupRequests[i] != NULL) {
				...
				if (listBoxControls[i].getSelectedItemIndex() == ctNetwork) {
					...
					if (switchSetupRequests[i]->getToSlotIndex() != -1) {
						...
						int switchFactionIdx =
							switchSetupRequests[i]->getCurrentSlotIndex();
						if (serverInterface->switchSlot
						(switchFactionIdx, newFactionIdx)) {
							try {
								...
								if (switchSetupRequests[i]->getNetworkPlayerName() != "") {
									...
									(SystemFlags::debugSystem).enabled)
										SystemFlags::OutputDebug(SystemFlags::debugSystem,
											"In [%s::%s Line %d] i = %d, labelPlayerNames[newFactionIdx].getText() [%s] switchSetupRequests[i]->getNetworkPlayerName() [%s]\n",
											extractFileFromDirectoryPath
											(__FILE__).c_str(),
											__FUNCTION__, __LINE__, i,
											labelPlayerNames
											[newFactionIdx].getText
											().c_str(),
											switchSetupRequests
						...
						try {
							...
							ServerInterface *serverInterface =
								NetworkManager::getInstance().getServerInterface();
							ConnectionSlot *slot =
								serverInterface->getSlot(factionIdx, true);
							...
							if (switchSetupRequests[i]->getToTeam() != -1) {
								listBoxTeams[i].setSelectedItemIndex(switchSetupRequests
									[i]->getToTeam());
							}
							...
							if ((switchSetupRequests[i]->getSwitchFlags() &
								...
								(SystemFlags::debugSystem).enabled)
									SystemFlags::OutputDebug(SystemFlags::debugSystem,
										"In [%s::%s Line: %d] i = %d, switchSetupRequests[i]->getSwitchFlags() = %d, switchSetupRequests[i]->getNetworkPlayerName() [%s], labelPlayerNames[i].getText() [%s]\n",
										extractFileFromDirectoryPath
										(__FILE__).c_str(),
										__FUNCTION__, __LINE__, i,
										switchSetupRequests
	...
	void MenuStateCustomGame::initFactionPreview(const GameSettings *
		gameSettings) {
		string factionVideoUrl = "";
		string factionVideoUrlFallback = "";

		string factionDefinitionXML =
			...
			if (currentFactionLogo != factionVideoUrl) {
				...
				if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false
					...
					SDL_Window *window =
						static_cast <
						ContextGl *
						>(c)->getPlatformContextGlPtr()->getScreenWindow();
					SDL_Surface *screen =
						static_cast <
						ContextGl *
						>(c)->getPlatformContextGlPtr()->getScreenSurface();
	...
	void MenuStateCustomGame::publishToMasterserver() {
		...
		publishToServerInfo["connectedClients"] =
			intToStr(slotCountConnectedPlayers);
		...
		publishToServerInfo["gameStatus"] =
			intToStr(game_status_waiting_for_players);
		if (slotCountHumans <= slotCountConnectedPlayers) {
			publishToServerInfo["gameStatus"] =
				intToStr(game_status_waiting_for_start);
		}
	...
	void MenuStateCustomGame::simpleTaskForMasterServer(BaseThread *
		callingThread) {
		try {
			...
			needToRepublishToMasterserver = false;
			std::map < string, string > newPublishToServerInfo =
				publishToServerInfo;
			publishToServerInfo.clear();
			...
			if (republish == true) {
				...
				MutexSafeWrapper
					safeMutexThreadOwner2(callingThread->getMutexThreadOwnerValid(),
						string(__FILE__) + "_" +
						intToStr(__LINE__));
				if (callingThread->getQuitStatus() == true
	...
	void MenuStateCustomGame::simpleTaskForClients(BaseThread *
		callingThread) {
		try {
			//printf("-=-=-=-=- IN MenuStateCustomGame simpleTask - A\n");
			...
			bool needPing =
				(difftime((long int) time(NULL), lastNetworkPing) >=
			...
			if (needPing == true) {
				...
				if (serverInterface != NULL) {
					...
					NetworkMessagePing *msg =
						new NetworkMessagePing(GameConstants::networkPingInterval,
							time(NULL));
	...
	void MenuStateCustomGame::loadGameSettings(GameSettings * gameSettings,
		...
		if (autoStartSettings != NULL) {
			...
			gameSettings->
				setDefaultResources(autoStartSettings->getDefaultResources());
			gameSettings->
		...
		if (Config::getInstance().getBool("EnableNetworkGameSynchChecks",
			"false") == true) {
		...
		if (Config::getInstance().getBool("EnableNetworkGameSynchMonitor",
			"false") == true) {
			//printf("*WARNING* - EnableNetworkGameSynchChecks is enabled\n");
			...
			if (ct != ctClosed) {
				...
				if (factionFiles[listBoxFactions[i].getSelectedItemIndex()] ==
					...
					if ((listBoxControls[i].getSelectedItemIndex() == ctCpuEasy
						...
						ctCpuUltra
						|| listBoxControls[i].getSelectedItemIndex() ==
						ctCpuGlest) && checkBoxScenario.getValue() == true) {
						...
						if (lastSelectedTeamIndex[i] >= 0
							...
							listBoxTeams[i].setSelectedItemIndex(lastSelectedTeamIndex
								[i]);
						} else {
					...
					if (serverInterface != NULL &&
						...
						gameSettings->setNetworkPlayerName(slotIndex,
							serverInterface->getSlot(i,
								true)->getName
								());
						gameSettings->setNetworkPlayerUUID(i,
							serverInterface->getSlot(i,
								true)->getUUID
								());
						gameSettings->setNetworkPlayerPlatform(i,
							serverInterface->getSlot
							(i,
								true)->getPlatform());
						labelPlayerNames[i].
							setText(serverInterface->getSlot(i, true)->getName());
					} else {
						...
						if (factionName != GameConstants::RANDOMFACTION_SLOTNAME
							...
							if (factionCRC == 0) {
								factionCRC =
									...
									(config.getPathListForType(ptTechs, ""),
										"/" + gameSettings->getTech() +
										"/factions/" + factionName + "/*", ".xml", NULL, true);
							}
		...
		if (gameSettings->getMap() != "") {
			...
			if (lastCheckedCRCMapName != gameSettings->getMap()) {
				Checksum checksum;
				string file =
					Config::getMapPath(gameSettings->getMap(), "", false);
					...
					if (serverInterface->getSlot(i, true) != NULL
						...
						if (clientConnectedTime == 0 ||
							(serverInterface->getSlot(i, true)->getConnectedTime() > 0
								&& serverInterface->getSlot(i,
									true)->getConnectedTime() <
								clientConnectedTime)) {
							clientConnectedTime =
								serverInterface->getSlot(i, true)->getConnectedTime();
							gameSettings->setMasterserver_admin(serverInterface->getSlot
							(i,
								true)->getSessionKey
								());
	...
	void MenuStateCustomGame::saveGameSettingsToFile(std::string fileName) {
		...
		GameSettings gameSettings;
		loadGameSettings(&gameSettings);
		CoreData::getInstance().saveGameSettingsToFile(fileName,
			&gameSettings,
	...
	void MenuStateCustomGame::KeepCurrentHumanPlayerSlots(GameSettings &
		gameSettings) {
		//look for human players
		...
		for (int index2 = 0; index2 < GameConstants::maxPlayers; ++index2) {
			ControlType ctFile =
				static_cast <ControlType>
				(gameSettings.getFactionControl(index2));
			if (ctFile == ctHuman) {
				ControlType ctUI =
					static_cast <ControlType>
		...
		for (int index = 0; index < GameConstants::maxPlayers; ++index) {
			...
			ControlType ctFile =
				static_cast <ControlType>
				(gameSettings.getFactionControl(index));
			if (ctFile == ctHuman) {
				gameSettings.setFactionControl(index, ctHuman);
			}
		}
	...
	GameSettings MenuStateCustomGame::loadGameSettingsFromFile(std::string
		fileName) {
		if (SystemFlags::
			getSystemSettingType(SystemFlags::debugSystem).enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
		...
		try {
			CoreData::getInstance().loadGameSettingsFromFile(fileName,
				&gameSettings);
	...
	void MenuStateCustomGame::setupUIFromGameSettings(const GameSettings &
		gameSettings) {
		...
		if (checkBoxScenario.getValue() == true) {
			...
			listBoxMapFilter.setSelectedItemIndex(0);
			listBoxMap.setItems(formattedPlayerSortedMaps[mapInfo.hardMaxPlayers]);
			...
			if (gameSettings.getMapFilter() == 0) {
				...
				listBoxMapFilter.setSelectedItem(intToStr
				(gameSettings.getMapFilter()));
			}
		...
		if (find(mapFiles.begin(), mapFiles.end(), mapFile) !=
			mapFiles.end()) {
			mapFile = formatString(mapFile);
			listBoxMap.setSelectedItem(mapFile);

			loadMapInfo(Config::getMapPath
			(getCurrentMapFile(), scenarioDir, true), &mapInfo,
				true);
			labelMapInfo.setText(mapInfo.desc);
		}
		...
		checkBoxAllowObservers.setValue(gameSettings.getAllowObservers() ==
			true ? true : false);
		...
		checkBoxEnableSwitchTeamMode.setValue((gameSettings.getFlagTypes1() &
			ft1_allow_team_switching) ==
			ft1_allow_team_switching ? true :
			false);
		...
		checkBoxAllowInGameJoinPlayer.setValue((gameSettings.getFlagTypes1() &
			...
			ft1_allow_in_game_joining ? true
			: false);

		checkBoxAllowTeamUnitSharing.setValue((gameSettings.getFlagTypes1() &
			ft1_allow_shared_team_units) ==
			ft1_allow_shared_team_units ?
			true : false);
		checkBoxAllowTeamResourceSharing.setValue((gameSettings.getFlagTypes1
		() &
		...
		checkBoxNetworkPauseGameForLaggedClients.
			setValue(gameSettings.getNetworkPauseGameForLaggedClients());
		...
		for (int i = 0; i < GameConstants::maxPlayers; ++i) {
			...
			listBoxRMultiplier[slotIndex].
				setSelectedItemIndex(gameSettings.getResourceMultiplierIndex(i));

			//if(slotIndex == 0) printf("#4 slotIndex = %d, i = %d, multiplier = %d\n",slotIndex,i,listBoxRMultiplier[i].getSelectedItemIndex());

			listBoxTeams[slotIndex].
				setSelectedItemIndex(gameSettings.getTeam(i));
			...
			if (SystemFlags::
				getSystemSettingType(SystemFlags::debugSystem).enabled)
				SystemFlags::OutputDebug(SystemFlags::debugSystem,
					"In [%s::%s Line: %d] i = %d, gameSettings.getNetworkPlayerName(i) [%s]\n",
					extractFileFromDirectoryPath
					(__FILE__).c_str(), __FUNCTION__,
					__LINE__, i,
					gameSettings.
	...
	void MenuStateCustomGame::loadMapInfo(string file, MapInfo * mapInfo,
		...
		try {
			...
			(file, mapInfo, lang.getString("MaxPlayers"),
				...
				// Though we prefer not to change the actual value of mapInfo->players,
				// which is the number of players assigned when making the map, we still know
			...
			if (checkBoxScenario.getValue() == true) {
				...
				for (int humanIndex = 0; humanIndex < 2; ++humanIndex) {
					for (int i = 0; i < mapInfo.players; ++i) {
						...
						listBoxControls[i].
							setSelectedItemIndex(scenarioInfo.factionControls[i]);
						if (SystemFlags::getSystemSettingType
						(SystemFlags::debugSystem).enabled)
							SystemFlags::OutputDebug(SystemFlags::debugSystem,
								"In [%s::%s Line %d]\n",
						...
						//2 humans
							...
							int closeSlotIndex =
								(humanIndex1 == i ? humanIndex2 : humanIndex1);
							int humanSlotIndex =
						...
						if (ct != ctClosed) {
							...
							listBoxFactions[i].setSelectedItem(formatString
				...
				setupMapList("");
				listBoxMap.setSelectedItem(formatString

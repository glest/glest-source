...
namespace Game {
	...
	MenuStateMasterserver::MenuStateMasterserver(Program * program,
		MainMenu *
		mainMenu) :MenuState
		(program, mainMenu, "masterserver"),
		mutexIRCClient(new Mutex(CODE_AT_LINE)) {
		...
		userButtonsYBase =
			serverLinesYBase - (serverLinesToRender) * serverLinesLineHeight - 90;
		...
		serverScrollBar.init(1000 - 20,
			serverLinesYBase -
			serverLinesLineHeight * (serverLinesToRender - 1),
			false, 200, 20);
		...
		lines[0].init(0,
			userButtonsYBase + userButtonsLineHeight +
			serverLinesLineHeight);
		lines[1].init(userButtonsXBase - 5, 0, 5,
			userButtonsYBase + 2 * userButtonsLineHeight);
		...
		// announcement
		announcementLabel.registerGraphicComponent(containerName,
			"announcementLabel");
		announcementLabel.init(10, 730);
		announcementLabel.setFont(CoreData::getInstance().getMenuFontBig());
		announcementLabel.setFont3D(CoreData::getInstance().
			getMenuFontBig3D());
		announcementLabel.setText("");

		// versionInfo
		versionInfoLabel.registerGraphicComponent(containerName,
			"versionInfoLabel");
		versionInfoLabel.init(10, 680);
		versionInfoLabel.setFont(CoreData::getInstance().getMenuFontBig());
		versionInfoLabel.setFont3D(CoreData::getInstance().
			getMenuFontBig3D());
		...
		buttonRefresh.registerGraphicComponent(containerName,
			"buttonRefresh");
		...
		labelAutoRefresh.registerGraphicComponent(containerName,
			"labelAutoRefresh");
		labelAutoRefresh.init(750, buttonPos + 30);

		listBoxAutoRefresh.registerGraphicComponent(containerName,
			"listBoxAutoRefresh");
		...
		ircOnlinePeopleLabel.registerGraphicComponent(containerName,
			"ircOnlinePeopleLabel");
		ircOnlinePeopleLabel.init(userButtonsXBase,
			userButtonsYBase + userButtonsLineHeight);
		ircOnlinePeopleLabel.setText(lang.getString("IRCPeopleOnline"));

		ircOnlinePeopleStatusLabel.registerGraphicComponent(containerName,
			"ircOnlinePeopleStatusLabel");
		ircOnlinePeopleStatusLabel.init(userButtonsXBase,
			userButtonsYBase +
			userButtonsLineHeight - 20);
		...
		snprintf(szIRCNick, 80, "G_%s_%d", ircname.c_str(), randomNickId);
		...
		chatManager.setFont3D(CoreData::getInstance().
			getMenuFontNormal3D());
		...
		if (netPlayerName == "newbie") {
			showMessageBox(lang.getString("PlayerNameNotSetPrompt"),
				lang.getString("PlayerNameNotSetTitle"), false);
		}
	...
	void MenuStateMasterserver::IRC_CallbackEvent(IRCEventType evt,
		const char *origin,
		const char **params,
		unsigned int count) {
		MutexSafeWrapper safeMutexIRCPtr(mutexIRCClient,
		...
		if (ircClient != NULL) {
			if (evt == IRC_evt_exitThread) {
				...
				char szBuf[8096] = "";
				snprintf(szBuf, 8096, "%s: %s", origin ? origin : "someone",
					params[1]);
	...
	void MenuStateMasterserver::cleanup() {
		...
		MutexSafeWrapper
			safeMutex((updateFromMasterserverThread !=
				NULL ? updateFromMasterserverThread->
		...
		if (updateFromMasterserverThread != NULL
			&& updateFromMasterserverThread->canShutdown(false) == true
		...
		else if (updateFromMasterserverThread != NULL
			&& updateFromMasterserverThread->canShutdown(true) == true) {
	...
	void MenuStateMasterserver::mouseClick(int x, int y,
		...
		if (mainMessageBox.getEnabled()) {
			...
			if (serverScrollBar.getElementCount() != 0) {
				for (int i = serverScrollBar.getVisibleStart();
					i <= serverScrollBar.getVisibleEnd(); ++i) {
					if (serverLines[i]->buttonMouseClick(x, y)) {
						...
						int connectServerPort =
							serverLines[i]->getMasterServerInfo()->
							getExternalConnectPort();
						bool connected =
						...
						if (connected) {
							...
							mainMenu->
								setState(new
									MenuStateConnectedGame(program, mainMenu,
										jmMasterserver));
						}
						break;
					}
			...
			if (!clicked && userScrollBar.getElementCount() != 0) {
				for (int i = userScrollBar.getVisibleStart();
					i <= userScrollBar.getVisibleEnd(); ++i) {
					if (userButtons[i]->getEnabled() == true
						&& userButtons[i]->mouseClick(x, y)) {
						//clicked=true;
	...
	void MenuStateMasterserver::mouseUp(int x, int y,
	...
	void MenuStateMasterserver::mouseMove(int x, int y,
		const MouseState * ms) {
		MutexSafeWrapper
			safeMutex((updateFromMasterserverThread !=
				NULL ? updateFromMasterserverThread->
	...
	void MenuStateMasterserver::render() {
		...
		if (mainMessageBox.getEnabled()) {
			...
			if (ircClient != NULL && ircClient->isConnected() == true
				...
				if (ircOnlinePeopleStatusLabel.getText() !=
					lang.getString("Connecting")) {
					ircOnlinePeopleStatusLabel.setText(lang.
						getString("Connecting"));
				}

				renderer.renderLabel(&ircOnlinePeopleLabel, &titleLabelColor);
				renderer.renderLabel(&ircOnlinePeopleStatusLabel,
					&titleLabelColor);
			}
			...
			for (int i = 0; i < (int) sizeof(lines) / (int) sizeof(lines[0]);
				++i) {
	...
	void MenuStateMasterserver::update() {
		...
		if (autoRefreshTime != 0
			&& difftime(time(NULL), lastRefreshTimer) >= autoRefreshTime) {
			needUpdateFromServer = true;
			lastRefreshTimer = time(NULL);
		}

		// calculate button linepos:
		setButtonLinePosition(serverLinesYBase -
			(serverLinesToRender) * serverLinesLineHeight -
			30);
		...
		if (ircClient != NULL) {
			...
			if (isNew) {
				...
				for (int i = 0; i < (int) nickList.size(); ++i) {
					...
					button->init(userButtonsXBase, userButtonsYBase,
						userButtonsWidth, userButtonsHeight);
					...
					if (strncmp(&nickList[i][0], "G_", 3) == 0) {
			...
			if (userScrollBar.getElementCount() != 0) {
				for (int i = userScrollBar.getVisibleStart();
					i <= userScrollBar.getVisibleEnd(); ++i) {
					userButtons[i]->setY(userButtonsYBase -
						userButtonsLineHeight * (i -
							userScrollBar.
							getVisibleStart
							()));
				}
		...
		if (serverScrollBar.getElementCount() != 0) {
			for (int i = serverScrollBar.getVisibleStart();
				i <= serverScrollBar.getVisibleEnd(); ++i) {
				serverLines[i]->setY(serverLinesYBase -
					serverLinesLineHeight * (i -
						serverScrollBar.
						getVisibleStart()));
			}
		}
	...
	void MenuStateMasterserver::simpleTask(BaseThread * callingThread,
		...
		MutexSafeWrapper safeMutex(callingThread->
			getMutexThreadObjectAccessor(),
			string(extractFileFromDirectoryPath
			(__FILE__).c_str()) + "_" +
			intToStr(__LINE__));
			...
			try {
				...
				try {
					if (Config::getInstance().getString("Masterserver", "") != "") {
						...
						std::string localServerInfoString =
							SystemFlags::getHTTP(baseURL + "showServersForGlest.php" +
								playerUUID, handle);
					...
					SystemFlags::OutputDebug(SystemFlags::debugError,
						"In [%s::%s Line %d] error during Internet game status update: [%s]\n",
						extractFileFromDirectoryPath(__FILE__).
						c_str(), __FUNCTION__, __LINE__,
						ex.what());
				}
			...
			if (serverInfo != "") {
				...
				for (int i = 0; i < (int) serverList.size(); i++) {
					...
					if ((int) serverEntities.size() >= MIN_FIELDS_EXPECTED) {
						...
						if (Config::getInstance().getString("Masterserver", "") == "") {
							labelTitle.setText("*** " +
								lang.getString("AvailableServers"));
						}
						...
						masterServerInfo->
							setNetworkSlots(strToInt(serverEntities[9]));
						masterServerInfo->
							setConnectedClients(strToInt(serverEntities[10]));
						masterServerInfo->
							setExternalConnectPort(strToInt(serverEntities[11]));
						...
						//printf("Ping time = %f\n",pingTime);
						char szBuf[8096] = "";
						snprintf(szBuf, 8096, "%s",
							masterServerInfo->getServerTitle().c_str());
						...
						serverLines.
							push_back(new
								ServerLine(masterServerInfo, i, serverLinesYBase,
									serverLinesLineHeight, containerName));
						...
						labelTitle.setText("*** " +
							lang.getString("AvailableServers") + "[" +
							intToStr(serverEntities.size()) + "][" +
							intToStr(MIN_FIELDS_EXPECTED) + "] [" +
							serverInfo + "]");

						if (masterserverParseErrorShown == false) {
							masterserverParseErrorShown = true;
							SystemFlags::OutputDebug(SystemFlags::debugError,
								"In [%s::%s Line %d] error, no masterserver defined!\n",
								extractFileFromDirectoryPath
								(__FILE__).c_str(), __FUNCTION__,
								__LINE__);
						}
	...
	bool MenuStateMasterserver::connectToServer(string ipString, int port) {
		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s] START  ipString='%s'\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__, ipString.c_str());
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s] try to connect to [%s] serverPort = %d\n",
				extractFileFromDirectoryPath(__FILE__).
				c_str(), __FUNCTION__,
				serverIp.getString().c_str(), serverPort);
		...
		if (clientInterface->isConnected() == false) {
			...
			Lang & lang = Lang::getInstance();
			showMessageBox(lang.getString("CouldNotConnect"),
				lang.getString("ConnectionFailed"), false);
			return false;
	...
	void MenuStateMasterserver::showMessageBox(const string & text,
		const string & header,
		bool toggle) {
		if (!toggle) {
			mainMessageBox.setEnabled(false);
	...
	void MenuStateMasterserver::keyDown(SDL_KeyboardEvent key) {
		...
		if (chatManager.getEditEnabled() == false) {
			...
			//else if(key == configKeys.getCharKey("SaveGUILayout")) {
				...
				Lang & lang = Lang::getInstance();
				consoleIRC.addLine(lang.getString("GUILayoutSaved") + " [" +
					(saved ? lang.getString("Yes") : lang.
						getString("No")) + "]");
			}
		}
	}

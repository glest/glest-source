	...
	namespace PlatformCommon {
		...
		void event_join(irc_session_t * session, const char * event, const char * origin, const char ** params, unsigned int count) {
			...
			if (ctx != NULL) {
				...
				if (ctx->getHasJoinedChannel() == false) {
					...
					irc_cmd_msg(session, "#glest-lobby", announceJoin);
					*/
						...
						if (count >= 4) {
							for (unsigned int i = 3; i < count && params[i]; ++i) {
								...
								for (unsigned int j = 0; j < tokens.size(); ++j) {
									...
									// Only show Glest users in the user list
									//if(strncmp(&realNick[0],"G_",3) == 0) {
									nickList.push_back(realNick);
									//}
								}
							}
		...
		void IRCThread::signalQuit() {
			...
			if (validSession == true) {
				...
				MutexSafeWrapper safeMutex1(&mutexIRCSession, string(__FILE__) + "_" + intToStr(__LINE__));
				if (ircSession != NULL) {
					irc_cmd_quit(ircSession, "Glest Bot is closing!");
				}
				safeMutex1.ReleaseLock();
				hasJoinedChannel = false;
			}
...
#if !defined(DISABLE_IRCCLIENT)
				MutexSafeWrapper safeMutex1(&mutexIRCSession, string(__FILE__) + "_" + intToStr(__LINE__));
				if (irc_connect(ircSession, argv[0].c_str(), IRC_SERVER_PORT, 0, this->nick.c_str(), this->username.c_str(), "glest")) {
					safeMutex1.ReleaseLock();

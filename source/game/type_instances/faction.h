...
namespace Game {
	...
	public:
		...
		void notifyUnitSkillTypeChange(const Unit * unit,
			const SkillType * newType);
		bool hasAliveUnits(bool filterMobileUnits,
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).
				...
				if (worldSynchThreadedLogList.empty() == false) {
					for (unsigned int index = 0;
						index < worldSynchThreadedLogList.size(); ++index) {
						SystemFlags::OutputDebug(SystemFlags::debugWorldSynch,
							worldSynchThreadedLogList[index].
							c_str());
					}
					worldSynchThreadedLogList.clear();
				}
		...
		inline void addLivingUnits(int id) {
			livingUnits.insert(id);
		}
		inline void addLivingUnitsp(Unit * unit) {
			livingUnitsp.insert(unit);
		}

		inline bool isUnitInLivingUnitsp(Unit * unit) {
			return (livingUnitsp.find(unit) != livingUnitsp.end());
		}
		inline void deleteLivingUnits(int id) {
			livingUnits.erase(id);
		}
		inline void deleteLivingUnitsp(Unit * unit) {
			livingUnitsp.erase(unit);
		}
		...
		void init(FactionType * factionType, ControlType control,
			TechTree * techTree, Game * game, int factionIndex,
			int teamIndex, int startLocationIndex, bool thisFaction,
		...
		inline const Resource *getResource(int i) const {
			return &resources[i];
		}
		int getStoreAmount(const ResourceType * rt, bool localFactionOnly =
			false) const;
		inline const FactionType *getType() const {
			return factionType;
		}
		...
		inline TechTree *getTechTree() const {
			return techTree;
		}
		...
		inline bool getCpuEasyControl() const {
			return control == ctCpuEasy;
		}
		inline bool getCpuUltraControl() const {
			return control == ctCpuUltra;
		}
		inline bool getCpuGlestControl() const {
			return control == ctCpuGlest;
		}
		...
		inline int getStartLocationIndex() const {
			return startLocationIndex;
		}
		inline bool getThisFaction() const {
			return thisFaction;
		}
		...
		Vec2i getClosestResourceTypeTargetFromCache(const Vec2i & pos,
			const ResourceType * type);
		void cleanupResourceTypeTargetCache(std::vector < Vec2i >
			*deleteListPtr, int frameIndex);
		...
		bool canCreateUnit(const UnitType * ut, bool checkBuild,
		...
		std::pair < int,
			string > getCRC_DetailsForWorldFrameIndex(int worldFrameIndex) const;

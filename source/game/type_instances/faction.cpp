...
namespace Game {
	bool CommandGroupUnitSorterId::operator () (const int l, const int r) {
		...
		if (!lUnit) {
			...
			for (unsigned int i = 0; i < (unsigned int) faction->getUnitCount();
				++i) {
				printf("%u / %d id = %d [%s]\n", i, faction->getUnitCount(),
					faction->getUnit(i)->getId(),
					faction->getUnit(i)->getType()->getName(false).c_str());
			}
		}
		if (!rUnit) {
	...
	bool CommandGroupUnitSorter::compare(const Unit * l, const Unit * r) {
		...
		if (l == NULL || r == NULL)
			...
			(l !=
				NULL ? l->getType()->getName(false).c_str() : "null"),
				(l != NULL ? l->getId() : -1),
				(r !=
					NULL ? r->getType()->getName(false).c_str() : "null"),
					(r != NULL ? r->getId() : -1));
		...
		if (command != NULL && command->getCommandType() != NULL &&
			...
			// If comparer command group id is less than current group id
				...
				float unitDistPeer =
					r->getCenteredPos().dist(commandPeer->getPos());
	...
	void Faction::sortUnitsByCommandGroups() {
		MutexSafeWrapper safeMutex(unitsMutex,
			string(__FILE__) + "_" +
			intToStr(__LINE__));
	...
	//              }
		...
		for (unsigned int i = 0; i < units.size(); ++i) {
			...
			if (this->findUnit(unitId) == NULL) {
				printf
				("#1 Error unitId not found for id = %d [%s] factionIndex = %d\n",
					unitId, units[i]->getType()->getName(false).c_str(),
					this->getIndex());

				for (unsigned int j = 0; j < units.size(); ++j) {
					printf("%u / %d id = %d [%s]\n", j, (int) units.size(),
						units[j]->getId(),
						units[j]->getType()->getName(false).c_str());
				}
			}
	...
	bool FactionThread::isSignalPathfinderCompleted(int frameIndex) {
		...
		bool result = (this->frameIndex.first == frameIndex
			&& this->frameIndex.second == true);
	...
	void FactionThread::execute() {
		...
		try {
			...
			for (; this->faction != NULL;) {
				...
				if (executeTask == true) {
					...
					static string mutexOwnerId2 =
						string(__FILE__) + string("_") + intToStr(__LINE__);
					MutexSafeWrapper safeMutex(faction->getUnitMutex(),
						mutexOwnerId2);

					//if(SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled) chrono.start();
					if (minorDebugPerformance)
						chrono.start();
					...
					for (int j = 0; j < unitCount; ++j) {
						...
						int64 elapsed1 = 0;
						if (minorDebugPerformance)
							elapsed1 = chrono.getMillis();

						bool update = unit->needToUpdate();

						codeLocation = "12";
						if (minorDebugPerformance
							&& (chrono.getMillis() - elapsed1) >= 1)
							printf
							("Faction [%d - %s] #1-unit threaded updates on frame: %d for [%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
								faction->getStartLocationIndex(),
								faction->getType()->getName(false).c_str(),
								currentTriggeredFrameIndex,
								faction->getUnitPathfindingListCount(), j, unitCount,
								(long long int) chrono.getMillis() - elapsed1);
						...
						if (update == true) {
							...
							if (SystemFlags::
								...
								int64 speed =
									unit->getCurrSkill()->getTotalSpeed(unit->
										getTotalUpgrade());
								...
								char szBuf[8096] = "";
								snprintf(szBuf, 8096,
									"unit->needToUpdate() returned: %d updateProgressValue: %lld speed: %lld changedActiveCommand: %d df: %lld hf: %lld",
									update, (long long int) updateProgressValue,
									(long long int) speed, changedActiveCommand,
									(long long int) df, (long long int) hf);
								unit->logSynchDataThreaded(__FILE__, __LINE__, szBuf);
							}

							int64 elapsed2 = 0;
							if (minorDebugPerformance)
								elapsed2 = chrono.getMillis();
							...
							world->getUnitUpdater()->updateUnitCommand(unit,
								currentTriggeredFrameIndex);

							codeLocation = "15";
							if (minorDebugPerformance
								&& (chrono.getMillis() - elapsed2) >= 1)
								printf
								("Faction [%d - %s] #2-unit threaded updates on frame: %d for [%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
									...
									currentTriggeredFrameIndex,
									faction->getUnitPathfindingListCount(), j, unitCount,
									(long long int) chrono.getMillis() - elapsed2);
						} else {
							codeLocation = "16";
							if (SystemFlags::
								getSystemSettingType(SystemFlags::debugWorldSynch).
					...
					codeLocation = "17";
					if (minorDebugPerformance && chrono.getMillis() >= 1)
						printf
						("Faction [%d - %s] threaded updates on frame: %d for [%d] units took [%lld] msecs\n",
							faction->getStartLocationIndex(),
							faction->getType()->getName(false).c_str(),
							currentTriggeredFrameIndex,
							faction->getUnitPathfindingListCount(),
			...
			SystemFlags::OutputDebug(SystemFlags::debugError,
				"In [%s::%s Line: %d] Loc [%s] Error [%s]\n",
				__FILE__, __FUNCTION__, __LINE__,
				codeLocation.c_str(), ex.what());
			if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
				enabled)
				SystemFlags::OutputDebug(SystemFlags::debugSystem,
					"In [%s::%s Line: %d]\n", __FILE__,
	...
	void Faction::notifyUnitTypeChange(const Unit * unit,
	...
	void Faction::notifyUnitSkillTypeChange(const Unit * unit,
		const SkillType * newType) {
		if (unit != NULL) {
	...
	bool Faction::hasAliveUnits(bool filterMobileUnits,
		bool filterBuiltUnits) const {
		bool result = false;
	...
	bool Faction::canUnitsPathfind() {
		...
		if (control == ctCpuEasy || control == ctCpu ||
			control == ctCpuUltra || control == ctCpuGlest) {
			...
			result =
				(getUnitPathfindingListCount() <= MAX_UNITS_PATHFINDING_PER_FRAME);
			if (result == false) {
	...
	void Faction::init(FactionType * factionType, ControlType control,
		TechTree * techTree, Game * game, int factionIndex,
		int teamIndex, int startLocationIndex,
		...
		if (loadWorldNode == NULL) {
			for (int index = 0; index < techTree->getResourceTypeCount();
				...
				int resourceAmount =
					giveResources ? factionType->getStartingResourceAmount(rt) : 0;
		...
		if (texture) {
			string playerTexture =
				getGameCustomCoreDataPath(data_path,
					...
					intToStr(startLocationIndex) + ".tga");
			texture->load(playerTexture);
		}

		if (loadWorldNode != NULL) {
			loadGame(loadWorldNode, this->index, game->getGameSettings(),
				game->getWorld());
		}
	...
	bool Faction::hasUnitTypeWithResourceCostInCache(const ResourceType * rt) const {
		...
		std::map < std::string, bool >::const_iterator iterFind =
			resourceTypeCostCache.find(resourceTypeName);
		if (iterFind != resourceTypeCostCache.end()) {
			return iterFind->second;
		}
		return false;
	}
	void Faction::updateUnitTypeWithResourceCostCache(const ResourceType *
		rt) {
		std::string resourceTypeName = rt->getName(false);

		if (resourceTypeCostCache.find(resourceTypeName) ==
			resourceTypeCostCache.end()) {
			resourceTypeCostCache[resourceTypeName] =
				hasUnitTypeWithResouceCost(rt);
		}
	}
	...
	const Resource *Faction::getResource(const ResourceType * rt,
		bool localFactionOnly) const {

		if (localFactionOnly == false &&
			world != NULL && world->getGame() != NULL) {

			Game *game = world->getGame();
			if (game->isFlagType1BitEnabled(ft1_allow_shared_team_resources) ==
				true) {
		...
		for (int i = 0; i < (int) resources.size(); ++i) {
			printf("Index %d [%s]", i,
				resources[i].getType()->getName().c_str());
		}
	...
	int Faction::getStoreAmount(const ResourceType * rt,
		bool localFactionOnly) const {
	...
	void Faction::finishUpgrade(const UpgradeType * ut) {
		...
		if (world->getThisFaction() != NULL
			&& this->getIndex() == world->getThisFaction()->getIndex()) {
			Console *console = world->getGame()->getConsole();
			console->addStdMessage("UpgradeFinished",
				": " + formatString(ut->getName(true)));
		}
	...
	bool Faction::applyCosts(const ProducibleType * p,
		...
		if (ct != NULL && ct->getClass() == ccMorph) {
			const MorphCommandType *mct =
				dynamic_cast <const MorphCommandType *>(ct);
			if (mct != NULL) {
	...
	void Faction::applyCostsOnInterval(const ResourceType * rtApply) {
		...
		for (int j = 0; j < getUnitCount(); ++j) {
			...
			if (unit->isOperative() == true) {
				for (int k = 0; k < unit->getType()->getCostCount(); ++k) {
					...
					if (resource->getType() == rtApply
						&& resource->getType()->getClass() == rcConsumable
						&& resource->getAmount() != 0) {
						if (resourceIntervalUsage.find(resource->getType()) ==
						...
						resourceIntervalUsage[resource->getType()].first +=
							-resource->getAmount();

						// If the cost > 0 then the unit is a consumer
						if (resource->getAmount() > 0) {
							resourceIntervalUsage[resource->getType()].second.
								push_back(unit);
						}
					}
				}
		...
		if (resourceIntervalUsage.empty() == false) {
			for (std::map < const ResourceType *, std::pair < int,
				std::vector < Unit * > > >::iterator iter =
				resourceIntervalUsage.begin();
				iter != resourceIntervalUsage.end(); ++iter) {
				...
				if (getResource(rt)->getAmount() < 0) {
					...
					for (int i = 0; i < (int) resourceConsumers.size(); ++i) {
						...
						//decrease unit hp
						if (scriptManager->getPlayerModifiers(this->index)->
							getConsumeEnabled() == true) {
							bool decHpResult =
								unit->decHp(unit->getType()->
									getTotalMaxHp(unit->getTotalUpgrade()) / 3);
							if (decHpResult) {
							...
							StaticSound *sound =
								static_cast <
								const DieSkillType *
								>(unit->getType()->getFirstStOfClass(scDie))->getSound();
							if (sound != NULL
								&& (thisFaction == true
									|| world->showWorldForPlayer(world->
										getThisTeamIndex()) ==
									true)) {
	...
	void Faction::incResourceAmount(const ResourceType * rt, int amount) {
		if (world != NULL && world->getGame() != NULL
			&& world->getGame()->
			isFlagType1BitEnabled(ft1_allow_shared_team_resources) == true) {
			for (int i = 0; i < (int) resources.size(); ++i) {
				Resource *r = &resources[i];
				if (r->getType() == rt) {
					r->setAmount(r->getAmount() + amount);
					if (r->getType()->getClass() != rcStatic
						&& (getResource(rt, false)->getAmount() + amount) >
						getStoreAmount(rt, false)) {
						r->setAmount(getStoreAmount(rt, false) -
							(getResource(rt, false)->getAmount() -
								r->getAmount()));
					}
					return;
				}
	...
	void Faction::addStore(const UnitType * unitType) {
		...
		for (int newUnitStoredResourceIndex = 0;
			...
			const Resource *newUnitStoredResource =
				unitType->getStoredResource(newUnitStoredResourceIndex);
			...
			for (int currentStoredResourceIndex = 0;
				...
				if (storedResource->getType() == newUnitStoredResource->getType()) {
					storedResource->setAmount(storedResource->getAmount() +
						newUnitStoredResource->getAmount());
				}
			}
		}
	}
	...
	void Faction::limitResourcesToStore() {
		if (world != NULL && world->getGame() != NULL
			...
			for (int i = 0; i < (int) resources.size(); ++i) {
				...
				const ResourceType *rt = r->getType();
				if (rt->getClass() != rcStatic
					&& (getResource(rt, false)->getAmount()) > getStoreAmount(rt,
						false)) {
					r->setAmount(getStoreAmount(rt, false) -
						(getResource(rt, false)->getAmount() -
							r->getAmount()));
	...
	bool Faction::isResourceTargetInCache(const Vec2i & pos,
		bool incrementUseCounter) {
		bool result = false;

		if (cachingDisabled == false) {
			if (cacheResourceTargetList.empty() == false) {
				std::map < Vec2i, int >::iterator iter =
					cacheResourceTargetList.find(pos);
	...
	void Faction::addResourceTargetToCache(const Vec2i & pos,
		...
		if (cachingDisabled == false) {
			...
			bool duplicateEntry =
				isResourceTargetInCache(pos, incrementUseCounter);
	...
	void Faction::addCloseResourceTargetToCache(const Vec2i & pos) {
		if (cachingDisabled == false) {
			if (cachedCloseResourceTargetLookupList.find(pos) ==
				cachedCloseResourceTargetLookupList.end()) {
					...
					for (int k = -harvestDistance; k <= harvestDistance; ++k) {
						...
						if (isResourceTargetInCache(newPos) == false) {
							if (map->isInside(newPos.x, newPos.y)) {
								Resource *r =
									map->getSurfaceCell(map->toSurfCoords(newPos))->
									getResource();
								if (r != NULL) {
	...
	Vec2i Faction::getClosestResourceTypeTargetFromCache(Unit * unit,
		const ResourceType *
		type,
		int frameIndex) {
		Vec2i result(-1);
		...
		//                      }
				...
				//int tryRadius = 0;
				if (tryRadius == 0) {
					for (int j = -harvestDistance;
						j <= harvestDistance && foundCloseResource == false; ++j) {
						for (int k = -harvestDistance;
							k <= harvestDistance && foundCloseResource == false; ++k) {
							Vec2i newPos = pos + Vec2i(j, k);
							if (map->isInside(newPos) == true
								&& isResourceTargetInCache(newPos) == false) {
								const SurfaceCell *sc =
									map->getSurfaceCell(map->toSurfCoords(newPos));
								if (sc != NULL && sc->getResource() != NULL) {
									...
									if (resource->getType() != NULL
										...
										if (result.x < 0
											|| unit->getPos().dist(newPos) <
											unit->getPos().dist(result)) {
				...
				if (foundCloseResource == false) {
					...
					for (std::map < Vec2i, int >::iterator iter =
						cacheResourceTargetList.begin();
						iter != cacheResourceTargetList.end()
						&& foundCloseResource == false; ++iter) {
						const Vec2i & cache = iter->first;
						if (map->isInside(cache) == true) {
							const SurfaceCell *sc =
								map->getSurfaceCell(map->toSurfCoords(cache));
							if (sc != NULL && sc->getResource() != NULL) {
								const Resource *resource = sc->getResource();
								if (resource->getType() != NULL
									&& resource->getType() == type) {
									if (result.x < 0
										|| unit->getPos().dist(cache) <
										unit->getPos().dist(result)) {
										if (unit->isBadHarvestPos(cache) == false) {
											result = cache;
											// Close enough to our position, no more looking
											if (unit->getPos().dist(result) <=
												(harvestDistance * 2)) {
												foundCloseResource = true;
												break;
											}
	...
	Vec2i Faction::getClosestResourceTypeTargetFromCache(const Vec2i & pos,
		const ResourceType *
		type) {
		Vec2i result(-1);
		if (cachingDisabled == false) {
							...
							if (map->isInside(newPos) == true
								...
								if (sc != NULL && sc->getResource() != NULL) {
									...
									if (resource->getType() != NULL
										&& resource->getType() == type) {
										if (result.x < 0
											|| pos.dist(newPos) < pos.dist(result)) {
											result = newPos;
											foundCloseResource = true;
											break;
										}
	...
	void Faction::cleanupResourceTypeTargetCache(std::vector < Vec2i >
		*deleteListPtr,
		int frameIndex) {
		if (cachingDisabled == false) {
			if (cacheResourceTargetList.empty() == false) {
				...
				if (deleteListPtr != NULL || needToCleanup == true) {
					...
					if (deleteListPtr != NULL) {
						...
						for (std::map < Vec2i, int >::iterator iter =
							...
							if (world->getMap()->
								getSurfaceCell(world->getMap()->toSurfCoords(cache)) !=
								NULL) {
								Resource *resource =
									world->getMap()->getSurfaceCell(world->getMap()->
										toSurfCoords(cache))->
									getResource();
								if (resource == NULL) {
									deleteList.push_back(cache);
	...
	const SwitchTeamVote *Faction::getFirstSwitchTeamVote() const {
		...
		if (switchTeamVotes.empty() == false) {
			for (std::map < int, SwitchTeamVote >::const_iterator iterMap =
				switchTeamVotes.begin(); iterMap != switchTeamVotes.end();
				++iterMap) {
	...
	bool Faction::canCreateUnit(const UnitType * ut, bool checkBuild,
		bool checkProduce, bool checkMorph) const {
		...
		for (int l = 0; l < this->getUnitCount() && foundUnit == false; ++l) {
			...
			for (int j = 0;
				j < unitType2->getCommandTypeCount() && foundUnit == false; ++j) {
	...
	uint64 Faction::getCacheKBytes(uint64 * cache1Size, uint64 * cache2Size,
		...
		for (std::map < Vec2i, int >::iterator iterMap1 =
			cacheResourceTargetList.begin();
			iterMap1 != cacheResourceTargetList.end(); ++iterMap1) {
			cache1Count++;
		}
		for (std::map < Vec2i, bool >::iterator iterMap1 =
			cachedCloseResourceTargetLookupList.begin();
			iterMap1 != cachedCloseResourceTargetLookupList.end(); ++iterMap1) {
			cache2Count++;
		}
		for (std::map < int, const Unit * >::iterator iterMap1 =
			aliveUnitListCache.begin(); iterMap1 != aliveUnitListCache.end();
			++iterMap1) {
			cache3Count++;
		}
		for (std::map < int, const Unit * >::iterator iterMap1 =
			mobileUnitListCache.begin();
			iterMap1 != mobileUnitListCache.end(); ++iterMap1) {
			cache4Count++;
		}
		for (std::map < int, const Unit * >::iterator iterMap1 =
			beingBuiltUnitListCache.begin();
	...
	string Faction::getCacheStats() {
		...
		char szBuf[8096] = "";
		snprintf(szBuf, 8096, "cache1Count [%d] cache2Count [%d] total KB: %s",
			cache1Count, cache2Count, formatNumber(totalBytes).c_str());
		result = szBuf;
		return result;
	}
	...
	std::string Faction::toString(bool crcMode) const {
		...
		result +=
			"startLocationIndex = " + intToStr(this->startLocationIndex) + "\n";
		if (crcMode == false) {
		...
		for (int idx = 0; idx < (int) resources.size(); idx++) {
			result +=
				"index = " + intToStr(idx) + " " + resources[idx].toString() +
				"\n";
		}
		...
		for (int idx = 0; idx < (int) store.size(); idx++) {
			result +=
				"index = " + intToStr(idx) + " " + store[idx].toString() + "\n";
		}

		result += "Allies = " + intToStr(allies.size()) + "\n";
		for (int idx = 0; idx < (int) allies.size(); idx++) {
			result +=
				"index = " + intToStr(idx) + " name: " +
				allies[idx]->factionType->getName(false) + " factionindex = " +
				intToStr(allies[idx]->index) + "\n";
		}
	...
	void Faction::saveGame(XmlNode * rootNode) {
		...
		for (unsigned int i = 0; i < allies.size(); ++i) {
			...
			allyNode->addAttribute("allyFactionIndex",
				intToStr(ally->getIndex()),
				mapTagReplacements);
		}
		for (unsigned int i = 0; i < units.size(); ++i) {
			Unit *unit = units[i];
		...
		factionNode->addAttribute("control", intToStr(control),
			mapTagReplacements);

		factionNode->addAttribute("overridePersonalityType",
			intToStr(overridePersonalityType),
			mapTagReplacements);
		factionNode->addAttribute("factiontype", factionType->getName(false),
			mapTagReplacements);
		factionNode->addAttribute("index", intToStr(index),
			mapTagReplacements);
		factionNode->addAttribute("teamIndex", intToStr(teamIndex),
			mapTagReplacements);
		factionNode->addAttribute("startLocationIndex",
			intToStr(startLocationIndex),
			mapTagReplacements);
		factionNode->addAttribute("thisFaction", intToStr(thisFaction),
			mapTagReplacements);

		for (std::map < Vec2i, int >::iterator iterMap =
			cacheResourceTargetList.begin();
			iterMap != cacheResourceTargetList.end(); ++iterMap) {
			XmlNode *cacheResourceTargetListNode =
				factionNode->addChild("cacheResourceTargetList");

			cacheResourceTargetListNode->addAttribute("key",
				iterMap->first.
				getString(),
				mapTagReplacements);
			cacheResourceTargetListNode->addAttribute("value",
				intToStr(iterMap->second),
				mapTagReplacements);
		}

		for (std::map < Vec2i, bool >::iterator iterMap =
			cachedCloseResourceTargetLookupList.begin();
			iterMap != cachedCloseResourceTargetLookupList.end(); ++iterMap) {
			XmlNode *cachedCloseResourceTargetLookupListNode =
				factionNode->addChild("cachedCloseResourceTargetLookupList");

			cachedCloseResourceTargetLookupListNode->addAttribute("key",
				iterMap->first.
				getString(),
				mapTagReplacements);
			cachedCloseResourceTargetLookupListNode->addAttribute("value",
				intToStr
				(iterMap->
					second),
				mapTagReplacements);
		...
		factionNode->addAttribute("random", intToStr(random.getLastNumber()),
			mapTagReplacements);
		factionNode->addAttribute("currentSwitchTeamVoteFactionIndex",
			intToStr(currentSwitchTeamVoteFactionIndex),
			mapTagReplacements);
		factionNode->addAttribute("allowSharedTeamUnits",
			intToStr(allowSharedTeamUnits),
			mapTagReplacements);

		for (std::set < const UnitType * >::iterator iterMap =
			lockedUnits.begin(); iterMap != lockedUnits.end(); ++iterMap) {
			XmlNode *lockedUnitsListNode =
				factionNode->addChild("lockedUnitList");
			const UnitType *ut = *iterMap;

			lockedUnitsListNode->addAttribute("value", ut->getName(false),
				mapTagReplacements);
		}

		for (std::map < int, int >::iterator iterMap = unitsMovingList.begin();
			iterMap != unitsMovingList.end(); ++iterMap) {
			XmlNode *unitsMovingListNode =
				factionNode->addChild("unitsMovingList");

			unitsMovingListNode->addAttribute("key", intToStr(iterMap->first),
				mapTagReplacements);
			unitsMovingListNode->addAttribute("value",
				intToStr(iterMap->second),
				mapTagReplacements);
		}

		for (std::map < int, int >::iterator iterMap =
			unitsPathfindingList.begin();
			iterMap != unitsPathfindingList.end(); ++iterMap) {
			XmlNode *unitsPathfindingListNode =
				factionNode->addChild("unitsPathfindingList");

			unitsPathfindingListNode->addAttribute("key",
				intToStr(iterMap->first),
				mapTagReplacements);
			unitsPathfindingListNode->addAttribute("value",
				intToStr(iterMap->second),
				mapTagReplacements);
		}
	}
	...
	void Faction::loadGame(const XmlNode * rootNode, int factionIndex,
		...
		vector < XmlNode * >factionNodeList =
			rootNode->getChildList("Faction");
		...
		if (factionNode != NULL) {
			...
			for (unsigned int i = 0; i < allyNodeList.size(); ++i) {
				...
				int allyFactionIndex =
					allyNode->getAttribute("allyFactionIndex")->getIntValue();
			...
			control =
				...
				(factionNode->getAttribute("control")->getIntValue());

			if (factionNode->hasAttribute("overridePersonalityType") == true) {
				overridePersonalityType =
					static_cast <FactionPersonalityType>
					(factionNode->getAttribute("overridePersonalityType")->
						getIntValue());
			}
			...
			startLocationIndex =
				factionNode->getAttribute("startLocationIndex")->getIntValue();

			thisFaction =
				factionNode->getAttribute("thisFaction")->getIntValue() != 0;

			if (factionNode->hasAttribute("allowSharedTeamUnits") == true) {
				allowSharedTeamUnits =
					factionNode->getAttribute("allowSharedTeamUnits")->
					getIntValue() != 0;
			}

			vector < XmlNode * >cacheResourceTargetListNodeList =
				factionNode->getChildList("cacheResourceTargetList");
			for (unsigned int i = 0; i < cacheResourceTargetListNodeList.size();
				++i) {
				XmlNode *cacheResourceTargetListNode =
					cacheResourceTargetListNodeList[i];

				Vec2i vec =
					Vec2i::strToVec2(cacheResourceTargetListNode->
						getAttribute("key")->getValue());
				cacheResourceTargetList[vec] =
					cacheResourceTargetListNode->getAttribute("value")->
					getIntValue();
			}
			vector < XmlNode * >cachedCloseResourceTargetLookupListNodeList =
				factionNode->getChildList("cachedCloseResourceTargetLookupList");
			for (unsigned int i = 0;
				i < cachedCloseResourceTargetLookupListNodeList.size(); ++i) {
				XmlNode *cachedCloseResourceTargetLookupListNode =
					cachedCloseResourceTargetLookupListNodeList[i];

				Vec2i vec =
					Vec2i::strToVec2(cachedCloseResourceTargetLookupListNode->
						getAttribute("key")->getValue());
				cachedCloseResourceTargetLookupList[vec] =
					cachedCloseResourceTargetLookupListNode->getAttribute("value")->
					getIntValue() != 0;
			}

			random.setLastNumber(factionNode->getAttribute("random")->
				getIntValue());

			vector < XmlNode * >lockedUnitsListNodeList =
				factionNode->getChildList("lockedUnitList");
			for (unsigned int i = 0; i < lockedUnitsListNodeList.size(); ++i) {
				XmlNode *lockedUnitsListNode = lockedUnitsListNodeList[i];

				string unitName =
					lockedUnitsListNode->getAttribute("value")->getValue();
				lockedUnits.insert(getType()->getUnitType(unitName));
			}

			vector < XmlNode * >unitsMovingListNodeList =
				factionNode->getChildList("unitsMovingList");
			for (unsigned int i = 0; i < unitsMovingListNodeList.size(); ++i) {
				XmlNode *unitsMovingListNode = unitsMovingListNodeList[i];

				int unitId =
					unitsMovingListNode->getAttribute("key")->getIntValue();
				unitsMovingList[unitId] =
					unitsMovingListNode->getAttribute("value")->getIntValue();
			}
			vector < XmlNode * >unitsPathfindingListNodeList =
				factionNode->getChildList("unitsPathfindingList");
			for (unsigned int i = 0; i < unitsPathfindingListNodeList.size();
				++i) {
				XmlNode *unitsPathfindingListNode = unitsPathfindingListNodeList[i];

				int unitId =
					unitsPathfindingListNode->getAttribute("key")->getIntValue();
				unitsPathfindingList[unitId] =
					unitsPathfindingListNode->getAttribute("value")->getIntValue();
			}
		}
	}
		...
		if (consoleDebug) {
			if (getWorld()->getFrameCount() % 40 == 0) {
				printf("#1 Frame #: %d Faction: %d CRC: %u\n",
					getWorld()->getFrameCount(), index,
					crcForFaction.getSum());
			}
		}
	...
	void Faction::addCRC_DetailsForWorldFrame(int worldFrameCount,
		...
		if ((unsigned int) crcWorldFrameDetails.size() > MAX_FRAME_CACHE) {
			...
			for (;
				(unsigned int) crcWorldFrameDetails.size() - MAX_FRAME_CACHE >
				0;) {
	...
	std::pair < int,
		string >
		Faction::getCRC_DetailsForWorldFrameIndex(int worldFrameIndex) const {
		if (crcWorldFrameDetails.empty()) {
			return make_pair < int, string >(0, "");
		}
		std::map < int, string >::const_iterator iterMap =
			crcWorldFrameDetails.begin();
	...
	string Faction::getCRC_DetailsForWorldFrames() const {
		string result = "";
		for (std::map < int, string >::const_iterator iterMap =
			crcWorldFrameDetails.begin();
			...
			result +=
				...
				("============================================================================\n");
			result +=
				string("** world frame: ") + intToStr(iterMap->first) +
				string(" detail: ") + iterMap->second;
		}
		return result;
	}

...
namespace Game {
	...
	static const string CORE_MISC_TEXTURES_PATH =
		CORE_PATH + "misc_textures/";

	static const string CORE_MENU_PATH = CORE_PATH + "menu/";
	static const string CORE_MENU_TEXTURES_PATH =
		CORE_MENU_PATH + "textures/";
	...
	void CoreData::loadTextureIfRequired(Texture2D ** tex, string data_path,
		...
		int texSystemId, bool setMipMap,
		bool setAlpha,
				...
				if (*tex) {
					...
					try {
						...
						if (loadUniqueFilePath == false) {
							...
							fileToLoad =
								getGameCustomCoreDataPath(data_path, uniqueFilePath);
						}
	...
	Texture2D *CoreData::getTeamColorTexture() {
		...
		loadTextureIfRequired(&teamColorTexture, data_path,
			...
			"team_color_texture.tga", tsyst_teamColorTexture,
			true, true, false);
	...
	Texture2D *CoreData::getLogoTexture() {
		...
		loadTextureIfRequired(&logoTexture, data_path,
			CORE_MENU_TEXTURES_PATH + "logo.png",
	...
	Texture2D *CoreData::getButtonSmallTexture() {
		...
		loadTextureIfRequired(&buttonSmallTexture, data_path,
			...
			tsyst_buttonSmallTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getButtonBigTexture() {
		...
		loadTextureIfRequired(&buttonBigTexture, data_path,
			...
			tsyst_buttonBigTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getHorizontalLineTexture() {
		...
		loadTextureIfRequired(&horizontalLineTexture, data_path,
			...
			tsyst_horizontalLineTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getVerticalLineTexture() {
		...
		loadTextureIfRequired(&verticalLineTexture, data_path,
			...
			tsyst_verticalLineTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getCheckedCheckBoxTexture() {
		...
		loadTextureIfRequired(&checkedCheckBoxTexture, data_path,
			...
			tsyst_checkedCheckBoxTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getOnServerDifferentTexture() {
		...
		loadTextureIfRequired(&onServerDifferentTexture, data_path,
			...
			tsyst_onServerDifferentTexture, true, false,
			false);
	...
	Texture2D *CoreData::getOnServerInstalledTexture() {
		...
		loadTextureIfRequired(&onServerInstalledTexture, data_path,
			...
			tsyst_onServerInstalledTexture, true, false,
			false);
	...
	Texture2D *CoreData::getGameWinnerTexture() {
		...
		loadTextureIfRequired(&gameWinnerTexture, data_path,
			...
			tsyst_gameWinnerTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getHealthbarTexture() {
		...
		loadTextureIfRequired(&healthbarTexture, data_path,
			...
			tsyst_healthbarTexture, true, false, false,
			true);
	...
	Texture2D *CoreData::getHealthbarBackgroundTexture() {
		...
		loadTextureIfRequired(&healthbarBackgroundTexture, data_path,
			CORE_MISC_TEXTURES_PATH +
			"healthbarBackground.png",
			tsyst_healthbarBackgroundTexture, true, false,
			false, true);
	...
	void CoreData::loadLogoTextureExtraIfRequired() {
		...
		if (itemLoadAttempted.find(loadAttemptLookupKey) ==
			...
			for (int index = 0; index < (int) logoFilenames.size(); ++index) {
				...
				if (strcmp("logo.png", logo.c_str()) != 0) {
			...
			if (logoTextureList.empty() == true) {
				...
				for (int index = 0; index < (int) logoFilenames.size(); ++index) {
					...
					if (strcmp("logo.png", logo.c_str()) != 0) {
						...
						Texture2D *logoTextureExtra = NULL;
						loadTextureIfRequired(&logoTextureExtra, data_path,
							data_path + CORE_MENU_TEXTURES_PATH +
							logo, tsyst_NONE, true, false, true);
						logoTextureList.push_back(logoTextureExtra);
					}
				}
			}
		...
		if (itemLoadAttempted.find(loadAttemptLookupKey) ==
			...
			for (int index = 0; index < 6; ++index) {
				...
				if (waterSounds[index] != NULL) {
					try {
						waterSounds[index]->load(getGameCustomCoreDataPath(data_path,
							CORE_WATER_SOUNDS_PATH
							+ "water" +
							intToStr
							(index) +
							".wav"));
	...
	void CoreData::loadMusicIfRequired() {
		...
		if (itemLoadAttempted.find(loadAttemptLookupKey) ==
			...
			xmlTree.
				...
				(data_path, CORE_MENU_PATH + "menu.xml"),
			...
			if (menuNode->hasChild("intro") == true) {
				...
				// intro info
				const XmlNode *menuPathNode =
					introNode->getChild("menu-music-path");
				menuMusicPath =
			...
			try {
				introMusic.open(getGameCustomCoreDataPath(data_path,
					CORE_PATH +
					menuMusicPath +
					menuIntroMusicFile));
				introMusic.setNext(&menuMusic);
				menuMusic.open(getGameCustomCoreDataPath(data_path,
					CORE_PATH +
					menuMusicPath +
					menuMusicFile));
	...
	void CoreData::loadIntroMedia(string data_path) {
		...
		introVideoFilenameFallback =
			config.getString("IntroVideoURLFallback", "");
	...
	void CoreData::loadBattleEndMedia(string data_path) {
		...
		if (battleEndWinVideoFilename == "") {
			string battleEndWinVideoPath =
				getGameCustomCoreDataPath(data_path,
					"") + CORE_MENU_VIDEOS_PATH +
				"battle_end_win.*";
		...
		battleEndWinMusicFilename =
			config.getString("BattleEndWinMusicFilename", "");
	...
	void CoreData::triggerFontChangedCallbacks(std::string fontUniqueId,
		...
		for (std::map < std::string,
			std::vector <
			FontChangedCallbackInterface * > >::const_iterator iterMap =
	...
	void CoreData::loadFonts() {
		...
		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			...
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				...
				displayFontName.c_str(), displayFontSize);
		...
		if (lang.hasString("FontDisplaySmallBaseSize") == true) {
			displayFontNameSmallSize =
				computeFontSize(strToInt
				(lang.getString("FontDisplaySmallBaseSize")));
		}
		string displayFontNameSmall =
			displayFontNameSmallPrefix + intToStr(displayFontNameSmallSize) +
			displayFontNameSmallPostfix;

		displayFontSmall =
			loadFont < Font2D >(displayFontSmall, displayFontNameSmall,
				displayFontNameSmallSize, "FontSmallDisplay",
				"FontSmallDisplayFamily", "displayFontSmall");
		displayFontSmall3D =
			loadFont < Font3D >(displayFontSmall3D, displayFontNameSmall,
				displayFontNameSmallSize, "FontSmallDisplay",
				"FontSmallDisplayFamily", "displayFontSmall3D");

		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] displayFontSmallName = [%s] displayFontSmallNameSize = %d\n",
				__FILE__, __FUNCTION__, __LINE__,
		...
		if (lang.hasString("FontMenuNormalBaseSize") == true) {
			menuFontNameNormalSize =
				computeFontSize(strToInt
				(lang.getString("FontMenuNormalBaseSize")));
		}
		string menuFontNameNormal =
			menuFontNameNormalPrefix + intToStr(menuFontNameNormalSize) +
			menuFontNameNormalPostfix;

		menuFontNormal =
			loadFont < Font2D >(menuFontNormal, menuFontNameNormal,
				menuFontNameNormalSize, "FontMenuNormal",
				"FontMenuNormalFamily", "menuFontNormal");
		menuFontNormal3D =
			loadFont < Font3D >(menuFontNormal3D, menuFontNameNormal,
				menuFontNameNormalSize, "FontMenuNormal",
				"FontMenuNormalFamily", "menuFontNormal3D");

		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] menuFontNormalName = [%s] menuFontNormalNameSize = %d\n",
				__FILE__, __FUNCTION__, __LINE__,
		...
		string menuFontNameBig =
			menuFontNameBigPrefix + intToStr(menuFontNameBigSize) +
			menuFontNameBigPostfix;

		menuFontBig =
			loadFont < Font2D >(menuFontBig, menuFontNameBig,
				menuFontNameBigSize, "FontMenuBig",
				"FontMenuBigFamily", "menuFontBig");
		menuFontBig3D =
			loadFont < Font3D >(menuFontBig3D, menuFontNameBig,
				menuFontNameBigSize, "FontMenuBig",
				"FontMenuBigFamily", "menuFontBig3D");

		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] menuFontNameBig = [%s] menuFontNameBigSize = %d\n",
				__FILE__, __FUNCTION__, __LINE__,
				menuFontNameBig.c_str(),
				menuFontNameBigSize);
		...
		if (lang.hasString("FontMenuVeryBigBaseSize") == true) {
			menuFontNameVeryBigSize =
				computeFontSize(strToInt
				(lang.getString("FontMenuVeryBigBaseSize")));
		}
		string menuFontNameVeryBig =
			menuFontNameVeryBigPrefix + intToStr(menuFontNameVeryBigSize) +
			menuFontNameVeryBigPostfix;

		menuFontVeryBig =
			loadFont < Font2D >(menuFontVeryBig, menuFontNameVeryBig,
				menuFontNameVeryBigSize, "FontMenuVeryBig",
				"FontMenuVeryBigFamily", "menuFontVeryBig");
		menuFontVeryBig3D =
			loadFont < Font3D >(menuFontVeryBig3D, menuFontNameVeryBig,
				menuFontNameVeryBigSize, "FontMenuVeryBig",
				"FontMenuVeryBigFamily", "menuFontVeryBig3D");

		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] menuFontNameVeryBig = [%s] menuFontNameVeryBigSize = %d\n",
				__FILE__, __FUNCTION__, __LINE__,
		...
		string consoleFontName =
			consoleFontNamePrefix + intToStr(consoleFontNameSize) +
			consoleFontNamePostfix;

		consoleFont =
			loadFont < Font2D >(consoleFont, consoleFontName,
				consoleFontNameSize, "FontConsole",
				"FontConsoleFamily", "consoleFont");
		consoleFont3D =
			loadFont < Font3D >(consoleFont3D, consoleFontName,
				consoleFontNameSize, "FontConsole",
				"FontConsoleFamily", "consoleFont3D");

		if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
			enabled)
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] consoleFontName = [%s] consoleFontNameSize = %d\n",
				__FILE__, __FUNCTION__, __LINE__,
				consoleFontName.c_str(),
				consoleFontNameSize);
	}


	template < typename T > T * CoreData::loadFont(Font * menuFont,
		string menuFontName,
		int menuFontNameSize,
		string fontType,
	...
	void CoreData::saveGameSettingsToFile(std::string fileName,
		...
		saveGameFile << "Description=" << gameSettings->
			getDescription() << std::endl;
		saveGameFile << "MapFilterIndex=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getMapFilter()) << std::endl;
		...
		saveGameFile << "DefaultUnits=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getDefaultUnits()) << std::endl;
		saveGameFile << "DefaultResources=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getDefaultResources()) << std::endl;
		saveGameFile << "DefaultVictoryConditions=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
				getDefaultVictoryConditions()) <<
			std::endl;
		saveGameFile << "FogOfWar=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getFogOfWar()) << std::endl;
		saveGameFile << "AdvancedIndex=" << Shared::PlatformByteOrder::
			toCommonEndian(advancedIndex) << std::endl;
		saveGameFile << "AllowObservers=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getAllowObservers()) << std::endl;
		saveGameFile << "FlagTypes1=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getFlagTypes1()) << std::endl;
		saveGameFile << "EnableObserverModeAtEndGame=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
				getEnableObserverModeAtEndGame())
			<< std::endl;
		saveGameFile << "AiAcceptSwitchTeamPercentChance=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
		...
		saveGameFile << "EnableServerControlledAI=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
				getEnableServerControlledAI()) <<
			std::endl;
		saveGameFile << "NetworkFramePeriod=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getNetworkFramePeriod()) << std::endl;
		saveGameFile << "NetworkPauseGameForLaggedClients=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
				getNetworkPauseGameForLaggedClients
				()) << std::endl;

		saveGameFile << "FactionThisFactionIndex=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
				getThisFactionIndex()) << std::
			endl;
		saveGameFile << "FactionCount=" << Shared::PlatformByteOrder::
			toCommonEndian(gameSettings->getFactionCount()) << std::endl;

		saveGameFile << "NetworkAllowNativeLanguageTechtree=" << Shared::
			PlatformByteOrder::toCommonEndian(gameSettings->
		...
		for (int i = 0; i < GameConstants::maxPlayers; ++i) {
			...
			saveGameFile << "FactionControlForIndex" << Shared::
				PlatformByteOrder::toCommonEndian(slotIndex) << "=" << Shared::
				PlatformByteOrder::toCommonEndian(gameSettings->
					getFactionControl(i)) << std::
				endl;
			saveGameFile << "ResourceMultiplierIndex" << Shared::
				PlatformByteOrder::toCommonEndian(slotIndex) << "=" << Shared::
				PlatformByteOrder::toCommonEndian(gameSettings->
					getResourceMultiplierIndex(i))
				<< std::endl;
			saveGameFile << "FactionTeamForIndex" << Shared::PlatformByteOrder::
				toCommonEndian(slotIndex) << "=" << Shared::PlatformByteOrder::
				toCommonEndian(gameSettings->getTeam(i)) << std::endl;
			saveGameFile << "FactionStartLocationForIndex" << Shared::
				PlatformByteOrder::toCommonEndian(slotIndex) << "=" << Shared::
				PlatformByteOrder::toCommonEndian(gameSettings->
					getStartLocationIndex(i)) <<
				std::endl;
			saveGameFile << "FactionTypeNameForIndex" << Shared::
				PlatformByteOrder::
				toCommonEndian(slotIndex) << "=" << gameSettings->
				getFactionTypeName(i) << std::endl;
			saveGameFile << "FactionPlayerNameForIndex" << Shared::
				PlatformByteOrder::
				toCommonEndian(slotIndex) << "=" << gameSettings->
				getNetworkPlayerName(i) << std::endl;

			saveGameFile << "FactionPlayerUUIDForIndex" << Shared::
				PlatformByteOrder::
				toCommonEndian(slotIndex) << "=" << gameSettings->
				getNetworkPlayerUUID(i) << std::endl;
		}
	...
	bool CoreData::loadGameSettingsFromFile(std::string fileName,
		...
		gameSettings->setDefaultResources(properties.
			getBool("DefaultResources"));
		gameSettings->setDefaultVictoryConditions(properties.
			getBool
		...
		gameSettings->setAllowObservers(properties.
			getBool("AllowObservers", "false"));
		gameSettings->setFlagTypes1(properties.getInt("FlagTypes1", "0"));

		uint32 valueFlags1 = gameSettings->getFlagTypes1();
		if (Config::getInstance().
			getBool("EnableNetworkGameSynchChecks", "false") == true) {
		...
		if (Config::getInstance().
			getBool("EnableNetworkGameSynchMonitor", "false") == true) {
			//printf("*WARNING* - EnableNetworkGameSynchChecks is enabled\n");
		...
		gameSettings->setEnableObserverModeAtEndGame(properties.
			getBool
			("EnableObserverModeAtEndGame"));
		gameSettings->setAiAcceptSwitchTeamPercentChance(properties.
			getInt
		...
		gameSettings->setPathFinderType(static_cast <PathFinderType>
			(properties.
				getInt("PathFinderType",
					intToStr(pfBasic).
					c_str())));
		gameSettings->setEnableServerControlledAI(properties.
			getBool
			("EnableServerControlledAI",
				"true"));
		gameSettings->setNetworkFramePeriod(properties.
			getInt("NetworkFramePeriod",
		...
		if (properties.hasString("NetworkAllowNativeLanguageTechtree") == true) {
			gameSettings->setNetworkAllowNativeLanguageTechtree(properties.
				getBool
		...
		for (int i = 0; i < GameConstants::maxPlayers; ++i) {
			gameSettings->setFactionControl(i,
				(ControlType) properties.
				getInt(string
				("FactionControlForIndex") +
					intToStr(i),
					intToStr(ctClosed).
					c_str()));
			...
			gameSettings->setTeam(i,
				...
				getInt(string("FactionTeamForIndex") +
					intToStr(i), "0"));
			gameSettings->setStartLocationIndex(i,
				properties.
				getInt(string
				("FactionStartLocationForIndex")
					+ intToStr(i),
					intToStr(i).c_str()));
			gameSettings->setFactionTypeName(i,
				properties.
				getString(string
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
				...
				SystemFlags::OutputDebug(SystemFlags::debugSystem,
					...
					gameSettings->getFactionTypeName(i).
					c_str());

...
namespace Game {
	...
	const float GameConstants::glestMultiplier = 2.5f;
	...
	const char *GameConstants::preCacheThreadCacheLookupKey =
		"preCacheThreadCache";
	const char *GameConstants::ircClientCacheLookupKey = "ircClientCache";
	const char *GameConstants::playerTextureCacheLookupKey =
		"playerTextureCache";
	const char *GameConstants::factionPreviewTextureCacheLookupKey =
		"factionPreviewTextureCache";
	const char *GameConstants::characterMenuScreenPositionListCacheLookupKey =
		"characterMenuScreenPositionListCache";
	const char *GameConstants::application_name = "Glest";

	const char *GameConstants::LOADING_SCREEN_FILE = "loading_screen";
	const char *GameConstants::LOADING_SCREEN_FILE_FILTER =
		"loading_screen*.*";
	const char *GameConstants::PREVIEW_SCREEN_FILE = "preview_screen";
	const char *GameConstants::PREVIEW_SCREEN_FILE_FILTER =
		"preview_screen*.*";
	...
	const char *GameConstants::saveNetworkGameFileServer =
		"glest-saved-server.xml";
	const char *GameConstants::saveNetworkGameFileServerCompressed =
		"glest-saved-server.zip";

	const char *GameConstants::saveNetworkGameFileClient =
		"glest-saved-client.xml";
	const char *GameConstants::saveNetworkGameFileClientCompressed =
		"glest-saved-client.zip";

	const char *GameConstants::saveGameFileDefault = "glest-saved.xml";
	const char *GameConstants::saveGameFileAutoTestDefault =
		"glest-auto-saved_%s.xml";
	const char *GameConstants::saveGameFilePattern = "glest-saved_%s.xml";
	...
	bool Config::tryCustomPath(std::pair < ConfigType, ConfigType > &type,
		std::pair < string, string > &file,
		string custom_path) {
		bool wasFound = false;
		if ((type.first == cfgMainGame && type.second == cfgUserGame &&
			file.first == glest_ini_filename
			&& file.second == glestuser_ini_filename)
	...
	Config::Config(std::pair < ConfigType, ConfigType > type,
		...
		if (getGameReadWritePath(GameConstants::path_ini_CacheLookupKey) != "") {
			fileName.first =
				getGameReadWritePath(GameConstants::path_ini_CacheLookupKey) +
				fileName.first;
			fileName.second =
				getGameReadWritePath(GameConstants::path_ini_CacheLookupKey) +
				fileName.second;
		...
		try {
			if (fileName.second != "" && fileExists(fileName.second) == false) {
				...
				userFile.open(fileName.second.c_str(),
			...
			if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
				enabled)
				SystemFlags::OutputDebug(SystemFlags::debugSystem,
					"In [%s::%s Line: %d] ERROR trying to auto-create cfgFile.second = [%s]\n",
					__FILE__, __FUNCTION__, __LINE__,
					fileName.second.c_str());
		}
	}
	...
	Config & Config::getInstance(std::pair < ConfigType, ConfigType > type,
		...
		if (configList.find(type.first) == configList.end()) {
			...
			configList.insert(map < ConfigType,
				Config >::value_type(type.first, config));
	...
	void Config::save(const string & path) {
		if (fileLoaded.second == true) {
			...
			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("In [%s::%s Line: %d] save file [%s]\n", __FILE__,
					__FUNCTION__, __LINE__, fileName.second.c_str());
		...
		if (SystemFlags::VERBOSE_MODE_ENABLED)
			printf("In [%s::%s Line: %d] save file [%s]\n", __FILE__,
				__FUNCTION__, __LINE__, fileName.first.c_str());
	...
	void Config::setUserProperties(const vector < pair < string,
		string > >&valueList) {
	...
	vector < string > Config::getPathListForType(PathType type,
		...
		if (userData != "") {
			...
			if (isdir(userData.c_str()) == false) {
				...
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userData.c_str());
			}
			...
			if (isdir(userDataMaps.c_str()) == false) {
				...
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userDataMaps.c_str());
			}
			string userDataScenarios =
				userData + GameConstants::folder_path_scenarios;
			if (isdir(userDataScenarios.c_str()) == false) {
				createDirectoryPaths(userDataScenarios);
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userDataScenarios.c_str());
			}
			...
			if (isdir(userDataTechs.c_str()) == false) {
				...
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userDataTechs.c_str());
			}
			string userDataTilesets =
				userData + GameConstants::folder_path_tilesets;
			if (isdir(userDataTilesets.c_str()) == false) {
				createDirectoryPaths(userDataTilesets);
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userDataTilesets.c_str());
			}
			string userDataTutorials =
				userData + GameConstants::folder_path_tutorials;
			if (isdir(userDataTutorials.c_str()) == false) {
				createDirectoryPaths(userDataTutorials);
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] creating path [%s]\n", __FILE__,
						__FUNCTION__, __LINE__, userDataTutorials.c_str());
			}
		}
		if (scenarioDir != "") {
		...
		switch (type) {
			case ptMaps:
				...
				if (userData != "") {
					pathList.push_back(userData +
						string(GameConstants::folder_path_maps));
				}
				break;
			case ptScenarios:
				...
				if (userData != "") {
					pathList.push_back(userData +
						string(GameConstants::folder_path_scenarios));
				}
				break;
			case ptTechs:
				...
				if (userData != "") {
					pathList.push_back(userData +
						string(GameConstants::folder_path_techs));
				}
				break;
			case ptTilesets:
				...
				if (userData != "") {
					pathList.push_back(userData +
						string(GameConstants::folder_path_tilesets));
				}
				break;
			case ptTutorials:
				...
				if (userData != "") {
					pathList.push_back(userData +
						string(GameConstants::folder_path_tutorials));
				}
				break;
		}
	...
	bool Config::replaceFileWithLocalFile(const vector < string > &dirList,
		string fileNamePart,
	...
	// static
	string Config::getMapPath(const string & mapName, string scenarioDir,
		...
		for (int idx = 0; idx < (int) pathList.size(); idx++) {
			...
			const string newFormat = map_path + mapName + ".mgm";
			const string oldFormat = map_path + mapName + ".gbm";
			if (fileExists(newFormat)) {
				return newFormat;
			} else if (fileExists(oldFormat)) {
				return oldFormat;
			}
		}

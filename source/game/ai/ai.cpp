...
namespace Game {
	...
	void
		...
		if (faction->
			getAIBehaviorStaticOverideValue(aibsvcMinMinWarriorsExpandCpuGlest)
			!= INT_MAX) {
			minMinWarriorsExpandCpuGlest =
				faction->
				getAIBehaviorStaticOverideValue
				(aibsvcMinMinWarriorsExpandCpuGlest);
			//printf("Discovered overriden static value for AI, minMinWarriorsExpandCpuGlest = %d\n",minMinWarriorsExpandCpuGlest);
		}
		if (faction->
	...
	void
		...
		for (int i = 0; i < unitCount; ++i) {
			...
			if (aiInterface->getControlType() == ctCpuGlest) {
			...
			if (alreadyAttacking == false && act != NULL
				...
				// Glest CPU does not send ( far away ) units which are currently producing something
				if (aiInterface->getControlType() == ctCpuGlest) {
					if (!isWarrior) {
		...
		} else if (aiInterface->getControlType() == ctCpuGlest) {
			minWarriors += minMinWarriorsExpandCpuGlest;
	...
	void
		...
		aiNode->addAttribute("minMinWarriorsExpandCpuEasy",
			...
			mapTagReplacements);
		//      int minMinWarriorsExpandCpuGlest;
		aiNode->addAttribute("minMinWarriorsExpandCpuGlest",
			intToStr(minMinWarriorsExpandCpuGlest),
			mapTagReplacements);
	...
	void
		...
		//      int minMinWarriorsExpandCpGlest;
		minMinWarriorsExpandCpuGlest =
			aiNode->getAttribute("minMinWarriorsExpandCpuGlest")->getIntValue();
		//      int minMinWarriorsExpandCpuUltra;
		minMinWarriorsExpandCpuUltra =

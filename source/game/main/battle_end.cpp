...
namespace Game {
	...
	BattleEnd::BattleEnd(Program * program, const Stats * stats,
		...
		if (originState != NULL) {
			Game *
				game = dynamic_cast <Game *>(originState);
			if (game != NULL) {
		...
		int
			buttonWidth = 125;
		int
			xLocation = (metrics.getVirtualW() / 2) - (buttonWidth / 2);
	...
	BattleEnd::~BattleEnd() {
		...
		if (CoreData::getInstance().hasMainMenuVideoFilename() == false) {
			SoundRenderer::getInstance().
				playMusic(CoreData::getInstance().getMenuMusic());
		}
	...
	std::pair < string, string > BattleEnd::getBattleEndVideo(bool won) {
		...
		if (gameSettings != NULL) {
			...
			string
				...
				gameSettings->getFactionTypeName(stats.getThisFactionIndex());

			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("#1 tech [%s] faction [%s] won = %d\n",
					currentTechName_factionPreview.c_str(),
					currentFactionName_factionPreview.c_str(), won);
			...
			if (factionDefinitionXML != ""
				...
				if (won == true) {
					if (factionNode->hasAttribute("battle-end-win-video") == true) {
						factionVideoUrl =
							factionNode->
							getAttribute("battle-end-win-video")->getValue();
					}
				} else {
					if (factionNode->hasAttribute("battle-end-lose-video") == true) {
						factionVideoUrl =
							factionNode->
							getAttribute("battle-end-lose-video")->getValue();
					}
				}
				...
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("#3 factionVideoUrl [%s] factionVideoUrlFallback [%s]\n",
						factionVideoUrl.c_str(),
						factionVideoUrlFallback.c_str());
		...
		if (factionVideoUrl != "") {
			...
			result.first =
				CoreData::getInstance().getBattleEndVideoFilename(won);
			result.second =
				CoreData::getInstance().getBattleEndVideoFilenameFallback(won);
		}
	...
	void
		BattleEnd::initBackgroundMusic() {
		string music = "";

		if (stats.getThisFactionIndex() >= 0
			&& stats.getTeam(stats.getThisFactionIndex()) !=
		...
		if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false
			...
			if (stats.getThisFactionIndex() >= 0
				...
				if (stats.getVictory(stats.getThisFactionIndex())) {
					...
					std::pair < string, string > lostVideos =
						getBattleEndVideo(false);
			...
			if (fileExists(videoFile) || fileExists(videoFileFallback)) {
				...
				menuBackgroundVideo = new VideoPlayer(&Renderer::getInstance(),
					videoFile,
					videoFileFallback,
					window,
					0, 0,
	...
	void
		...
		if (menuBackgroundVideo == NULL && renderToTexture != NULL) {
			...
			TextRenderer2D *
				textRenderer2D = renderer.getTextRenderer();
			TextRenderer3D *
				textRenderer3D = renderer.getTextRenderer3D();
			TextRenderer *
				textRenderer = NULL;
			...
			if (menuBackgroundVideo != NULL) {
				...
				renderer.
					renderBackground(CoreData::
						getInstance().getBackgroundTexture());
			}
			...
			for (int i = 0; i < stats.getFactionCount(); ++i) {
				...
				int
					enemykills = stats.getEnemyKills(i);
				...
				int
					unitsProduced = stats.getUnitsProduced(i);
				...
				int
					resourcesHarvested = stats.getResourcesHarvested(i);
			...
			bool
				disableStatsColorCoding =
				Config::getInstance().getBool("DisableBattleEndColorCoding",
					"false");

			if (Renderer::renderText3DEnabled == true) {
				textRenderer3D->
					begin(CoreData::getInstance().getMenuFontNormal3D());
			} else {
				textRenderer2D->
					begin(CoreData::getInstance().getMenuFontNormal());
			}
			...
			for (int i = 0; i < stats.getFactionCount(); ++i) {
				...
				realPlayerCount++;
				int
					textX = lm + 60 + (realPlayerCount * 100);
				int
					team = stats.getTeam(i) + 1;
				int
					kills = stats.getKills(i);
				int
					enemykills = stats.getEnemyKills(i);
				...
				if (stats.getPlayerName(i) != "") {
					...
					textRenderer->render(textToRender.c_str(), textX, bm + 400,
						false, &color);
				} else {
					textRenderer->render((lang.getString("Player") + " " +
						intToStr(i + 1)).c_str(), textX,
						bm + 400, false, &color);
				}
				...
				if (stats.getPersonalityType(i) == fpt_Observer) {
					textRenderer->render(lang.getString("GameOver").c_str(), textX,
						bm + 360);
				} else {
					if (stats.getVictory(i)) {
						textRenderer->render(stats.
							getVictory(i) ?
							lang.getString("Victory").
							c_str() : lang.getString("Defeat").
							c_str(), textX, bm + 360, false,
							&highliteColor);
					} else {
						textRenderer->render(stats.
							getVictory(i) ?
				...
				textRenderer->render(stats.getFactionTypeName(i), textX,
					bm + 280);
				...
				if (enemykills == bestEnemyKills) {
					textRenderer->render(intToStr(enemykills).c_str(), textX,
						bm + 180, false, &highliteColor);
				} else {
					textRenderer->render(intToStr(enemykills).c_str(), textX,
						bm + 180);
				}
				if (deaths == leastDeaths) {
					...
					textRenderer->render(intToStr(deaths).c_str(), textX,
						bm + 160);
				}
				if (unitsProduced == bestUnitsProduced) {
					textRenderer->render(intToStr(unitsProduced).c_str(), textX,
						bm + 120, false, &highliteColor);
				} else {
					textRenderer->render(intToStr(unitsProduced).c_str(), textX,
						bm + 120);
				}
				if (resourcesHarvested == bestResourcesHarvested) {
					textRenderer->render(intToStr(resourcesHarvested).c_str(),
						textX, bm + 80, false, &highliteColor);
				} else {
					textRenderer->render(intToStr(resourcesHarvested).c_str(),
						textX, bm + 80);
				}
			...
			textRenderer->render(lang.getString("ResourcesHarvested"), lm,
				bm + 80);
			...
			if (Renderer::renderText3DEnabled == true) {
				textRenderer3D->
					begin(CoreData::getInstance().getMenuFontVeryBig3D());
			} else {
				textRenderer2D->
					begin(CoreData::getInstance().getMenuFontVeryBig());
			}
			...
			header2 =
				lang.getString("GameMaxConcurrentUnitCount") + ": " +
				intToStr(stats.getMaxConcurrentUnitCount());
			textRenderer->render(header2, lm + 250, bm + 510);

			header2 =
				lang.getString("GameTotalEndGameConcurrentUnitCount") + ": " +
	...
	void
		BattleEnd::mouseDownLeft(int x, int y) {
		SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s %d]\n",
			__FILE__, __FUNCTION__, __LINE__);
		//program->setState(new MainMenu(program));
	...
	void
		BattleEnd::mouseMove(int x, int y, const MouseState * ms) {
		mouseX = x;
		mouseY = y;
	...
	void
		BattleEnd::showMessageBox(const string & text, const string & header,
			bool toggle) {
		if (toggle == false) {
			mainMessageBox.setEnabled(false);
		}

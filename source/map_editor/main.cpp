...
namespace MapEditor {
	const string MainWindow::winHeader = string("Glest Map Editor ") + MapEditorVersionString;
	...
	bool App::OnInit() {
		...
		if (argc == 2) {
			if (argv[1][0] == '-') {   // any flag gives help and exits program.
				std::cout << std::endl << "Glest Map Editor " << MapEditorVersionString << " [Using " << (const char *) wxConvCurrent->cWX2MB(wxVERSION_STRING) << "]" << std::endl << std::endl;
				std::cout << "Creates or edits Glest maps. [.mgm]" << std::endl << std::endl;

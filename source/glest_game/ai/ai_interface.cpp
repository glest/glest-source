      ...
      if (executeCommandOverNetwork () == true)
      {
        ...
        result =
          commander->tryGiveCommand (unit,
                                     unit->getType ()->getFirstCtOfClass
                                         ...
                                         int unitGroupCommandId)
    {
      ...
      if (unit == NULL)
      {
        ...
        snprintf (szBuf, 8096,
                  ...
                  __FILE__, __FUNCTION__, __LINE__, unitIndex, factionIndex);
        throw megaglest_runtime_error (szBuf);
      }
      const UnitType *unitType = unit->getType ();
      if (unitType == NULL)
...
// ==================== get data ====================
    ...
    {
      return world->getMap ()->getStartLocation (world->getFaction
                                                 (factionIndex)->
...
//returns if there is a resource next to a unit, in "resourcePos" is stored the relative position of the resource
    bool
      AiInterface::isResourceNear (const Vec2i & pos,
                                   const ResourceType * rt,
                                   Vec2i & resourcePos,
                                   Faction * faction,
                                   bool
    ...
    bool
      AiInterface::factionUsesResourceType (const FactionType *
                                            factionType,
                                            const ResourceType * rt)
    {
      bool

...
// =====================================================
  ...
  AiInterfaceThread::AiInterfaceThread(AiInterface * aiIntf):
    ...
    AiInterface::AiInterface(Game & game, int factionIndex, int teamIndex,
      ...
      {
#ifdef WIN32
        fp = _wfopen(::Shared::Platform::utf8_decode(aiLogFile).c_str(),
                     L"wt");
#else
        ...
        fprintf(fp,
                ...
                this->world->getFaction(this->factionIndex)->
                getType()->getName().c_str(), this->factionIndex);
      }
...
// ==================== interaction ====================
    ...
    bool AiInterface::executeCommandOverNetwork()
    {
      ...
      return faction->getCpuControl(enableServerControlledAI,
                                    isNetworkGame, role);
    }
                                          ...
                                          const Vec2i & pos)
    {
      ...
      if (executeCommandOverNetwork() == true)
      {
        ...
        result =
            commander->tryGiveCommand(unit,
                                      unit->
                                      getType()->getFirstCtOfClass
                                      (commandClass), pos, unit->getType(),
      ...
      else
      {
        Command *c =
            new Command(world->
                        getFaction(factionIndex)->getUnit(unitIndex)->
                        getType()->getFirstCtOfClass(commandClass), pos);
        ...
        result =
            ...
            getUnit(unitIndex)->giveCommand(c);
        if (SystemFlags::
            getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                   "In [%s::%s Line: %d]\n", __FILE__,
                                          ...
                                          int unitGroupCommandId)
    {
      ...
      const CommandType *ct =
          unit->getType()->findCommandTypeById(commandType->getId());
      if (ct == NULL)
      {
        char
         szBuf[8096] = "";
                                          ...
                                          int unitGroupCommandId)
    {
      ...
      else
      {
        ...
        result =
            world->getFaction(factionIndex)->
            getUnit(unitIndex)->giveCommand(cmd);
                                          ...
                                          const UnitType * ut)
    {
      ...
      else
      {
        ...
        result =
            world->getFaction(factionIndex)->
            getUnit(unitIndex)->giveCommand(new
                                          ...
                                          Unit * u)
    {
      ...
      const CommandType *ct =
          (commandType !=
           NULL ? unit->getType()->
           findCommandTypeById(commandType->getId()) : NULL);
      if (ct == NULL)
      {
        char
         szBuf[8096] = "";
      ...
      else
      {
        ...
        result =
            world->getFaction(factionIndex)->
            getUnit(unitIndex)->giveCommand(new Command(commandType, u));
...
// ==================== get data ====================
    ...
    {
      return world->getMap()->
          getStartLocation(world->getFaction
                           (factionIndex)->getStartLocationIndex());
    }
    ...
    int
     ...
     AiInterface::getMyUpgradeCount() const {
      return
          world->getFaction(factionIndex)->getUpgradeManager
          ()->getUpgradeCount();
    }
//int AiInterface::onSightUnitCount() {//    int count=0;//      Map *map= world->getMap();//      for(int i=0; i<world->getFactionCount(); ++i) {//              for(int j=0; j<world->getFaction(i)->getUnitCount(); ++j) {//                      Unit *unit = world->getFaction(i)->getUnit(j);//                      SurfaceCell *sc= map->getSurfaceCell(Map::toSurfCoords(unit->getPos()));//                      bool cannotSeeUnit = (unit->getType()->hasCellMap() == true &&//                                                                unit->getType()->getAllowEmptyCellMap() == true &&//                                                                unit->getType()->hasEmptyCellMap() == true);//                      if(sc->isVisible(teamIndex) && cannotSeeUnit == false) {//                              count++;//                      }//              }//      }//    return count;//} const Resource *
...
//}
    ...
    bool AiInterface::isResourceInRegion(const Vec2i & pos,
      ...
      for (int i = -range; i <= range; ++i)
      {
        for (int j = -range; j <= range; ++j)
        {
          ...
          {
            Resource *r =
                map->getSurfaceCell(map->toSurfCoords(Vec2i(pos.x + ii,
                                                            pos.y +
                                                            jj)))->
                getResource();
            if (r != NULL)
...
//returns if there is a resource next to a unit, in "resourcePos" is stored the relative position of the resource
    bool AiInterface::isResourceNear(const Vec2i & pos,
                                     const ResourceType * rt,
                                     Vec2i & resourcePos,
                                     Faction * faction,
                                     bool
                                     fallbackToPeersHarvestingSameResource)
        const {
      const Map *map = world->getMap();
      int
       size = 1;
      for (int i = -1; i <= size; ++i)
      {
        for (int j = -1; j <= size; ++j)
        {
          ...
          {
            Resource *r =
                map->getSurfaceCell(map->toSurfCoords(Vec2i(pos.x + i,
                                                            pos.y +
                                                            j)))->
                getResource();
            if (r != NULL)
          ...
          if (fallbackToPeersHarvestingSameResource == true
              && faction != NULL)
      {
        ...
        Vec2i
            result =
            faction->getClosestResourceTypeTargetFromCache(pos, rt);
        if (result.x >= 0)
        {
          resourcePos = result;
    ...
    bool AiInterface::getNearestSightedResource(const ResourceType * rt,
                                                const Vec2i & pos,
                                                Vec2i & resultPos,
                                                bool
                                                usableResourceTypeOnly)
    {
      Faction *faction = world->getFaction(factionIndex);
      //float tmpDist=0;
      float
    ...
    void
     AiInterface::removeEnemyWarningPositionFromList(Vec2i & checkPos) {
      for (int i = (int)enemyWarningPositionList.size() - 1; i >= 0; --i)
      {
        ...
        if (checkPos == pos)
        {
          enemyWarningPositionList.erase(enemyWarningPositionList.begin() +
                                         i);
          break;
        }
      }
    ...
    const Unit *AiInterface::getFirstOnSightEnemyUnit(Vec2i & pos,
                                                      Field & field,
                                                      int radius) {
      Map *map = world->getMap();
        ...
        for (int j = 0; j < world->getFaction(i)->getUnitCount(); ++j)
        {
              ...
              isAlly(unit) == false && unit->isAlive() == true)
          {
            ...
            if (pos.dist(getHomeLocation()) < radius)
            {
                   ...
                   aiX < pos.x + CHECK_RADIUS; ++aiX)
              {
                     ...
                     aiY < pos.y + CHECK_RADIUS; ++aiY)
                {
                  ...
                  if (map->isInside(checkPos)
                      && map->isInsideSurface(map->toSurfCoords(checkPos)))
                  {
                    Cell *cAI = map->getCell(checkPos);
                    SurfaceCell *scAI =
                        map->getSurfaceCell(Map::toSurfCoords(checkPos));
                    if (scAI != NULL && cAI != NULL
                        && cAI->getUnit(field) != NULL
                        && sc->isVisible(teamIndex))
                    {
                      const Unit *checkUnit = cAI->getUnit(field);
                      if (foundEnemyList.find(checkUnit->getId()) ==
                          foundEnemyList.end())
                      {
                        bool cannotSeeUnitAI =
                            (checkUnit->getType()->hasCellMap() == true
                             && checkUnit->
                             getType()->getAllowEmptyCellMap() == true
                             && checkUnit->getType()->hasEmptyCellMap() ==
                             true);
              ...
              {
                if (std::find(enemyWarningPositionList.begin(),
                                              ...
                                              const ResourceType * rt)
    {
      bool factionUsesResourceType =
          factionType->factionUsesResourceType(rt);
...
//    int teamIndex;
      aiInterfaceNode->
          addAttribute("teamIndex", intToStr(teamIndex),
                       mapTagReplacements);
//      //config
//      bool redir;
      aiInterfaceNode->
...
//    std::map<const ResourceType *,int> cacheUnitHarvestResourceLookup;
      ...
      {
        XmlNode *cacheUnitHarvestResourceLookupNode =
            aiInterfaceNode->addChild("cacheUnitHarvestResourceLookup");

        cacheUnitHarvestResourceLookupNode->addAttribute("key",
                                                         iterMap->first->
                                                         getName(),
                                                         mapTagReplacements);
        cacheUnitHarvestResourceLookupNode->addAttribute("value",
                                                         intToStr(iterMap->
                                                                  second),
                                                         mapTagReplacements);
    }}
// AiInterface::AiInterface(Game &game, int factionIndex, int teamIndex, int useStartLocation) { void

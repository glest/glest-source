        ...
        if (u->getHpRatio() < 1.f)
        {
               ...
               j < u->getType()->getCommandTypeCount(); ++j)
          {
            ...
            if (ct->getClass() == ccProduce || ct->getClass() == ccMorph)
            {
              ...
              if (pt != NULL)
              {
                const UnitType *ut = dynamic_cast < const
                    UnitType * >(pt);
                if (ut != NULL
          ...
          if (unitCanProduceWorker == true)
          {
            ...
            for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1)
            {
              ...
              const RepairCommandType *rct = static_cast < const
                  RepairCommandType *
              ...
              if (rct != NULL)
              {
                ...
                else if (u1->getCurrSkill()->getClass() == scRepair)
                {
                  ...
                  if (cmd != NULL
                      && cmd->getCommandType()->getClass() == ccRepair)
                  {
                    if (cmd->getUnit() != NULL
                        && cmd->getUnit()->getId() == u->getId())
                    {
                ...
                && rct->isRepairableUnitType(damagedUnit->getType()))
            {
                  ...
                  minUnitsRepairingCastle)
              {
                ...
                aiInterface->giveCommand(i, rct,
                                         damagedUnit->getPosWithCellMapSet
                                         (), unitGroupCommandId);
...
// =====================================================
  ...
  AiRuleBuildOneFarm::AiRuleBuildOneFarm(Ai * ai):
    ...
    bool AiRuleBuildOneFarm::test()
    {
      ...
      {
        const UnitType *ut =
            aiInterface->getMyFactionType()->getUnitType(i);
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j)
        {
          ...
          {
            const UnitType *producedType = static_cast < const
                ProduceCommandType * >(ct)->getProducedUnit();
            ...
            for (int k = 0; k < producedType->getCostCount(); ++k)
            {
                  ...
                  && ai->getCountOfType(ut) == 0)
              {
                if (aiInterface->reqsOk(ct)
                    && aiInterface->getMyFaction()->canCreateUnit(ut, true,
                                                                  true,
                                                                  true) ==
                    true)
                {
  ...
  AiRuleProduceResourceProducer::AiRuleProduceResourceProducer(Ai * ai):
    ...
    bool AiRuleProduceResourceProducer::test()
    {
           ...
           i < aiInterface->getTechTree()->getResourceTypeCount(); ++i)
      {
        ...
        bool factionUsesResourceType =
            aiInterface->
            factionUsesResourceType(aiInterface->getMyFactionType(), rt);
      ...
      if (aiInterface->
          getMyFactionType()->getAIBehaviorStaticOverideValue
...
// =====================================================
  ...
  AiRuleProduce::AiRuleProduce(Ai * ai):
        ...
        if (ai->outputAIBehaviourToConsole())
          printf("AiRuleProduce producing [%s]\n",
                 (produceTask->getUnitType() !=
                  NULL ? produceTask->getUnitType()->
                  getName(false).c_str() : "null"));
        if (aiInterface->isLogLevelEnabled(4) == true)
        {
          char
           szBuf[8096] = "";
    ...
    bool AiRuleProduce::canUnitTypeOfferResourceType(const UnitType * ut,
                                                     const ResourceType *
                                                     rt) {
          ...
          && aiInterface->reqsOk(ut))
      {
        ...
        else
        {
               ...
               commandIndex < ut->getCommandTypeCount(); ++commandIndex)
          {
            ...
            if (ct->getClass() == ccProduce || ct->getClass() == ccMorph)
            {
              const UnitType *producedUnit = static_cast < const
    ...
    bool AiRuleProduce::setAIProduceTaskForResourceType(const ProduceTask *
                                                        pt,
                                                        AiInterface *
                                                        aiInterface)
    {
      ...
      if (aiInterface->
          getMyFactionType()->getAIBehaviorUnits
      ...
      if (aiInterface->isLogLevelEnabled(4) == true)
      {
        ...
        snprintf(szBuf, 8096,
                 ...
                 pt->getResourceType()->getName(false).c_str(), taskAdded);
        aiInterface->printLog(4, szBuf);
      }
    ...
    void
     AiRuleProduce::produceGenericNew(const ProduceTask * pt) {
      ...
      {
        if (aiInterface->
            getMyFactionType()->getAIBehaviorUnits(aibcWorkerUnits).
            size() > 0)
        {
          const
           std::vector <
      ...
      {
        if (aiInterface->
            getMyFactionType()->getAIBehaviorUnits(aibcWarriorUnits).
            size() > 0)
        {
          const
           std::vector <
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount(); ++i)
      {
        if (aiInterface->getMyUnit(i)->getCurrCommand() != NULL
            && aiInterface->getMyUnit(i)->
            getCurrCommand()->getCommandType()->getClass() == ccBuild)
        {
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j)
        {
          ...
          if (ct->getClass() == ccProduce || ct->getClass() == ccMorph)
          {
            ...
            {
              const Resource *r =
                  producedUnit->getCost(pt->getResourceType());
              if (r != NULL)
              {
                if (ai->outputAIBehaviourToConsole())
                  printf
...
//              }
      ...
      if (ableUnits.empty() == false)
      {
        ...
        if (aiInterface->isLogLevelEnabled(4) == true)
        {
          ...
          snprintf(szBuf, 8096,
                   ...
                   ableUnits[randomUnitTypeIndex]->getName(false).c_str());
          aiInterface->printLog(4, szBuf);
        }
    ...
    void
     AiRuleProduce::produceGeneric(const ProduceTask * pt) {
      typedef
       vector < const UnitType *> UnitTypes;
      UnitTypes ableUnits;
      AiInterface *aiInterface = ai->getAiInterface();
    ...
    void
     AiRuleProduce::produceSpecific(const ProduceTask * pt) {
      ...
      if (aiInterface->reqsOk(pt->getUnitType()))
      {
                ...
                if (aiInterface->reqsOk(ct))
                {
                      ...
                      || ct->getClass() == ccMorph)
                  {
                    if (ctypeForCostCheck != NULL
                        && ct->getClass() == ccMorph)
                    {
                      const MorphCommandType *mct = dynamic_cast < const
                          MorphCommandType * >(ct);
                      if (mct == NULL)
                      {
        ...
        if (aiInterface->checkCosts(pt->getUnitType(), ctypeForCostCheck)
            == false)
        {
          if (aiInterface->isLogLevelEnabled(4) == true)
          {
        ...
        if (producers.empty() == false)
        {
            ...
            if (currentProducerIndex >= aiInterface->getMyUnitCount())
            {
              ...
              printf
                  ("In [%s::%s Line: %d] currentProducerIndex >= aiInterface->getMyUnitCount(), currentProducerIndex = %d, aiInterface->getMyUnitCount() = %d, i = %u,producers.size() = "
                   ...
                   aiInterface->getMyUnitCount(), i, producers.size());
              snprintf(szBuf, 8096,
          ...
          {                     // mega cpu trys to balance the commands to the producers
            int
             randomstart = ai->getRandom()->randRange(0,
                                                      (int) producers.
                                                      size() - 1);
            int
              ...
              if (prIndex >= (int) producers.size())
              {
                ...
                printf
                    ("In [%s::%s Line: %d] prIndex >= producers.size(), currentProducerIndex = %d, i = %u,producers.size() = "
                     MG_SIZE_T_SPECIFIER " \n", __FILE__,
                     __FUNCTION__, __LINE__, prIndex, i, producers.size());
                snprintf(szBuf, 8096,
                         "In [%s::%s Line: %d] currentProducerIndex >= producers.size(), currentProducerIndex = %d, i = %u,producers.size() = "
                         MG_SIZE_T_SPECIFIER "", __FILE__,
                         __FUNCTION__, __LINE__,
                         currentProducerIndex, i, producers.size());
                throw megaglest_runtime_error(szBuf);
              }

              currentCommandCount =
                  aiInterface->
                  getMyUnit(currentProducerIndex)->getCommandSize();
              if (currentCommandCount == 1
                  && aiInterface->
                  getMyUnit(currentProducerIndex)->getCurrCommand()->
                  getCommandType()->getClass() == ccStop)
            ...
            if (bestIndex >= 0)
            {
              if (aiInterface->getMyUnit(bestIndex)->getCommandSize() > 2)
              {
                ...
                if (aiInterface->reqsOk
                    (aiInterface->getMyUnit(bestIndex)->getType()))
                {
                  if (ai->getCountOfClass(ucBuilding) > 5)
                  {
                    ai->addTask(new
                                BuildTask(aiInterface->getMyUnit
                                          (bestIndex)->getType()));
                  }
                }
                ...
                {
                  const UnitType *ut =
                      aiInterface->getMyUnit(i)->getType();
                  //for each command
                  for (int j = 0; j < ut->getCommandTypeCount(); ++j)
                  {
                    const CommandType *ct = ut->getCommandType(j);
                    ...
                    if (ct->getClass() == ccProduce)
                    {
                      const UnitType *unitType = static_cast < const
                          UnitType * >(ct->getProduced());
                      if (unitType->hasSkillClass(scAttack)
                          && !unitType->hasCommandClass(ccHarvest)
                ...
                if (!backupProducers.empty())
                {
                  ...
                  int
                   lowestCommandCount = 1000000;
                  int
                   currentProducerIndex = backupProducers[randomstart];
                  int
                   bestIndex = -1;
                  for (unsigned int i = randomstart;
                       i < backupProducers.size() + randomstart; i++)
                  {
                    int
                     prIndex = i;
                    ...
                    int
                     currentCommandCount =
                        aiInterface->getMyUnit
                        (currentProducerIndex)->getCommandSize();
                    if (currentCommandCount == 1
                        &&
                        aiInterface->getMyUnit
                        (currentProducerIndex)->getCurrCommand()->
                        getCommandType()->getClass() == ccStop)
                    {           // special for non buildings
                      currentCommandCount = 0;
                    }
                  ...
                  if (bestIndex >= 0)
                  {
                    ...
                    int
                     commandIndex =
                        productionCommandIndexes[ai->getRandom()->randRange
                                                 (0,
                                                  (int)
                                                  productionCommandIndexes.size
                                                  () - 1)];
                    ...
                    if (ai->outputAIBehaviourToConsole())
                      printf
                          ("mega #1 produceSpecific giveCommand to unit [%s] commandType [%s]\n",
                           aiInterface->getMyUnit(bestIndex)->getType()->
                           getName().c_str(),
                           ut->getCommandType(commandIndex)->getName().
                           c_str());
                    if (aiInterface->isLogLevelEnabled(4) == true)
                    {
                      char
                      ...
                      producersDefaultCommandType.end())
                  {
                    int
                     bestCommandTypeCount =
                        (int)
                        producersDefaultCommandType[bestIndex].size();
                    int
                     bestCommandTypeIndex = ai->getRandom()->randRange(0,
                                                                       bestCommandTypeCount
                                                                       -
                                                                       1);
              ...
              else
              {
                    ...
                    producersDefaultCommandType.end())
                {
                  ...
                  int
                   bestCommandTypeCount =
                      (int) producersDefaultCommandType[bestIndex].size();
                  int
                   bestCommandTypeIndex = ai->getRandom()->randRange(0,
                                                                     bestCommandTypeCount
                                                                     - 1);
          ...
          {
            int
             pIndex = ai->getRandom()->randRange(0,
                                                 (int) producers.size() -
                                                 1);
            int
                ...
                producersDefaultCommandType.end())
            {
              ...
              int
               bestCommandTypeCount =
                  (int) producersDefaultCommandType[producerIndex].size();
              int
               bestCommandTypeIndex = ai->getRandom()->randRange(0,
                                                                 bestCommandTypeCount
                                                                 - 1);
            ...
            if (SystemFlags::getSystemSettingType
              ...
              SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                       ...
                                       __FILE__, __FUNCTION__, __LINE__,
                                       producers.size(), producerIndex,
                                       pIndex,
                                       producersDefaultCommandType.size());
  ...
  AiRuleBuild::AiRuleBuild(Ai * ai):
    ...
    void
     AiRuleBuild::buildGeneric(const BuildTask * bt) {
                ...
                true)
            {
              ...
              if (aiInterface->isLogLevelEnabled(4) == true)
              {
                ...
                snprintf(szBuf, 8096,
                         ...
                         bt->getResourceType()->getName().c_str(),
                         unitList.size(),
      ...
      {
        if (aiInterface->
            getMyFactionType()->getAIBehaviorUnits(aibcBuildingUnits).
            size() > 0)
        {
          const
           std::vector <
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount(); ++i)
      {
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j)
        {
          ...
          {
            const BuildCommandType *bct = static_cast < const
                BuildCommandType * >(ct);
      ...
      if (!buildings.empty())
      {
        ...
        for (int i = 0; i < 10 && !buildingFound; ++i)
        {
          ...
          if (i > 0)
          {
            ...
            for (int j = 0;
                 j < (int) buildings.size() && buildingFound == false; ++j)
            {
      ...
      if (aiInterface->reqsOk(bt->getUnitType()))
      {
        ...
        if (builders.empty() == false)
        {
          ...
          Vec2i
              searchPos =
              bt->getForcePos()? bt->
              getPos() : ai->getRandomHomePosition();
          if (bt->getForcePos() == false)
          {
            ...
            ai->getAiInterface()->getWorld()->
                getUnitUpdater()->findEnemiesForCell(searchPos,
                                                     bt->getUnitType()->
                                                     getSize(),
              ...
              for (int i1 = 0; i1 < 25 && enemies.empty() == false; ++i1)
              {
                for (int j1 = 0; j1 < 25 && enemies.empty() == false; ++j1)
                {
                  ...
                  const int
                   spacing = 1;
                  if (ai->
                      getAiInterface()->isFreeCells(tryPos -
                                                    Vec2i(spacing),
                                                    bt->
                                                    getUnitType()->getSize
                                                    () + spacing * 2,
                                                    fLand))
                  {
                    enemies.clear();
                    ai->getAiInterface()->
                        getWorld()->getUnitUpdater()->findEnemiesForCell
                        (tryPos, bt->getUnitType()->getSize(),
                         enemySightDistanceToAvoid,
                         ai->getAiInterface()->getMyFaction(), enemies,
                         true);
          ...
          if (ai->findPosForBuilding(bt->getUnitType(), searchPos, pos))
          {
                ...
                buildersDefaultCommandType.end())
            {
              ...
              int
               bestCommandTypeCount =
                  (int) buildersDefaultCommandType[builderIndex].size();
              int
               bestCommandTypeIndex = ai->getRandom()->randRange(0,
                                                                 bestCommandTypeCount
                                                                 - 1);
    ...
    bool AiRuleBuild::isWarriorProducer(const UnitType * building)
    {
      for (int i = 0; i < building->getCommandTypeCount(); i++)
      {
        ...
        if (ct->getClass() == ccProduce)
        {
          const UnitType *ut = static_cast < const
              ProduceCommandType * >(ct)->getProducedUnit();
  ...
  AiRuleUpgrade::AiRuleUpgrade(Ai * ai):
    ...
    void
     AiRuleUpgrade::upgradeGeneric(const UpgradeTask * upgt) {
      ...
      typedef
       vector < const UpgradeType *> UpgradeTypes;
      AiInterface *aiInterface = ai->getAiInterface();
        ...
        for (unsigned int i = 0; i < upgradeList.size(); ++i)
        {
          ...
          for (int k = 0; k < aiInterface->getMyUnitCount(); ++k)
          {
            ...
            for (int j = 0; j < ut->getCommandTypeCount(); ++j)
            {
              ...
              {
                const UpgradeCommandType *upgct = dynamic_cast < const
                ...
                if (upgct != NULL)
                {
                  ...
                  if (upgrade == priorityUpgrade)
                  {
                    if (aiInterface->reqsOk(upgct) == true &&
                        aiInterface->
                        getMyFaction()->getUpgradeManager
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount(); ++i)
      {
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j)
        {
          ...
          if (ct->getClass() == ccUpgrade)
          {
            const UpgradeCommandType *upgct = static_cast < const
                UpgradeCommandType * >(ct);
            const UpgradeType *upgrade = upgct->getProducedUpgrade();
      ...
      if (aiInterface->reqsOk(upgt->getUpgradeType()))
      {
        ...
        for (int i = 0; i < aiInterface->getMyUnitCount(); ++i)
        {
          ...
          for (int j = 0; j < ut->getCommandTypeCount(); ++j)
          {
            ...
            if (ct->getClass() == ccUpgrade)
            {
              const UpgradeCommandType *uct = static_cast < const
                  UpgradeCommandType * >(ct);
    ...
    bool AiRuleExpand::test()
    {
           ...
           i < aiInterface->getTechTree()->getResourceTypeCount(); ++i)
      {
        const ResourceType *rt =
            aiInterface->getTechTree()->getResourceType(i);
        if (rt->getClass() == rcTech)
        {
          ...
          if (factionUsesResourceType == true)
          {
            ...
            if (aiInterface->getNearestSightedResource(rt,
              ...
              for (int j = 0; j < unitCount; ++j)
              {
                ...
                if (ut->getStore(rt) > 0)
                {
                  ...
                  int
                   distance =
                      static_cast <
                      int >(u->getPosNotThreadSafe().dist(expandPos));

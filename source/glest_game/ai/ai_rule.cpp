...
// =====================================================
    ...
    AiRuleWorkerHarvest::AiRuleWorkerHarvest (Ai * ai):AiRule (ai)
    {
  ...
  AiRuleRepair::AiRuleRepair (Ai * ai):
    ...
    void AiRuleRepair::execute ()
    {
      ...
      for (int i1 = 0; i1 < aiInterface->getMyUnitCount (); ++i1)
      {
        const Unit *u1 = aiInterface->getMyUnit (i1);
        const RepairCommandType *rct = static_cast < const
          RepairCommandType * >(u1->getType ()->getFirstCtOfClass (ccRepair));
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount (); ++i)
      {
        const Unit *u = aiInterface->getMyUnit (i);
        const RepairCommandType *rct = static_cast < const
          RepairCommandType * >(u->getType ()->getFirstCtOfClass (ccRepair));
    ...
    bool AiRuleProduceResourceProducer::test ()
    {
          ...
          (aibsvcMinStaticResourceCount) != INT_MAX)
      {
        targetStaticResourceCount =
          aiInterface->getMyFactionType ()->getAIBehaviorStaticOverideValue
...
// =====================================================
  ...
  AiRuleProduce::AiRuleProduce (Ai * ai):
    ...
    bool
      AiRuleProduce::setAIProduceTaskForResourceType (const ProduceTask *
                                                      pt,
                                                      AiInterface *
                                                      aiInterface)
    {
          ...
          (aibcResourceProducerUnits).size () > 0)
      {
        ...
        for (unsigned int i = 0; i < unitList.size (); ++i)
        {
          ...
          if (ai->getCountOfType (ut) < priorityUnit.second &&
              ...
              canCreateUnit (priorityUnit.first, false, true, true) == true)
          {
      ...
      if (aiInterface->reqsOk (pt->getUnitType ()))
      {
        ...
        if (producers.empty () == false)
        {
          ...
          {                     // mega cpu trys to balance the commands to the producers
            int randomstart = ai->getRandom ()->randRange (0,
                                                           (int)
                                                           producers.size () -
                                                           1);
                 ...
                 i < producers.size () + randomstart; i++)
            {
              ...
              if (currentCommandCount == 1
                  && aiInterface->getMyUnit (currentProducerIndex)->
                  getCurrCommand ()->getCommandType ()->getClass () == ccStop)
              ...
              if (aiInterface->getMyUnit (bestIndex)->getCommandSize () > 2)
              {
                ...
                if (!backupProducers.empty ())
                {
                  ...
                  if (bestIndex >= 0)
                  {
                    ...
                    int
                      commandIndex =
                      productionCommandIndexes[ai->getRandom ()->randRange (0,
                                                                            (int) productionCommandIndexes.size () - 1)];
                ...
                {               // do it like normal CPU
                      ...
                      producersDefaultCommandType.end ())
                  {
                    ...
                    int bestCommandTypeIndex = ai->getRandom ()->randRange (0,
                                                                            bestCommandTypeCount
                                                                            -
                                                                            1);
          ...
          {
            int pIndex = ai->getRandom ()->randRange (0,
                                                      (int) producers.
                                                      size () - 1);
    ...
    void AiRuleBuild::buildSpecific (const BuildTask * bt)
    {
      ...
      if (aiInterface->reqsOk (bt->getUnitType ()))
      {
        ...
        if (builders.empty () == false)
        {
          ...
          Vec2i
            searchPos =
            bt->getForcePos ()? bt->getPos () : ai->getRandomHomePosition ();
          if (bt->getForcePos () == false)
          {
            const int enemySightDistanceToAvoid = 18;
            vector < Unit * >enemies;
            ai->getAiInterface ()->getWorld ()->getUnitUpdater ()->
              findEnemiesForCell (searchPos, bt->getUnitType ()->getSize (),
                                  enemySightDistanceToAvoid,
                                  ai->getAiInterface ()->getMyFaction (),
                                  enemies, true);
            if (enemies.empty () == false)
            {
              for (int i1 = 0; i1 < 25 && enemies.empty () == false; ++i1)
              {
                for (int j1 = 0; j1 < 25 && enemies.empty () == false; ++j1)
                {
                  ...
                  const int spacing = 1;
                  if (ai->getAiInterface ()->isFreeCells (tryPos -
                                                          Vec2i (spacing),
                                                          bt->getUnitType
                                                          ()->getSize () +
                                                          spacing * 2, fLand))
                ...
                if (upgct != NULL)
                {
                  ...
                  if (upgrade == priorityUpgrade)
                  {
                    if (aiInterface->reqsOk (upgct) == true &&
                        aiInterface->getMyFaction ()->getUpgradeManager
                        ()->isUpgradingOrUpgraded (priorityUpgrade) == false)
                    {
                      //if(ai->getRandom()->randRange(0, 1)==0) {

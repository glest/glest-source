...
// =====================================================
    ...
    void Ai::init (AiInterface * aiInterface, int useStartLocation)
    {
      ...
      {
        scoutResourceRange =
          faction->getAIBehaviorStaticOverideValue (aibsvcScoutResourceRange);
                           ...
                           UnitClass * additionalUnitClassToExcludeFromCount)
    {
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount (); ++i)
      {
        if (aiInterface->getMyUnit (i)->getType ()->isOfClass (uc))
        {
          ...
          {
            if (aiInterface->getMyUnit (i)->getType ()->isOfClass
...
// ==================== actions ====================
    ...
    void Ai::massiveAttack (const Vec2i & pos, Field field, bool ultraAttack)
    {
      ...
      for (int i = 0; i < unitCount; ++i)
      {
        ...
        bool
          alreadyAttacking = (unit->getCurrSkill ()->getClass () == scAttack);
...
//                                                                      unit->getId(), enemy->getFullName().c_str(), enemy->getId());
            ...
            && (ultraAttack || isWarrior) && unitSignalledToAttack == false)
        {
          ...
          {
            shouldAttack =
              (attackerWorkersHarvestingCount > minWorkerAttackersHarvesting);
            if (shouldAttack == false)
            {
              attackerWorkersHarvestingCount++;
            }
          ...
          for (int i = -1; i <= 1; ++i)
          {
            for (int j = -1; j <= 1; ++j)
            {
                  ...
                  && map->isInsideSurface (map->toSurfCoords (pos)))
              {
                ...
                {
                  bool
                    canUnitMoveToCell = map->aproxCanMove (u, unitPos, pos);
               ...
               iterMap2 != iterMap->second.end (); ++iterMap2)
          {
                ...
                NULL)
            {
                   ...
                   ++moveAttempt)
              {
                ...
                bool
                  canUnitMoveToCell = map->aproxCanMove (adjacentUnit,
                                                         adjacentUnit->
                                                         getPosNotThreadSafe
                                                         (), pos);

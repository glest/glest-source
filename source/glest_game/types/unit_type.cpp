...
namespace Game {
	auto_ptr < CommandType >
		UnitType::
		ctHarvestEmergencyReturnCommandType(new
	...
	const Level *Level::loadGame(const XmlNode * rootNode,
		const UnitType * ut) {
	...
	UnitType::~UnitType() {
		...
		skillTypes.clear();
		deleteValues(selectionSounds.getSounds().begin(),
			selectionSounds.getSounds().end());
		selectionSounds.clearSounds();
		deleteValues(commandSounds.getSounds().begin(),
			commandSounds.getSounds().end());
	...
	void UnitType::loadUnit(int id, const string & dir,
		...
		try {
			...
			if (parametersNode->hasChild("count-in-victory-conditions") == true) {
				bool countUnit =
					parametersNode->getChild("count-in-victory-conditions")->
			...
			size =
				parametersNode->getChild("size")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->size), this->size);
			renderSize = size;
			if (parametersNode->hasChild("render-size")) {
				renderSize =
					parametersNode->getChild("render-size")->getAttribute("value")->
					getIntValue();
			}
			aiBuildSize = size;
			if (parametersNode->hasChild("ai-build-size")) {
				aiBuildSize =
					parametersNode->getChild("ai-build-size")->
			...
			height =
				parametersNode->getChild("height")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->height), this->height);
			...
			if (parametersNode->hasChild("target-height")) {
				targetHeight =
					parametersNode->getChild("target-height")->
					getAttribute("value")->getIntValue();
			...
			if (parametersNode->hasChild("target-height")) {
				burnHeight =
					parametersNode->getChild("burn-height")->getAttribute("value")->
					getIntValue();
			...
			maxHp =
				parametersNode->getChild("max-hp")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->maxHp), this->maxHp);
			...
			hpRegeneration =
				parametersNode->getChild("max-hp")->getAttribute("regeneration")->
				getIntValue();
			addItemToVault(&(this->hpRegeneration), this->hpRegeneration);
			...
			maxEp =
				parametersNode->getChild("max-ep")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->maxEp), this->maxEp);
			...
			if (maxEp != 0) {
				...
				epRegeneration =
					...
					getAttribute("regeneration")->getIntValue();
			}
			addItemToVault(&(this->epRegeneration), this->epRegeneration);
			...
			if (parametersNode->getChild("max-hp")->
				hasAttribute("start-percentage")) {
				startHpPercentage =
					parametersNode->getChild("max-hp")->
					getAttribute("start-percentage")->getIntValue();
			...
			if (parametersNode->getChild("max-ep")->
				hasAttribute("start-percentage")) {
				startEpPercentage =
					parametersNode->getChild("max-ep")->
					getAttribute("start-percentage")->getIntValue();
			...
			if (parametersNode->hasChild("max-unit-count")) {
				...
				maxUnitCount =
					parametersNode->getChild("max-unit-count")->
					getAttribute("value")->getIntValue();
			}
			addItemToVault(&(this->maxUnitCount), this->maxUnitCount);
			...
			armor =
				parametersNode->getChild("armor")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->armor), this->armor);

			//armor type string
			string armorTypeName =
				parametersNode->getChild("armor-type")->getAttribute("value")->
				getRestrictedValue();
			...
			sight =
				parametersNode->getChild("sight")->getAttribute("value")->
				getIntValue();
			addItemToVault(&(this->sight), this->sight);

			//prod time
			productionTime =
				parametersNode->getChild("time")->getAttribute("value")->
				getIntValue();
			...
			if (parametersNode->hasChild("uniform-selection")) {
				uniformSelect =
					parametersNode->getChild("uniform-selection")->
					getAttribute("value")->getBoolValue();
			}
			...
			if (parametersNode->hasChild("commandable")) {
				commandable =
					parametersNode->getChild("commandable")->getAttribute("value")->
					getBoolValue();
			}
			//cellmap
			allowEmptyCellMap = false;
			...
			bool hasCellMap =
				cellMapNode->getAttribute("value")->getBoolValue();
			if (hasCellMap == true) {
				if (cellMapNode->getAttribute("allowEmpty", false) != NULL) {
					allowEmptyCellMap =
						cellMapNode->getAttribute("allowEmpty")->getBoolValue();
				}
			...
			for (int i = 0; i < (int) levels.size(); ++i) {
				...
				levels[i].init(levelNode->getAttribute("name")->
					getRestrictedValue(),
			...
			//properties
			const XmlNode *propertiesNode =
				parametersNode->getChild("properties");
			for (int i = 0; i < (int) propertiesNode->getChildCount(); ++i) {
				const XmlNode *propertyNode =
					propertiesNode->getChild("property", i);
				string propertyName =
					propertyNode->getAttribute("value")->getRestrictedValue();
				bool found = false;
				for (int i = 0; i < pCount; ++i) {
			...
			if (parametersNode->hasChild("damage-particles")) {
				const XmlNode *particleNode =
					parametersNode->getChild("damage-particles");
				...
				if (particleEnabled) {
					for (int i = 0; i < (int) particleNode->getChildCount(); ++i) {
						...
						unitParticleSystemType->load(particleFileNode, dir,
							currentPath + path,
							&Renderer::getInstance(),
							loadedFileList, sourceXMLFile,
							techTree->getPath());
						loadedFileList[currentPath +
							path].push_back(make_pair(sourceXMLFile,
								particleFileNode->
								getAttribute
								("path")->
								getRestrictedValue
						...
						if (particleFileNode->getAttribute("minHp", false) != NULL
							&& particleFileNode->getAttribute("maxHp", false) != NULL) {
							unitParticleSystemType->setMinmaxEnabled(true);
							unitParticleSystemType->setMinHp(particleFileNode->
								getAttribute("minHp")->
								getIntValue());
							unitParticleSystemType->setMaxHp(particleFileNode->
								getAttribute("maxHp")->
								getIntValue());

							if (particleFileNode->
								getAttribute("ispercentbased", false) != NULL) {
								unitParticleSystemType->
									setMinmaxIsPercent(particleFileNode->
			...
			if (parametersNode->hasChild("healthbar")) {
				const XmlNode *healthbarNode =
					parametersNode->getChild("healthbar");
			...
			if (light) {
				lightColor.x =
					lightNode->getAttribute("red")->getFloatValue(0.f, 1.f);
				lightColor.y =
					lightNode->getAttribute("green")->getFloatValue(0.f, 1.f);
				lightColor.z =
					lightNode->getAttribute("blue")->getFloatValue(0.f, 1.f);
			}

			//rotationAllowed
			if (parametersNode->hasChild("rotationAllowed")) {
				const XmlNode *rotationAllowedNode =
					parametersNode->getChild("rotationAllowed");
				rotationAllowed =
					rotationAllowedNode->getAttribute("value")->getBoolValue();
			} else {
			...
			const XmlNode *unitRequirementsNode =
				parametersNode->getChild("unit-requirements");
			...
			if (hasDup) {
				printf
				("WARNING, unit type [%s] has one or more duplicate unit requirements\n",
			...
			const XmlNode *upgradeRequirementsNode =
				parametersNode->getChild("upgrade-requirements");
			...
			if (hasDup) {
				printf
				("WARNING, unit type [%s] has one or more duplicate upgrade requirements\n",
					this->getName(false).c_str());
			}
			...
			const XmlNode *resourceRequirementsNode =
				parametersNode->getChild("resource-requirements");
			...
			if (hasDup) {
				printf
				("WARNING, unit type [%s] has one or more duplicate resource requirements\n",
					this->getName(false).c_str());
			}
			...
			//resources stored
			const XmlNode *resourcesStoredNode =
				parametersNode->getChild("resources-stored");
			storedResources.resize(resourcesStoredNode->getChildCount());
			for (int i = 0; i < (int) storedResources.size(); ++i) {
				const XmlNode *resourceNode =
					resourcesStoredNode->getChild("resource", i);
				string name =
					resourceNode->getAttribute("name")->getRestrictedValue();
				int amount = resourceNode->getAttribute("amount")->getIntValue();
			...
			if (hasDup) {
				printf
				("WARNING, unit type [%s] has one or more duplicate stored resources\n",
					this->getName(false).c_str());
			}
			...
			for (std::map < string, int >::iterator iterMap =
				sortedItems.begin(); iterMap != sortedItems.end(); ++iterMap) {
				try {
					storedResources[index].init(techTree->
			...
			if (parametersNode->hasChild("resources-death")) {
				const XmlNode *deathResourcesNode =
					parametersNode->getChild("resources-death");

				for (unsigned int i = 0; i < deathResourcesNode->getChildCount();
					++i) {
					const XmlNode *resourceNode =
						deathResourcesNode->getChild("resource", i);
					string name =
						resourceNode->getAttribute("name")->getRestrictedValue();
					...
					if (resourceNode->hasAttribute("amount-value")) {
						resource.setAmountValue(resourceNode->
							getAttribute("amount-value")->
							getIntValue());
					} else {
					...
					if (resourceNode->hasAttribute("amount-faction-percent")) {
						resource.setAmountFactionPercent(resourceNode->
							getAttribute
							("amount-faction-percent")->
							getIntValue());
					} else {
					...
					if (resourceNode->hasAttribute("loss-value")) {
						resource.setLossValue(resourceNode->
							getAttribute("loss-value")->
							getIntValue());
					} else {
					...
					if (resourceNode->hasAttribute("loss-faction-percent")) {
						resource.setLossFactionPercent(resourceNode->
							getAttribute
					...
					if (resourceNode->hasAttribute("allow-negative")) {
						resource.setNegativeAllowed(resourceNode->
							getAttribute("allow-negative")->
							getBoolValue());
					} else {
					...
					if (std::
						find(lootableResources.begin(), lootableResources.end(),
							resource) != lootableResources.end()) {
						printf
						("WARNING, unit type [%s] has one or more duplicate lootable resources\n",
							this->getName(false).c_str());
					}
			...
			if (parametersNode->hasChild("tags")) {
				...
				for (unsigned int i = 0; i < tagsNode->getChildCount(); ++i) {
					...
					string tag =
						resourceNode->getAttribute("value")->getRestrictedValue();
					tags.insert(tag);
				}
			}
			...
			//image cancel
			const XmlNode *imageCancelNode =
				parametersNode->getChild("image-cancel");
			cancelImage = Renderer::getInstance().newTexture2D(rsGame);
			if (cancelImage) {
				cancelImage->load(imageCancelNode->getAttribute("path")->
					getRestrictedValue(currentPath));
			}
			loadedFileList[imageCancelNode->getAttribute("path")->
				getRestrictedValue(currentPath)].
				push_back(make_pair
				(sourceXMLFile,
					imageCancelNode->getAttribute("path")->
					getRestrictedValue()));

			//meeting point
			const XmlNode *meetingPointNode =
				parametersNode->getChild("meeting-point");
			meetingPoint =
				meetingPointNode->getAttribute("value")->getBoolValue();
			if (meetingPoint) {
				meetingPointImage = Renderer::getInstance().newTexture2D(rsGame);
				if (meetingPointImage) {
					meetingPointImage->load(meetingPointNode->
						getAttribute("image-path")->
						getRestrictedValue(currentPath));
				}
				loadedFileList[meetingPointNode->getAttribute("image-path")->
					getRestrictedValue(currentPath)].
					push_back(make_pair
					(sourceXMLFile,
						meetingPointNode->getAttribute("image-path")->
						getRestrictedValue()));
			}

			//countUnitDeathInStats
			if (parametersNode->hasChild("count-unit-death-in-stats")) {
				const XmlNode *countUnitDeathInStatsNode =
					parametersNode->getChild("count-unit-death-in-stats");
				countUnitDeathInStats =
					countUnitDeathInStatsNode->getAttribute("value")->
					getBoolValue();
			} else {
			...
			if (parametersNode->hasChild("count-unit-production-in-stats")) {
				const XmlNode *countUnitProductionInStatsNode =
					parametersNode->getChild("count-unit-production-in-stats");
				countUnitProductionInStats =
					countUnitProductionInStatsNode->getAttribute("value")->
					getBoolValue();
			} else {
			...
			if (parametersNode->hasChild("count-unit-kill-in-stats")) {
				const XmlNode *countUnitKillInStatsNode =
					parametersNode->getChild("count-unit-kill-in-stats");
				countUnitKillInStats =
					countUnitKillInStatsNode->getAttribute("value")->getBoolValue();
			} else {
			...
			if (parametersNode->hasChild("count-kill-for-unit-upgrade")) {
				const XmlNode *countKillForUnitUpgradeNode =
					parametersNode->getChild("count-kill-for-unit-upgrade");
				countKillForUnitUpgrade =
					countKillForUnitUpgradeNode->getAttribute("value")->
					getBoolValue();
			} else {
			...
			//selection sounds
			const XmlNode *selectionSoundNode =
				parametersNode->getChild("selection-sounds");
			if (selectionSoundNode->getAttribute("enabled")->getBoolValue()) {
				...
				for (int i = 0; i < (int) selectionSounds.getSounds().size(); ++i) {
					const XmlNode *soundNode =
						selectionSoundNode->getChild("sound", i);
					string path =
						soundNode->getAttribute("path")->
						getRestrictedValue(currentPath);
			...
			//command sounds
			const XmlNode *commandSoundNode =
				parametersNode->getChild("command-sounds");
			if (commandSoundNode->getAttribute("enabled")->getBoolValue()) {
				...
				for (int i = 0; i < (int) commandSoundNode->getChildCount(); ++i) {
					const XmlNode *soundNode =
						commandSoundNode->getChild("sound", i);
					string path =
						soundNode->getAttribute("path")->
						getRestrictedValue(currentPath);
					StaticSound *sound = new StaticSound();
			...
			for (int i = 0; i < (int) skillTypes.size(); ++i) {
				...
				string classId =
					typeNode->getAttribute("value")->getRestrictedValue();
				SkillType *skillType =
					SkillTypeFactory::getInstance().newInstance(classId);
			...
			for (int i = 0; i < (int) commandTypes.size(); ++i) {
				...
				CommandType *commandType =
					CommandTypeFactory::getInstance().newInstance(classId);
	...
	const HarvestCommandType *UnitType::
		getFirstHarvestCommand(const ResourceType * resourceType,
	...
	const HarvestEmergencyReturnCommandType *UnitType::
		getFirstHarvestEmergencyReturnCommand() const {
		const HarvestEmergencyReturnCommandType *result =
			dynamic_cast <
			const HarvestEmergencyReturnCommandType *
			>(ctHarvestEmergencyReturnCommandType.get());
		return result;
	}
	...
	const AttackStoppedCommandType *UnitType::
		getFirstAttackStoppedCommand(Field field) const {
		//printf("$$$ Unit [%s] commandTypes.size() = %d\n",this->getName().c_str(),(int)commandTypes.size());
	...
	const RepairCommandType *UnitType::getFirstRepairCommand(const UnitType *
		repaired) const {
		for (int i = 0; i < (int) commandTypes.size(); ++i) {
			if (commandTypes[i]->getClass() == ccRepair) {
				const RepairCommandType *rct =
					static_cast <const RepairCommandType *>(commandTypes[i]);
	...
	const SkillType *UnitType::getSkillType(const string & skillName,
	...
	int UnitType::getTotalMaxHpRegeneration(const TotalUpgrade *
		totalUpgrade) const {
		checkItemInVault(&(this->hpRegeneration), this->hpRegeneration);
	...
	int UnitType::getTotalMaxEpRegeneration(const TotalUpgrade *
		totalUpgrade) const {
		checkItemInVault(&(this->epRegeneration), this->epRegeneration);
	...
	bool UnitType::isOfClass(UnitClass uc) const {
		switch (uc) {
			...
			case ucWorker:return hasSkillClass(scBuild)
				|| hasSkillClass(scRepair) || hasSkillClass(scHarvest);
			case ucBuilding:return hasSkillClass(scBeBuilt);
	...
	void UnitType::computeFirstStOfClass() {
		for (int j = 0; j < scCount; ++j) {
			...
			for (int i = 0; i < (int) skillTypes.size(); ++i) {
				if (skillTypes[i] != NULL
					&& skillTypes[i]->getClass() == SkillClass(j)) {
	...
	void UnitType::computeFirstCtOfClass() {
		for (int j = 0; j < ccCount; ++j) {
			...
			for (int i = 0; i < (int) commandTypes.size(); ++i) {
				if (commandTypes[i] != NULL
					&& commandTypes[i]->getClass() == CommandClass(j)) {
	...
	std::string UnitType::toString() const {
		...
		if (armorType != NULL) {
			result +=
				" armorType Name: [" + armorType->getName(false) + " id = " +
				intToStr(armorType->getId());
		}
		...
		if (cellMap != NULL) {
			...
			for (int i = 0; i < size; ++i) {
				for (int j = 0; j < size; ++j) {
					result +=
						" i = " + intToStr(i) + " j = " + intToStr(j) + " value = " +
						intToStr(cellMap[i * size + j]);
				}
			}
		}
		...
		for (int i = 0; i < (int) skillTypes.size(); ++i) {
			result +=
				" i = " + intToStr(i) + " " + skillTypes[i]->toString(false);
		}

		result += " commandTypes: [" + intToStr(commandTypes.size()) + "]";
		for (int i = 0; i < (int) commandTypes.size(); ++i) {
			result +=
				" i = " + intToStr(i) + " " + commandTypes[i]->toString(false);
		}

		result +=
			" storedResources: [" + intToStr(storedResources.size()) + "]";
		for (int i = 0; i < (int) storedResources.size(); ++i) {
			result +=
				" i = " + intToStr(i) + " " +
				storedResources[i].getDescription(false);
		}
		...
		result +=
			" countInVictoryConditions = " + intToStr(countInVictoryConditions);

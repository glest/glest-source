...
#ifndef _SKILLTYPE_H_
#define _SKILLTYPE_H_
...
namespace Game {
	...
	protected:
		...
		static int getNextAttackBoostId() {
			return ++nextAttackBoostId;
		}
		...
		void loadAttackBoost(const XmlNode * attackBoostsNode,
			const XmlNode * attackBoostNode,
			const FactionType * ft, string parentLoader,
			const string & dir, string currentPath,
			std::map < string, vector < pair < string,
			string > > >&loadedFileList, const TechTree * tt);
	...
	public:
		...
		bool CanCycleNextRandomAnimation(const int *animationRandomCycleCount)
			const;

		static void resetNextAttackBoostId() {
			nextAttackBoostId = 0;
		}
		...
		Model *getAnimation(float animProgress = 0, const Unit * unit =
			NULL, int *lastAnimationIndex =
			NULL, int *animationRandomCycleCount = NULL) const;

		float getShakeStartTime() const {
			return shakeStartTime;
		}
		bool getShake() const {
			return shake;
		}
		...
		const SkillSoundList *getSkillSoundList() const {
			return &skillSoundList;
		}

		bool getShakeSelfEnabled() const {
			return shakeSelfEnabled;
		}
		bool getShakeSelfVisible() const {
			return shakeSelfVisible;
		}
		bool getShakeSelfInCameraView() const {
			return shakeSelfInCameraView;
		}
		bool getShakeSelfCameraAffected() const {
			return shakeSelfCameraAffected;
		}
		bool getShakeTeamEnabled() const {
			return shakeTeamEnabled;
		}
		bool getShakeTeamVisible() const {
			return shakeTeamVisible;
		}
		bool getShakeTeamInCameraView() const {
			return shakeTeamInCameraView;
		}
		bool getShakeTeamCameraAffected() const {
			return shakeTeamCameraAffected;
		}
		bool getShakeEnemyEnabled() const {
			return shakeEnemyEnabled;
		}
		bool getShakeEnemyVisible() const {
			return shakeEnemyVisible;
		}
		bool getShakeEnemyInCameraView() const {
			return shakeEnemyInCameraView;
		}
		bool getShakeEnemyCameraAffected() const {
			return shakeEnemyCameraAffected;
		}


		bool isAttackBoostEnabled() const {
			return attackBoost.enabled;
		}
		const AttackBoost *getAttackBoost() const {
			return &attackBoost;
		}
		...
		virtual int getTotalSpeed(const TotalUpgrade *) const {
			return speed;
		}
	...
	public:
		...
		//get
		inline int getAttackStrength() const {
			return attackStrength;
		}
		inline int getAttackVar() const {
			return attackVar;
		}
		...
		inline string getSpawnUnit() const {
			return spawnUnit;
		}
		inline int getSpawnUnitCount() const {
			return spawnUnitcount;
		}
		inline bool getSpawnUnitAtTarget() const {
			return spawnUnitAtTarget;
		}

		//get proj
		inline bool getProjectile() const {
			return projectile;
		}
		inline StaticSound *getProjSound() const {
			return projSounds.getRandSound();
		}

		//get splash
		inline bool getSplash() const {
			return splash;
		}
		inline int getSplashRadius() const {
			return splashRadius;
		}
		inline bool getSplashDamageAll() const {
			return splashDamageAll;
		}
	...
	public:
		ProduceSkillType();
		bool getAnimProgressBound() const {
			return animProgressBound;
		}
		virtual void load(const XmlNode * sn, const XmlNode * attackBoostsNode,
			const string & dir, const TechTree * tt,
			const FactionType * ft, std::map < string,
	...
	public:
		...
		inline int getSpawnStartTime() const {
		...
		inline int getSpawnProbability() const {
	...
	public:
		FogOfWarSkillType();
		bool getFowEnable() const {
			return fowEnable;
		}
		bool getApplyToTeam() const {
			return applyToTeam;
		}

  ...
  namespace Game
  {
    ...
    bool AttackBoost::isAffected (const Unit * source, const Unit * dest) const
    {
          ...
          (includeSelf == true || source != dest))
      {
        ...
        {
          float distance =
            source->getCenteredPos ().dist (dest->getCenteredPos ());
          if (distance <= radius)
          {
            result = true;
          }
    ...
    string AttackBoost::getDesc (bool translatedValue) const
    {
      ...
      if (enabled)
      {
        ...
        str +=
          indent + lang.getString ("effectRadius") + ": " +
          intToStr (radius) + "\n";
        if (allowMultipleBoosts == false)
        {
          ...
          str +=
            indent + lang.getString ("allowMultiBoost") + ": " + allowIt +
            "\n";
        }
        ...
        else
        {
          ...
          for (unitIter = outputUnits.begin ();
               unitIter != outputUnits.end (); ++unitIter)
          {
                                ...
                                const SkillType * skillType)
    {
      ...
      enabled =
        (attackBoostNode->getAttribute ("enabled")->getIntValue () != 0);
      allowMultipleBoosts =
        (attackBoostNode->getAttribute ("allowMultipleBoosts")->
         getIntValue () != 0);
      radius = attackBoostNode->getAttribute ("radius")->getIntValue ();
      targetType =
        static_cast < AttackBoostTargetType >
      ...
      if (attackBoostNode->hasChild ("UnitType") == true)
      {
        ...
        for (unsigned int i = 0; i < attackBoostNodeList.size (); ++i)
        {
          ...
          const UnitType *unitType =
            faction->getType ()->getUnitType (unitTypeName);
          if (unitType != NULL)
          {
      ...
      {
        vector < XmlNode * >tagNodeList =
          attackBoostNode->getChildList ("tag");
...
//      bool allowMultipleBoosts;
      attackBoostNode->addAttribute ("allowMultipleBoosts",
                                     intToStr (allowMultipleBoosts),
                                     mapTagReplacements);
//      int radius;
      attackBoostNode->addAttribute ("radius", intToStr (radius),
                                     mapTagReplacements);
...
//      vector<const UnitType *> boostUnitList;
      ...
      for (unitIter = boostUnitList.begin ();
           unitIter != boostUnitList.end (); ++unitIter)
      {
        const UnitType *unit = *unitIter;
...
// =====================================================
                                                      ...
                                                      attackBoostNode)
    {
      ...
      if (attackBoostsNode != NULL && attackBoostName != "")
      {
        for (int i = 0; i < (int) attackBoostsNode->getChildCount (); ++i)
        {
          ...
          string sharedName =
            abn->getAttribute ("name")->getRestrictedValue ();
    ...
    void SkillType::loadAttackBoost (const XmlNode * attackBoostsNode,
                                     const XmlNode * attackBoostNode,
                                     const FactionType * ft,
                                     string parentLoader, const string & dir,
                                     string currentPath, std::map < string,
                                     vector < pair < string,
                                     ...
                                     const TechTree * tt)
    {
      ...
      if (attackBoostNode->hasAttribute ("name") == true)
      {
        ...
        attackBoostNode =
          findAttackBoostDetails (attackBoost.name, attackBoostsNode,
                                  attackBoostNode);
      }
      else
      {
      ...
      string targetType =
        attackBoostNode->getChild ("target")->getAttribute ("value")->
        getValue ();
      ...
      attackBoost.radius =
        attackBoostNode->getChild ("radius")->getAttribute ("value")->
        getIntValue ();

      attackBoost.includeSelf = false;
      if (attackBoostNode->getChild ("target")->
          hasAttribute ("include-self") == true)
      {
        attackBoost.includeSelf =
          attackBoostNode->getChild ("target")->
          getAttribute ("include-self")->getBoolValue ();
      }
      ...
      vector < XmlNode * >targetNodes =
        targetNode->getChildList ("unit-type");
      ...
      if (attackBoostNode->hasChild ("particles") == true)
      {
        ...
        if (particleEnabled == true)
        {
          if (particleNode->hasChild ("originator-particle-file") == true)
          {
            ...
            attackBoost.unitParticleSystemTypeForSourceUnit->
              load (particleFileNode, dir, currentPath + path,
    ...
    void SkillType::load (const XmlNode * sn,
                          const XmlNode * attackBoostsNode,
                          const string & dir, const TechTree * tt,
                          const FactionType * ft, std::map < string,
                          vector < pair < string, string > > >&loadedFileList,
                          string parentLoader)
    {
      //name
      name =
        sn->getChild ("name")->getAttribute ("value")->getRestrictedValue ();

      //ep cost
      if (sn->hasChild ("ep-cost") == true)
      {
        mpCost =
          sn->getChild ("ep-cost")->getAttribute ("value")->getIntValue ();
      }
      else
      {
        mpCost = 0;
      ...
      {
        hpCost =
          sn->getChild ("hp-cost")->getAttribute ("value")->getIntValue ();
      }
      else
      {
        hpCost = 0;
      ...
      {
        speed =
          sn->getChild ("speed")->getAttribute ("value")->getIntValue ();
      }
      else
      {
        speed = 0;
      ...
      if (sn->hasChild ("animation") == true)
      {
        ...
        {
          string path =
            animationList[i]->getAttribute ("path")->
        ...
        if (particleEnabled)
        {
          for (int i = 0; i < (int) particleNode->getChildCount (); ++i)
          {
            ...
            if (particleNode->getChild (i)->hasAttribute ("start-time"))
            {
              ...
              unitParticleSystemType->setStartTime (particleNode->
                                                    getChild (i)->
                                                    getAttribute
                                                    ("start-time")->
                                                    getFloatValue ());
            ...
            else if (particleNode->hasAttribute ("end-time"))
            {
              ...
              unitParticleSystemType->setEndTime (particleNode->
                                                  getAttribute ("end-time")->
                                                  getFloatValue ());
            }
                ...
                && particleNode->getChild (i)->hasAttribute ("maxHp"))
            {
              ...
              unitParticleSystemType->setMinHp (particleNode->getChild (i)->
                                                getAttribute ("minHp")->
                                                getIntValue ());
              unitParticleSystemType->setMaxHp (particleNode->getChild (i)->
                                                getAttribute ("maxHp")->
                                                getIntValue ());

              if (particleNode->getChild (i)->hasAttribute ("ispercentbased"))
              {
                unitParticleSystemType->setMinmaxIsPercent (particleNode->
                                                            getChild (i)->
                                                            getAttribute
                                                            ("ispercentbased")->
                                                            getBoolValue ());
      ...
      for (unsigned int i = 0; i < soundNodeList.size (); ++i)
      {
        ...
        if (soundNode->getAttribute ("enabled")->getBoolValue ())
        {
          ...
          skillSound->getSoundContainer ()->resize ((int) soundNode->
                                                    getChildCount ());
      ...
      if (sn->hasChild ("attack-boost") == true)
      {
        ...
        loadAttackBoost (attackBoostsNode, attackBoostNode, ft, parentLoader,
                         dir, currentPath, loadedFileList, tt);
      }
      ...
      if (sn->hasChild ("shake"))
      {
        ...
        shakeIntensity =
          shakeNode->getAttribute ("intensity")->getIntValue ();
        shakeDuration = shakeNode->getAttribute ("duration")->getIntValue ();
        if (shakeNode->hasAttribute ("start-time"))
        {
          shakeStartTime =
            shakeNode->getAttribute ("start-time")->getFloatValue ();
        }
        if (shakeNode->hasChild ("self"))
        {
          shakeSelfEnabled =
            shakeNode->getChild ("enemy")->getAttribute ("enabled")->
            getBoolValue ();
          shakeSelfVisible =
            shakeNode->getChild ("self")->getAttribute ("visible")->
            getBoolValue ();
          shakeSelfInCameraView =
            shakeNode->getChild ("self")->getAttribute ("in-camera-view")->
            getBoolValue ();
          shakeSelfCameraAffected =
            shakeNode->getChild ("self")->
            getAttribute ("camera-distance-affected")->getBoolValue ();
        }
        if (shakeNode->hasChild ("team"))
        {
          shakeTeamEnabled =
            ...
            getBoolValue ();
          shakeTeamVisible =
            shakeNode->getChild ("team")->getAttribute ("visible")->
            getBoolValue ();
          shakeTeamInCameraView =
            shakeNode->getChild ("team")->getAttribute ("in-camera-view")->
            getBoolValue ();
          shakeTeamCameraAffected =
            shakeNode->getChild ("team")->
            getAttribute ("camera-distance-affected")->getBoolValue ();
        }
        if (shakeNode->hasChild ("enemy"))
        {
          shakeEnemyEnabled =
            ...
            getBoolValue ();
          shakeEnemyVisible =
            shakeNode->getChild ("enemy")->getAttribute ("visible")->
            getBoolValue ();
          shakeEnemyInCameraView =
            shakeNode->getChild ("enemy")->getAttribute ("in-camera-view")->
            getBoolValue ();
          shakeEnemyCameraAffected =
            shakeNode->getChild ("enemy")->
            getAttribute ("camera-distance-affected")->getBoolValue ();
        }
    ...
    bool SkillType::
      CanCycleNextRandomAnimation (const int *animationRandomCycleCount) const
    {
      bool result = true;
                                    ...
                                    int *animationRandomCycleCount) const
    {
      ...
      if (animations.size () > 1)
      {
        ...
        {
          bool canCycle =
            CanCycleNextRandomAnimation (animationRandomCycleCount);
          if (canCycle == true)
          {
...
//      int mpCost;
      skillTypeNode->addAttribute ("mpCost", intToStr (mpCost),
                                   mapTagReplacements);
//      int hpCost;
      skillTypeNode->addAttribute ("hpCost", intToStr (hpCost),
                                   mapTagReplacements);
//    int speed;
      skillTypeNode->addAttribute ("speed", intToStr (speed),
                                   mapTagReplacements);
//    int animSpeed;
...
//      RandomGen random;
      skillTypeNode->addAttribute ("random",
                                   intToStr (random.getLastNumber ()),
                                   mapTagReplacements);
//      AttackBoost attackBoost;
      attackBoost.saveGame (skillTypeNode);
//      static int nextAttackBoostId;
      skillTypeNode->addAttribute ("nextAttackBoostId",
                                   intToStr (nextAttackBoostId),
                                   mapTagReplacements);
//      UnitParticleSystemTypes unitParticleSystemTypes;
      for (UnitParticleSystemTypes::iterator it =
           unitParticleSystemTypes.begin ();
...
//varios
    ...
    AttackSkillType::~AttackSkillType ()
    {
      ...
      splashParticleSystemType = NULL;
      deleteValues (projSounds.getSounds ().begin (),
                    projSounds.getSounds ().end ());
      projSounds.clearSounds ();
    }

    void AttackSkillType::load (const XmlNode * sn,
                                const XmlNode * attackBoostsNode,
                                const string & dir, const TechTree * tt,
                                const FactionType * ft, std::map < string,
                                vector < pair < string,
                                ...
                                string parentLoader)
    {
      ...
      attackVar =
        sn->getChild ("attack-var")->getAttribute ("value")->getIntValue ();
      ...
      string attackTypeName =
        sn->getChild ("attack-type")->getAttribute ("value")->
        getRestrictedValue ();
      attackType = tt->getAttackType (attackTypeName);
      attackStartTime =
        sn->getChild ("attack-start-time")->getAttribute ("value")->
        getFloatValue ();

      if (sn->hasChild ("unit"))
      {
        spawnUnit =
          sn->getChild ("unit")->getAttribute ("value")->getValue ();
        spawnUnitcount =
          sn->getChild ("unit")->getAttribute ("amount")->getIntValue ();
        if (sn->getChild ("unit")->hasAttribute ("spawnAtTarget"))
        {
          spawnUnitAtTarget =
            sn->getChild ("unit")->getAttribute ("spawnAtTarget")->
            getBoolValue ();
        }
        else
        {
        ...
        if (projectile)
        {
          ...
          if (projectileNode->hasChild ("particle"))
          {
            const XmlNode *particleNode =
              projectileNode->getChild ("particle");
            bool particleEnabled =
              particleNode->getAttribute ("value")->getBoolValue ();
            if (particleEnabled)
            {
              ...
              loadedFileList[currentPath +
                             path].push_back (make_pair (parentLoader,
                                                         ...
                                                         ()));
              projectileType->
                setProjectileParticleSystemType
      ...
      else
      {
        ...
        for (unsigned int i = 0; i < projectilesNodeList.size (); ++i)
        {
          ...
          ProjectileType *projectileType = new ProjectileType ();
          projectileType->load (projectileNode, dir, tt->getPath (),
                                loadedFileList, parentLoader);
      ...
      {
        splashRadius =
          splashNode->getChild ("radius")->getAttribute ("value")->
          getIntValue ();
        splashDamageAll =
          splashNode->getChild ("damage-all")->getAttribute ("value")->
          getBoolValue ();
        ...
        if (particleEnabled)
        {
          ...
          splashParticleSystemType->load (particleNode, dir,
                                          currentPath + path,
                                          &Renderer::getInstance (),
                                          loadedFileList, parentLoader,
                                          tt->getPath ());
...
// Get the amount to boost the attack animation speed by (based on attack-speed upgrades)
    int AttackSkillType::getAnimSpeedBoost (const TotalUpgrade * totalUpgrade) const
    {
      ...
      {
        return animSpeed * (totalUpgrade->getAttackSpeed (NULL) /
                            (double) 100);
      }
      else
      {
...
//get totals
    int AttackSkillType::getTotalAttackStrength (const TotalUpgrade *
                                                 totalUpgrade) const
    {
    ...
    int AttackSkillType::getTotalAttackRange (const TotalUpgrade *
                                              totalUpgrade) const
    {
...
//    int attackStrength;
      attackSkillTypeNode->addAttribute ("attackStrength",
...
//    int attackVar;
      attackSkillTypeNode->addAttribute ("attackVar", intToStr (attackVar),
                                         mapTagReplacements);
//    int attackRange;
      attackSkillTypeNode->addAttribute ("attackRange",
                                         intToStr (attackRange),
                                         mapTagReplacements);
//      const AttackType *attackType;
      if (attackType != NULL)
      {
        attackSkillTypeNode->addAttribute ("attackType",
                                           attackType->getName (false),
                                           mapTagReplacements);
      }
...
//      float attackStartTime;
      attackSkillTypeNode->addAttribute ("attackStartTime",
                                         ...
                                         mapTagReplacements);
//      string spawnUnit;
      attackSkillTypeNode->addAttribute ("spawnUnit", spawnUnit,
                                         mapTagReplacements);
//      int spawnUnitcount;
      attackSkillTypeNode->addAttribute ("spawnUnitcount",
                                         intToStr (spawnUnitcount),
                                         mapTagReplacements);
...
//    bool splash;
      attackSkillTypeNode->addAttribute ("splash", intToStr (splash),
                                         mapTagReplacements);
//    int splashRadius;
      attackSkillTypeNode->addAttribute ("splashRadius",
                                         intToStr (splashRadius),
                                         mapTagReplacements);
//    bool splashDamageAll;
      attackSkillTypeNode->addAttribute ("splashDamageAll",
                                         intToStr (splashDamageAll),
                                         mapTagReplacements);
...
// =====================================================
    ...
    void ProduceSkillType::load (const XmlNode * sn,
                                 const XmlNode * attackBoostsNode,
                                 const string & dir, const TechTree * tt,
                                 const FactionType * ft, std::map < string,
                                 vector < pair < string,
                                 ...
                                 string parentLoader)
    {
      ...
      {
        animProgressBound =
          sn->getChild ("anim-progress-bound")->getAttribute ("value")->
          getBoolValue ();
      }
      else
      {
    ...
    void ProduceSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      produceSkillTypeNode->addAttribute ("animProgressBound",
                                          intToStr (animProgressBound),
                                          mapTagReplacements);
    }
// =====================================================
...
// =====================================================
    ...
    void UpgradeSkillType::load (const XmlNode * sn,
                                 const XmlNode * attackBoostsNode,
                                 const string & dir, const TechTree * tt,
                                 const FactionType * ft, std::map < string,
                                 vector < pair < string,
    ...
    void UpgradeSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      upgradeSkillTypeNode->addAttribute ("animProgressBound",
                                          intToStr (animProgressBound),
                                          mapTagReplacements);
    }
...
// =====================================================
    ...
    void BeBuiltSkillType::load (const XmlNode * sn,
                                 const XmlNode * attackBoostsNode,
                                 const string & dir, const TechTree * tt,
                                 const FactionType * ft, std::map < string,
                                 vector < pair < string,
                                 ...
                                 string parentLoader)
    {
      ...
      else if (sn->hasChild ("anim-hp-bound"))
      {                         // deprecated!!!! remove it when you see it after 15th July 2011
        animProgressBound =
          sn->getChild ("anim-hp-bound")->getAttribute ("value")->
          getBoolValue ();
      }
      else
      {
    ...
    void BeBuiltSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      beBuiltSkillTypeNode->addAttribute ("animProgressBound",
                                          intToStr (animProgressBound),
                                          mapTagReplacements);
    }
...
// =====================================================
    ...
    void MorphSkillType::load (const XmlNode * sn,
                               const XmlNode * attackBoostsNode,
                               const string & dir, const TechTree * tt,
                               const FactionType * ft, std::map < string,
                               vector < pair < string,
    ...
    void MorphSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      morphSkillTypeNode->addAttribute ("animProgressBound",
                                        intToStr (animProgressBound),
                                        mapTagReplacements);
    }
...
// =====================================================
    ...
    void DieSkillType::load (const XmlNode * sn,
                             const XmlNode * attackBoostsNode,
                             const string & dir, const TechTree * tt,
                             const FactionType * ft, std::map < string,
                             vector < pair < string,
                             string > > >&loadedFileList, string parentLoader)
    {
      ...
      if (sn->hasChild ("spawn"))
      {
        ...
        spawn = true;
        spawnStartTime =
          spawnNode->getAttribute ("start-time")->getFloatValue ();
        spawnUnit =
          spawnNode->getChild ("unit")->getAttribute ("value")->getValue ();
        spawnUnitcount =
          spawnNode->hasChild ("amount") ? spawnNode->getChild ("amount")->
          getAttribute ("value")->getIntValue () : 0;
        if (spawnNode->hasChild ("health-percent"))
        {
          spawnUnitHealthPercentMin =
            spawnNode->getChild ("health-percent")->getAttribute ("min")->
            getIntValue ();
          spawnUnitHealthPercentMax =
            spawnNode->getChild ("health-percent")->getAttribute ("max")->
            getIntValue ();
        }
        spawnProbability =
          spawnNode->hasChild ("probability") ? spawnNode->
          getChild ("probability")->getAttribute ("value")->
          getIntValue () : spawnProbability;
      }                         // else keep defaults
    }
    ...
    void DieSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      dieSkillTypeNode->addAttribute ("fade", intToStr (fade),
                                      mapTagReplacements);
...
// =====================================================
    ...
    void FogOfWarSkillType::load (const XmlNode * sn,
                                  const XmlNode * attackBoostsNode,
                                  const string & dir, const TechTree * tt,
                                  const FactionType * ft, std::map < string,
                                  vector < pair < string,
                                  ...
                                  string parentLoader)
    {
      ...
      fowEnable =
        sn->getChild ("enable-fog")->getAttribute ("value")->getBoolValue ();
      applyToTeam =
    ...
    void FogOfWarSkillType::saveGame (XmlNode * rootNode)
    {
      ...
      fogSkillTypeNode->addAttribute ("apply-team", intToStr (applyToTeam),
                                      mapTagReplacements);
      fogSkillTypeNode->addAttribute ("duration",
                                      floatToStr (durationTime, 6),
                                      mapTagReplacements);
    }

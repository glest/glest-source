	...
	namespace Game {
		...
		void Lang::loadGameStrings(string uselanguage, bool loadFonts,
			...
			loadGameStringProperties(uselanguage,
				gameStringsAllLanguages[this->language], true,
				fallbackToDefault);
			...
			if (languageChanged == true) {
				...
				if (lang.hasString("FONT_SCALE_SIZE")) {
					Font::scaleFontValue =
						strToFloat(lang.getString("FONT_SCALE_SIZE"));
				}
				...
				if (lang.hasString("FONT_MULTIBYTE")) {
					Font::fontIsMultibyte =
						strToBool(lang.getString("FONT_MULTIBYTE"));
				}

				if (lang.hasString("FONT_RIGHTTOLEFT")) {
					Font::fontIsRightToLeft =
						strToBool(lang.getString("FONT_RIGHTTOLEFT"));
				}
				...
				if (lang.hasString("FONT_BASE_SIZE_WINDOWS")) {
					...
					// 30000 for Chinese
					Font::baseSize =
						strToInt(lang.getString("FONT_BASE_SIZE_WINDOWS"));
				}
				...
				if (lang.hasString("FONT_HEIGHT_TEXT_WINDOWS")) {
					Font::langHeightText =
						lang.getString("FONT_HEIGHT_TEXT_WINDOWS",
				...
				if (lang.hasString("FONT_CHARCOUNT_WINDOWS")) {
					...
					// 30000 for Chinese
					Font::charCount =
						strToInt(lang.getString("FONT_CHARCOUNT_WINDOWS"));
				}
		...
		void Lang::loadGameStringProperties(string uselanguage,
			...
			string languageFile =
				getGameCustomCoreDataPath(data_path,
					"data/lang/" + uselanguage + ".lng");
			if (fileMustExist == false && fileExists(languageFile) == false) {
				return;
			} else if (fileExists(languageFile) == false
		...
		bool Lang::loadTechTreeStrings(string techTree, bool forceLoad) {
			...
			string path =
				techTreeFolder + "lang/" + techTree + "_" + language + ".lng";
			string pathDefault =
				techTreeFolder + "lang/" + techTree + "_default.lng";
			if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
				enabled)
				SystemFlags::OutputDebug(SystemFlags::debugSystem,
					"In [%s::%s Line: %d] path = [%s] pathDefault = [%s]\n",
					__FILE__, __FUNCTION__, __LINE__,
					path.c_str(), pathDefault.c_str());
			...
			if (fileExists(path)) {
				...
				if (forceLoad == true ||
					...
					techTreeStringsAllLanguages[techTree][this->language].
					getpath() && techTree != techNameLoaded) {
					techNameLoaded = techTree;
				}
			} else {
				if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
				...
				path =
					techTreeFolder + "lang/" + techTree + "_" + default_language +
					".lng";
				if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).
					enabled)
					SystemFlags::OutputDebug(SystemFlags::debugSystem,
		...
		void Lang::loadTilesetStrings(string tileset) {
			...
			string path =
				tilesetFolder + "lang/" + tileset + "_" + language + ".lng";
		...
		string Lang::getLanguageFile(string uselanguage) {
			...
			if (foundMatch == false) {
				...
				for (unsigned int i = 0;
					i < langResults.size() && foundMatch == false; ++i) {
					string testLanguageFile =
		...
		map < string,
			string > Lang::getDiscoveredLanguageList(bool searchKeyIsLangName) {
			...
			for (unsigned int i = 0; i < langResults.size(); ++i) {
				...
				string nativeName =
					getNativeLanguageName(testLanguage, testLanguageFile);
			...
			for (unsigned int i = 0; i < langResults2.size(); ++i) {
				...
				if (std::
					find(langResults.begin(), langResults.end(),

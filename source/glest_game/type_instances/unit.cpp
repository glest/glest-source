...
//map<void *,bool> Unit::deletedUnits;
    ...
    void UnitPathBasic::dumpMemoryList ()
    {
      ...
      for (std::map < UnitPathBasic *, bool >::iterator iterMap =
           UnitPathBasic::mapMemoryList.begin ();
           iterMap != UnitPathBasic::mapMemoryList.end (); ++iterMap)
      {
        printf ("************** ==> Unfreed UnitPathBasic pointer [%p]\n",
                iterMap->first);

        if (Unit::mapMemoryList2.find (iterMap->first) !=
            Unit::mapMemoryList2.end ())
        {
    ...
    {
      std::string result =
        "unit path blockCount = " + intToStr (blockCount) +
        "\npathQueue size = " + intToStr (pathQueue.size ());
      for (int idx = 0; idx < (int) pathQueue.size (); ++idx)
      {
        result +=
          " index = " + intToStr (idx) + " value = " +
          pathQueue[idx].getString ();
      }
...
//      int blockCount;
      unitPathBasicNode->addAttribute ("blockCount", intToStr (blockCount),
                                       mapTagReplacements);
//      vector<Vec2i> pathQueue;
      for (unsigned int i = 0; i < pathQueue.size (); ++i)
      {
        ...
        pathQueueNode->addAttribute ("vec", vec.getString (),
                                     mapTagReplacements);
      }
    }
    ...
    void UnitPathBasic::loadGame (const XmlNode * rootNode)
    {
      ...
      blockCount =
        unitPathBasicNode->getAttribute ("blockCount")->getIntValue ();

      pathQueue.clear ();
      vector < XmlNode * >pathqueueNodeList =
        unitPathBasicNode->getChildList ("pathQueue");
      for (unsigned int i = 0; i < pathqueueNodeList.size (); ++i)
      {
        XmlNode *node = pathqueueNodeList[i];

        Vec2i vec =
          Vec2i::strToVec2 (node->getAttribute ("vec")->getValue ());
...
// =====================================================
    ...
    void UnitReference::saveGame (XmlNode * rootNode)
    {
      ...
      if (faction != NULL)
      {
        unitRefNode->addAttribute ("factionIndex",
                                   intToStr (faction->getIndex ()),
                                   mapTagReplacements);
      }
    }
    ...
    void UnitReference::loadGame (const XmlNode * rootNode, World * world)
    {
      ...
      {
        int factionIndex =
          unitRefNode->getAttribute ("factionIndex")->getIntValue ();
    ...
    UnitAttackBoostEffect::~UnitAttackBoostEffect ()
    {
      ...
      {
        bool particleValid =
          Renderer::getInstance ().validateParticleSystemStillExists (ups,
                                                                      rsGame);
    ...
    void UnitAttackBoostEffect::
      applyLoadedAttackBoostParticles (UnitParticleSystemType * upstPtr,
                                       const XmlNode * node, Unit * unit)
    {
      if (upstPtr != NULL)
      {
        bool showUnitParticles =
          Config::getInstance ().getBool ("UnitParticles", "true");
    ...
    void UnitAttackBoostEffect::loadGame (const XmlNode * rootNode,
                                          Unit * unit, World * world,
                                          bool applyToOriginator)
    {
      ...
      {
        unitId =
          unitAttackBoostEffectNode->getAttribute ("source")->getIntValue ();
        unitPtr = unit;
      ...
      if (unitAttackBoostEffectNode->hasAttribute ("source-faction") == true)
      {
        ...
        if (unitType != NULL)
        {
          const SkillType *skillType =
            unitType->getSkillType (skillTypeName, skillClass);
          if (skillType != NULL)
          {
...
//                              unit->getId(),unit->getType()->getName(false).c_str(),applyToOriginator,boost->unitParticleSystemTypeForSourceUnit,boost->unitParticleSystemTypeForAffectedUnit,boost->enabled,boost->getDesc(false).c_str());
      ...
      {
        printf
          ("******!!!! unit [%d - %s] applyToOriginator: %d NO BOOST FOUND!!!\n",
    ...
    {
      std::map < string, string > mapTagReplacements;
      XmlNode *unitAttackBoostEffectNode =
        rootNode->addChild ("UnitAttackBoostEffect");
...
//      UnitParticleSystemType *upst;
    ...
    void UnitAttackBoostEffectOriginator::loadGame (const XmlNode * rootNode,
                                                    Unit * unit,
                                                    World * world)
    {
      const XmlNode *unitAttackBoostEffectOriginatorNode =
        rootNode->getChild ("UnitAttackBoostEffectOriginator");
    ...
    {
      std::map < string, string > mapTagReplacements;
      XmlNode *unitAttackBoostEffectOriginatorNode =
        rootNode->addChild ("UnitAttackBoostEffectOriginator");
...
// ============================ Constructor & destructor =============================
                ...
                CardinalDir placeFacing):BaseColorPickEntity (), id (id)
    {
      ...
      maxQueuedCommandDisplayCount =
        config.getInt ("MaxQueuedCommandDisplayCount", "15");
...
//      }
    ...
    Unit::~Unit ()
    {
      ...
      Renderer::getInstance ().removeParticleSystemsForParticleOwner (this,
                                                                      rsGame);
    ...
    void Unit::dumpMemoryList ()
    {
      ...
      for (std::map < Unit *, bool >::iterator iterMap =
           Unit::mapMemoryList.begin ();
    ...
    bool Unit::isNetworkCRCEnabled ()
    {
      ...
      if (game != NULL)
      {
        if (game->isFlagType1BitEnabled (ft1_network_synch_checks_verbose) ==
            true)
        {
          isNetworkCRCEnabled = true;
        }
        else if (game->isFlagType1BitEnabled (ft1_network_synch_checks) ==
                 true)
        {
          isNetworkCRCEnabled = true;
        }
    ...
    string Unit::getParticleInfo () const
    {
      ...
      if (networkCRCParticleInfoList.empty () == false)
      {
        for (unsigned int index = 0;
             index < networkCRCParticleInfoList.size (); ++index)
        {
    ...
    void Unit::end (ParticleSystem * particleSystem)
    {
      ...
      vector < ParticleSystem * >::iterator iterFind =
        find (attackParticleSystems.begin (), attackParticleSystems.end (),
              particleSystem);
      ...
      vector < UnitParticleSystem * >::iterator iterFind1 =
        find (smokeParticleSystems.begin (), smokeParticleSystems.end (),
              particleSystem);
      ...
      iterFind =
        find (fireParticleSystems.begin (), fireParticleSystems.end (),
              particleSystem);
      ...
      iterFind1 =
        find (damageParticleSystems.begin (), damageParticleSystems.end (),
              particleSystem);
      ...
      iterFind1 =
        find (unitParticleSystems.begin (), unitParticleSystems.end (),
              particleSystem);
...
// ====================================== get ======================================
    ...
    Vec2f Unit::getFloatCenteredPos () const
    {
      ...
      return Vec2f (truncateDecimal <
                    float >(pos.x - 0.5f + type->getSize () / 2.f, 6),
                    truncateDecimal <
                    float >(pos.y - 0.5f + type->getSize () / 2.f, 6));
    }
    ...
    void Unit::calculateXZRotation ()
    {
      ...
      if (lastPos != pos)
      {                         // targetPosCalc ( maybe also sometimes needed if no move ? terrain flatting... )
    ...
    int Unit::getProductionPercent () const
    {
      if (anyCommand ())
      {
        const ProducibleType *produced =
          commands.front ()->getCommandType ()->getProduced ();
        if (produced != NULL)
        {
          ...
          return clamp (progress2 * 100 / produced->getProductionTime (), 0,
                        100);
        }
      }
      return -1;
...
// ====================================== is ======================================
    ...
    bool Unit::isAnimProgressBound () const
    {
      ...
      {
        const BeBuiltSkillType *skill =
          dynamic_cast < const BeBuiltSkillType * >(currSkill);
        if (skill != NULL)
        {
      ...
      {
        const ProduceSkillType *skill =
          dynamic_cast < const ProduceSkillType * >(currSkill);
        if (skill != NULL)
        {
          result = skill->getAnimProgressBound ();
        }
      ...
      {
        const UpgradeSkillType *skill =
          dynamic_cast < const UpgradeSkillType * >(currSkill);
        if (skill != NULL)
        {
          result = skill->getAnimProgressBound ();
        }
      ...
      {
        const MorphSkillType *skill =
          dynamic_cast < const MorphSkillType * >(currSkill);
        if (skill != NULL)
        {
          result = skill->getAnimProgressBound ();
        }
    ...
    bool Unit::isBuildCommandPending () const
    {
      ...
      if (command != NULL)
      {
        const BuildCommandType *bct =
          dynamic_cast <
          const BuildCommandType * >(command->getCommandType ());
        if (bct != NULL)
        {
...
// ====================================== set ======================================
    ...
    void Unit::setCurrSkill (const SkillType * currSkill)
    {
      ...
      if (faction != NULL)
        faction->notifyUnitSkillTypeChange (this, currSkill);
      ...
      {
        //printf("File: %s line: %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__);
        game->getScriptManager ()->onUnitTriggerEvent (this,
                                                       utet_SkillChanged);
        //printf("File: %s line: %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__);
      }
    }
    ...
    FowAlphaCellsLookupItem Unit::getFogOfWarRadius (bool useCache) const
    {
      ...
      while (pci.next ())
      {
        ...
        {
          alpha =
            clamp (1.f - (dist - sightRange) / (World::indirectSightRange),
                   0.f, maxAlpha);
        }
    ...
    void Unit::setVisible (const bool visible)
    {
      ...
      {
        for (UnitParticleSystems::iterator it =
      ...
      {
        if (currentAttackBoostOriginatorEffect.currentAppliedEffect->ups !=
            NULL)
        {
          bool particleValid =
...
// =============================== Render related ==================================
    ...
    Model *Unit::getCurrentModelPtr ()
    {
      ...
      Model *result =
        currSkill->getAnimation (getAnimProgressAsFloat (), this,
                                 &lastModelIndexForCurrSkillType,
                                 &animationRandomCycleCount);
      if (currentModelIndexForCurrSkillType != lastModelIndexForCurrSkillType)
      {
        animationRandomCycleCount++;
        if (currSkill != NULL
            && animationRandomCycleCount >= currSkill->getAnimationCount ())
        {
...
//}
      ...
      if (currSkill != NULL && currSkill->getAnimationCount () > 1)
      {
        if (lastModelIndexForCurrSkillType >= 0)
        {
          const AnimationAttributes attributes =
            currSkill->getAnimationAttribute (lastModelIndexForCurrSkillType);
          if (attributes.fromHp != 0 || attributes.toHp != 0)
          {
            ...
            if (this->getHp () < attributes.fromHp
                || this->getHp () > attributes.toHp)
            {
    ...
    Vec3f Unit::getCurrVectorAsTarget () const
    {
      ...
      Vec3f result =
        getCurrVectorFlat () + Vec3f (0.f, type->getTargetHeight () / 2.f,
                                      0.f);
      result.x = truncateDecimal < float >(result.x, 6);
      result.y = truncateDecimal < float >(result.y, 6);
      result.z = truncateDecimal < float >(result.z, 6);
    ...
    Vec3f Unit::getCurrBurnVector () const
    {
      ...
      Vec3f result =
        getCurrVectorFlat () + Vec3f (0.f, type->getBurnHeight () / 2.f, 0.f);
      result.x = truncateDecimal < float >(result.x, 6);
      result.y = truncateDecimal < float >(result.y, 6);
      result.z = truncateDecimal < float >(result.z, 6);
    ...
    {
      float result =
        (static_cast < float >(progress) / static_cast <
         float >(PROGRESS_SPEED_MULTIPLIER));
      result = truncateDecimal < float >(result, 6);
      return result;
    }

    Vec3f Unit::getVectorFlat (const Vec2i & lastPosValue,
                               const Vec2i & curPosValue) const
    {
      ...
      if (currSkill->getClass () == scMove)
      {
        ...
        v.x =
          lastPosValue.x + progressAsFloat * (curPosValue.x - lastPosValue.x);
        v.z =
          lastPosValue.y + progressAsFloat * (curPosValue.y - lastPosValue.y);
...
//any command
    bool Unit::anyCommand (bool validateCommandtype) const
    {
      ...
      {
        for (Commands::const_iterator it = commands.begin ();
             it != commands.end (); ++it)
        {
           ...
           it != commands.end (); ++it)
      {
        ...
        if (ct->getClass () == ccProduce || ct->getClass () == ccMorph)
        {
          const UnitType *producedUnitType =
            static_cast < const UnitType * >(ct->getProduced ());
...
// =================== route stack ===================
    ...
    void Unit::born (const CommandType * ct)
    {
      ...
      {
        this->hp =
          type->getTotalMaxHp (&totalUpgrade) *
          type->getStartHpPercentage () / 100;
      }
      ...
      {
        this->ep =
          type->getTotalMaxEp (&totalUpgrade) *
          type->getStartEpPercentage () / 100;
      }
    }
...
//}
    ...
    {
      for (Observers::iterator it = observers.begin ();
           it != observers.end (); ++it)
      {
...
// =================== Other ===================
                                                 ...
                                                 targetUnit) const
    {
      ...
      if (targetUnit != NULL)
      {
        ...
        {
          commandType =
            type->getFirstAttackCommand (targetUnit->getCurrField ());
        }
        //repair allies
        else
        {
          if (targetUnit->isBuilt () == false
              || targetUnit->isDamaged () == true)
          {
            commandType =
              type->getFirstRepairCommand (targetUnit->getType ());
          }
      ...
      else
      {
        ...
        {
          commandType =
            type->getFirstHarvestCommand (resource->getType (),
                                          this->getFaction ());
        }
      }
    ...
    int64 Unit::getUpdateProgress ()
    {
      ...
      if (currSkill->getClass () != scDie)
      {
        ...
        {
          if (changedActiveCommandFrame - lastChangedActiveCommandFrame >=
              MIN_FRAMECOUNT_CHANGE_COMMAND_SPEED)
          {
...
//              }
      ...
      if (currSkill->getClass () == scMove)
      {
        ...
        heightFactor =
          clamp (speedMultiplier + heightDiff / (5 * speedMultiplier),
                 (2 * (speedMultiplier / 10)), 5 * speedMultiplier);
      }
    ...
    {
      int64 speedDenominator =
        (speedDivider * updateFPS) * PROGRESS_SPEED_MULTIPLIER;
      return speedDenominator;
    }
    int64 Unit::getUpdatedProgress (int64 currentProgress, int64 updateFPS,
                                    int64 speed, int64 diagonalFactor,
                                    int64 heightFactor)
    {

      int64 speedDenominator = getSpeedDenominator (updateFPS);
      int64 newProgress = currentProgress;
      int64 progressIncrease =
      ...
      if (speed > 0 && diagonalFactor > 0 && heightFactor > 0
          && progressIncrease == 0)
      {
        progressIncrease = 1;
      }
      ...
      if (isNetworkCRCEnabled () == true)
      {
        ...
        {
          SurfaceCell *sc = map->getSurfaceCell (map->toSurfCoords (pos));
          if (sc != NULL && sc->getObject () != NULL
              && sc->getObject ()->getType () != NULL)
          {
        ...
        snprintf (szBuf, 8095,
                  ...
                  ((speed * diagonalFactor * heightFactor) /
                   speedDenominator), height, airHeight, cellUnitHeight,
                  cellObjectHeight,
                  (currSkill !=
                   NULL ? currSkill->getName ().c_str () : "none"),
                  pos.getString ().c_str (), lastPos.getString ().c_str ());
...
//      }
      ...
      if (currSkill != currentAttackBoostOriginatorEffect.skillType)
      {
        ...
        if (currSkill->isAttackBoostEnabled () == true)
        {
          ...
          UnitUpdater *unitUpdater =
            this->game->getWorld ()->getUnitUpdater ();
      ...
      else
      {
        if (currSkill->isAttackBoostEnabled () == true)
        {
          ...
          vector < Unit * >candidates =
            unitUpdater->findUnitsInRange (this, attackBoost->radius);
              ...
              currentAttackBoostUnits.empty () == false)
          {
                 ...
                 size () - 1; i >= 0; --i)
            {
              ...
              std::vector < int >::iterator iterFound =
                std::find (candidateValidIdList.begin (),
              ...
              if (iterFound == candidateValidIdList.end ())
              {
                ...
                if (affectedUnit != NULL)
                {
                  ...
                  if (debugBoost)
                    printf
                      ("Removed attack boost from Unit [%d - %s] since they are NO LONGER in range\n",
                       affectedUnit->id,
    ...
    bool Unit::update ()
    {
      ...
      if (isAnimProgressBound () == true)
      {
        ...
        float targetProgressIntValue =
          targetProgress * ANIMATION_SPEED_MULTIPLIER;
        if (this->animProgress < targetProgressIntValue)
        {
          float diff = targetProgressIntValue - this->animProgress;
          float progressIncrease =
            static_cast < float >(this->animProgress) + diff / static_cast <
            float >(GameConstants::updateFps);
          // Ensure we increment at least a value of 1 of the action will be stuck infinitely
          if (diff > 0.f && GameConstants::updateFps > 0
              && progressIncrease == 0.f)
          {
      ...
      else
      {
        ...
        int64 progressIncrease =
          (animSpeed * heightFactor) / speedDenominator;
        // Ensure we increment at least a value of 1 of the action will be stuck infinitely
        if (currSkill->getAnimSpeed () > 0 && heightFactor > 0
            && progressIncrease == 0)
        {
          progressIncrease = 1;
        }
      ...
      if (currSkill->getClass () != scStop)
      {
        ...
        if (getProgressAsFloat () < 1.f / rotFactor)
        {
          if (type->getFirstStOfClass (scMove))
          {
            ...
            {
              float rotationTerm =
                targetRotation > lastRotation ? -360.f : +360.f;
              rotation =
                lastRotation + (targetRotation - lastRotation +
                                rotationTerm) * getProgressAsFloat () *
                rotFactor;
            }
          }
        }
      ...
      {
        bool canCycle =
          currSkill->CanCycleNextRandomAnimation (&animationRandomCycleCount);
        this->animProgress =
          currSkill->getClass () == scDie ? ANIMATION_SPEED_MULTIPLIER : 0;
        if (canCycle == true)
        {
          this->lastModelIndexForCurrSkillType = -1;
        }
    ...
    void Unit::updateTimedParticles ()
    {
      ...
      {
        for (int i = (int)queuedUnitParticleSystemTypes.size () - 1; i >= 0;
             i--)
        {
          UnitParticleSystemType *pst = queuedUnitParticleSystemTypes[i];
          if (pst != NULL)
          {
            if (truncateDecimal < float >(pst->getStartTime (), 6) <=
                truncateDecimal < float >(getAnimProgressAsFloat (), 6))
            {
        ...
        for (int i = (int)unitParticleSystems.size () - 1; i >= 0; i--)
        {
          ...
          if (ps != NULL)
          {
                ...
                true)
            {
              ...
              {
                float animProgressTime =
                  truncateDecimal < float >(getAnimProgressAsFloat (), 6);
                if (animProgressTime >= 0.99
                    || animProgressTime >= particleEndTime)
                {

                  ps->fade ();
                  unitParticleSystems.erase (unitParticleSystems.begin () +
                                             i);
                }
              }
            }
    ...
    bool Unit::unitHasAttackBoost (const AttackBoost * boost,
                                   const Unit * source)
    {
      bool result = false;
      for (unsigned int i = 0; i < currentAttackBoostEffects.size (); ++i)
      {
        ...
        if (effect != NULL && effect->boost->name == boost->name &&
            effect->getSource ()->getType ()->getId () ==
    ...
    bool Unit::applyAttackBoost (const AttackBoost * boost,
                                 const Unit * source)
    {
      if (boost == NULL)
      {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf
            ("=== APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: %d\n",
             this->getType ()->getName (false).c_str (), this->getId (),
             source->getType ()->getName (false).c_str (), source->getId (),
             hp);
        ...
        if (wasAlive == true)
        {
          ...
          if (this->hp <= 0)
          {
            ...
            StaticSound *sound =
              static_cast <
              const DieSkillType *
              >(this->getType ()->getFirstStOfClass (scDie))->getSound ();
            if (sound != NULL
            ...
            if (this->isDead ()
                && this->getCurrSkill ()->getClass () != scDie)
            {
              this->kill ();
            }
          }
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf
          ("APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: %d\n",
           this->getType ()->getName (false).c_str (), this->getId (),
           source->getType ()->getName (false).c_str (), source->getId (),
           hp);
    ...
    void Unit::deapplyAttackBoost (const AttackBoost * boost,
                                   const Unit * source)
    {
      if (boost == NULL)
      {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf
          ("=== DE-APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: %d\n",
           this->getType ()->getName (false).c_str (), this->getId (),
           source->getType ()->getName (false).c_str (), source->getId (),
           hp);
      ...
      int prevMaxHp = totalUpgrade.getMaxHp ();
      int prevMaxHpRegen = totalUpgrade.getMaxHpRegeneration ();
      totalUpgrade.deapply (source->getId (), &boost->boostUpgrade,
                            this->getId ());
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf
          ("DE-APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: %d\n",
           this->getType ()->getName (false).c_str (), this->getId (),
           source->getType ()->getName (false).c_str (), source->getId (),
           hp);
      ...
      if (isAlive ())
      {
        ...
        if (type->getTotalMaxHpRegeneration (&totalUpgrade) != 0)
        {
          if (currSkill->getClass () != scBeBuilt)
          {
            ...
            {
              bool decHpResult =
                decHp (-type->getTotalMaxHpRegeneration (&totalUpgrade));
              if (decHpResult)
              {
...
//decrements HP and returns if dead
    ...
    {
      char szBuf[8096] = "";
      snprintf (szBuf, 8095, "this->hp = %d, decrementValue = %d", this->hp,
                decrementValue);
    ...
    string Unit::getDescExtension (bool translatedValue) const
    {
      ...
      if (commands.empty () == false && commands.size () > 1)
      {
        ...
        for (unsigned int i = 0;
             i < min ((size_t) maxQueuedCommandDisplayCount,
                      commands.size ()); ++i)
        {
          const CommandType *ct = (*it)->getCommandType ();
          if (i == 0)
    ...
    string Unit::getDesc (bool translatedValue) const
    {
      ...
      {
        str +=
          lang.getString ("MaxUnitCount") + ": " +
          intToStr (faction->getCountForMaxUnitCount (type)) + "/" +
          intToStr (type->getMaxUnitCount ());
      }

      str +=
        "\n" + lang.getString ("Hp") + ": " + intToStr (hp) + "/" +
        intToStr (type->getTotalMaxHp (&totalUpgrade));
      if (type->getHpRegeneration () != 0
          || totalUpgrade.getMaxHpRegeneration () != 0)
      {
        str +=
          " (" + lang.getString ("Regeneration") + ": " +
          intToStr (type->getHpRegeneration ());
        if (totalUpgrade.getMaxHpRegeneration () != 0)
        {
      ...
      {
        str +=
          "\n" + lang.getString ("Ep") + ": " + intToStr (ep) + "/" +
          intToStr (type->getTotalMaxEp (&totalUpgrade));
      }
      if (type->getEpRegeneration () != 0
          || totalUpgrade.getMaxEpRegeneration () != 0)
      {
        str +=
          " (" + lang.getString ("Regeneration") + ": " +
          intToStr (type->getEpRegeneration ());
      ...
      //armor
      str +=
        "\n" + lang.getString ("Armor") + ": " +
        intToStr (getType ()->getArmor ());
      if (totalUpgrade.getArmor () != 0)
      {
        str += "+" + intToStr (totalUpgrade.getArmor ());
      }
      str +=
        " (" + getType ()->getArmorType ()->getName (translatedValue) + ")";

      //sight
      str +=
        "\n" + lang.getString ("Sight") + ": " +
        intToStr (getType ()->getSight ());
      ...
      if (enemyKills > 0 || nextLevel != NULL)
      {
        ...
        {
          str +=
            " (" + nextLevel->getName (translatedValue) + ": " +
            intToStr (nextLevel->getKills ()) + ")";
        }
      }
      ...
      {
        str +=
          "\n" + lang.getString ("Load") + ": " + intToStr (loadCount) +
          "  " + loadType->getName (translatedValue);
      }
      ...
      for (int i = 0; i < getType ()->getCostCount (); ++i)
      {
        ...
        if (r->getType ()->getClass () == rcConsumable)
        {
          ...
          str +=
            r->getAmount () <
            0 ? lang.getString ("Produce") +
            ": " : lang.getString ("Consume") + ": ";
          str +=
            intToStr (abs (r->getAmount ())) + " " +
            r->getType ()->getName (translatedValue);
        }
      }
    ...
    {
      faction->getUpgradeManager ()->computeTotalUpgrade (this,
                                                          &totalUpgrade);
    }
...
// ==================== PRIVATE ====================
    ...
    std::pair < CommandResult, string > Unit::checkCommand (Command * command) const
    {
           ...
           false))
      {
        ...
        if (command->getUnit () == this &&
            ...
            this->getType ()->getFirstRepairCommand (this->getType ()) !=
            NULL)
        {
      ...
      if (produced != NULL)
      {
                                 ...
                                 command->getCommandType ()) == false)
        {
          ...
          result.second =
            " - " + lang.getString ("Reqs") + " : " +
            produced->getResourceReqDesc (false,
                                          this->showTranslatedTechTree ());
          return result;
        }
      }
      ...
      if (command->getCommandType ()->getClass () == ccBuild)
      {
        ...
        if (faction->checkCosts (builtUnit, NULL) == false)
        {
          ...
          result.second =
            " - " + lang.getString ("Reqs") + " : " +
            builtUnit->getResourceReqDesc (false,
                                           this->showTranslatedTechTree ());
          return result;
        }
      }
      ...
      else if (command->getCommandType ()->getClass () == ccUpgrade)
      {
        const UpgradeCommandType *uct =
          static_cast <
          const UpgradeCommandType * >(command->getCommandType ());
    ...
    CommandResult Unit::undoCommand (Command * command)
    {
      ...
      if (getCurrCommand () == command
          && command->getCommandType ()->getClass () == ccMorph
      ...
      if (command->getCommandType ()->getClass () == ccBuild)
      {
        if (currSkill->getClass () != scBuild
            && currSkill->getClass () != scDie)
        {
                ...
                true)
            {
                   ...
                   iterMap != damageParticleSystemsInUse.end (); ++iterMap)
              {
                if (iterMap->second == ps)
                {
                  ...
                  {
                    pst =
                      type->damageParticleSystemTypes[foundParticleIndexType];
                    break;
                  }
                }
              }
    ...
    void Unit::
      checkCustomizedUnitParticleListTriggers (const UnitParticleSystemTypes &
                                               unitParticleSystemTypesList,
                                               bool applySkillChangeParticles)
    {
      if (showUnitParticles == true)
      {
        ...
        if (unitParticleSystems.empty () == false)
        {
          for (int index = (int)unitParticleSystems.size () - 1; index >= 0;
               index--)
          {
            ...
            if (ps != NULL)
            {
                  ...
                  true)
              {
                     ...
                     ps->getParticleType ()->getMinmaxEnabled ()))
                {
                  ...
                  if (ps->getParticleType () != NULL)
                  {
                    if (ps->getParticleType ()->getMinmaxIsPercent () ==
                        false)
                    {
                      if (hp < ps->getParticleType ()->getMinHp ()
                          || hp > ps->getParticleType ()->getMaxHp ())
                      {
                        stopParticle = true;
                    ...
                    {
                      int hpPercent =
                        (hp / type->getTotalMaxHp (&totalUpgrade) * 100);
                      if (hpPercent < ps->getParticleType ()->getMinHp ()
                          || hpPercent > ps->getParticleType ()->getMaxHp ())
                      {
                        stopParticle = true;
        ...
        if (unitParticleSystemTypesList.empty () == false)
        {
          ...
          for (UnitParticleSystemTypes::const_iterator iterParticleType =
               unitParticleSystemTypesList.begin ();
               iterParticleType != unitParticleSystemTypesList.end ();
               ++iterParticleType)
          {
            UnitParticleSystemType *pst = *iterParticleType;

            vector < ParticleSystemTypeInterface * >::iterator iterFind =
              std::find (systemTypesInUse.begin (), systemTypesInUse.end (),
                         pst);
            ...
            if (pst->getMinmaxEnabled () == true)
            {
              ...
              if (iterFind == systemTypesInUse.end ())
              {
                ...
                else
                {
                  int hpPercent =
                    (hp / type->getTotalMaxHp (&totalUpgrade) * 100);
                  if (hpPercent >= pst->getMinHp ()
                      && hpPercent <= pst->getMaxHp ())
                  {
                    showParticle = true;
    ...
    void Unit::
      queueTimedParticles (const UnitParticleSystemTypes &
                           unitParticleSystemTypesList)
    {
      if (showUnitParticles == true)
      {
    ...
    void Unit::setMeshPosInParticleSystem (UnitParticleSystem * ups)
    {
      if (ups->getMeshName () != "")
      {
        ...
        model->updateInterpolationVertices (currentAnimProgress, isAlive ()
                                            && !isAnimProgressBound ());
    ...
    void Unit::checkCustomizedParticleTriggers (bool force)
    {
      ...
      if (damageParticleSystems.empty () == false)
      {
        for (int i = (int)damageParticleSystems.size () - 1; i >= 0; --i)
        {
          ...
          if (force == true
              || (pst != NULL && pst->getMinmaxEnabled () == true))
          {
      ...
      {
        for (unsigned int i = 0; i < type->damageParticleSystemTypes.size ();
             ++i)
        {
          UnitParticleSystemType *pst = type->damageParticleSystemTypes[i];

          if (pst->getMinmaxEnabled () == true
              && damageParticleSystemsInUse.find (i) ==
              damageParticleSystemsInUse.end ())
          {
    ...
    {
      if (hp < type->getTotalMaxHp (&totalUpgrade) / 2 && hp > 0
          && alive == true)
      {
        //start additional particles
        if (showUnitParticles &&
        ...
        {
          for (unsigned int i = 0;
               i < type->damageParticleSystemTypes.size (); ++i)
          {
            UnitParticleSystemType *pst = type->damageParticleSystemTypes[i];
...
//                              rotation,
                  ...
                  (unitPath !=
                   NULL ? unitPath->toString ().c_str () : "NULL"));

        if (lastSynchDataString != string (szBuf) ||
            lastFile != file || lastLine != line || lastSource != source)
        {
...
//}
    ...
    void Unit::cleanupOldBadHarvestPos ()
    {
      ...
      if (needToCleanup == true)
      {
        ...
        for (std::map < Vec2i, int >::iterator iter =
             badHarvestPosList.begin (); iter != badHarvestPosList.end ();
             ++iter)
        {
    ...
    void Unit::setLastHarvestResourceTarget (const Vec2i * pos)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if (lastHarvestResourceTarget.second - getFrameCount () >=
              addInterval)
          {
...
//}
    ...
    bool Unit::isLastPathfindFailedFrameWithinCurrentFrameTolerance () const
    {
      ...
      if (enablePathfinderEnlargeMaxNodes)
      {
        ...
        result =
          (getFrameCount () - lastPathfindFailedFrame >=
           MIN_FRAME_ELAPSED_RETRY);
      }
      return result;
    }
    ...
    bool Unit::isLastStuckFrameWithinCurrentFrameTolerance (bool evalMode)
    {
      ...
      if (lastStuckFrame < MAX_BLOCKED_FRAME_THRESHOLD)
      {
        ...
        {
          MIN_FRAME_ELAPSED_RETRY =
            random.randRange (2, 6, intToStr (__LINE__));
        }
      }
      else
      {
        ...
        {
          MIN_FRAME_ELAPSED_RETRY =
            random.randRange (6, 8, intToStr (__LINE__));
        }
      }
      bool result (getFrameCount () - lastStuckFrame <=
                   (MIN_FRAME_ELAPSED_RETRY * 100));
      return result;
    }
    ...
    {
      Vec2i cellMapPos =
        this->getType ()->getFirstOccupiedCellInCellMap (pos);
      return cellMapPos;
    }
    ...
    {
      return (this->game !=
              NULL ? this->game->showTranslatedTechTree () : true);
    }
    ...
    string Unit::getNetworkCRCDecHpList () const
    {
      ...
      if (networkCRCDecHpList.empty () == false)
      {
        for (unsigned int index = 0; index < networkCRCDecHpList.size ();
             ++index)
        {
    ...
    std::string Unit::toString (bool crcMode) const
    {
      ...
      {
        result +=
          " name [" + this->type->getName (false) + "][" +
          intToStr (this->type->getId ()) + "]";
      }
      ...
      if (this->faction != NULL)
      {
        ...
        result +=
          "teamIndex = " + intToStr (this->faction->getTeam ()) + "\n";
        result +=
          "startLocationIndex = " +
          intToStr (this->faction->getStartLocationIndex ()) + "\n";
        if (crcMode == false)
        {
          result +=
            "thisFaction = " + intToStr (this->faction->getThisFaction ()) +
            "\n";
          result +=
            "control = " + intToStr (this->faction->getControlType ()) + "\n";
        }
        if (this->faction->getType () != NULL)
        {
          result +=
            "factionName = " + this->faction->getType ()->getName (false) +
            "\n";
        }
      }
      ...
      if (this->targetRef.getUnitId () >= 0)
      {
        ...
        result +=
          " targetRef = " + intToStr (this->targetRef.getUnitId ()) +
          " - factionIndex = " +
          intToStr (this->targetRef.getUnitFaction ()->getIndex ());
      }
      ...
      int cmdIdx = 0;
      for (Commands::const_iterator iterList = commands.begin ();
           iterList != commands.end (); ++iterList)
      {
...
//    }
      ...
      result +=
        "retryCurrCommandCount = " + intToStr (retryCurrCommandCount) + "\n";
      ...
      result +=
        "pathFindRefreshCellCount = " + intToStr (pathFindRefreshCellCount) +
        "\n";
      ...
      {
        result +=
          "attackParticleSystems count = " +
          intToStr (attackParticleSystems.size ()) + "\n";
      }
      if (networkCRCParticleLogInfo != "")
      {
        result +=
          "networkCRCParticleLogInfo = " + networkCRCParticleLogInfo + "\n";
      }
      if (networkCRCDecHpList.empty () == false)
      {
        result +=
          "getNetworkCRCDecHpList() = " + getNetworkCRCDecHpList () + "\n";
      }
      ...
      for (unsigned int index = 0; index < attackParticleSystems.size ();
           ++index)
      {
        ParticleSystem *ps = attackParticleSystems[index];
        if (ps != NULL &&
            Renderer::getInstance ().validateParticleSystemStillExists (ps,
                                                                        rsGame)
            == true)
        {

          result +=
            "attackParticleSystems #" + intToStr (index) + " = " +
            ps->toString () + "\n";
        }
      }
...
//      const int id;
      ...
      unitNode->addAttribute ("name", type->getName (false),
                              mapTagReplacements);
...
//    int loadCount;
      unitNode->addAttribute ("loadCount", intToStr (loadCount),
                              mapTagReplacements);
//    int deadCount;
      unitNode->addAttribute ("deadCount", intToStr (deadCount),
                              mapTagReplacements);
//    float progress;                   //between 0 and 1
      unitNode->addAttribute ("progress", intToStr (progress),
                              mapTagReplacements);
//      float lastAnimProgress; //between 0 and 1
      unitNode->addAttribute ("lastAnimProgress", intToStr (lastAnimProgress),
                              mapTagReplacements);
//      float animProgress;             //between 0 and 1
      unitNode->addAttribute ("animProgress", intToStr (animProgress),
                              mapTagReplacements);
//      float highlight;
      unitNode->addAttribute ("highlight", floatToStr (highlight, 6),
                              mapTagReplacements);
//      int progress2;
      unitNode->addAttribute ("progress2", intToStr (progress2),
                              mapTagReplacements);
//      int kills;
      unitNode->addAttribute ("kills", intToStr (kills), mapTagReplacements);
//      int enemyKills;
      unitNode->addAttribute ("enemyKills", intToStr (enemyKills),
                              mapTagReplacements);
//      UnitReference targetRef;
      targetRef.saveGame (unitNode);
//
//      Field currField;
      unitNode->addAttribute ("currField", intToStr (currField),
                              mapTagReplacements);
//    Field targetField;
      unitNode->addAttribute ("targetField", intToStr (targetField),
                              mapTagReplacements);
//      const Level *level;
      if (level != NULL)
      {
...
//      Vec2i lastPos;
      unitNode->addAttribute ("lastPos", lastPos.getString (),
                              mapTagReplacements);
//    Vec2i targetPos;          //absolute target pos
      unitNode->addAttribute ("targetPos", targetPos.getString (),
                              mapTagReplacements);
//      Vec3f targetVec;
      unitNode->addAttribute ("targetVec", targetVec.getString (),
                              mapTagReplacements);
//      Vec2i meetingPos;
      unitNode->addAttribute ("meetingPos", meetingPos.getString (),
                              mapTagReplacements);
//
//      float lastRotation;             //in degrees
      unitNode->addAttribute ("lastRotation", floatToStr (lastRotation, 6),
                              mapTagReplacements);
//      float targetRotation;
      unitNode->addAttribute ("targetRotation",
                              floatToStr (targetRotation, 6),
                              mapTagReplacements);
//      float rotation;
      unitNode->addAttribute ("rotation", floatToStr (rotation, 6),
                              mapTagReplacements);
//      float targetRotationZ;
      unitNode->addAttribute ("targetRotationZ",
                              floatToStr (targetRotationZ, 6),
                              mapTagReplacements);
//      float targetRotationX;
      unitNode->addAttribute ("targetRotationX",
                              floatToStr (targetRotationX, 6),
                              mapTagReplacements);
//      float rotationZ;
      unitNode->addAttribute ("rotationZ", floatToStr (rotationZ, 6),
                              mapTagReplacements);
//      float rotationX;
      unitNode->addAttribute ("rotationX", floatToStr (rotationX, 6),
                              mapTagReplacements);
//    const UnitType *type;
      unitNode->addAttribute ("type", type->getName (false),
                              mapTagReplacements);

      unitNode->addAttribute ("preMorph_type",
                              (preMorph_type !=
                               NULL ? preMorph_type->getName (false) : ""),
                              mapTagReplacements);

//    const ResourceType *loadType;
      if (loadType != NULL)
      {
        unitNode->addAttribute ("loadType", loadType->getName (),
                                mapTagReplacements);
      }
//    const SkillType *currSkill;
      if (currSkill != NULL)
      {
        unitNode->addAttribute ("currSkillName", currSkill->getName (),
                                mapTagReplacements);
        unitNode->addAttribute ("currSkillClass",
                                intToStr (currSkill->getClass ()),
                                mapTagReplacements);
      }
//    int lastModelIndexForCurrSkillType;
      unitNode->addAttribute ("lastModelIndexForCurrSkillType",
                              intToStr (lastModelIndexForCurrSkillType),
                              mapTagReplacements);
//    int animationRandomCycleCount;
      unitNode->addAttribute ("animationRandomCycleCount",
                              intToStr (animationRandomCycleCount),
                              mapTagReplacements);
//
//    bool toBeUndertaken;
      unitNode->addAttribute ("toBeUndertaken", intToStr (toBeUndertaken),
                              mapTagReplacements);
//      bool alive;
      unitNode->addAttribute ("alive", intToStr (alive), mapTagReplacements);
//      bool showUnitParticles;
      unitNode->addAttribute ("showUnitParticles",
                              intToStr (showUnitParticles),
                              mapTagReplacements);
//    Faction *faction;
//      ParticleSystem *fire;
      int linkFireIndex = -1;
...
//
//    Commands commands;
      for (Commands::iterator it = commands.begin (); it != commands.end ();
           ++it)
      {
...
//      vector<UnitParticleSystem*> unitParticleSystems;
      if (unitParticleSystems.empty () == false)
      {
        XmlNode *unitParticleSystemsNode =
          unitNode->addChild ("unitParticleSystems");
...
//      vector<UnitParticleSystemType*> queuedUnitParticleSystemTypes;
      if (queuedUnitParticleSystemTypes.empty () == false)
      {
        XmlNode *queuedUnitParticleSystemTypesNode =
          unitNode->addChild ("queuedUnitParticleSystemTypes");
        for (unsigned int i = 0; i < queuedUnitParticleSystemTypes.size ();
             ++i)
        {
...
//      UnitParticleSystems damageParticleSystems;
      if (damageParticleSystems.empty () == false)
      {
        XmlNode *damageParticleSystemsNode =
          unitNode->addChild ("damageParticleSystems");
...
//      std::map<int, UnitParticleSystem *> damageParticleSystemsInUse;
      ...
      {
        XmlNode *damageParticleSystemsInUseNode =
          unitNode->addChild ("damageParticleSystemsInUse");

        for (std::map < int, UnitParticleSystem * >::const_iterator iterMap =
             damageParticleSystemsInUse.begin ();
             iterMap != damageParticleSystemsInUse.end (); ++iterMap)
        {
...
//      vector<ParticleSystem*> fireParticleSystems;
      if (fireParticleSystems.empty () == false)
      {
        XmlNode *fireParticleSystemsNode =
          unitNode->addChild ("fireParticleSystems");

        if (linkFireIndex >= 0)
        {
          fireParticleSystemsNode->addAttribute ("fireParticleLink",
                                                 intToStr (linkFireIndex),
                                                 mapTagReplacements);
        }
...
//      vector<UnitParticleSystem*> smokeParticleSystems;
      if (smokeParticleSystems.empty () == false)
      {
        XmlNode *smokeParticleSystemsNode =
          unitNode->addChild ("smokeParticleSystems");
...
//      CardinalDir modelFacing;
      unitNode->addAttribute ("modelFacing", intToStr (modelFacing),
                              mapTagReplacements);

//      std::string lastSynchDataString;
      unitNode->addAttribute ("lastSynchDataString", lastSynchDataString,
                              mapTagReplacements);
//      std::string lastFile;
      unitNode->addAttribute ("lastFile", lastFile, mapTagReplacements);
//      int lastLine;
      unitNode->addAttribute ("lastLine", intToStr (lastLine),
                              mapTagReplacements);
//      std::string lastSource;
      unitNode->addAttribute ("lastSource", lastSource, mapTagReplacements);
//      int lastRenderFrame;
      unitNode->addAttribute ("lastRenderFrame", intToStr (lastRenderFrame),
                              mapTagReplacements);
//      bool visible;
      unitNode->addAttribute ("visible", intToStr (visible),
                              mapTagReplacements);
//      int retryCurrCommandCount;
      unitNode->addAttribute ("retryCurrCommandCount",
                              intToStr (retryCurrCommandCount),
                              mapTagReplacements);
//      Vec3f screenPos;
      unitNode->addAttribute ("screenPos", screenPos.getString (),
                              mapTagReplacements);
//      string currentUnitTitle;
      unitNode->addAttribute ("currentUnitTitle", currentUnitTitle,
                              mapTagReplacements);
//
//      bool inBailOutAttempt;
      unitNode->addAttribute ("inBailOutAttempt", intToStr (inBailOutAttempt),
                              mapTagReplacements);
//      //std::vector<std::pair<Vec2i,Chrono> > badHarvestPosList;
//      std::map<Vec2i,int> badHarvestPosList;
      for (std::map < Vec2i, int >::const_iterator iterMap =
           badHarvestPosList.begin (); iterMap != badHarvestPosList.end ();
           ++iterMap)
      {
        XmlNode *badHarvestPosListNode =
          unitNode->addChild ("badHarvestPosList");

        badHarvestPosListNode->addAttribute ("key",
                                             iterMap->first.getString (),
                                             mapTagReplacements);
        badHarvestPosListNode->addAttribute ("value",
...
//      bool ignoreCheckCommand;
      unitNode->addAttribute ("ignoreCheckCommand",
                              intToStr (ignoreCheckCommand),
                              mapTagReplacements);
//      uint32 lastStuckFrame;
      unitNode->addAttribute ("lastStuckFrame", uIntToStr (lastStuckFrame),
                              mapTagReplacements);
//      Vec2i lastStuckPos;
      unitNode->addAttribute ("lastStuckPos", lastStuckPos.getString (),
                              mapTagReplacements);
//      uint32 lastPathfindFailedFrame;
      unitNode->addAttribute ("lastPathfindFailedFrame",
                              intToStr (lastPathfindFailedFrame),
                              mapTagReplacements);
//      Vec2i lastPathfindFailedPos;
      unitNode->addAttribute ("lastPathfindFailedPos",
                              lastPathfindFailedPos.getString (),
                              mapTagReplacements);
//      bool usePathfinderExtendedMaxNodes;
      unitNode->addAttribute ("usePathfinderExtendedMaxNodes",
                              intToStr (usePathfinderExtendedMaxNodes),
                              mapTagReplacements);
//      int maxQueuedCommandDisplayCount;
      unitNode->addAttribute ("maxQueuedCommandDisplayCount",
                              intToStr (maxQueuedCommandDisplayCount),
                              mapTagReplacements);
...
//      bool changedActiveCommand;
      unitNode->addAttribute ("changedActiveCommand",
                              intToStr (changedActiveCommand),
                              mapTagReplacements);
//      int lastAttackerUnitId;
      unitNode->addAttribute ("lastAttackerUnitId",
                              intToStr (lastAttackerUnitId),
                              mapTagReplacements);
//      int lastAttackedUnitId;
      unitNode->addAttribute ("lastAttackedUnitId",
                              intToStr (lastAttackedUnitId),
                              mapTagReplacements);
//      CauseOfDeathType causeOfDeath;
      unitNode->addAttribute ("causeOfDeath", intToStr (causeOfDeath),
                              mapTagReplacements);

      //pathfindFailedConsecutiveFrameCount
      unitNode->addAttribute ("pathfindFailedConsecutiveFrameCount",
                              intToStr (pathfindFailedConsecutiveFrameCount),
                              mapTagReplacements);

      unitNode->addAttribute ("currentPathFinderDesiredFinalPos",
                              currentPathFinderDesiredFinalPos.getString (),
                              mapTagReplacements);

      unitNode->addAttribute ("random", intToStr (random.getLastNumber ()),
                              mapTagReplacements);
      unitNode->addAttribute ("pathFindRefreshCellCount",
                              intToStr (pathFindRefreshCellCount),
                              mapTagReplacements);
    }

    Unit *Unit::loadGame (const XmlNode * rootNode, GameSettings * settings,
                          Faction * faction, World * world)
    {
      ...
      Vec2i newUnitPos =
        Vec2i::strToVec2 (unitNode->getAttribute ("pos")->getValue ());
      string newUnitType = unitNode->getAttribute ("type")->getValue ();
      const UnitType *ut = faction->getType ()->getUnitType (newUnitType);
      CardinalDir newModelFacing =
        static_cast < CardinalDir >
        (unitNode->getAttribute ("modelFacing")->getIntValue ());
...
//                         const UnitType *type, Faction *faction, Map *map, CardinalDir placeFacing) : BaseColorPickEntity(), id(id) {
      ...
      {
        string newUnitType_preMorph =
          unitNode->getAttribute ("preMorph_name")->getValue ();
        const UnitType *ut_premorph =
          faction->getType ()->getUnitType (newUnitType_preMorph);
        result->preMorph_type = ut_premorph;
      }

      result->lastRotation =
        unitNode->getAttribute ("lastRotation")->getFloatValue ();
      result->targetRotation =
        unitNode->getAttribute ("targetRotation")->getFloatValue ();
      result->rotation =
        unitNode->getAttribute ("rotation")->getFloatValue ();
      ...
      result->lastPos =
        Vec2i::strToVec2 (unitNode->getAttribute ("lastPos")->getValue ());
      result->meetingPos =
        Vec2i::strToVec2 (unitNode->getAttribute ("meetingPos")->getValue ());
      // Attempt to improve performance
      //result->exploreCells();
...
//    int loadCount;
      result->loadCount =
        unitNode->getAttribute ("loadCount")->getIntValue ();
//    int deadCount;
      result->deadCount =
        unitNode->getAttribute ("deadCount")->getIntValue ();
//    float progress;                   //between 0 and 1
      try
      {
        result->progress =
          unitNode->getAttribute ("progress")->getIntValue ();
      }
#ifdef WIN32
...
#endif
        result->progress =
          unitNode->getAttribute ("progress")->getFloatValue ();
      }
//      float lastAnimProgress; //between 0 and 1
      try
      {
        result->lastAnimProgress =
          unitNode->getAttribute ("lastAnimProgress")->getIntValue ();
      }
#ifdef WIN32
      catch (const exception &)
      {
...
#endif
        result->lastAnimProgress =
          unitNode->getAttribute ("lastAnimProgress")->getFloatValue ();
      }
...
//      float animProgress;             //between 0 and 1
      ...
      {
        result->animProgress =
          unitNode->getAttribute ("animProgress")->getIntValue ();
      }
#ifdef WIN32
      catch (const exception &)
      {
...
#endif
        result->animProgress =
          unitNode->getAttribute ("animProgress")->getFloatValue ();
      }

//      float highlight;
      result->highlight =
        unitNode->getAttribute ("highlight")->getFloatValue ();
//      int progress2;
      result->progress2 =
        unitNode->getAttribute ("progress2")->getIntValue ();
//      int kills;
      result->kills = unitNode->getAttribute ("kills")->getIntValue ();
//      int enemyKills;
      result->enemyKills =
        unitNode->getAttribute ("enemyKills")->getIntValue ();
//      UnitReference targetRef;
//      targetRef.saveGame(unitNode);
...
//      Field currField;
      result->currField =
        static_cast < Field >
        (unitNode->getAttribute ("currField")->getIntValue ());
//    Field targetField;
      result->targetField =
        static_cast < Field >
        (unitNode->getAttribute ("targetField")->getIntValue ());
//      const Level *level;
//      if(level != NULL) {
//              level->saveGame(unitNode);
//      }
...
//    Vec2i targetPos;          //absolute target pos
      result->targetPos =
        Vec2i::strToVec2 (unitNode->getAttribute ("targetPos")->getValue ());
//      Vec3f targetVec;
      result->targetVec =
        Vec3f::strToVec3 (unitNode->getAttribute ("targetVec")->getValue ());
//      Vec2i meetingPos;
      result->meetingPos =
        Vec2i::strToVec2 (unitNode->getAttribute ("meetingPos")->getValue ());
//
...
//      float rotation;
      result->rotation =
        unitNode->getAttribute ("rotation")->getFloatValue ();
//      float targetRotationZ;
      result->targetRotationZ =
        unitNode->getAttribute ("targetRotationZ")->getFloatValue ();
//      float targetRotationX;
      result->targetRotationX =
        unitNode->getAttribute ("targetRotationX")->getFloatValue ();
//      float rotationZ;
      result->rotationZ =
        unitNode->getAttribute ("rotationZ")->getFloatValue ();
//      float rotationX;
      result->rotationX =
        unitNode->getAttribute ("rotationX")->getFloatValue ();
//    const UnitType *type;
...
//      }
      ...
      {
        string loadTypeName =
          unitNode->getAttribute ("loadType")->getValue ();
        result->loadType =
          world->getTechTree ()->getResourceType (loadTypeName);
      }
//    const SkillType *currSkill;
//      if(currSkill != NULL) {
...
//      }
      ...
      {
        string skillTypeName =
          unitNode->getAttribute ("currSkillName")->getValue ();
        SkillClass skillClass =
          static_cast < SkillClass >
          (unitNode->getAttribute ("currSkillClass")->getIntValue ());
...
//
//    bool toBeUndertaken;
      result->toBeUndertaken =
        unitNode->getAttribute ("toBeUndertaken")->getIntValue () != 0;
//      bool alive;
      result->setAlive (unitNode->getAttribute ("alive")->getIntValue () !=
                        0);
//      bool showUnitParticles;
      result->showUnitParticles =
        unitNode->getAttribute ("showUnitParticles")->getIntValue () != 0;
//    Faction *faction;
//      ParticleSystem *fire;
//      if(fire != NULL) {
//              fire->saveGame(unitNode);
...
//              (*it)->saveGame(unitNode);
//      }
      vector < XmlNode * >commandNodeList =
        unitNode->getChildList ("Command");
...
//      }
      ...
      {
        XmlNode *unitParticleSystemsNode =
          unitNode->getChild ("unitParticleSystems");
...
//      }
      ...
      {
        XmlNode *damageParticleSystemsNode =
          unitNode->getChild ("damageParticleSystems");
        vector < XmlNode * >unitParticleSystemNodeList =
...
//      }
      ...
      {
        XmlNode *fireParticleSystemsNode =
          unitNode->getChild ("fireParticleSystems");

        int linkFireIndex = -1;
        if (fireParticleSystemsNode->hasAttribute ("fireParticleLink") ==
            true)
        {
          linkFireIndex =
        ...
        vector < XmlNode * >unitParticleSystemNodeList =
          fireParticleSystemsNode->getChildList ("FireParticleSystem");
        for (int i = 0; i < (int) unitParticleSystemNodeList.size (); ++i)
        {
          ...
          if (result->fire == NULL || linkFireIndex != i)
          {
            ...
            if (result->fire == NULL && linkFireIndex >= 0
                && linkFireIndex == i)
            {
              result->fire = ups;
            }
...
//      }
      ...
      {
        XmlNode *smokeParticleSystemsNode =
          unitNode->getChild ("smokeParticleSystems");
        vector < XmlNode * >unitParticleSystemNodeList =
...
//      unitNode->addAttribute("lastSource",lastSource, mapTagReplacements);
//      int lastRenderFrame;
      result->lastRenderFrame =
        unitNode->getAttribute ("lastRenderFrame")->getIntValue ();
//      bool visible;
      result->visible =
        unitNode->getAttribute ("visible")->getIntValue () != 0;
//      int retryCurrCommandCount;
      result->retryCurrCommandCount =
        unitNode->getAttribute ("retryCurrCommandCount")->getIntValue ();
//      Vec3f screenPos;
      result->screenPos =
        Vec3f::strToVec3 (unitNode->getAttribute ("screenPos")->getValue ());
//      string currentUnitTitle;
      result->currentUnitTitle =
        unitNode->getAttribute ("currentUnitTitle")->getValue ();
//
//      bool inBailOutAttempt;
      result->inBailOutAttempt =
        unitNode->getAttribute ("inBailOutAttempt")->getIntValue () != 0;
//      //std::vector<std::pair<Vec2i,Chrono> > badHarvestPosList;
//      std::map<Vec2i,int> badHarvestPosList;
...
//      //time_t lastBadHarvestListPurge;
//      std::pair<Vec2i,int> lastHarvestResourceTarget;
      const XmlNode *lastHarvestResourceTargetNode =
        unitNode->getChild ("lastHarvestResourceTarget");
...
//
//      bool ignoreCheckCommand;
      result->ignoreCheckCommand =
        unitNode->getAttribute ("ignoreCheckCommand")->getIntValue () != 0;
//      uint32 lastStuckFrame;
      result->lastStuckFrame =
...
//      }
      ...
      {
        vector < XmlNode * >unitParticleSystemNodeList =
          unitNode->getChildList ("UnitAttackBoostEffect");
        for (unsigned int i = 0; i < unitParticleSystemNodeList.size (); ++i)
        {
          XmlNode *node = unitParticleSystemNodeList[i];

          UnitAttackBoostEffect *attackBoostEffect =
            new UnitAttackBoostEffect ();
...
//
//      bool changedActiveCommand;
      result->changedActiveCommand =
        unitNode->getAttribute ("changedActiveCommand")->getIntValue () != 0;
//      int lastAttackerUnitId;
      result->lastAttackerUnitId =
        unitNode->getAttribute ("lastAttackerUnitId")->getIntValue ();
//      int lastAttackedUnitId;
      result->lastAttackedUnitId =
        unitNode->getAttribute ("lastAttackedUnitId")->getIntValue ();
//      CauseOfDeathType causeOfDeath;
      result->causeOfDeath =
        static_cast < CauseOfDeathType >
      ...
      {
        result->pathFindRefreshCellCount =
          unitNode->getAttribute ("pathFindRefreshCellCount")->getIntValue ();
      }
    ...
    Checksum Unit::getCRC ()
    {
      ...
      if (consoleDebug)
        printf ("#9c Unit: %d CRC: %u\n", id, crcForUnit.getSum ());

      crcForUnit.addInt (alive);
      //bool showUnitParticles;

      if (consoleDebug)
        printf ("#10 Unit: %d CRC: %u\n", id, crcForUnit.getSum ());

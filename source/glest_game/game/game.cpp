  ...
  Glest
{
    ...
    Game
  {
    ...
    string
      GameSettings::playerDisconnectedText = "";
    Game *
      thisGamePtr = NULL;
...
// ===================== PUBLIC ========================
    ...
    const float
      PHOTO_MODE_MAXHEIGHT = 500.0;

    const int
      CREATE_NEW_TEAM = -100;
    const int
      CANCEL_SWITCH_TEAM = -1;

    const int
      CANCEL_DISCONNECT_PLAYER = -1;

    const float
      Game::highlightTime = 0.5f;

    int
      fadeMusicMilliseconds = 3500;

// Check every x seconds if we should switch disconnected players to AI
    const int
      NETWORK_PLAYER_CONNECTION_CHECK_SECONDS = 5;

    int
      GAME_STATS_DUMP_INTERVAL = 60 * 10;
    ...
    ProgramState (NULL)
    {
      ...
      popupMenuDisconnectPlayer.
      setEnabled (false);
      popupMenuDisconnectPlayer.
      setVisible (false);

      switchTeamConfirmMessageBox.
      setEnabled (false);
      ...
      fadeMusicMilliseconds =
        Config::getInstance ().getInt ("GameStartStopFadeSoundMilliseconds",
                                       intToStr (fadeMusicMilliseconds).
                                       c_str ());
      GAME_STATS_DUMP_INTERVAL =
        Config::getInstance ().getInt ("GameStatsDumpIntervalSeconds",
                                       intToStr (GAME_STATS_DUMP_INTERVAL).
                                       c_str ());
    }
    ...
    Game::resetMembers ()
    {
      ...
      photoModeEnabled =
        Config::getInstance ().getBool ("PhotoMode", "false");
    ...
    isFirstRender (true)
    {
      ...
      Lang::getInstance ().setAllowNativeLanguageTechtree (this->gameSettings.
                                                           getNetworkAllowNativeLanguageTechtree
                                                           ());
    ...
    Game::endGame ()
    {
      ...
      if (originalDisplayMsgCallback != NULL)
        {
          NetworkInterface::
            setDisplayMessageFunction (originalDisplayMsgCallback);
        }
      ...
      logger.add (Lang::getInstance ().
                  getString ("LogScreenGameLoading", "", true), false);
      logger.hideProgress ();
...
//      world.end();    //must die before selection because of referencers
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] aiInterfaces.size() = %d\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__,
                                  aiInterfaces.size ());
    ...
    bool
    Game::showTranslatedTechTree () const
    {
      return
        this->
        gameSettings.
...
// ==================== init and load ====================
    ...
    int
    Game::ErrorDisplayMessage (const char *msg, bool exitApp)
    {
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] %s\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, msg);
...
//}
    ...
    string
    Game::extractScenarioLogoFile (const GameSettings * settings,
                                   string & result, bool & loadingImageUsed,
                                   Logger * logger, string factionLogoFilter)
    {
      string
        scenarioDir = "";
      if (settings->getScenarioDir () != "")
        {
          ...
          if (EndsWith (scenarioDir, ".xml") == true)
            {
              ...
              scenarioDir =
                scenarioDir.erase (scenarioDir.size () -
                                   settings->getScenario ().size (),
                                   settings->getScenario ().size () + 1);
            }
          ...
          string
            logoFullPathFilter = scenarioDir + factionLogoFilter;
          ...
          if (loadScreenList.empty () == false)
            {
              ...
              if (loadScreenList.size () > 1
                  && EndsWith (factionLogoFilter, ".xml") == false)
                {
                  ...
                  int
                    bestMinWidthDiff = INT_MAX;
                  int
                    bestMinHeightDiff = INT_MAX;
                  // Now find the best texture for our screen
                  // Texture2D *result = preloadTexture(logoFilename);
                  for (unsigned int logoIndex = 0;
                       logoIndex < loadScreenList.size (); ++logoIndex)
                    {
                      string
                        senarioLogo =
                        scenarioDir + loadScreenList[bestLogoIndex];
                      if (SystemFlags::
                          getSystemSettingType (SystemFlags::debugSystem).
                          enabled)
                        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                                  "In [%s::%s Line: %d] looking for loading screen '%s'\n",
                      ...
                      if (SystemFlags::VERBOSE_MODE_ENABLED)
                        printf ("Looking for best logo: %u [%s]\n", logoIndex,
                                senarioLogo.c_str ());
                      ...
                      if (fileExists (senarioLogo) == true)
                        {
                          ...
                          Texture2D *
                            checkLogo =
                            Renderer::preloadTexture (senarioLogo);
                          if (checkLogo != NULL)
                            {
                              const
                                Metrics &
                              ...
                              int
                                minWidthDifference =
                                abs (metrics.getScreenW () -
                                     checkLogo->getPixmapConst ()->getW ());
                              int
                                minHeightDifference =
                                abs (metrics.getScreenH () -
                                     checkLogo->getPixmapConst ()->getH ());
              ...
              string
                senarioLogo = scenarioDir + loadScreenList[bestLogoIndex];
              if (fileExists (senarioLogo) == true)
                {
                  if (SystemFlags::
                      getSystemSettingType (SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                              "In [%s::%s] found scenario loading screen '%s'\n",
                                              extractFileFromDirectoryPath
                                              (__FILE__).c_str (),
                                              __FUNCTION__,
                                              senarioLogo.c_str ());
          ...
          if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
            ...
            SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                      ...
                                      extractFileFromDirectoryPath (__FILE__).
                                      c_str (), __FUNCTION__, __LINE__,
                                      settings->getScenarioDir ().c_str (),
                                      settings->getScenario ().c_str (),
                                      scenarioDir.c_str ());
        }
      return scenarioDir;
    }
    ...
    string
    Game::extractFactionLogoFile (bool & loadingImageUsed,
                                  const string & factionName,
                                  string scenarioDir, const string & techName,
                                  Logger * logger, string factionLogoFilter)
    {
      string
        result = "";
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] Searching for faction loading screen\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__);
      ...
      if (factionName == formatString (GameConstants::OBSERVER_SLOTNAME))
        {
          ...
          const string
            factionLogo = data_path + "data/core/misc_textures/observer.jpg";
      ...
      //else if(settings->getFactionTypeName(i) == formatString(GameConstants::RANDOMFACTION_SLOTNAME)) {
               ...
               formatString (GameConstants::RANDOMFACTION_SLOTNAME))
        {
          string
            ...
            getGameReadWritePath (GameConstants::path_data_CacheLookupKey);
          const string
            factionLogo = data_path + "data/core/misc_textures/random.jpg";
      ...
      else
        {
          ...
          for (int idx = 0; idx < (int) pathList.size (); idx++)
            {
              string
                currentPath = pathList[idx];
              endPathWithSlash (currentPath);
              //string path = currentPath + techName + "/" + "factions" + "/" + settings->getFactionTypeName(i);
              string
                path =
                currentPath + techName + "/" + "factions" + "/" + factionName;
              if (SystemFlags::
                  getSystemSettingType (SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                          "In [%s::%s Line: %d] possible loading screen dir '%s'\n",
                                          extractFileFromDirectoryPath
                                          (__FILE__).c_str (), __FUNCTION__,
                                          __LINE__, path.c_str ());
              if (isdir (path.c_str ()) == true)
                {
                  ...
                  vector < string > loadScreenList;
                  string
                    logoFullPathFilter = path + factionLogoFilter;
                  findAll (logoFullPathFilter, loadScreenList, false, false);
                  if (loadScreenList.empty () == false)
                    {
                      int
                      ...
                      if (loadScreenList.size () > 1
                          && EndsWith (factionLogoFilter, ".xml") == false)
                        {
                          ...
                          for (unsigned int logoIndex = 0;
                               logoIndex <
                               (unsigned int) loadScreenList.size ();
                               ++logoIndex)
                            {
                              string
                                factionLogo =
                                path + loadScreenList[logoIndex];
                              if (SystemFlags::
                              ...
                              if (fileExists (factionLogo) == true)
                                {
                                  ...
                                  Texture2D *
                                    checkLogo =
                                    Renderer::preloadTexture (factionLogo);
                                  if (checkLogo != NULL)
                                    {
                                      const
                                        Metrics &
                                      ...
                                      if (SystemFlags::VERBOSE_MODE_ENABLED)
                                        printf
                                          ("Logo info: %d x %d (%d,%d)\n",
                                           ...
                                           getW (),
                                           checkLogo->getPixmapConst ()->
                                           getH (), minWidthDifference,
                                           minHeightDifference);

                                      if (minWidthDifference <
                                          bestMinWidthDiff)
                                        {
                                          bestMinWidthDiff =
                                            minWidthDifference;
                                          ...
                                          if (SystemFlags::
                                            ...
                                            printf
                                              ("#1 New best logo is [%s]\n",
                                               factionLogo.c_str ());
                                        }
                                      else if (minWidthDifference ==
                                               bestMinWidthDiff
                                               && minHeightDifference <
                                               bestMinHeightDiff)
                                        {
                      ...
                      string
                        factionLogo = path + loadScreenList[bestLogoIndex];
                      if (SystemFlags::
                          getSystemSettingType (SystemFlags::debugSystem).
                          enabled)
                        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                  ...
                  // Check if this is a linked faction
                  else
                    {
                      //!!!
                      string
                        factionXMLFile = path + factionName + ".xml";
                      ...
                      if (fileExists (factionXMLFile) == true)
                        {
                          XmlTree
                          xmlTreeFaction (XML_RAPIDXML_ENGINE);
                          std::map < string,
                            string > mapExtraTagReplacementValues;
                          xmlTreeFaction.load (factionXMLFile,
                                               Properties::
                          ...
                          const XmlNode *
                            rootNode = xmlTreeFaction.getRootNode ();
                          ...
                          if (rootNode->getName () == "link"
                              && rootNode->hasChild ("techtree") == true)
                            {
                              const XmlNode *
                                linkNode = rootNode;
                              ...
                              const XmlNode *
                                techtreeNode =
                                linkNode->getChild ("techtree");

                              string
                                linkedTechTreeName =
                                techtreeNode->getAttribute ("name")->
                                getValue ();
                                  ...
                                  if (isdir (path.c_str ()) == true)
                                    {
                                      ...
                                      findAll (path + factionLogoFilter,
                                               loadScreenList, false, false);
                                      if (loadScreenList.empty () == false)
                                        {
                                          string
                                            factionLogo =
                                            path + loadScreenList[0];
                                          if (SystemFlags::
                                              getSystemSettingType
                                              (SystemFlags::debugSystem).
                                              enabled)
                                            SystemFlags::
                                              OutputDebug (SystemFlags::
                                                           debugSystem,
                                                           "In [%s::%s Line: %d] looking for loading screen '%s'\n",
                                                           extractFileFromDirectoryPath
                                                           (__FILE__).
    ...
    string
    Game::extractTechLogoFile (string scenarioDir, const string & techName,
                               bool & loadingImageUsed, Logger * logger,
                               const string & factionLogoFilter)
    {
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] Searching for tech loading screen\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__);
      Config & config = Config::getInstance ();
      vector < string > pathList =
          ...
          if (isdir (path.c_str ()) == true)
            {
              ...
              vector < string > loadScreenList;
              findAll (path + factionLogoFilter, loadScreenList, false,
                       false);
              if (loadScreenList.empty () == false)
                {
                  string
                    factionLogo = path + loadScreenList[0];
                  if (SystemFlags::
                      getSystemSettingType (SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                              "In [%s::%s Line: %d] looking for loading screen '%s'\n",
    ...
    void
    Game::loadHudTexture (const GameSettings * settings)
    {
      string
        factionName = "";
      string
        techName = settings->getTech ();
      string
        scenarioDir =
        extractDirectoryPathFromFile (settings->getScenarioDir ());
      //printf("In loadHudTexture, scenarioDir [%s]\n",scenarioDir.c_str());

      for (int i = 0; i < settings->getFactionCount (); ++i)
        {
          if ((settings->getFactionControl (i) == ctHuman)
              || (settings->getFactionControl (i) == ctNetwork
      ...
      if (factionName != "")
        {
          ...
          vector < string > pathList =
            config.getPathListForType (ptTechs, scenarioDir);
          for (int idx = 0; hudFound == false && idx < (int) pathList.size ();
               idx++)
            {
              string
                currentPath = pathList[idx];
              ...
              string
                ...
                currentPath + techName + "/" + "factions" + "/" + factionName;
              endPathWithSlash (path);
              findAll (path + GameConstants::HUD_SCREEN_FILE_FILTER, hudList,
                       false, false);
              if (hudList.empty () == false)
                {
                  for (unsigned int hudIdx = 0;
                       hudFound == false
                       && hudIdx < (unsigned int) hudList.size (); ++hudIdx)
                    {
                      string
                        hudImageFileName = path + hudList[hudIdx];
                      if (SystemFlags::VERBOSE_MODE_ENABLED)
                        printf
                          ("In [%s::%s Line: %d] looking for a HUD [%s]\n",
                           extractFileFromDirectoryPath (__FILE__).c_str (),
                           __FUNCTION__, __LINE__, hudImageFileName.c_str ());
                      if (SystemFlags::
                          getSystemSettingType (SystemFlags::debugSystem).
                          enabled)
                        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                      ...
                      if (fileExists (hudImageFileName) == true)
                        {
                          ...
                          Texture2D *
                            texture =
                            Renderer::findTexture (hudImageFileName);
    ...
    string
    Game::findFactionLogoFile (const GameSettings * settings, Logger * logger,
                               const string & factionLogoFilter)
    {
      string
        result = "";
      if (settings == NULL)
        ...
        {
          string
            mapName = settings->getMap ();
          string
            tilesetName = settings->getTileset ();
          string
            techName = settings->getTech ();
          string
            scenarioName = settings->getScenario ();
          bool
            loadingImageUsed = false;
              ...
              if (scenarioName.empty ())
                {
                  string
                    scenarioDir =
                    extractDirectoryPathFromFile (settings->
                                                  getScenarioDir ());
                  TechTree
                  techTree (Config::getInstance ().
                            getPathListForType (ptTechs, scenarioDir));

                  logger->setSubtitle (formatString (mapName) + " - " +
                                       formatString (tilesetName) + " - " +
                                       formatString (techTree.
                                                     getTranslatedName
                                                     (techName)));
                }
              else
          ...
          string
            scenarioDir =
            extractScenarioLogoFile (settings, result, loadingImageUsed,
              ...
              for (int i = 0; i < settings->getFactionCount (); ++i)
                {
                  if (settings->getFactionControl (i) == ctHuman ||
                       ...
                       && settings->getThisFactionIndex () == i))
                    {
                      ...
                      result =
                        extractFactionLogoFile (loadingImageUsed,
                                                settings->
                                                getFactionTypeName (i),
                                                scenarioDir, techName, logger,
                                                factionLogoFilter);
                      break;
                    }
                }
    ...
    vector < Texture2D * >Game::processTech (string techName)
    {
      ...
      bool
        enableFactionTexturePreview =
        Config::getInstance ().getBool ("FactionPreview", "true");
              ...
              if (factions.empty () == false)
                {
                  for (unsigned int factionIdx = 0;
                       factionIdx < (unsigned int) factions.size ();
                       ++factionIdx)
                    {
                      bool
                        loadingImageUsed = false;
                      ...
                      string
                        factionLogo =
                        Game::extractFactionLogoFile (loadingImageUsed,
                                                      currentFactionName_factionPreview,
                                                      "",
                                                      techName,
                                                      NULL,
                      ...
                      if (factionLogo != "")
                        {
                          Texture2D *
                            texture = Renderer::preloadTexture (factionLogo);
    ...
    Game::load (int loadTypes)
    {
      ...
      originalDisplayMsgCallback =
        NetworkInterface::getDisplayMessageFunction ();
      NetworkInterface::setDisplayMessageFunction (ErrorDisplayMessage);

      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] loadTypes = %d, gameSettings = [%s]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, loadTypes,
                                  this->gameSettings.toString ().c_str ());
      ...
      string
        mapName = gameSettings.getMap ();
      string
        tilesetName = gameSettings.getTileset ();
      string
        techName = gameSettings.getTech ();
      string
        scenarioName = gameSettings.getScenario ();
      string
        data_path =
        getGameReadWritePath (GameConstants::path_data_CacheLookupKey);
      // loadHints
      ...
      string
        user_data_path = config.getString ("UserData_Root", "");
      if (user_data_path != "")
        {
      ...
      string
        ...
        getGameCustomCoreDataPath (data_path,
                                   "data/lang/hint/hint_" +
                                   Lang::getInstance ().
                                   getDefaultLanguage () + ".lng");
      string
        languageFile =
        getGameCustomCoreDataPath (data_path,
                                   "data/lang/hint/hint_" +
                                   Lang::getInstance ().getLanguage () +
                                   ".lng");
      string
        languageFileUserData =
        user_data_path + "data/lang/hint/hint_" +
        Lang::getInstance ().getLanguage () + ".lng";
      ...
      if (fileExists (englishFile) == false)
        {
          SystemFlags::OutputDebug (SystemFlags::debugError,
                                    "In [%s::%s Line: %d] file [%s] not found\n",
      ...
      const string
        markCellTextureFilename =
        data_path + "data/core/misc_textures/mark_cell.png";
      markCellTexture = Renderer::findTexture (markCellTextureFilename);
      const string
        unmarkCellTextureFilename =
        data_path + "data/core/misc_textures/unmark_cell.png";
      unmarkCellTexture = Renderer::findTexture (unmarkCellTextureFilename);
      const string
        highlightCellTextureFilename =
        data_path + "data/core/misc_textures/pointer.png";
      highlightCellTexture =
        Renderer::findTexture (highlightCellTextureFilename);
      ...
      if ((loadTypes & lgt_TechTree) == lgt_TechTree)
        {
          ...
          world.loadTech (config.getPathListForType (ptTechs, scenarioDir),
                          techName, factions, &checksum, loadedFileList);

          if (world.getTechTree () == NULL
              || world.getTechTree ()->getNameUntranslated () == "")
            {
              char
                szBuf[8096] = "";
              snprintf (szBuf, 8096,
      ...
      if ((loadTypes & lgt_Map) == lgt_Map)
        {
          if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
            ...
            SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                      ...
                                      extractFileFromDirectoryPath (__FILE__).
                                      c_str (), __FUNCTION__, __LINE__);
          world.loadMap (Config::getMapPath (mapName, scenarioDir),
                         &checksum);
        }
          ...
          if (scenarioName.empty () == false)
            {
              ...
              bool
                isTutorial =
                Scenario::isGameTutorial (gameSettings.getScenarioDir ());
              ...
              Lang::getInstance ().loadScenarioStrings (gameSettings.
                                                        getScenarioDir (),
                                                        scenarioName,
                                                        isTutorial);
    ...
    void
    Game::init (bool initForPreviewOnly)
    {
      bool
        showPerfStats =
        Config::getInstance ().getBool ("ShowPerfStats", "false");
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s Line: %d] initForPreviewOnly = %d\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__,
                                  initForPreviewOnly);
        ...
        if (ex.wantStackTrace () == true)
          {
            char
              szErrBuf[8096] = "";
            snprintf (szErrBuf, 8096, "In [%s::%s %d]", __FILE__,
                      __FUNCTION__, __LINE__);
            sErrBuf =
      ...
      catch (const exception & ex)
      {
        char
          szErrBuf[8096] = "";
        snprintf (szErrBuf, 8096, "In [%s::%s %d]", __FILE__, __FUNCTION__,
                  __LINE__);
        string
          sErrBuf =
          string (szErrBuf) + string ("\nerror [") + string (ex.what ()) +
      ...
      if (initForPreviewOnly == false)
        {
          ...
          bool
            ...
            this->gameSettings.getEnableServerControlledAI ();
          bool
            isNetworkGame = this->gameSettings.isNetworkGame ();
          ...
          for (int i = 0; i < world.getFactionCount (); ++i)
            {
              Faction *
                faction = world.getFaction (i);

              //printf("Controltype = %d for index = %d\n",faction->getControlType(),i);

              if (faction->
                  getCpuControl (enableServerControlledAI, isNetworkGame,
                                 role) == true)
                {
                  //printf("** Loading AI player for Controltype = %d for index = %d\n",faction->getControlType(),i);

                  aiInterfaces[i] =
                    new AiInterface (*this, i, faction->getTeam ());
                  if (loadGameNode != NULL)
                    {
                  ...
                  slaveThreadList.push_back (aiInterfaces[i]->
                                             getWorkerThread ());
                }
              else
                {
          ...
          if (Config::getInstance ().
              getBool ("EnableNewThreadManager", "false") == true)
            {
              ...
              if (world.getTileset ()->getWeather () == wRainy)
                {
                  ...
                  weatherParticleSystem->setSpeed (12.f /
                                                   GameConstants::updateFps);
                  weatherParticleSystem->setPos (gameCamera.getPos ());
                  renderer.manageParticleSystem (weatherParticleSystem,
                                                 rsGame);
                }
              else if (world.getTileset ()->getWeather () == wSnowy)
                {
                  ...
                  weatherParticleSystem->setSpeed (1.5f /
                                                   GameConstants::updateFps);
                  weatherParticleSystem->setPos (gameCamera.getPos ());
                  weatherParticleSystem->setTexture (coreData.
                                                     getSnowTexture ());
                  renderer.manageParticleSystem (weatherParticleSystem,
                                                 rsGame);
                }
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                  "In [%s::%s] Initializing renderer\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__);
      logger.add (Lang::getInstance ().
                  getString ("LogScreenGameLoadingInitRenderer", "", true),
                  true);
      ...
      if (initForPreviewOnly == false)
        {
          ...
          if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
              enabled)
            SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                      "In [%s::%s] Waiting for network\n",
                                      extractFileFromDirectoryPath (__FILE__).
                                      c_str (), __FUNCTION__);
          logger.add (Lang::getInstance ().
                      getString ("LogScreenGameLoadingWaitForNetworkPlayers",
                                 "", true), true);
          networkManager.getGameNetworkInterface ()->
          ...
          if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
            ...
            SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                      ...
                                      c_str (), __FUNCTION__, __LINE__);
          logger.add (Lang::getInstance ().
                      getString ("LogScreenGameLoadingStartingMusic", "",
                                 true), true);
          ...
          if (this->masterserverMode == false)
            {
              if (world.getThisFaction () == NULL)
                {
                  throw
                  megaglest_runtime_error ("world.getThisFaction() == NULL");
                }
          ...
          if (tileset->getWeather () == wRainy
            ...
            {
              logger.add (Lang::getInstance ().
                          getString ("LogScreenGameLoadingStartingAmbient",
                                     "", true), true);
          ...
          if (this->masterserverMode == false)
            {
              StrSound *
                gameMusic = world.getThisFaction ()->getType ()->getMusic ();
              soundRenderer.playMusic (gameMusic);
            }

          logger.add (Lang::getInstance ().
                      getString ("LogScreenGameLoadingLaunchGame", "", true));
        }
      ...
      for (int i = 0; i < world.getFactionCount (); ++i)
        {
          ...
          if (faction != NULL
              && faction->getTeam () ==
              GameConstants::maxPlayers - 1 + fpt_Observer)
            {
              faction->setPersonalityType (fpt_Observer);
              world.getStats ()->setPersonalityType (i,
                                                     faction->
                                                     getPersonalityType ());
            }
        }
      ...
      if (role == nrClient)
        {
          ...
          if (clientInterface != NULL
              && clientInterface->getResumeInGameJoin () == true)
            {
      ...
      if (isFlagType1BitEnabled (ft1_network_synch_checks_verbose) == true)
        {
          printf
            ("*Note: Monitoring Network CRC VERBOSE synchronization...\n");
        }
    ...
    void
    Game::initCamera (Map * map)
    {
      gameCamera.init (map->getW (), map->getH ());

      // camera default height calculation
      if (map->getCameraHeight () > 0
          && gameCamera.getCalculatedDefault () < map->getCameraHeight ())
        {
          gameCamera.setCalculatedDefault (map->getCameraHeight ());
        }
      else if (gameCamera.getCalculatedDefault () <
               map->getMaxMapHeight () + 13.0f)
        {
      ...
      if (world.getThisFaction () != NULL)
        {
          const
            Vec2i &
            v =
            map->getStartLocation (world.getThisFaction ()->
                                   getStartLocationIndex ());
          // This args are set in map.cpp - Map::getStartLocation()
          gameCamera.
            setPos (Vec2f
                    (v.x, v.y + gameCamera.getCalculatedDefault () / 2));
          //
...
// ==================== update ====================
    ...
    void
    Game::setupPopupMenus (bool checkClientAdminOverrideOnly)
    {
      Lang & lang = Lang::getInstance ();
      NetworkManager & networkManager = NetworkManager::getInstance ();
      NetworkRole
      ...
      bool
        allowAdminMenuItems = false;
      bool
        forceJoinInProgressUpdate = false;
      if (role == nrServer)
        {
          ...
          if (disconnectPlayerPopupMenuIndex == -1)
            {
              ...
              if (serverInterface != NULL
                  && checkClientAdminOverrideOnly == true)
                {
                  for (int i = 0; i < world.getFactionCount (); ++i)
                    {
                      Faction *
                      ...
                      MutexSafeWrapper
                      safeMutex (serverInterface->
                                 getSlotMutex (faction->
                                               getStartLocationIndex ()),
                                 CODE_AT_LINE);
                      ConnectionSlot *
                        slot =
                        serverInterface->getSlot (faction->
                                                  getStartLocationIndex (),
                                                  false);
                      if (slot != NULL
                          && slot->getConnectHasHandshaked () == true
      ...
      if (checkClientAdminOverrideOnly == false ||
            ...
            && clientInterface->isMasterServerAdminOverride () == true)))
        {
          ...
          if (checkClientAdminOverrideOnly == true && clientInterface != NULL)
            {
              gameSettings.setMasterserver_admin (clientInterface->
                                                  getSessionKey ());
              gameSettings.
                setMasterserver_admin_faction_index (clientInterface->
                                                     getPlayerIndex ());
            }
          //PopupMenu popupMenu;
          ...
          if ((gameSettings.getFlagTypes1 () & ft1_allow_team_switching) ==
              ft1_allow_team_switching && world.getThisFaction () != NULL
              && world.getThisFaction ()->getPersonalityType () !=
              fpt_Observer)
            {
              ...
              if (gameSettings.isNetworkGame () == true)
                {
                  menuItems.push_back ("  " +
                                       lang.
                                       getString ("DisconnectNetorkPlayer") +
                                       "  ");
                  disconnectPlayerPopupMenuIndex =
                    (int) menuItems.size () - 1;
                }
            }
          ...
          popupMenu.setW (100);
          popupMenu.setH (100);
          popupMenu.init ("  " + lang.getString ("GameMenuTitle") + "  ",
                          menuItems);
          popupMenu.setEnabled (false);
          popupMenu.setVisible (false);
    ...
    void
    Game::processNetworkSynchChecksIfRequired ()
    {
      bool
        isNetworkGame = this->gameSettings.isNetworkGame ();
              ...
              if (calculateNetworkCRC == true)
                {
                  ...
                  NetworkInterface *
                    netIntf = networkManager.getGameNetworkInterface ();
                  for (int index = 0; index < GameConstants::maxPlayers;
                       ++index)
                    {
                      if (index < world.getFactionCount ())
                        {
                          Faction *
                            faction = world.getFaction (index);
                          netIntf->setNetworkPlayerFactionCRC (index,
                                                               faction->
                                                               getCRC ().
                                                               getSum ());
                          ...
                          if (settings != NULL)
                            {
                              if (isFlagType1BitEnabled
                                  (ft1_network_synch_checks_verbose) == true)
                                {
                                  faction->addCRC_DetailsForWorldFrame (world.
                                                                        getFrameCount
                                                                        (),
                                                                        role
                                                                        ==
...
//update
    ...
    Game::update ()
    {
      try
      {
        ...
        bool
          pendingQuitError = (quitPendingIndicator == true ||
                              (NetworkManager::getInstance ().
                               getGameNetworkInterface () != NULL
                               && NetworkManager::getInstance ().
                               getGameNetworkInterface ()->getQuit ()));
        ...
        if (this->masterserverMode == false)
          {
            if (world.getFactionCount () > 0
                && world.getThisFaction ()->getFirstSwitchTeamVote () != NULL)
              {
                const SwitchTeamVote *
                  vote = world.getThisFaction ()->getFirstSwitchTeamVote ();
                GameSettings *
                  settings = world.getGameSettingsPtr ();
                ...
                if (lang.hasString ("AllowPlayerJoinTeam") == true)
                  {
                    snprintf (szBuf, 8096,
                              lang.getString ("AllowPlayerJoinTeam").c_str (),
                              settings->getNetworkPlayerName (vote->
                                                              factionIndex).
                              c_str (), vote->oldTeam, vote->newTeam);
                  }
                else
                  {
                    snprintf (szBuf, 8096,
                              "Allow player [%s] to join your team\n(changing from team# %d to team# %d)?",
                              settings->getNetworkPlayerName (vote->
                                                              factionIndex).
                              c_str (), vote->oldTeam, vote->newTeam);
                  }
                ...
                switchTeamConfirmMessageBox.init (lang.getString ("Yes"),
                                                  lang.getString ("No"));
                switchTeamConfirmMessageBox.setEnabled (true);

                world.getThisFactionPtr ()->
        ...
        if (role == nrClient && updateLoops == 1
            && world.getFrameCount () >=
            (gameSettings.getNetworkFramePeriod () * 2))
          {
            ClientInterface *
              clientInterface =
              dynamic_cast <
            ...
            if (clientInterface != NULL)
              {
                uint64
                  ...
                  clientInterface->getCachedLastPendingFrameCount ();
                ...
                receivedTooEarlyInFrames[(index +
                                          1) %
                                         GameConstants::
                                         networkSmoothInterval] = -1;
                framesNeededToWaitForServerMessage[(index +
                                                    1) %
                                                   GameConstants::
                                                   networkSmoothInterval] =
                  -1;

                if (receivedTooEarlyInFrames[index] == -1)
                  {
                    // we need to check if we already received something for next frame
                    if (lastNetworkFrameFromServer > 0
                        && lastNetworkFrameFromServer >
                        (uint64) world.getFrameCount ())
                      {
                        receivedTooEarlyInFrames[index] =
                          lastNetworkFrameFromServer - world.getFrameCount ();
                      }
                  }
                if (framesNeededToWaitForServerMessage[index] == -1)
                  {
                    ...
                    int64
                      timeClientWaitedForLastMessage =
                      clientInterface->getTimeClientWaitedForLastMessage ();
                    if (timeClientWaitedForLastMessage > 0)
                      {
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                          printf
                            ("world.getFrameCount():%d index %d Client waited:%d ms\n",
                             world.getFrameCount (), index,
                             (int) timeClientWaitedForLastMessage);
                        framesNeededToWaitForServerMessage[index] =
                          timeClientWaitedForLastMessage *
                          GameConstants::updateFps / 1000;
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                          printf
                            ("ClienttimeClientWaitedForLastMessage:%d ms  which is %d frames \n",
                             (int) timeClientWaitedForLastMessage,
                             framesNeededToWaitForServerMessage[index]);
                      }
                    else
                ...
                int
                  minimum = 0;
                int
                  allowedMaxFallback = 5;
                int
                  countOfMessagesReceivedTooEarly = 0;
                int
                  countOfMessagesReceivedTooLate = 0;
                int
                  sumOfTooLateFrames = 0;
                bool
                  cleanupStats = false;
                    ...
                    if (framesNeededToWaitForServerMessage[i] > 0)
                      {
                        ...
                        sumOfTooLateFrames +=
                          framesNeededToWaitForServerMessage[i];
                      }
                  }
                ...
                if (countOfMessagesReceivedTooEarly == GameConstants::networkSmoothInterval - 1)        // -1 because slot for next frame is already initialized
                  {             // all packages where too early
                    ...
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                      printf
                        ("Worldframe %d : Client will speed up: %d frames\n",
                         world.getFrameCount (), framesToCatchUpAsClient);
                  }
                else if (countOfMessagesReceivedTooLate > 3)
                  {
                    framesToSlowDownAsClient =
                      sumOfTooLateFrames / countOfMessagesReceivedTooLate;
                    framesToCatchUpAsClient = 0;
                    cleanupStats = true;
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                      printf
                        ("Worldframe %d : Client will slow down: %d frames\n",
                ...
                if (cleanupStats == true)
                  {
                    ...
                    for (int i = 0; i < GameConstants::networkSmoothInterval;
                         i++)
                      {
                        receivedTooEarlyInFrames[i] = -1;
                        framesNeededToWaitForServerMessage[i] = -1;
        ...
        addPerformanceCount ("CalculateNetworkUpdateLoops",
        ...
        if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
            enabled && chrono.getMillis () > 0)
          SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                    "In [%s::%s] Line: %d took msecs: %lld [before ReplaceDisconnectedNetworkPlayersWithAI]\n",
                                    extractFileFromDirectoryPath (__FILE__).
                                    c_str (), __FUNCTION__, __LINE__,
                                    chrono.getMillis ());
        if (showPerfStats)
        ...
        if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
            enabled && chrono.getMillis () > 0)
          SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                    "In [%s::%s] Line: %d took msecs: %lld [after ReplaceDisconnectedNetworkPlayersWithAI]\n",
                                    extractFileFromDirectoryPath (__FILE__).
                                    c_str (), __FUNCTION__, __LINE__,
                                    chrono.getMillis ());
        if (showPerfStats)
        ...
        if (updateLoops > 0)
          {
            ...
            int64
              lastReplaySecond = -1;
            int
              replayCommandsPlayed = 0;
            int
              replayTotal = commander.getReplayCommandListForFrameCount ();
            if (replayTotal > 0)
              {
                chronoReplay.start ();
              }
            ...
            do
              {
                if (replayTotal > 0)
                  {
                    replayCommandsPlayed =
                      (replayTotal -
                       commander.getReplayCommandListForFrameCount ());
                  }
                for (int i = 0; i < updateLoops; ++i)
                  {
                    ...
                    if (commander.hasReplayCommandListForFrame () == false)
                      {
                        ...
                        addPerformanceCount ("CalculateNetworkCRCSynchChecks",
                                             chronoGamePerformanceCounts.
                                             getMillis ());
                        ...
                        const bool
                          newThreadManager =
                          Config::getInstance ().
                          getBool ("EnableNewThreadManager", "false");
                        if (newThreadManager == true)
                          {
                            int
                              currentFrameCount = world.getFrameCount ();
                            masterController.
                              signalSlaves (&currentFrameCount);
                        ...
                        else
                          {
                            ...
                            for (int j = 0; j < world.getFactionCount (); ++j)
                              {
                                ...
                                if (faction->
                                    getCpuControl (enableServerControlledAI,
                                                   isNetworkGame,
                                                   role) == true
                                    && scriptManager.getPlayerModifiers (j)->
                                    getAiEnabled () == true)
                                  {
                                    ...
                                    if (SystemFlags::
                                      ...
                                      SystemFlags::OutputDebug (SystemFlags::
                                                                debugPerformance,
                                                                "In [%s::%s Line: %d] [i = %d] faction = %d, factionCount = %d, took msecs: %lld [before AI updates]\n",
                                                                extractFileFromDirectoryPath
                                                                (__FILE__).
                                                                c_str (),
                                                                __FUNCTION__,
                            ...
                            if (hasAIPlayer == true)
                              {
                                ...
                                const int
                                  MAX_FACTION_THREAD_WAIT_MILLISECONDS =
                                  20000;
                                for (;
                                     chronoAI.getMillis () <
                                     MAX_FACTION_THREAD_WAIT_MILLISECONDS;)
                                    ...
                                    for (int j = 0;
                                         j < world.getFactionCount (); ++j)
                                      {
                                        ...
                                        if (faction->
                                            getCpuControl
                                            (enableServerControlledAI,
                                             isNetworkGame, role) == true
                                            && scriptManager.
                                            getPlayerModifiers (j)->
                                            getAiEnabled () == true)
                                          {
                                            if (aiInterfaces[j]->
                                                isWorkerThreadSignalCompleted
                                                (world.getFrameCount ()) ==
                                                false)
                                              {
                            ...
                            addPerformanceCount ("ProcessAIWorkerThreads",
                                                 chronoGamePerformanceCounts.
                                                 getMillis ());
                          }
                        ...
                        if (lastReplaySecond < chronoReplay.getSeconds ())
                          {
                            ...
                            char
                              szBuf[8096] = "";
                            snprintf (szBuf, 8096,
                                      "Please wait, loading game with replay [%d / %d]...",
                                      replayCommandsPlayed, replayTotal);
                            string
                              text = szBuf;
                            if (Renderer::renderText3DEnabled)
                              {
                                ...
                                int
                                  renderX =
                                  (w / 2) -
                                  (font->getMetrics ()->getTextWidth (text) /
                                   2);
                                int
                                ...
                                int
                                  renderY =
                                  (h / 2) +
                                  (font->getMetrics ()->getHeight (text) / 2);

                                renderer.renderText3D (text, font,
                                                       Vec3f (1.f, 1.f, 0.f),
                                                       renderX, renderY,
                                                       false);
                              }
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [AI updates]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis ());
                    ...
                    addPerformanceCount ("ProcessWorldUpdate",
                                         chronoGamePerformanceCounts.
                                         getMillis ());
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [world update i = %d]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis (), i);
                    ...
                    if (currentCameraFollowUnit != NULL)
                      {
                        Vec3f
                          c =
                          currentCameraFollowUnit->getCurrMidHeightVector ();
                        int
                          rotation = currentCameraFollowUnit->getRotation ();
                        float
                          angle = rotation + 180;
                        ...
                        c.y =
                          c.y +
                          currentCameraFollowUnit->getType ()->getHeight () /
                          2.f + 2.0f;
                    ...
                    addPerformanceCount ("ProcessNetworkUpdate",
                                         chronoGamePerformanceCounts.
                                         getMillis ());
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [commander updateNetwork i = %d]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis (), i);
                    ...
                    addPerformanceCount ("ProcessGUIUpdate",
                                         chronoGamePerformanceCounts.
                                         getMillis ());
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [gui updating i = %d]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis (), i);
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [weather particle updating i = %d]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis (), i);
                    ...
                    addPerformanceCount ("ProcessParticleManager",
                                         chronoGamePerformanceCounts.
                                         getMillis ());
                    ...
                    if (SystemFlags::
                        ...
                        enabled && chrono.getMillis () > 0)
                      SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                                "In [%s::%s] Line: %d took msecs: %lld [particle manager updating i = %d]\n",
                                                extractFileFromDirectoryPath
                                                (__FILE__).c_str (),
                                                __FUNCTION__, __LINE__,
                                                chrono.getMillis (), i);
        ...
        if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
            enabled && chrono.getMillis () > 0)
          SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                    "In [%s::%s] Line: %d took msecs: %lld [chatManager.updateNetwork]\n",
                                    extractFileFromDirectoryPath (__FILE__).
                                    c_str (), __FUNCTION__, __LINE__,
                                    chrono.getMillis ());
        if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
        ...
        if (NetworkManager::getInstance ().getGameNetworkInterface () != NULL
            && NetworkManager::getInstance ().getGameNetworkInterface ()->
            getQuit () && mainMessageBox.getEnabled () == false
            && errorMessageBox.getEnabled () == false)
          {
            if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
                enabled)
        ...
        addPerformanceCount ("ProcessMiscNetwork",
                             chronoGamePerformanceCounts.getMillis ());
        ...
        if (role == nrServer)
          {
                ...
                if (getPaused () == false)
                  {
                    ...
                    printf
                      ("[CMDR] Pausing game for lagging client(s), current world frame [%d]\n",
                       world.getFrameCount ());
                ...
                if (getPaused () == true)
                  {
                    ...
                    printf
                      ("[CMDR] Resuming game after Pause for lagging client(s), current world frame [%d]\n",
                       world.getFrameCount ());
            ...
            ServerInterface *
              server = NetworkManager::getInstance ().getServerInterface ();
            if (server->getPauseForInGameConnection () == true)
              {

                bool
                  clientNeedsGameSetup = false;
                for (int i = 0; i < world.getFactionCount (); ++i)
                  {
                    Faction *
                      faction = world.getFaction (i);
                    ...
                    for (int i = 0; i < world.getFactionCount (); ++i)
                      {
                        ...
                        if (faction->getControlType () != ctNetwork &&
                            ...
                            server->isClientConnected (faction->
                                                       getStartLocationIndex
                                                       ()) == true)
                          {
                            ...
                            server->gameSettings.setFactionControl (i,
                                                                    ctNetwork);
                            ...
                            this->gameSettings.setFactionControl (i,
                                                                  ctNetwork);
                            ...
                            this->gameSettings.setNetworkPlayerPlatform (i,
                                                                         ...
                                                                         (i));
                            this->gameSettings.setNetworkPlayerStatuses (i,
                                                                         npst_None);
                          }
                      }
            ...
            else if (server->getStartInGameConnectionLaunch () == true)
              {
                ...
                //Lang &lang= Lang::getInstance();
                bool
                  pauseAndSaveGameForNewClient = false;
                for (int i = 0; i < world.getFactionCount (); ++i)
                  {
                    Faction *
                      faction = world.getFaction (i);
                    ...
                    if (saveNetworkGame == true)
                      {
                        ...
                        string
                          file =
                          this->
                          saveGame (GameConstants::saveNetworkGameFileServer,
                                    "temp/");

                        string
                          saveGameFilePath = "temp/";
                        string
                          saveGameFileCompressed =
                          saveGameFilePath +
                          string (GameConstants::
        ...
        if (world.getQueuedScenario () != "")
          {
            string
              name = world.getQueuedScenario ();
            bool
              keepFactions = world.getQueuedScenarioKeepFactions ();
            ...
            string
              scenarioFile = Scenario::getScenarioPath (dirList, name);
            ...
            try
            {
              ...
              bool
                isTutorial = Scenario::isGameTutorial (scenarioFile);
              ScenarioInfo
                scenarioInfo;
              ...
              GameSettings
                gameSettings;
              Scenario::loadGameSettings (dirList, &scenarioInfo,
                                          &gameSettings, scenarioFile);
              ...
              if (keepFactions == false)
              ...
              else
                {
                  ...
                  this->setGameSettings (&gameSettings);
                  this->
                    load (lgt_FactionPreview | lgt_TileSet | lgt_Map |
                          lgt_Scenario);
                  try
                  {
                  ...
                  catch (const exception & ex)
                  {
                    ...
                    snprintf (szBuf, 8096,
                              "In [%s::%s Line: %d]\nError [%s]\n",
                              extractFileFromDirectoryPath (__FILE__).
                              c_str (), __FUNCTION__, __LINE__, ex.what ());
                  ...
                  // camera default height calculation
                  if (map->getCameraHeight () > 0
                      && gameCamera.getCalculatedDefault () <
                      map->getCameraHeight ())
                    {
                      gameCamera.setCalculatedDefault (map->
                                                       getCameraHeight ());
                    }
                  else if (gameCamera.getCalculatedDefault () <
                           map->getMaxMapHeight () + 13.0f)
    ...
    void
    Game::addPerformanceCount (string key, int64 value)
    {
      gamePerformanceCounts[key] = value + gamePerformanceCounts[key] / 2;
    }

    string
    Game::getGamePerformanceCounts (bool displayWarnings) const
    {
      ...
      bool
        displayWarningHeader = true;
      bool
        WARN_TO_CONSOLE =
        Config::getInstance ().getBool ("PerformanceWarningEnabled", "false");
      int
        WARNING_MILLIS =
        Config::getInstance ().getInt ("PerformanceWarningMillis", "7");
      int
        WARNING_RENDER_MILLIS =
        Config::getInstance ().getInt ("PerformanceWarningRenderMillis",
                                       "40");

      string
        result = "";
      for (std::map < string, int64 >::const_iterator iterMap =
           gamePerformanceCounts.begin ();
           iterMap != gamePerformanceCounts.end (); ++iterMap)
        {
          ...
          string
            perfStat =
            iterMap->first + " = avg millis: " + intToStr (iterMap->second);
                               ...
                               bool onlyNetworkUnassigned)
    {
      ...
      MutexSafeWrapper
      safeMutex (serverInterface->getSwitchSetupRequestsMutex (),
                 CODE_AT_LINE);
      SwitchSetupRequest **
        switchSetupRequests = serverInterface->getSwitchSetupRequests ();
          ...
          if (switchSetupRequests[i] != NULL)
            {
              ...
              if (SystemFlags::
                  getSystemSettingType (SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                          "In [%s::%s Line: %d] switchSetupRequests[i]->getSwitchFlags() = %d\n",
                                          extractFileFromDirectoryPath
                                          (__FILE__).c_str (), __FUNCTION__,
                                          __LINE__,
                                          switchSetupRequests[i]->
                                          getSwitchFlags ());
              ...
              if (gameSettings.getFactionControl (i) == ctNetwork ||
                  ...
                  (gameSettings.getFactionControl (i) != ctHuman))
                {
                  if (SystemFlags::
                      getSystemSettingType (SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                              "In [%s::%s Line: %d] switchSetupRequests[i]->getToFactionIndex() = %d\n",
                                              extractFileFromDirectoryPath
                                              (__FILE__).c_str (),
                                              __FUNCTION__, __LINE__,
                                              switchSetupRequests[i]->
                  ...
                  if (switchSetupRequests[i]->getToSlotIndex () != -1)
                    {
                      ...
                      int
                        switchSlotIdx =
                        switchSetupRequests[i]->getCurrentSlotIndex ();
                      if (serverInterface->
                          switchSlot (switchSlotIdx, newSlotIdx))
                        {
                          ...
                          int
                            oldFactionIndex =
                            gameSettings.
                            getFactionIndexForStartLocation (switchSlotIdx);
                          int
                            newFactionIndex =
                            gameSettings.
                            getFactionIndexForStartLocation (newSlotIdx);
                          ...
                          gameSettings.setNetworkPlayerName (oldFactionIndex,
                                                             "");
                          serverInterface->gameSettings.
                            setNetworkPlayerName (oldFactionIndex, "");
                          gameSettings.setNetworkPlayerUUID (oldFactionIndex,
                                                             "");
                          serverInterface->gameSettings.
                            setNetworkPlayerUUID (oldFactionIndex, "");

                          gameSettings.
                            setNetworkPlayerPlatform (oldFactionIndex, "");
                          serverInterface->gameSettings.
                            setNetworkPlayerPlatform (oldFactionIndex, "");

                          gameSettings.setFactionControl (newFactionIndex,
                                                          ctNetwork);
                          serverInterface->gameSettings.
                            setFactionControl (newFactionIndex, ctNetwork);
                          ...
                          try
                          {
                            ...
                            if (switchSetupRequests[i]->
                                getNetworkPlayerName () != "")
                              {
                                ...
                                gameSettings.
                                  setNetworkPlayerName (newFactionIndex,
                                                        switchSetupRequests
                                                        [i]->
                                                        getNetworkPlayerName
                                                        ());
...
//                                                      }
                  ...
                  else
                    {
                      try
                      {
                        ...
                        if ((switchSetupRequests[i]->
                             getSwitchFlags () & ssrft_NetworkPlayerName) ==
      ...
      try
      {
        ...
        if (gameNetworkInterface != NULL &&
            gameNetworkInterface->getMarkedCellList (false).empty () == false)
          {
            ...
            std::vector < MarkedCell > chatList =
              gameNetworkInterface->getMarkedCellList (true);
            for (int idx = 0; idx < (int) chatList.size (); idx++)
              {
                ...
                if (mc.getFactionIndex () >= 0)
                  {
                    mc.setFaction ((const Faction *) world.
                                   getFaction (mc.getFactionIndex ()));
                  }

                Map *
                  map = world.getMap ();
                Vec2i
                  surfaceCellPos = map->toSurfCoords (mc.getTargetPos ());
    ...
    void
    Game::updateNetworkUnMarkedCells ()
    {
      try
      {
        GameNetworkInterface *
        ...
        if (gameNetworkInterface != NULL &&
            gameNetworkInterface->getUnMarkedCellList (false).empty () ==
            false)
          {
            //Lang &lang= Lang::getInstance();

            std::vector < UnMarkedCell > chatList =
              gameNetworkInterface->getUnMarkedCellList (true);
            for (int idx = 0; idx < (int) chatList.size (); idx++)
              {
                UnMarkedCell
                  mc = chatList[idx];
    ...
    void
    Game::updateNetworkHighligtedCells ()
    {
      try
      {
        GameNetworkInterface *
        ...
        if (gameNetworkInterface != NULL &&
            gameNetworkInterface->getHighlightedCellList (false).empty () ==
            false)
          {
            //Lang &lang= Lang::getInstance();
            std::vector < MarkedCell > highlighList =
              gameNetworkInterface->getHighlightedCellList (true);
            for (int idx = 0; idx < (int) highlighList.size (); idx++)
              {
                MarkedCell
                  mc = highlighList[idx];       // I want a copy here
                if (mc.getFactionIndex () >= 0)
                  {
    ...
    void
    Game::addOrReplaceInHighlightedCells (MarkedCell mc)
    {
      if (mc.getFactionIndex () >= 0)
        {
      ...
      if (this->masterserverMode == false)
        {
          ...
          const Faction *
            faction = mc.getFaction ();
          if (getWorld ()->getThisFaction () == NULL)
            {
              throw
              megaglest_runtime_error
                ("getWorld()->getThisFaction() == NULL");
            }
                                                   ...
                                                   NetworkRole role)
    {
      if (role == nrServer && isNetworkGame == true &&
          ...
          NETWORK_PLAYER_CONNECTION_CHECK_SECONDS)
        {
          lastNetworkPlayerConnectionCheck = time (NULL);
          Logger & logger = Logger::getInstance ();
          ServerInterface *
                  ...
                  if (aiInterfaces[i] == NULL &&
                      ...
                      false)
                    {
                      ...
                      if (faction->getPersonalityType () != fpt_Observer)
                        {
                          DumpCRCWorldLogIfRequired ("_faction_" +
                                                     intToStr (i));
                        }
                      ...
                      for (unsigned int j = 0;
                           j < (unsigned int) languageList.size (); ++j)
                        {
                          if (isPlayerObserver == false)
                            {
                              ...
                              if (lang.
                                  hasString ("GameSwitchPlayerToAI",
                                             languageList[j], true))
                                {
                                  msg =
                          ...
                          else
                            {
                              ...
                              if (lang.
                                  hasString ("GameSwitchPlayerObserverToAI",
                                             languageList[j], true))
                                {
                                  msg =
                                    lang.
                                    getString ("GameSwitchPlayerObserverToAI",
                                               languageList[j], true);
                                }
                              snprintf (szBuf, 8096, msg.c_str (), i + 1,
                                        this->gameSettings.
                          ...
                          bool
                            localEcho =
                            (languageList[j] == lang.getLanguage ());
          ...
          if (newAIPlayerCreated == true
              && Config::getInstance ().getBool ("EnableNewThreadManager",
                                                 "false") == true)
            {
              bool
                enableServerControlledAI =
      ...
      else
        {
          ...
          if (connectedClients == 0)
          ...
          else
            {
              ...
              if (difftime
                  ((long int) time (NULL),
                   lastMasterServerGameStatsDump) >= GAME_STATS_DUMP_INTERVAL)
                {
...
//              }
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %d [render3d]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        chrono.start ();
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %d [render2d]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %d [swap buffers]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
    }
...
// ==================== tick ====================
    ...
    void
    Game::removeUnitFromSelection (const Unit * unit)
    {
      try
      {
        Selection *
          selection = gui.getSelectionPtr ();
        for (int i = 0; i < selection->getCount (); ++i)
          {
            const Unit *
              currentUnit = selection->getUnit (i);
            if (currentUnit == unit)
              {
    ...
    bool
    Game::addUnitToSelection (Unit * unit)
    {
      bool
        result = false;
      try
    ...
    void
    Game::addUnitToGroupSelection (Unit * unit, int groupIndex)
    {
      try
      {
        Selection *
    ...
    void
    Game::removeUnitFromGroupSelection (int unitId, int groupIndex)
    {
      try
      {
        Selection *
    ...
    void
    Game::recallGroupSelection (int groupIndex)
    {
      try
      {
        Selection *
    ...
    Game::tick ()
    {
      ...
      const int
        CHECK_AVG_FPS_EVERY_X_TICKS = 15;
      if (tickCount % CHECK_AVG_FPS_EVERY_X_TICKS == 0)
        {
          avgRenderFps =
            currentAvgRenderFpsTotal / CHECK_AVG_FPS_EVERY_X_TICKS;
          currentAvgRenderFpsTotal = 0;
        }
...
// ==================== events ====================
    ...
    int
    Game::getFirstUnusedTeamNumber ()
    {
      std::map < int,
        bool >
        uniqueTeamNumbersUsed;
      for (unsigned int i = 0; i < (unsigned int) world.getFactionCount ();
           ++i)
        {
          Faction *
            faction = world.getFaction (i);
    ...
    void
    Game::tryPauseToggle (bool pauseValue)
    {
      bool
        allowAdminMenuItems = false;
      NetworkManager & networkManager = NetworkManager::getInstance ();
    ...
    {
      int
        totalMarkedCellsForPlayer = 0;
      if (world.getThisFaction () != NULL)
        {
          for (std::map < Vec2i, MarkedCell >::iterator iterMap =
               mapMarkedCellList.begin ();
               iterMap != mapMarkedCellList.end (); ++iterMap)
            {
              MarkedCell & bm = iterMap->second;
              if (bm.getPlayerIndex () ==
      ...
      if (totalMarkedCellsForPlayer < MAX_MARKER_COUNT)
        ...
        {
          Lang & lang = Lang::getInstance ();
          console.addLine (lang.getString ("MaxMarkerCount") + " " +
                           intToStr (MAX_MARKER_COUNT));
        }
    }

    void
    Game::processInputText (string text, bool cancelled)
    {
      isMarkCellTextEnabled = false;
...
//              renderer.forceQuadCacheUpdate();
    ...
    void
    Game::addCellMarker (Vec2i cellPos, MarkedCell cellData)
    {
      //printf("Note [%s]\n",text.c_str());

      string
        text = cellData.getNote ();
      if (text.find ("\\n") != text.npos)
        {
          replaceAll (text, "\\n", "\n");
        }
      ...
      gameNetworkInterface->sendMarkCellMessage (cellData.getTargetPos (),
                                                 factionIndex,
                                                 cellData.getNote (),
                                                 playerIndex);
    ...
    void
    Game::removeCellMarker (Vec2i surfaceCellPos, const Faction * faction)
    {
      //Vec2i surfaceCellPos = map->toSurfCoords(Vec2i(xCell,yCell));
      Map *
        map = world.getMap ();
      SurfaceCell *
        sc = map->getSurfaceCell (surfaceCellPos);
      Vec3f
        vertex = sc->getVertex ();
      Vec2i
      targetPos (vertex.x, vertex.z);
      ...
      if (mapMarkedCellList.find (surfaceCellPos) != mapMarkedCellList.end ())
        {
          MarkedCell
            mc = mapMarkedCellList[surfaceCellPos];
    ...
    void
    Game::showMarker (Vec2i cellPos, MarkedCell cellData)
    {
      //setMarker = true;
      //if(setMarker) {
      ...
      GameNetworkInterface *
        gameNetworkInterface =
        NetworkManager::getInstance ().getGameNetworkInterface ();
      gameNetworkInterface->sendHighlightCellMessage (cellData.
                                                      getTargetPos (),
                                                      cellData.
                                                      getFactionIndex ());
      //}
    ...
    void
    Game::mouseDownLeft (int x, int y)
    {
      if (this->masterserverMode == true)
        {
          return;
      ...
      try
      {
        ...
        NetworkManager & networkManager = NetworkManager::getInstance ();
        bool
          messageBoxClick = false;
        bool
          originalIsMarkCellEnabled = isMarkCellEnabled;
        bool
          originalIsUnMarkCellEnabled = isUnMarkCellEnabled;

        if (popupMenu.mouseClick (x, y))
          {
            std::pair < int,
              string >
              result = popupMenu.mouseClickedMenuItem (x, y);
            ...
            if (result.first == exitGamePopupMenuIndex)
              {
                showMessageBox (Lang::getInstance ().
                                getString ("ExitBattleQuestion"), "", true);
              }
            else if (result.first == joinTeamPopupMenuIndex)
              {
                ...
                for (unsigned int i = 0;
                     i < (unsigned int) world.getFactionCount (); ++i)
                  {
                    Faction *
                      faction = world.getFaction (i);

                    if (faction->getPersonalityType () != fpt_Observer &&
                        uniqueTeamNumbersUsed.find (faction->getTeam ()) ==
                        uniqueTeamNumbersUsed.end ())
                      {
                        uniqueTeamNumbersUsed[faction->getTeam ()] = true;
                      }

                    if (faction->getPersonalityType () != fpt_Observer &&
                        world.getThisFaction ()->getIndex () !=
                        faction->getIndex ()
                        && world.getThisFaction ()->getTeam () !=
                        faction->getTeam ())
                      {
                        char
                          szBuf[8096] = "";
                        if (lang.hasString ("JoinPlayerTeam") == true)
                          {
                            snprintf (szBuf, 8096,
                                      ("  " +
                                       lang.getString ("JoinPlayerTeam") +
                                       "  ").c_str (), faction->getIndex (),
                                      this->gameSettings.
                                      getNetworkPlayerName (i).c_str (),
                                      faction->getTeam ());
                          }
                        ...
                        switchTeamIndexMap[(int) menuItems.size () - 1] =
                          faction->getTeam ();
                      }
                  }
                ...
                popupMenuSwitchTeams.init ("  " +
                                           lang.getString ("SwitchTeams") +
                                           "  ", menuItems);
                ...
                if (role == nrServer)
                  {
                    serverInterface =
                      dynamic_cast <
                      ServerInterface *
                      >(networkManager.getServerInterface ());
                  }
                disconnectPlayerIndexMap.clear ();
                std::vector < string > menuItems;
                for (unsigned int i = 0;
                     i < (unsigned int) world.getFactionCount (); ++i)
                  {
                    ...
                    bool
                      isSlotJoinInProgressClient = false;
                    if (serverInterface != NULL)
                      {
                    ...
                    if (isSlotJoinInProgressClient == true ||
                        (faction->getPersonalityType () != fpt_Observer &&
                         ...
                         faction->getIndex ()
                         && faction->getControlType () == ctNetwork
                         && this->gameSettings.getNetworkPlayerStatuses (i) !=
                         npst_Disconnected))
                      {

                        char
                          szBuf[8096] = "";
                        if (lang.hasString ("DisconnectNetorkPlayerIndex") ==
                            true)
                          {
                            snprintf (szBuf, 8096,
                                      ("  " +
                        ...
                        disconnectPlayerIndexMap[(int) menuItems.size () -
                                                 1] = faction->getIndex ();
                      }
                  }

                menuItems.push_back ("  " + lang.getString ("Cancel") + "  ");
                disconnectPlayerIndexMap[(int) menuItems.size () - 1] =
                  CANCEL_DISCONNECT_PLAYER;

                popupMenuDisconnectPlayer.setW (100);
                popupMenuDisconnectPlayer.setH (100);
                popupMenuDisconnectPlayer.init ("  " +
                                                lang.
                                                getString
                                                ("DisconnectNetorkPlayer") +
                                                "  ", menuItems);
              ...
              {
                MainMenu *
                  newMenu = new MainMenu (program);     // open keyboard shortcuts setup screen
                ...
                newMenu->
                  setState (new
                            MenuStateKeysetup (program, newMenu,
                                               &currentUIState));
              }
        ...
        else if (popupMenuSwitchTeams.mouseClick (x, y))
          {
            ...
            std::pair < int,
              string >
              result = popupMenuSwitchTeams.mouseClickedMenuItem (x, y);
            //printf("In popup callback menuItemSelected [%s] menuIndexSelected = %d\n",result.second.c_str(),result.first);
            ...
            int
              teamIndex = switchTeamIndexMap[result.first];
            switch (teamIndex)
              {
              case CREATE_NEW_TEAM:
                {
                  int
                    newTeam = getFirstUnusedTeamNumber ();
                  //if(isNetworkGame == true) {
                  const Faction *
                    faction = world.getThisFaction ();
        ...
        else if (popupMenuDisconnectPlayer.mouseClick (x, y))
          {
            ...
            std::pair < int,
              string >
              result = popupMenuDisconnectPlayer.mouseClickedMenuItem (x, y);
            //printf("In popup callback menuItemSelected [%s] menuIndexSelected = %d\n",result.second.c_str(),result.first);
            ...
            int
              factionIndex = disconnectPlayerIndexMap[result.first];
            switch (factionIndex)
              {
...
//                                      }
                ...
                char
                  szBuf[8096] = "";
                if (lang.hasString ("DisconnectNetorkPlayerIndexConfirm") ==
                    true)
                  {
                    snprintf (szBuf, 8096,
                              ("  " +
                               lang.
                               getString
                               ("DisconnectNetorkPlayerIndexConfirm") +
                               "  ").c_str (), factionIndex + 1,
                  ...
                  {
                    snprintf (szBuf, 8096,
                              "  Confirm disconnection for player #%d - %s?  ",
                              factionIndex + 1,
                              settings->getNetworkPlayerName (factionIndex).
                              c_str ());
                  }
                ...
                disconnectPlayerConfirmMessageBox.init (lang.
                                                        getString ("Yes"),
                                                        lang.
                                                        getString ("No"));
                disconnectPlayerConfirmMessageBox.setEnabled (true);

                playerIndexDisconnect =
                  world.getFaction (factionIndex)->getStartLocationIndex ();
                ...
                if (gameNetworkInterface != NULL)
                  {
                    ...
                    for (unsigned int i = 0;
                         i < (unsigned int) languageList.size (); ++i)
                      {
                        char
                          szMsg[8096] = "";
                        if (lang.
                            hasString ("DisconnectNetorkPlayerIndexConfirmed",
                                       languageList[i]) == true)
                          {
                            snprintf (szMsg, 8096,
                                      lang.
                                      getString
                                      ("DisconnectNetorkPlayerIndexConfirmed",
                          ...
                          {
                            snprintf (szMsg, 8096,
                                      "Notice - Admin is warning to disconnect player #%d - %s!",
                                      factionIndex + 1,
                                      settings->
                                      getNetworkPlayerName (factionIndex).
                                      c_str ());
                        ...
                        bool
                          localEcho = lang.isLanguageLocal (languageList[i]);
            ...
            if (switchTeamConfirmMessageBox.mouseClick (x, y, button))
              {
                ...
                SwitchTeamVote *
                  vote =
                  world.getThisFactionPtr ()->getSwitchTeamVote (world.
                                                                 getThisFaction
                                                                 ()->
                                                                 getCurrentSwitchTeamVoteFactionIndex
                                                                 ());
                vote->voted = true;
                ...
                if (button == 0)
                  {
                    const Faction *
                      faction = world.getThisFaction ();
                    commander.tryDisconnectNetworkPlayer (faction,
                                                          playerIndexDisconnect);
                  }
              }
          }
        ...
        if (messageBoxClick == false)
          {
            if (metrics.isInMinimap (x, y))
              {
                int
                  xm = x - metrics.getMinimapX ();
                int
                  ym = y - metrics.getMinimapY ();
                int
                  xCell =
                  static_cast <
                  int >(xm *
                ...
                if (map->isInside (xCell, yCell)
                                                           ...
                                                           (xCell, yCell))))
                  {
                        ...
                        if (!setMarker)
                          {
                            ...
                            gameCamera.
                              setPos (Vec2f
                                      (static_cast < float >(xCell),
                                       static_cast < float >(yCell)));
                          }
                      }
                    ...
                    if (setMarker)
                      {
                        Vec2i
                          surfaceCellPos =
                          map->toSurfCoords (Vec2i (xCell, yCell));
                        SurfaceCell *
                          sc = map->getSurfaceCell (surfaceCellPos);
                        Vec3f
                          vertex = sc->getVertex ();
                        ...
                        MarkedCell
                        mc (targetPos, world.getThisFaction (), "none",
                            world.getThisFaction ()->
                            getStartLocationIndex ());
                        addOrReplaceInHighlightedCells (mc);
                        ...
                        gameNetworkInterface->sendHighlightCellMessage (mc.
                                                                        getTargetPos
                                                                        (),
                                                                        mc.
                                                                        getFaction
                    ...
                    if (originalIsMarkCellEnabled == true
                        && isMarkCellEnabled == true)
                      {
                        Vec2i
                          surfaceCellPos =
                        ...
                        MarkedCell
                        mc (targetPos, world.getThisFaction (),
                    ...
                    if (originalIsUnMarkCellEnabled == true
                        && isUnMarkCellEnabled == true)
                      {
                        Vec2i
                          surfaceCellPos =
...
//                                              }
                        ...
                        removeCellMarker (surfaceCellPos,
                                          world.getThisFaction ());
                        //printf("#1 ADDED in marked list pos [%s] markedCells.size() = " MG_SIZE_T_SPECIFIER "\n",surfaceCellPos.getString().c_str(),mapMarkedCellList.size());
            ...
            //display panel
              ...
              {
                int
                  xd = x - metrics.getDisplayX ();
                int
                  yd = y - metrics.getDisplayY ();
            ...
            else
              {
                ...
                if (originalIsMarkCellEnabled == true
                    && isMarkCellEnabled == true)
                  {
                    ...
                    targetPos = getMouseCellPos ();
                    Vec2i
                      surfaceCellPos = map->toSurfCoords (targetPos);
...
//                                      }
    ...
    void
    Game::mouseDownRight (int x, int y)
    {
      if (this->masterserverMode == true)
        {
          return;
        ...
        if (metrics.isInMinimap (x, y))
          {
            ...
            if (map->isInside (xCell, yCell)
                && map->isInsideSurface (map->
                                         toSurfCoords (Vec2i (xCell, yCell))))
              {
    ...
    void
    Game::mouseUpCenter (int x, int y)
    {
      if (this->masterserverMode == true)
        {
          return;
    ...
    void
    Game::mouseDoubleClickLeft (int x, int y)
    {
      if (this->masterserverMode == true)
        {
          return;
    ...
    void
    Game::mouseMove (int x, int y, const MouseState * ms)
    {
      if (this->masterserverMode == true)
        {
          return;
      ...
      try
      {
        ...
        if (ms->get (mbCenter))
            ...
            if (currentCameraFollowUnit == NULL)
              {
                ...
                Vec2i
                  oldPos =::Shared::Platform::Window::getOldMousePos ();
                int
                  oldx =
                  (oldPos.x * metrics.getVirtualW () / metrics.getScreenW ());
                int
                  oldy =
                  ((metrics.getScreenH () -
                    oldPos.y) * metrics.getVirtualH () /
                   metrics.getScreenH ());
                lastMousePos.x = oldx;
                lastMousePos.y = oldy;
                gameCamera.transitionVH (-(y - oldy) * ymult,
                                         (oldx - x) * xmult);
              }
        ...
        else if (currentCameraFollowUnit == NULL)
          {
            ...
            if (!camLeftButtonDown && !camRightButtonDown && !camUpButtonDown
                && !camDownButtonDown)
              {
                if (ms->get (mbLeft) && metrics.isInMinimap (x, y))
                  ...
                  {
                    bool
                      mouseMoveScrollsWorld =
                      Config::getInstance ().getBool ("MouseMoveScrollsWorld",
                                                      "true");
            ...
            if (!gui.isSelectingPos ())
              {
                if (metrics.isInDisplay (x, y))
                  {
                    gui.mouseMoveDisplay (x - metrics.getDisplayX (),
                                          y - metrics.getDisplayY ());
                  }
                else
                  {
    ...
    bool
    Game::isValidMouseCellPos () const
    {
    ...
    void
    Game::eventMouseWheel (int x, int y, int zDelta)
    {
      if (this->masterserverMode == true)
        {
          return;
    ...
    Game::startCameraFollowUnit ()
    {
      ...
      if (selection->getCount () == 1)
        {
          Unit *
            currentUnit = selection->getUnitPtr (0);
          if (currentUnit != NULL)
            {
              currentCameraFollowUnit = currentUnit;
              getGameCameraPtr ()->setState (GameCamera::sUnit);
              getGameCameraPtr ()->setPos (currentCameraFollowUnit->
    ...
    bool
    Game::textInput (std::string text)
    {
    ...
    bool
    Game::sdlKeyDown (SDL_KeyboardEvent key)
    {
      if (this->masterserverMode == true)
        {
          return false;
      ...
      for (int idx = 1; idx <= Selection::maxGroups; idx++)
        {
          string
            keyName = "GroupUnitsKey" + intToStr (idx);

          SDL_Keycode
            groupHotKey = configKeys.getSDLKey (keyName.c_str ());
          if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
              enabled)
            SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                      "In [%s::%s Line: %d] keyName [%s] group index = %d, key = [%c] [%d]\n",
                                      extractFileFromDirectoryPath (__FILE__).
                                      c_str (), __FUNCTION__, __LINE__,
                                      keyName.c_str (), idx, groupHotKey,
                                      groupHotKey);
    ...
    void
    Game::keyDown (SDL_KeyboardEvent key)
    {
      if (this->masterserverMode == true)
        {
          return;
      ...
      try
      {
        if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
            enabled)
          SystemFlags::OutputDebug (SystemFlags::debugSystem,
                                    "In [%s::%s Line: %d] key = [%c] [%d] gameStarted [%d]\n",
                                    extractFileFromDirectoryPath (__FILE__).
                                    c_str (), __FUNCTION__, __LINE__,
        ...
        if (chatManager.getEditEnabled () == false)
          {
            ...
            bool
              setMarkerKeyAllowsModifier = false;
            ...
            //else if(key == configKeys.getCharKey("ShowFullConsole")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("ShowFullConsole"), key,
                   false) == true)
              {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("SetMarker"), key,
                   setMarkerKeyAllowsModifier) == true)
              {
                setMarker = true;
              }
            //else if(key == configKeys.getCharKey("TogglePhotoMode")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("TogglePhotoMode"), key,
                   false) == true)
              {
            ...
            //Toggle Healthbars
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("ToggleHealthbars"), key,
                   false) == true)
              {
                switch (healthbarMode)
                  ...
                  case hbvSelected:
                    ...
                    console.addLine (lang.getString ("Healthbar2") + ": " +
                                     lang.
                                     getString
                                     ("HealthbarsSelectedOrNeeded"));
                    break;
                  case (hbvSelected | hbvIfNeeded):
                    healthbarMode = hbvUndefined;
                    console.addLine (lang.getString ("Healthbar2") + ": " +
                                     lang.
                                     getString ("HealthbarsFactionDefault"));
                    break;
                  default:
                    printf
                    ...
                    if (gameMusic != NULL)
                      {
                        float
                          configVolume =
                          (config.getInt ("SoundVolumeMusic") / 100.f);
                        float
                          currentVolume = gameMusic->getVolume ();
                        if (currentVolume > 0)
                          {
                        ...
                        else
                          {
                            ...
                            gameMusic->
                              setVolume (configVolume ? configVolume : 0.9);
                            console.addLine (lang.getString ("GameMusic"));
                          }
                      }
                  }
            ...
            //else if(key == configKeys.getCharKey("CameraModeLeft")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("CameraModeLeft"), key,
                   false) == true)
              {
            ...
            //else if(key == configKeys.getCharKey("CameraModeDown")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("CameraModeDown"), key,
                   false) == true)
              {
                gameCamera.setMoveZ (-1);
            ...
            //else if(key == configKeys.getCharKey("FreeCameraMode")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("FreeCameraMode"), key,
                   false) == true)
              {
                if (gameCamera.getState () == GameCamera::sFree)
                  {
                    ...
                    string
                      stateString =
                      gameCamera.getState () ==
                      GameCamera::sGame ? lang.
                      getString ("GameCamera") : lang.
            ...
            //else if(key == configKeys.getCharKey("GameSpeedIncrease")) {
                   ...
                   false) == true)
              {
                bool
                  speedChangesAllowed =
                  !NetworkManager::getInstance ().
                  isNetworkGameWithConnectedClients ();
            ...
            //else if(key == configKeys.getCharKey("GameSpeedDecrease")) {
              ...
              if (isKeyPressed
                  (configKeys.getSDLKey ("BookmarkRemove"), key,
                   false) == true)
              {
                isUnMarkCellEnabled = true;
              }
            ...
            if (gameCamera.getState () != GameCamera::sFree)
            ...
            else
              {
                ...
                //else if(key == configKeys.getCharKey("CameraRotateRight")){
                  ...
                  if (isKeyPressed
                      (configKeys.getSDLKey ("CameraRotateRight"),
                       key) == true)
                  {
                    gameCamera.setRotate (1);
                  }
                ...
                //else if(key == configKeys.getCharKey("CameraRotateDown")) {
                  ...
                  if (isKeyPressed
                      (configKeys.getSDLKey ("CameraRotateDown"),
                       key) == true)
                  {
                    gameCamera.setMoveY (-1);
                  }
    ...
    void
    Game::keyUp (SDL_KeyboardEvent key)
    {
      if (this->masterserverMode == true)
        {
          return;
    ...
    void
    Game::keyPress (SDL_KeyboardEvent c)
    {
      if (this->masterserverMode == true)
        {
          return;
    ...
    void
    Game::DumpCRCWorldLogIfRequired (string fileSuffix)
    {
      bool
        isNetworkGame = this->gameSettings.isNetworkGame ();
      if (isNetworkGame == true)
        {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf
              ("Check save world CRC to log. isNetworkGame = %d fileSuffix = %s\n",
          ...
          if (settings != NULL &&
            ...
            {
              string
                debugCRCWorldLogFile =
                Config::getInstance ().getString ("DebugCRCWorldLogFile",
                                                  "debugCRCWorld.log");
              ...
              FILE *
                fp =
                _wfopen (utf8_decode (debugCRCWorldLogFile).c_str (), L"w");
              std::ofstream logFile (fp);
#else
              std::ofstream logFile;
              logFile.open (debugCRCWorldLogFile.c_str (),
              ...
              for (unsigned int worldFrameIndex = 0;
                   worldFrameIndex <
                   world.getFaction (0)->getCRC_DetailsForWorldFrameCount ();
                   ++worldFrameIndex)
                {
                  //factions (and their related info)
                  for (int i = 0; i < world.getFactionCount (); ++i)
                    {
                      logFile << "Faction detail for index: " << i << std::
                        endl;
                      logFile << "--------------------------" << std::endl;
                      ...
                      std::pair < int,
                        ...
                        details =
                        world.getFaction (i)->
                        getCRC_DetailsForWorldFrameIndex (worldFrameIndex);

                      logFile << string ("** world frame: ") << details.
                        first << std::endl;
    ...
    void
    saveStatsToSteam (Game * game, Stats & endStats)
    {
      Steam *
        steamInstance =
        CacheManager::getCachedItem <
                   ...
                   ++factionIndex)
                {
                  if (factionIndex ==
                      game->getWorld ()->getThisFactionIndex ())
                    {
                      ...
                      steamInstance->setStatAsDouble (EnumParser <
                                                      SteamStatName >::
                                                      getString
              ...
              if (factionIndex == game->getWorld ()->getThisFactionIndex ())
                {
                  ...
                  if (endStats.getVictory (factionIndex))
                    {
                      if (steamInstance->
                          isUnlocked (EnumParser <
                                      SteamAchievementName >::
                                      getString (ACH_WIN_ONE_GAME).
                                      c_str ()) == false)
                        {
                          steamInstance->unlock (EnumParser <
                                                 SteamAchievementName >::
                                                 getString (ACH_WIN_ONE_GAME).
                                                 c_str ());
                        }
                      if (NetworkManager::getInstance ().isNetworkGame ())
                        {
                          if (steamInstance->
                              isUnlocked (EnumParser <
                                          SteamAchievementName >::
                                          getString (ACH_WIN_ONE_GAME_ONLINE).
                                          c_str ()) == false)
                            {
                              steamInstance->unlock (EnumParser <
                                                     SteamAchievementName >::
    ...
    void
    Game::exitGameState (Program * program, Stats & endStats)
    {
      if (SystemFlags::getSystemSettingType (SystemFlags::debugSystem).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugSystem,
      ...
      Game *
        game = dynamic_cast < Game * >(program->getState ());
      ...
      ProgramState *
        newState = new BattleEnd (program, &endStats, game);
...
// ==================== PRIVATE ====================
    ...
    void
    Game::highlightUnit (int unitId, float radius, float thickness,
                         Vec4f color)
    {
    ...
    void
    Game::unhighlightUnit (int unitId)
    {
...
// ==================== render ====================
    ...
    Game::render3d ()
    {
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [reset3d]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
...
//      if(SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [loadGameCameraMatrix]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [computeVisibleQuad]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [setupLighting]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderShadowsToTexture]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s] Line: %d renderFps = %d took msecs: %lld\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderSurface]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderSelectionEffects]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if ((renderExtraTeamColor & renderTeamColorCircleBit) > 0)
        {
          ...
          if (SystemFlags::
              ...
              && chrono.getMillis () > 0)
            SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                      "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderObjects]\n",
                                      extractFileFromDirectoryPath (__FILE__).
                                      c_str (), __FUNCTION__, __LINE__,
                                      renderFps, chrono.getMillis ());
          if (SystemFlags::
              getSystemSettingType (SystemFlags::debugPerformance).enabled
              && chrono.getMillis () > 0)
            chrono.start ();
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderUnits]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderWater]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderParticleManager]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderMouse3d]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
          enabled && chrono.getMillis () > 0)
        SystemFlags::OutputDebug (SystemFlags::debugPerformance,
                                  "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderUnitsToBuild]\n",
                                  extractFileFromDirectoryPath (__FILE__).
                                  c_str (), __FUNCTION__, __LINE__, renderFps,
                                  chrono.getMillis ());
      if (SystemFlags::getSystemSettingType (SystemFlags::debugPerformance).
    ...
    void
    Game::updateWorldStats ()
    {
      world.getStats ()->setWorldTimeElapsed (world.getTimeFlow ()->
                                              getTime ());
      ...
      if (difftime ((long int) time (NULL), lastMaxUnitCalcTime) >= 1)
        {
          ...
          if (world.getStats ()->getMaxConcurrentUnitCount () <
              totalUnitcount)
            {
              world.getStats ()->setMaxConcurrentUnitCount (totalUnitcount);
            }
          world.getStats ()->
            setTotalEndGameConcurrentUnitCount (totalUnitcount);
          world.getStats ()->setFramesPlayed (world.getFrameCount ());
        }
    }

    string
    Game::getDebugStats (std::map < int, string > &factionDebugInfo)
    {
      string
        str = "";
      ...
      if (this->masterserverMode == false)
        {
          str +=
            "MouseXY: " + intToStr (mouseX) + "," + intToStr (mouseY) + "\n";

          if (world.getMap ()->
              isInsideSurface (world.getMap ()->
                               toSurfCoords (mouseCellPos)) == true)
            {
              str +=
                "MouseXY cell coords: " + intToStr (mouseCellPos.x) + "," +
                intToStr (mouseCellPos.y) + "\n";
            }

          str +=
            "PosObjWord: " + intToStr (gui.getPosObjWorld ().x) + "," +
            intToStr (gui.getPosObjWorld ().y) + "\n";
        }

      str +=
        "Render FPS: " + intToStr (lastRenderFps) + "[" +
        intToStr (avgRenderFps) + "]\n";
      str +=
        "Update FPS: " + intToStr (lastUpdateFps) + "[" +
        intToStr (avgUpdateFps) + "]\n";

      if (this->masterserverMode == false)
        {
          str +=
            "GameCamera pos: " + floatToStr (gameCamera.getPos ().x) + "," +
      ...
      str +=
        "Time: " + floatToStr (world.getTimeFlow ()->getTime (),
                               2) + " [" +
        floatToStr ((double) world.getStats ()->
                    getFramesToCalculatePlaytime () /
                    (float) GameConstants::updateFps / 60.0, 2) + "]\n";

      if (SystemFlags::getThreadedLoggerRunning () == true)
        {
          str +=
            "Log buffer count: " +
            intToStr (SystemFlags::getLogEntryBufferCount ()) + "\n";
        }

      str +=
        "UnitRangeCellsLookupItemCache: " +
        world.getUnitUpdater ()->getUnitRangeCellsLookupItemCacheStats () +
        "\n";
      str +=
        "ExploredCellsLookupItemCache: " +
        world.getExploredCellsLookupItemCacheStats () + "\n";
      str +=
        "FowAlphaCellsLookupItemCache: " +
        world.getFowAlphaCellsLookupItemCacheStats () + "\n";

      const string
        selectionType =
        toLower (Config::getInstance ().
                 getString ("SelectionType", Config::colorPicking));
      str += "Selection type: " + toLower (selectionType) + "\n";

      if (selectionType == Config::colorPicking)
        {
          str +=
            "Color picking used color list size: " +
            intToStr (BaseColorPickEntity::getUsedColorIDListSize ()) + "\n";
        }
      ...
      if (this->masterserverMode == false)
        {
          Renderer & renderer = Renderer::getInstance ();
          str +=
            "Triangle count: " + intToStr (renderer.getTriangleCount ()) +
            "\n";
          str +=
            "Vertex count: " + intToStr (renderer.getPointCount ()) + "\n";
        }
      ...
      if (this->masterserverMode == false)
        {
          Renderer & renderer = Renderer::getInstance ();
          Quad2i
            visibleQuad = renderer.getVisibleQuad ();
          ...
          for (int i = 0; i < 4; ++i)
            {
              str +=
                "(" + intToStr (visibleQuad.p[i].x) + "," +
                intToStr (visibleQuad.p[i].y) + ") ";
            }
          //              str+= "\n";
          ...
          str +=
            "Visible quad area:        " + floatToStr (visibleQuad.area ()) +
            "\n";
      ...
      if (this->masterserverMode == false)
        {
          ...
          int
            visibleUnitCount = (int) qCache.visibleQuadUnitList.size ();
          str +=
            "Visible unit count: " + intToStr (visibleUnitCount) +
            " total: " + intToStr (totalUnitcount) + "\n";

          int
            visibleObjectCount = (int) qCache.visibleObjectList.size ();
          str +=
            "Visible object count: " + intToStr (visibleObjectCount) + "\n";
        }
      else
        {
      ...
      for (int i = 0; i < world.getFactionCount (); ++i)
        {
          string
            factionInfo = this->gameSettings.getNetworkPlayerName (i);
          //factionInfo += " [" + this->gameSettings.getNetworkPlayerUUID(i) + "]";
          float
            multi = world.getStats ()->getResourceMultiplier (i);
          string
            multiplier = "[" + floatToStr (multi, 1) + "]";
...
//              }
    ...
    Game::render2d ()
    {
      ...
      renderer.renderVisibleMarkedCells (true, lastMousePos.x,
                                         lastMousePos.y);
      ...
      //script display text
      if (!scriptManager.getDisplayText ().empty ()
          && !scriptManager.getMessageBoxEnabled ())
        {
          Vec4f
            fontColor = getGui ()->getDisplay ()->getColor ();
      ...
      if (renderer.getShowDebugUI () == true ||
          (perfLogging == true
           && difftime ((long int) time (NULL), lastRenderLog2d) >= 1))
        {
          str = getDebugStats (factionDebugInfo);
        }
      ...
      if (renderer.getShowDebugUI () == true)
        {
          ...
          if (Renderer::renderText3DEnabled == true)
            {
              renderer.renderTextShadow3D (str,
                                           coreData.getMenuFontNormal3D (),
                                           fontColor, 10,
          ...
          int
            ...
            (Renderer::renderText3DEnabled ==
             true ? coreData.getMenuFontNormal3D ()->getMetrics ()->
             getHeight ("W") : coreData.getMenuFontNormal ()->getMetrics ()->
             getHeight ("W"));
          int
            fontHeightBig =
            (Renderer::renderText3DEnabled ==
             true ? coreData.getMenuFontBig3D ()->getMetrics ()->
             getHeight ("W") : coreData.getMenuFontBig ()->getMetrics ()->
             getHeight ("W"));
          int
            playerPosY = (int) lineTokens.size () * fontHeightNormal;
          ...
          for (int i = 0; i < world.getFactionCount (); ++i)
            {
              string
                factionInfo = factionDebugInfo[i];
              Vec3f
                playerColor =
                world.getFaction (i)->getTexture ()->getPixmapConst ()->
                getPixel3f (0, 0);
          ...
          if ((renderer.getShowDebugUILevel () & debugui_unit_titles) ==
              debugui_unit_titles)
            {
              ...
              if (Renderer::renderText3DEnabled == true)
                {
                  renderer.renderUnitTitles3D (coreData.
                                               getMenuFontNormal3D (),
                                               Vec3f (1.0f));
                }
              else
      ...
      if (renderNetworkStatus == true)
        {
          if (NetworkManager::getInstance ().getGameNetworkInterface () !=
              NULL)
            {
              const
                Metrics &
                metrics = Metrics::getInstance ();
              int
                mx = metrics.getMinimapX ();
              //int my= metrics.getMinimapY();
              int
                mw = metrics.getMinimapW ();
              ...
              if (Renderer::renderText3DEnabled == true)
                {
                  renderer.renderTextShadow3D (NetworkManager::getInstance ().
                                               getGameNetworkInterface ()->
                                               getNetworkStatus (),
                                               coreData.
                                               getMenuFontNormal3D (),
                                               fontColor, mx + mw + 5,
                                               metrics.getVirtualH () - 30 -
                                               20, false);
                }
              else
                {
                  renderer.renderTextShadow (NetworkManager::getInstance ().
                                             getGameNetworkInterface ()->
                                             getNetworkStatus (),
                                             coreData.getMenuFontNormal (),
                                             fontColor, mx + mw + 5,
                                             metrics.getVirtualH () - 30 - 20,
                                             false);
                }
            }
        }
      ...
      if (photoModeEnabled == false)
        {
          ...
          renderer.renderConsole (&console,
                                  showFullConsole ? consoleFull :
                                  consoleNormal);
        }

      //2d mouse
      renderer.renderMouse2d (mouseX, mouseY, mouse2d,
                              gui.isSelectingPos ()? 1.f : 0.f);
...
// ==================== misc ====================
    ...
    Game::setEndGameTeamWinnersAndLosers ()
    {
      //bool lose= false;
      bool
        checkTeamIndex = !(this->masterserverMode == false
                           && world.getThisFaction ()->
                           getPersonalityType () != fpt_Observer);
          ...
          if (checkTeamIndex == false || i != world.getThisFactionIndex ())
            {
              if (factionLostGame (world.getFaction (i)) == false)
                {
                  teamsAlive[world.getFaction (i)->getTeam ()] =
                    teamsAlive[world.getFaction (i)->getTeam ()] + 1;
                }
            }
        }
      ...
      if (teamsAlive.size () <= 1)
        {
          for (int i = 0; i < world.getFactionCount (); ++i)
            {
              if (checkTeamIndex == false
                  || i != world.getThisFactionIndex ())
                {
                  if (teamsAlive.find (world.getFaction (i)->getTeam ()) !=
                      teamsAlive.end ())
                    {
...
//              }
    ...
    Game::getTeamsAlive ()
    {
      std::map < int, int >
        teamsAlive;
      for (int factionIndex = 0; factionIndex < world.getFactionCount ();
           ++factionIndex)
        {
    ...
    void
    Game::checkWinnerStandardHeadlessOrObserver ()
    {
      // lookup int is team #, value is players alive on team
      std::map < int, int >
        teamsAlive = getTeamsAlive ();
                  ...
                  if (this->masterserverMode == true)
                    {
                      ...
                      printf ("Player: %s is on the winning team #: %d\n",
                              this->gameSettings.
                              getNetworkPlayerName (factionIndex).c_str (),
                              faction->getTeam ());
                    }
                }
            }
          bool
            wasGameOverAlready = gameOver;
          gameOver = true;
    ...
    void
    Game::checkWinnerStandardPlayer ()
    {
      //lose
      bool
        lose = false;
          ...
          if (isFlagType1BitEnabled (ft1_allow_shared_team_units) == true)
            {
              ...
              for (int factionIndex = 0;
                   factionIndex < world.getFactionCount (); ++factionIndex)
                {

                  if (world.getFaction (factionIndex)->
                      getPersonalityType () != fpt_Observer)
                    {
                      if (world.getFaction (factionIndex)->
                          isAlly (world.getThisFaction ()) == true
                          && factionLostGame (world.
              ...
              for (int factionIndex = 0;
                   factionIndex < world.getFactionCount (); ++factionIndex)
                {
                  ...
                  if (world.getFaction (factionIndex)->
                    ...
                    {
                      if (world.getFaction (factionIndex)->
                          isAlly (world.getThisFaction ()) == false
                          && factionLostGame (world.
                                              getFaction (factionIndex)) ==
                          false)
                        {
              ...
              if (factionIndex != world.getThisFactionIndex ())
                {
                  if (world.getFaction (factionIndex)->
                      getPersonalityType () != fpt_Observer)
                    {
                      ...
                      if (factionLostGame (world.getFaction (factionIndex)) ==
                          false
                          && world.getFaction (factionIndex)->isAlly (world.
                                                                      getThisFaction
                                                                      ()) ==
                          false)
                        {
          ...
          if (wasGameOverAlready == false)
            {
              ...
              if (this->masterserverMode == true ||
                ...
                {
                  scriptManager.onGameOver (scriptManager.
                                            getPlayerModifiers (world.
                                                                getThisFactionIndex
                                                                ())->
                                            getWinner ());

                  if (scriptManager.
                      getPlayerModifiers (world.getThisFactionIndex ())->
                      getWinner ())
                    {
                      showWinMessageBox ();
                    }
    ...
    bool
    Game::isFlagType1BitEnabled (FlagTypes1 type) const
    {
      return ((gameSettings.getFlagTypes1 () & (uint32) type) ==
              (uint32) type);
    }

    bool
    Game::factionLostGame (int factionIndex)
    {
      return factionLostGame (world.getFaction (factionIndex));
    }

    bool
    Game::factionLostGame (const Faction * faction)
    {
      if (faction != NULL)
        {
          for (int factionIndex = 0; factionIndex < faction->getUnitCount ();
               ++factionIndex)
            {
              const UnitType *
                ut = faction->getUnit (factionIndex)->getType ();
              if (ut->getCountInVictoryConditions () == ucvcNotSet)
                {
                  if (faction->getUnit (factionIndex)->getType ()->
                      hasSkillClass (scBeBuilt))
                    {
                      return false;
                    }
                }
...
//}
      ...
      if (this->speed < Config::getInstance ().getInt ("FastSpeedLoops"))
        {
          ...
          console.addLine (lang.getString ("GameSpeedSet") + " " +
                           ((this->speed ==
                             0) ? lang.getString ("Slow") : (this->speed ==
                                                             1) ? lang.
                            getString ("Normal") : "x" +
                            intToStr (this->speed)));
        }
    ...
    void
    Game::setPaused (bool value, bool forceAllowPauseStateChange,
                     bool clearCaches, bool joinNetworkGame)
    {
      bool
        speedChangesAllowed =
        !NetworkManager::getInstance ().isNetworkGame ();
      ...
      if (SystemFlags::getSystemSettingType (SystemFlags::debugWorldSynch).
          enabled)
        SystemFlags::OutputDebug (SystemFlags::debugWorldSynch,
                                  "game.cpp line: %d setPaused value: %d clearCaches: %d forceAllowPauseStateChange: %d speedChangesAllowed: %d pausedForJoinGame: %d joinNetworkGame: %d\n",
                                  __LINE__, value, clearCaches,
                                  forceAllowPauseStateChange,
                                  speedChangesAllowed, pausedForJoinGame,
                                  joinNetworkGame);
      ...
      if (forceAllowPauseStateChange == true || speedChangesAllowed == true)
        {
          ...
          if (pausedForJoinGame == true && joinNetworkGame == false &&
              networkManager.getNetworkRole () == nrServer)
            {
              ...
              bool
                haveClientConnectedButNoReady = false;
              for (int i = 0; i < world.getFactionCount (); ++i)
                {
                  Faction *
                    faction = world.getFaction (i);
                  ...
                  MutexSafeWrapper
                  safeMutex (server->
                             getSlotMutex (faction->getStartLocationIndex ()),
                             CODE_AT_LINE);
                  ConnectionSlot *
                    slot =
                    server->getSlot (faction->getStartLocationIndex (),
                      ...
                      for (unsigned int i = 0; i < languageList.size (); ++i)
                        {
                          ...
                          char
                            szMsg[8096] = "";
                          if (lang.
                              hasString ("JoinPlayerToCurrentGameLaunch",
                                         languageList[i]) == true)
                            {
                              snprintf (szMsg, 8096,
                                        lang.
                                        getString
                                        ("JoinPlayerToCurrentGameLaunch",
                                         languageList[i]).c_str (),
                                        slot->getName ().c_str ());
                            }
                          else
                            {
                              snprintf (szMsg, 8096,
                                        "Player: %s is about to join the game, please wait...",
                                        slot->getName ().c_str ());
                            }
                          ...
                          server->sendTextMessage (szMsg, -1, localEcho,
                                                   languageList[i]);
                          sleep (1);
          ...
          if (value == false)
            {
              ...
              bool
                wasPausedForJoinGame = pausedForJoinGame;
              paused = false;
              pausedForJoinGame = false;
              pausedBeforeJoinGame = false;
              ...
              if (clearCaches == true)
                {
                  if (SystemFlags::
                      ...
                      enabled)
                    SystemFlags::OutputDebug (SystemFlags::debugWorldSynch,
                                              "game.cpp line: %d Clear Caches for resume in progress game\n",
                                              __LINE__);
      ...
      if (speedChangesAllowed)
        {
          if (popupMenu.getVisible () == true
              || popupMenuSwitchTeams.getVisible () == true)
            {
              return true;
            }
...
//              }
    ...
    void
    Game::showLoseMessageBox ()
    {
      Lang & lang = Lang::getInstance ();

      NetworkManager & networkManager = NetworkManager::getInstance ();
      if (networkManager.isNetworkGame () == true
          && networkManager.getNetworkRole () == nrServer)
        {
          showMessageBox (lang.getString ("YouLose") + " " +
                          lang.getString ("ExitBattleServerQuestion"),
                          lang.getString ("BattleOver"), false);
        }
      else
        {
          showMessageBox (lang.getString ("YouLose") + " " +
                          lang.getString ("ExitBattleQuestion"),
                          lang.getString ("BattleOver"), false);
        }
    }

    void
    Game::showWinMessageBox ()
    {
      Lang & lang = Lang::getInstance ();
      ...
      if (this->masterserverMode == true
          || world.getThisFaction ()->getPersonalityType () == fpt_Observer)
        {
          showMessageBox (lang.getString ("GameOver") + " " +
                          lang.getString ("ExitBattleQuestion"),
                          lang.getString ("BattleOver"), false);
        }
      else
        {
          showMessageBox (lang.getString ("YouWin") + " " +
                          lang.getString ("ExitBattleQuestion"),
                          lang.getString ("BattleOver"), false);
        }
    }
    ...
    void
    Game::showMessageBox (const string & text, const string & header,
                          bool toggle)
    {
      if (toggle == false)
        {
    ...
    void
    Game::showErrorMessageBox (const string & text, const string & header,
                               bool toggle)
    {
      if (toggle == false)
        {
    ...
    void
    Game::endPerformanceTimer ()
    {
      captureAvgTestStatus = false;
    }

    Vec2i
    Game::getPerformanceTimerResults ()
    {
      Vec2i
      results (this->updateFpsAvgTest, this->renderFpsAvgTest);
      return results;
    }

    void
    Game::consoleAddLine (string line)
    {
      console.addLine (line);
    }
    void
    Game::toggleTeamColorMarker ()
    {
                                         ...
                                         int worldFrameCount)
    {
      ...
      if (config.getBool ("SaveCommandsForReplay", "false") == true)
        {
          replayCommandList.
            push_back (make_pair (worldFrameCount, *networkCommand));
        }
    }

    void
    Game::renderVideoPlayer ()
    {
      if (videoPlayer != NULL)
        {
    ...
    void
    Game::playStaticVideo (const string & playVideo)
    {
...
//              player.PlayVideo();
    ...
    void
    Game::playStreamingVideo (const string & playVideo)
    {
      if (videoPlayer == NULL)
        {
          if (GlobalStaticFlags::getIsNonGraphicalModeEnabled () == false
              &&::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer () ==
              true)
            {
              Context *
              ...
              PlatformContextGl *
                glCtx =
                static_cast < ContextGl * >(c)->getPlatformContextGlPtr ();
              SDL_Window *
              ...
              videoPlayer =
                new::Shared::Graphics::VideoPlayer (&Renderer::getInstance (),
                                                    playVideo, "", window, 0,
                                                    0, screen->w, screen->h,
                                                    screen->format->
                                                    BitsPerPixel, false,
          ...
          if (videoPlayer->isPlaying () == false)
            {
              ...
              if (GlobalStaticFlags::getIsNonGraphicalModeEnabled () == false
                  &&::Shared::Graphics::VideoPlayer::
                  hasBackEndVideoPlayer () == true)
                {
                  Context *
                    c =
                  ...
                  PlatformContextGl *
                    glCtx =
                    static_cast <
                    ContextGl * >(c)->getPlatformContextGlPtr ();
                  SDL_Window *
                    window = glCtx->getScreenWindow ();
                  SDL_Surface *
                    screen = glCtx->getScreenSurface ();
                  ...
                  string
                    vlcPluginsPath =
                    Config::getInstance ().
                    getString ("VideoPlayerPluginsPath", "");
    ...
    void
    Game::stopStreamingVideo (const string & playVideo)
    {
      if (videoPlayer != NULL)
        {
    ...
    bool
    Game::clientLagHandler (int slotIndex,
                            bool networkPauseGameForLaggedClients)
    {
      ...
      this->networkPauseGameForLaggedClientsRequested =
        networkPauseGameForLaggedClients;
      this->networkResumeGameForLaggedClientsRequested =
        !networkPauseGameForLaggedClients;
      return true;
    }
    ...
    {
      string
        file = this->saveGame (GameConstants::saveGameFilePattern);
      char
        szBuf[8096] = "";
      Lang & lang = Lang::getInstance ();
    ...
    string
    Game::saveGame (string name, const string & path)
    {
      Config & config = Config::getInstance ();
      ...
      if (config.getBool ("SaveCommandsForReplay", "false") == true)
        {
          ...
          XmlTree
          xmlTreeSaveGame (XML_RAPIDXML_ENGINE);

          xmlTreeSaveGame.init ("megaglest-saved-game");
          XmlNode *
            rootNodeReplay = xmlTreeSaveGame.getRootNode ();
          ...
          rootNodeReplay->addAttribute ("version", glestVersionString,
                                        mapTagReplacements);
          rootNodeReplay->addAttribute ("timestamp", szBuf,
                                        mapTagReplacements);

          XmlNode *
            gameNodeReplay = rootNodeReplay->addChild ("Game");
          gameSettings.saveGame (gameNodeReplay);

          gameNodeReplay->addAttribute ("LastWorldFrameCount",
                                        intToStr (world.getFrameCount ()),
                                        mapTagReplacements);
          ...
          for (unsigned int i = 0; i < replayCommandList.size (); ++i)
            {
              ...
              XmlNode *
                networkCommandNode = cmd.second.saveGame (gameNodeReplay);
              networkCommandNode->addAttribute ("worldFrameCount",
                                                intToStr (cmd.first),
                                                mapTagReplacements);
            }

          string
            replayFile = saveGameFile + ".replay";
          if (SystemFlags::VERBOSE_MODE_ENABLED)
      ...
      XmlNode *
        gameNode = rootNode->addChild ("Game");
      //World world;
      ...
      //misc
      //Checksum checksum;
      gameNode->addAttribute ("checksum", intToStr (checksum.getSum ()),
                              mapTagReplacements);
      //string loadingText;
//    int mouse2d;
      gameNode->addAttribute ("mouse2d", intToStr (mouse2d),
                              mapTagReplacements);
//    int mouseX;
      gameNode->addAttribute ("mouseX", intToStr (mouseX),
                              mapTagReplacements);
//    int mouseY; //coords win32Api
      gameNode->addAttribute ("mouseY", intToStr (mouseY),
                              mapTagReplacements);
//    Vec2i mouseCellPos;
      gameNode->addAttribute ("mouseCellPos", mouseCellPos.getString (),
                              mapTagReplacements);
//      int updateFps, lastUpdateFps, avgUpdateFps;
//      int totalRenderFps, renderFps, lastRenderFps, avgRenderFps,currentAvgRenderFpsTotal;
      //Uint64 tickCount;
      gameNode->addAttribute ("tickCount", intToStr (tickCount),
                              mapTagReplacements);

      //bool paused;
      gameNode->addAttribute ("paused", intToStr (paused),
                              mapTagReplacements);
      //bool gameOver;
      gameNode->addAttribute ("gameOver", intToStr (gameOver),
                              mapTagReplacements);
      //bool renderNetworkStatus;
      //bool showFullConsole;
      //bool mouseMoved;
      //float scrollSpeed;
      gameNode->addAttribute ("scrollSpeed", floatToStr (scrollSpeed, 6),
                              mapTagReplacements);
      ...
      //Vec2i lastMousePos;
      gameNode->addAttribute ("lastMousePos", lastMousePos.getString (),
                              mapTagReplacements);
      //time_t lastRenderLog2d;
      gameNode->addAttribute ("lastRenderLog2d", intToStr (lastRenderLog2d),
                              mapTagReplacements);
      //DisplayMessageFunction originalDisplayMsgCallback;
      //bool isFirstRender;
      gameNode->addAttribute ("isFirstRender", intToStr (isFirstRender),
                              mapTagReplacements);

      //bool quitTriggeredIndicator;
      //int original_updateFps;
      gameNode->addAttribute ("original_updateFps",
                              intToStr (original_updateFps),
                              mapTagReplacements);
      //int original_cameraFps;
      gameNode->addAttribute ("original_cameraFps",
                              intToStr (original_cameraFps),
                              mapTagReplacements);

      //bool captureAvgTestStatus;
      gameNode->addAttribute ("captureAvgTestStatus",
                              intToStr (captureAvgTestStatus),
                              mapTagReplacements);
      //int updateFpsAvgTest;
      gameNode->addAttribute ("updateFpsAvgTest", intToStr (updateFpsAvgTest),
                              mapTagReplacements);
      //int renderFpsAvgTest;
      gameNode->addAttribute ("renderFpsAvgTest", intToStr (renderFpsAvgTest),
                              mapTagReplacements);

      //int renderExtraTeamColor;
      gameNode->addAttribute ("renderExtraTeamColor",
                              intToStr (renderExtraTeamColor),
                              mapTagReplacements);
      ...
      gameNode->addAttribute ("photoModeEnabled", intToStr (photoModeEnabled),
                              mapTagReplacements);
      //bool visibleHUD;
      gameNode->addAttribute ("visibleHUD", intToStr (visibleHUD),
                              mapTagReplacements);

      //bool timeDisplay
      gameNode->addAttribute ("timeDisplay", intToStr (timeDisplay),
                              mapTagReplacements);

      //bool withRainEffect;
      gameNode->addAttribute ("withRainEffect", intToStr (withRainEffect),
                              mapTagReplacements);
      //Program *program;

      //bool gameStarted;
      gameNode->addAttribute ("gameStarted", intToStr (gameStarted),
                              mapTagReplacements);

      //time_t lastMaxUnitCalcTime;
      gameNode->addAttribute ("lastMaxUnitCalcTime",
                              intToStr (lastMaxUnitCalcTime),
                              mapTagReplacements);
      ...
      gameNode->addAttribute ("lastNetworkPlayerConnectionCheck",
                              intToStr (lastNetworkPlayerConnectionCheck),
                              mapTagReplacements);

      //time_t lastMasterServerGameStatsDump;
      gameNode->addAttribute ("lastMasterServerGameStatsDump",
                              intToStr (lastMasterServerGameStatsDump),
                              mapTagReplacements);

      XmlNode *
        unitHighlightListNode = gameNode->addChild ("unitHighlightList");
      //for(unsigned int i = 0; i < unitHighlightList.size(); ++i) {
      for (std::map < int, HighlightSpecialUnitInfo >::iterator iterMap =
           unitHighlightList.begin (); iterMap != unitHighlightList.end ();
           ++iterMap)
        {
          HighlightSpecialUnitInfo & info = iterMap->second;
          XmlNode *
            infoNode = unitHighlightListNode->addChild ("info");
          infoNode->addAttribute ("unitid", intToStr (iterMap->first),
                                  mapTagReplacements);
          infoNode->addAttribute ("radius", floatToStr (info.radius, 6),
                                  mapTagReplacements);
          infoNode->addAttribute ("thickness", floatToStr (info.thickness, 6),
                                  mapTagReplacements);
          infoNode->addAttribute ("color", info.color.getString (),
                                  mapTagReplacements);
        }

      gameNode->addAttribute ("disableSpeedChange",
                              intToStr (disableSpeedChange),
                              mapTagReplacements);
    ...
    void
    Game::loadGame (string name, Program * programPtr,
                    bool isMasterserverMode,
                    const GameSettings * joinGameSettings)
    {
      Config & config = Config::getInstance ();
      // This condition will re-play all the commands from a replay file
      // INSTEAD of saving from a saved game.
      if (joinGameSettings == NULL
          && config.getBool ("SaveCommandsForReplay", "false") == true)
        {
          XmlTree
          xmlTreeReplay (XML_RAPIDXML_ENGINE);
          std::map < string, string > mapExtraTagReplacementValues;
          xmlTreeReplay.load (name + ".replay",
                              Properties::
                              getTagReplacementValues
                              (&mapExtraTagReplacementValues), true);

          const XmlNode *
            rootNode = xmlTreeReplay.getRootNode ();
          ...
          Lang & lang = Lang::getInstance ();
          string
            gameVer = versionNode->getAttribute ("version")->getValue ();
          ...
          XmlNode *
            gameNode = rootNode->getChild ("Game");

          GameSettings
            newGameSettingsReplay;
          newGameSettingsReplay.loadGame (gameNode);
          //printf("Loading scenario [%s]\n",newGameSettingsReplay.getScenarioDir().c_str());
          if (newGameSettingsReplay.getScenarioDir () != ""
              && fileExists (newGameSettingsReplay.getScenarioDir ()) ==
              false)
            {
              newGameSettingsReplay.
                setScenarioDir (Scenario::
                                getScenarioPath (Config::getInstance ().
                                                 getPathListForType
                                                 (ptScenarios),
                                                 newGameSettingsReplay.
                                                 getScenario ()));
          ...
          Game *
            newGame =
            new Game (programPtr, &newGameSettingsReplay, isMasterserverMode);
          newGame->lastworldFrameCountForReplay =
            gameNode->getAttribute ("LastWorldFrameCount")->getIntValue ();
          ...
          for (unsigned int i = 0; i < networkCommandNodeList.size (); ++i)
            {
              XmlNode *
                node = networkCommandNodeList[i];
              int
                worldFrameCount =
                node->getAttribute ("worldFrameCount")->getIntValue ();
              NetworkCommand
                command;
              command.loadGame (node);
              newGame->commander.addToReplayCommandList (command,
                                                         worldFrameCount);
            }
      ...
      xmlTree.load (name,
                    Properties::
                    getTagReplacementValues (&mapExtraTagReplacementValues),
                    true);
      if (SystemFlags::VERBOSE_MODE_ENABLED)
      ...
      if (joinGameSettings != NULL)
        {
          ...
          XmlNode *
            worldNode = gameNode->getChild ("World");
          XmlNode *
            guiNode = gameNode->getChild ("Gui");
          XmlNode *
            selectionNode = guiNode->getChild ("Selection");
          XmlNode *
            statsNode = worldNode->getChild ("Stats");
          XmlNode *
            minimapNode = worldNode->getChild ("Minimap");
      ...
      else
        {
          ...
          if (newGameSettings.getScenarioDir () != ""
              && fileExists (newGameSettings.getScenarioDir ()) == false)
            {
              newGameSettings.
                setScenarioDir (Scenario::
                                getScenarioPath (Config::getInstance ().
      ...
      Game *
        newGame = new Game (programPtr, &newGameSettings, isMasterserverMode);
...
//      int totalRenderFps, renderFps, lastRenderFps, avgRenderFps,currentAvgRenderFpsTotal;
      //Uint64 tickCount;
      newGame->tickCount =
        gameNode->getAttribute ("tickCount")->getIntValue ();

      //bool paused;
      if (newGame->inJoinGameLoading == true)
        {
          newGame->paused =
            gameNode->getAttribute ("paused")->getIntValue () != 0;
        }
      else
        {
      ...
      if (newGame->paused)
        newGame->console.addLine (lang.getString ("GamePaused"));
      //bool gameOver;
      newGame->gameOver =
        gameNode->getAttribute ("gameOver")->getIntValue () != 0;
      //bool renderNetworkStatus;
      //bool showFullConsole;
      //bool mouseMoved;
      //float scrollSpeed;
...
//      gameNode->addAttribute("renderFpsAvgTest",intToStr(renderFpsAvgTest), mapTagReplacements);
      ...
      //int renderExtraTeamColor;
      newGame->renderExtraTeamColor =
        gameNode->getAttribute ("renderExtraTeamColor")->getIntValue ();
      ...
      newGame->photoModeEnabled =
        gameNode->getAttribute ("photoModeEnabled")->getIntValue () != 0;
      //bool visibleHUD;
      //gameNode->addAttribute("visibleHUD",intToStr(visibleHUD), mapTagReplacements);
      newGame->visibleHUD =
        gameNode->getAttribute ("visibleHUD")->getIntValue () != 0;
      newGame->timeDisplay =
        gameNode->getAttribute ("timeDisplay")->getIntValue () != 0;
      //bool withRainEffect;
      //gameNode->addAttribute("withRainEffect",intToStr(withRainEffect), mapTagReplacements);
      newGame->withRainEffect =
        gameNode->getAttribute ("withRainEffect")->getIntValue () != 0;
      //Program *program;
      ...
      if (joinGameSettings == NULL)
        {
          if (gameNode->hasChild ("unitHighlightList") == true)
            {
              XmlNode *
                unitHighlightListNode =
                gameNode->getChild ("unitHighlightList");
              vector < XmlNode * >infoNodeList =
                unitHighlightListNode->getChildList ("info");
              for (unsigned int i = 0; i < infoNodeList.size (); ++i)
                {
                  XmlNode *
                    infoNode = infoNodeList[i];

                  int
                    unitId =
                    infoNode->getAttribute ("radius")->getIntValue ();
                  HighlightSpecialUnitInfo
                    info;
                  info.radius =
                    infoNode->getAttribute ("radius")->getFloatValue ();
                  info.thickness =
                    infoNode->getAttribute ("thickness")->getFloatValue ();
                  info.color =
                    Vec4f::strToVec4 (infoNode->getAttribute ("color")->
                                      getValue ());
      ...
      if (gameNode->hasAttribute ("disableSpeedChange") == true)
        {
          newGame->disableSpeedChange =
            gameNode->getAttribute ("disableSpeedChange")->getIntValue () !=
            0;
        }

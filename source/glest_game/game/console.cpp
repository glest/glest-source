...
namespace Glest {
	namespace Game {
		...
		Console::Console() {
			...
			CoreData::getInstance().registerFontChangedCallback(this->
				getFontCallbackName
				(), this);
			...
			yPos = 20;
			lineHeight =
				Config::getInstance().getInt("FontConsoleBaseSize", "18") + 2;
		...
		Console::~Console() {
			CoreData::getInstance().unRegisterFontChangedCallback(this->
				getFontCallbackName
				());
		}

...
namespace
	...
	namespace
		...
		void
			ChatManager::init(Console * console, int thisTeamIndex,
				const bool inMenu, string manualPlayerNameOverride) {
			this->console = console;
			this->
				thisTeamIndex = thisTeamIndex;
			this->
				disableTeamMode = false;
			this->
				inMenu = inMenu;
			this->
				manualPlayerNameOverride = manualPlayerNameOverride;
		}

		void
			ChatManager::setDisableTeamMode(bool value) {
		...
		void
			ChatManager::keyUp(SDL_KeyboardEvent key) {
		...
		bool
			ChatManager::textInput(std::string inputText) {
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
					...
					if (disableTeamMode == true) {
						if (!inMenu) {
							console->addLine(lang.
								getString
								("ChatModeDisabledToAvoidCheating"));
						}
					} else {
						SystemFlags::OutputDebug(SystemFlags::debugSystem,
							"In [%s::%s Line: %d] key = [%c] [%d]\n",
						...
						if (editEnabled == true) {
							...
							if (text.empty() == false) {
								...
								if (customCB == NULL) {
									...
									int
										playerIndex =
										gameNetworkInterface->getHumanPlayerIndex();
					...
					if (text.empty() == false) {
						...
						if (autoCompleteResult == "") {
							...
							if (matchedIndexes.empty() == false) {
								...
								if (newMatchedIndex >= 0) {
									autoCompleteResult =
										autoCompleteTextList[newMatchedIndex].
										substr(autoCompleteName.length());
								}
							}
						}
						...
						if (autoCompleteResult != "") {
							if (replaceCurrentAutoCompleteName >= 0) {
								deleteText((int) currentAutoCompleteName.length(),
									false);

								autoCompleteResult =
									autoCompleteName + autoCompleteResult;
		...
		void
			ChatManager::keyPress(SDL_KeyboardEvent c) {
			SystemFlags::OutputDebug(SystemFlags::debugSystem,
				"In [%s::%s Line: %d] key = [%c] [%d]\n",
				__FILE__, __FUNCTION__, __LINE__,
		...
		void
			ChatManager::switchOnEdit(CustomInputCallbackInterface * customCB,
		...
		void
			ChatManager::deleteText(int deleteCount, bool addToAutoCompleteBuffer) {
			if (text.empty() == false && deleteCount >= 0) {
				for (unsigned int i = 0; i < (unsigned int) deleteCount; ++i) {
					if (textCharLength.empty() == false) {
						...
						if (textCharLength[textCharLength.size() - 1] >
							(int) text.length()) {
							textCharLength[(int) textCharLength.size() - 1] =
								(int) text.length();
						}
						for (unsigned int i = 0;
							i <
							(unsigned int) textCharLength[textCharLength.size() -
							1]; ++i) {
		...
		void
			ChatManager::appendText(string addText, bool validateChars,
		...
		void
			ChatManager::updateAutoCompleteBuffer() {
			if (text.empty() == false) {
				int
					startPos = -1;
				for (int i = (int) text.size() - 1; i >= 0; --i) {

...
namespace Glest {
	namespace Game {
		...
		MenuStateScenario::MenuStateScenario(Program * program,
			MainMenu * mainMenu,
			bool isTutorialMode,
			const vector < string > &dirList,
			string
			autoloadScenarioName) :MenuState
			(program, mainMenu, "scenario") {
			...
			labelScenarioName.setFont3D(CoreData::getInstance().
				getMenuFontBig3D());
			...
			try {
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf
					("In [%s::%s Line: %d] listBoxScenario.getSelectedItemIndex() = %d scenarioFiles.size() = %d\n",
						extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
						__LINE__, listBoxScenario.getSelectedItemIndex(),
						(int) scenarioFiles.size());

				if (listBoxScenario.getItemCount() > 0
					&& listBoxScenario.getSelectedItemIndex() >= 0
					&& listBoxScenario.getSelectedItemIndex() <
					(int) scenarioFiles.size()) {
					string scenarioPath =
						Scenario::getScenarioPath(dirList,
							scenarioFiles[listBoxScenario.
			...
			if (scenarioErrors.empty() == false) {
				...
				for (std::map < string, string >::iterator iterMap =
					scenarioErrors.begin(); iterMap != scenarioErrors.end();
					++iterMap) {
					errorMsg +=
						"scenario: " + iterMap->first + " error text: " +
						iterMap->second.substr(0, 400) + "\n";
				}
		...
		void MenuStateScenario::reloadUI() {
			...
			labelScenarioName.setFont(CoreData::getInstance().
				getMenuFontNormal());
			labelScenarioName.setFont3D(CoreData::getInstance().
				getMenuFontNormal3D());
		...
		void MenuStateScenario::cleanupPreviewTexture() {
			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("In [%s::%s Line: %d] scenarioLogoTexture [%p]\n",
					extractFileFromDirectoryPath(__FILE__).c_str(),
					__FUNCTION__, __LINE__, scenarioLogoTexture);

			if (scenarioLogoTexture != NULL) {
				Renderer::getInstance().endTexture(rsGlobal, scenarioLogoTexture,
					false);
			}
			scenarioLogoTexture = NULL;
		}
		...
		void MenuStateScenario::mouseClick(int x, int y, MouseButton mouseButton) {
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (::Shared::Platform::Window::isKeyStateModPressed(KMOD_SHIFT) ==
					true) {
					const wchar_t lastKey = ::Shared::Platform::Window::
						extractLastKeyPressed();
					//                      xxx:
					//                      string hehe=lastKey;
		...
		void MenuStateScenario::update() {
			...
			if (this->autoloadScenarioName != "") {
				...
				("[%s:%s] Line: %d this->autoloadScenarioName [%s] scenarioPath [%s]\n",
					extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
					__LINE__, this->autoloadScenarioName.c_str(),
					scenarioPath.c_str());
				...
				//if(SystemFlags::VERBOSE_MODE_ENABLED) printf("[%s:%s] Line: %d this->autoloadScenarioName [%s] scenarioPath [%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,this->autoloadScenarioName.c_str(),scenarioPath.c_str());
				printf
				("[%s:%s] Line: %d this->autoloadScenarioName [%s] scenarioPath [%s] file [%s]\n",
					extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
					__LINE__, this->autoloadScenarioName.c_str(),
					scenarioPath.c_str(), scenarioInfo.file.c_str());
				...
				if (listBoxScenario.getSelectedItem() != this->autoloadScenarioName) {
					...
					showMessageBox("Could not find scenario name: " +
						this->autoloadScenarioName, "Scenario Missing",
						false);
					this->autoloadScenarioName = "";
				} else {
					try {
						...
						if (listBoxScenario.getItemCount() > 0
							...
							printf
							("[%s:%s] Line: %d scenarioFiles[listBoxScenario.getSelectedItemIndex()] [%s]\n",
								extractFileFromDirectoryPath(__FILE__).c_str(),
							...
							printf("[%s:%s] Line: %d scenarioInfo.file [%s]\n",
								extractFileFromDirectoryPath(__FILE__).c_str(),
								__FUNCTION__, __LINE__, scenarioInfo.file.c_str());

							labelInfo.setText(scenarioInfo.desc);
							if (scenarioInfo.namei18n != "") {
								labelScenarioName.setText(scenarioInfo.namei18n);
							} else {
								labelScenarioName.setText(listBoxScenario.
									getSelectedItem());
							}
		...
		void MenuStateScenario::launchGame() {
			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("In [%s::%s Line: %d] scenarioInfo.file [%s] [%s][%s][%s]\n",
					extractFileFromDirectoryPath(__FILE__).c_str(),
					__FUNCTION__, __LINE__, scenarioInfo.file.c_str(),
					scenarioInfo.tilesetName.c_str(),
			...
			if (scenarioInfo.file != "" && scenarioInfo.tilesetName != ""
				...
				if (SystemFlags::VERBOSE_MODE_ENABLED)
					printf("In [%s::%s Line: %d] gameSettings.getScenarioDir() [%s]\n",
						extractFileFromDirectoryPath(__FILE__).c_str(),
						__FUNCTION__, __LINE__,
						gameSettings.getScenarioDir().c_str());
				...
				if (listBoxScenario.getItemCount() > 0
					...
					if (SystemFlags::VERBOSE_MODE_ENABLED)
						printf("In [%s::%s Line: %d] scenarioLogo [%s]\n",
							extractFileFromDirectoryPath(__FILE__).c_str(),
							__FUNCTION__, __LINE__, scenarioLogo.c_str());
		...
		void MenuStateScenario::loadGameSettings(const ScenarioInfo *
			scenarioInfo,
			GameSettings * gameSettings) {
			if (listBoxScenario.getSelectedItemIndex() < 0) {
				char szBuf[8096] = "";
			...
			Scenario::loadGameSettings(dirList, scenarioInfo, gameSettings,
				formatString(scenarioFiles
					[listBoxScenario.
					getSelectedItemIndex()]));
		}

		void MenuStateScenario::showMessageBox(const string & text,
			const string & header,
			bool toggle) {
			if (!toggle) {
				mainMessageBox.setEnabled(false);

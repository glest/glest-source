...
namespace
	...
	namespace
		...
		// =====================================================
		MenuStateOptions::MenuStateOptions(Program * program,
			MainMenu * mainMenu,
			...
			labelDisableScreenshotConsoleText("Options",
				"labelDisableScreenshotConsoleText"),
			checkBoxDisableScreenshotConsoleText("Options",
				"checkBoxDisableScreenshotConsoleText"),
			labelMouseMoveScrollsWorld("Options", "labelMouseMoveScrollsWorld"),
			checkBoxMouseMoveScrollsWorld("Options",
			...
			try {
				...
				//MiscSection
				buttonMiscSection.
					init(400, 700, tabButtonWidth, tabButtonHeight + 20);
				buttonMiscSection.
					setFont(CoreData::getInstance().getMenuFontVeryBig());
				buttonMiscSection.
					setFont3D(CoreData::getInstance().getMenuFontVeryBig3D());
				...
				labelLang.
					setText(lang.getString("Language"));

				listBoxLang.
					init(currentColumnStart, currentLine, 375);
				vector <
					string >
					langResults;
				...
				listBoxLang.
					setSelectedItem(defaultLang.second + "-" + defaultLang.first);
				...
				labelPlayerName.setText(config.
					getString("NetPlayerName",
						Socket::getHostName().c_str()));
				labelPlayerName.setFont(CoreData::getInstance().getMenuFontBig());
				labelPlayerName.setFont3D(CoreData::getInstance().
					getMenuFontBig3D());
				...
				labelFontSizeAdjustment.setText(lang.
					getString("FontSizeAdjustment"));
				...
				listBoxScreenShotType.setSelectedItem(config.
					getString
					("ScreenShotFileType", "jpg"));

				currentLine -= lineOffset;

				labelDisableScreenshotConsoleText.init(currentLabelStart,
					currentLine);
				labelDisableScreenshotConsoleText.setText(lang.
					getString
					("ScreenShotConsoleText"));

				checkBoxDisableScreenshotConsoleText.init(currentColumnStart,
					currentLine);
				checkBoxDisableScreenshotConsoleText.setValue(!config.
					getBool
				...
				labelMouseMoveScrollsWorld.setText(lang.
					getString("MouseScrollsWorld"));

				checkBoxMouseMoveScrollsWorld.init(currentColumnStart, currentLine);
				checkBoxMouseMoveScrollsWorld.setValue(config.
					getBool
				...
				listCameraMoveSpeed.
					...
					((int)
						(config.getFloat("CameraMoveSpeed", "15"))));
				currentLine -= lineOffset;
				...
				listBoxHealthBars.pushBackItem(lang.
					getString
					("HealthbarsFactionDefault"));
				...
				listBoxHealthBars.pushBackItem(lang.
					getString("HealthbarsIfNeeded"));
				listBoxHealthBars.pushBackItem(lang.
					getString("HealthbarsSelected"));
				listBoxHealthBars.pushBackItem(lang.
					getString
					("HealthbarsSelectedOrNeeded"));

				int
					hpMode = config.getInt("HealthBarMode", "4");
				int
					hpIndex = 0;
				switch (hpMode) {
					case hbvUndefined:
				...
				checkBoxChatStaysActive.setValue(config.
					getBool("ChatStaysActive",
						"false"));
				...
				labelTransifexUserLabel.setText(lang.
					getString("TransifexUserName"));
				...
				labelTransifexUser.setText(config.
				...
				labelTransifexPwd.setText(config.
				...
				labelTransifexI18N.setText(config.
					getString("TranslationGetURLLanguage",
						"en"));
				currentLine -= lineOffset;

				buttonGetNewLanguageFiles.init(currentLabelStart, currentLine, 250);
				buttonGetNewLanguageFiles.setText(lang.
					getString
					("TransifexGetLanguageFiles"));

				buttonDeleteNewLanguageFiles.init(currentLabelStart + 260,
					currentLine, 250);
				buttonDeleteNewLanguageFiles.setText(lang.
					getString
					("TransifexDeleteLanguageFiles"));
		...
		void
			MenuStateOptions::setupTransifexUI() {
			buttonGetNewLanguageFiles.setEnabled(checkBoxCustomTranslation.
				getValue());
			buttonDeleteNewLanguageFiles.setEnabled(checkBoxCustomTranslation.
				getValue());
			labelTransifexUserLabel.setEnabled(checkBoxCustomTranslation.
				getValue());
			labelTransifexUser.setEnabled(checkBoxCustomTranslation.getValue());
			labelTransifexPwdLabel.setEnabled(checkBoxCustomTranslation.
				getValue());
			labelTransifexPwd.setEnabled(checkBoxCustomTranslation.getValue());
			labelTransifexI18NLabel.setEnabled(checkBoxCustomTranslation.
				getValue());
			labelTransifexI18N.setEnabled(checkBoxCustomTranslation.getValue());
		}

		void
			MenuStateOptions::showMessageBox(const string & text,
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (currentFontSizeAdjustment != selectedFontSizeAdjustment) {
					...
					showMessageBox(lang.getString("RestartNeeded"),
						lang.getString("FontSizeAdjustmentChanged"),
						false);
					return;
				}
				saveConfig();
				...
				if (labelTransifexI18N.getText() != "") {
					Lang & lang = Lang::getInstance();
					string
						language = lang.getLanguageFile(labelTransifexI18N.getText());
					replaceAll(language, "(", "");
					replaceAll(language, ")", "");

					if (language != "") {
						bool
							foundFilesToDelete = false;

						Config & config = Config::getInstance();
						string
							data_path = config.getString("UserData_Root", "");
						if (data_path != "") {
						...
						if (data_path != "") {
							...
							if (scenarioPaths.size() > 1) {
								...
								for (unsigned int i = 0; i < scenarioList.size(); ++i) {
									...
									findAll(scenarioPath + scenario + "/*.lng", langResults,
										false, false);
									for (unsigned int j = 0; j < langResults.size(); ++j) {
										string
											testLanguage = langResults[j];

										string
											removeLngFile =
											scenarioPath + scenario + "/" + testLanguage;

										if (EndsWith(testLanguage, language + ".lng") == true) {

											for (unsigned int k = 0;
												k < languageFileMappings.size(); ++k) {
												string
													mapping = languageFileMappings[k];
							...
							if (tutorialPaths.size() > 1) {
								...
								for (unsigned int i = 0; i < tutorialList.size(); ++i) {
									string
										tutorial = tutorialList[i];

									vector < string > langResults;
									findAll(tutorialPath + tutorial + "/*.lng", langResults,
										false, false);
									for (unsigned int j = 0; j < langResults.size(); ++j) {
										string
											testLanguage = langResults[j];
										...
										string
											removeLngFile =
											tutorialPath + tutorial + "/" + testLanguage;
										if (EndsWith(testLanguage, language + ".lng") == true) {
							...
							string
								mainLngFile = data_path + "data/lang/" + language + ".lng";
							if (fileExists(mainLngFile) == true) {

								for (unsigned int k = 0; k < languageFileMappings.size();
									++k) {
									string
										mapping = languageFileMappings[k];
									replaceAll(mapping, "$language", language);
							...
							string
								hintLngFile =
								data_path + "data/lang/hint/hint_" + language + ".lng";
						...
						if (foundFilesToDelete == true) {
							mainMessageBoxState = 0;
							Lang & lang = Lang::getInstance();
							showMessageBox(lang.getString("TransifexDeleteSuccess"),
				...
				if (languageFiles.empty() == false) {
					...
					for (unsigned int i = 0; i < languageFiles.size(); ++i) {
						...
						if (langName == "") {
							...
							size_t
								posDetails = fileDataDetails.find(search_detail_key, 0);
							if (posDetails != fileDataDetails.npos) {
								posDetails =
									fileDataDetails.find("\"name\": \"",
										posDetails +
										search_detail_key.length());

								if (posDetails != fileDataDetails.npos) {

									size_t
										posDetailsEnd =
										fileDataDetails.find("\"", posDetails + 9);

									langName =
										fileDataDetails.substr(posDetails + 9,
											posDetailsEnd - (posDetails + 9));
									replaceAll(langName, ",", "");
									replaceAll(langName, "\\", "");
									replaceAll(langName, "/", "");
									replaceAll(langName, "?", "");
						...
						if (pos != fileData.npos) {
							...
							string
								outputFile = languageFileMappings[i];
							...
							string
								lngFile = getGameCustomCoreDataPath(data_path, outputFile);

							string
								lngPath = extractDirectoryPathFromFile(lngFile);
					...
					if (gotDownloads == true) {
						...
						showMessageBox(lang.getString("TransifexDownloadSuccess") +
							"\n" + langName, lang.getString("Notice"),
							false);
					}
				}
				return;
			} else if (labelPlayerName.mouseClick(x, y)
				&& (activeInputLabel != &labelPlayerName)) {
				setActiveInputLable(&labelPlayerName);
			} else if (labelTransifexUser.mouseClick(x, y)
				&& (activeInputLabel != &labelTransifexUser)) {
				setActiveInputLable(&labelTransifexUser);
			} else if (labelTransifexPwd.mouseClick(x, y)
				&& (activeInputLabel != &labelTransifexPwd)) {
				setActiveInputLable(&labelTransifexPwd);
			} else if (labelTransifexI18N.mouseClick(x, y)
				&& (activeInputLabel != &labelTransifexI18N)) {
		...
		bool
			MenuStateOptions::isInSpecialKeyCaptureEvent() {
			return (activeInputLabel != NULL);
		}

		void
			MenuStateOptions::keyDown(SDL_KeyboardEvent key) {
		...
		bool
			MenuStateOptions::textInput(std::string text) {
			if (activeInputLabel != NULL) {
				//printf("[%d]\n",c); fflush(stdout);
		...
		void
			MenuStateOptions::keyPress(SDL_KeyboardEvent c) {
			if (activeInputLabel != NULL) {
		...
		void
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (buttonGetNewLanguageFiles.getEnabled())
					renderer.renderButton(&buttonGetNewLanguageFiles);
				if (buttonDeleteNewLanguageFiles.getEnabled())
					renderer.renderButton(&buttonDeleteNewLanguageFiles);
				if (labelTransifexUserLabel.getEnabled())
					renderer.renderLabel(&labelTransifexUserLabel);
				if (labelTransifexPwdLabel.getEnabled())
					renderer.renderLabel(&labelTransifexPwdLabel);
				if (labelTransifexI18NLabel.getEnabled())
					renderer.renderLabel(&labelTransifexI18NLabel);
				if (labelTransifexUser.getEnabled())
					renderer.renderLabel(&labelTransifexUser);
				if (labelTransifexPwd.getEnabled())
					renderer.renderLabel(&labelTransifexPwd);
				if (labelTransifexI18N.getEnabled())
					renderer.renderLabel(&labelTransifexI18N);
		...
		void
			...
			config.setString("FontSizeAdjustment",
				listFontSizeAdjustment.getSelectedItem());
			config.setString("ScreenShotFileType",
				listBoxScreenShotType.getSelectedItem());

			config.setBool("DisableScreenshotConsoleText",
				!checkBoxDisableScreenshotConsoleText.getValue());
			config.setBool("MouseMoveScrollsWorld",
				checkBoxMouseMoveScrollsWorld.getValue());
			config.setString("CameraMoveSpeed",
				listCameraMoveSpeed.getSelectedItem());

			int
				hpIndex = listBoxHealthBars.getSelectedItemIndex();
			int
				hpMode = hbvUndefined;
			switch (hpIndex) {
				case 0:
		...
		void
			MenuStateOptions::setActiveInputLable(GraphicLabel * newLable) {

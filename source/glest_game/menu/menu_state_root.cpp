...
namespace Glest {
	namespace Game {
		...
		MenuStateRoot::MenuStateRoot(Program * program,
				...
				//exit message box, has to be the last thing to do in this function
					...
					if (ftpMessageBox.mouseClick(x, y, button)) {
						...
						if (button == 0) {
							...
							if (ftpClientThread != NULL)
								ftpClientThread->addTempFileToRequests(ftpFileName,
									ftpFileURL);
				...
				if (result.first == ftp_crt_SUCCESS) {
					...
					if (result == false) {
						...
						printf("#2 Rename: [%s] to [%s] result = %d errno = %d\n",
							binaryNameOld.c_str(), binaryName.c_str(), resultRename,
							errno);
					}
		...
		void MenuStateRoot::render() {
			...
			int logoMainH = h;
			logoMainX =
				Config::getInstance().getInt(string(containerName) + "_MainLogo_x",
					intToStr(logoMainX).c_str());
			logoMainY =
				Config::getInstance().getInt(string(containerName) + "_MainLogo_y",
					intToStr(logoMainY).c_str());
			logoMainW =
				Config::getInstance().getInt(string(containerName) + "_MainLogo_w",
		...
		void MenuStateRoot::simpleTask(BaseThread * callingThread,
			void *userdata) {
			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("In [%s::%s Line %d]\n", __FILE__, __FUNCTION__, __LINE__);
				...
				if (curlResult != CURLE_OK) {
					...
					if (SystemFlags::VERBOSE_MODE_ENABLED)
						printf("In [%s::%s Line %d] curlError [%s]..\n", __FILE__,
							__FUNCTION__, __LINE__, curlError.c_str());
		...
		void MenuStateRoot::keyDown(SDL_KeyboardEvent key) {
			...
			if (SystemFlags::VERBOSE_MODE_ENABLED)
				printf("In [%s::%s Line: %d] key = [%d - %c]\n", __FILE__,
					__FUNCTION__, __LINE__, key.keysym.sym, key.keysym.sym);
		...
		void MenuStateRoot::showMessageBox(const string & text,
		...
		void MenuStateRoot::showFTPMessageBox(const string & text,
			const string & header, bool toggle,
			bool okOnly) {
			if (toggle == false) {
				ftpMessageBox.setEnabled(false);

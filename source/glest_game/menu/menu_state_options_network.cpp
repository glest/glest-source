//
//      menu_state_options_network.cpp:
//
...
namespace
	...
	namespace
		...
		MenuStateOptionsNetwork::MenuStateOptionsNetwork(Program * program,
			...
			labelEnableFTPServerInternetTilesetXfer("Options_Network",
				"labelEnableFTPServerInternetTilesetXfer"),
			checkBoxEnableFTPServerInternetTilesetXfer("Options_Network",
				"checkBoxEnableFTPServerInternetTilesetXfer"),
			labelEnableFTPServerInternetTechtreeXfer("Options_Network",
				"labelEnableFTPServerInternetTechtreeXfer"),
			checkBoxEnableFTPServerInternetTechtreeXfer("Options_Network",
			...
			try {
				...
				buttonNetworkSettings.
					init(600, 700, tabButtonWidth, tabButtonHeight + 20);
				buttonNetworkSettings.
					setFont(CoreData::getInstance().getMenuFontVeryBig());
				buttonNetworkSettings.
					setFont3D(CoreData::getInstance().getMenuFontVeryBig3D());
				...
				labelPublishServerExternalPort.
					init(currentLabelStart, currentLine, 150);
				labelPublishServerExternalPort.
					setText(lang.getString("PublishServerExternalPort"));

				labelExternalPort.
					init(currentColumnStart, currentLine);
				string
					extPort = config.getString("PortExternal", "not set");
				...
				string
					currentPort =
					config.getString("PortServer",
						intToStr(GameConstants::serverPort).c_str());
				int
					portSelectionIndex = 0;
				for (int idx = 0; idx < (int) portList.size(); idx++) {
					if (portList[idx] != ""
						&& IsNumeric(portList[idx].c_str(), false)) {
				...
				int
					FTPPort =
					config.getInt("FTPServerPort",
						intToStr(ServerSocket::getFTPServerPort()).
						c_str());
				...
				labelFTPServerDataPortsLabel.setText(lang.
					getString
					("FTPServerDataPort"));
				...
				checkBoxEnableFTPServer.setValue(config.
					getBool("EnableFTPServer",
						"true"));
				currentLine -= lineOffset;
				// FTP Config - start
				...
				labelEnableFTPServerInternetTilesetXfer.init(currentLabelStart,
					currentLine);
				labelEnableFTPServerInternetTilesetXfer.setText(lang.
					getString
					("EnableFTPServerInternetTilesetXfer"));

				checkBoxEnableFTPServerInternetTilesetXfer.init(currentColumnStart,
					currentLine);
				checkBoxEnableFTPServerInternetTilesetXfer.setValue(config.
					getBool
					("EnableFTPServerInternetTilesetXfer",
						"true"));
				...
				labelEnableFTPServerInternetTechtreeXfer.init(currentLabelStart,
					currentLine);
				labelEnableFTPServerInternetTechtreeXfer.setText(lang.
					getString
					("EnableFTPServerInternetTechtreeXfer"));

				checkBoxEnableFTPServerInternetTechtreeXfer.init(currentColumnStart,
					currentLine);
				checkBoxEnableFTPServerInternetTechtreeXfer.setValue(config.
					getBool
					("EnableFTPServerInternetTechtreeXfer",
						"true"));
				...
				checkBoxEnablePrivacy.setValue(config.
					getBool("PrivacyPlease", "false"));
				//currentLine-=lineOffset;
				// end
		...
		void
			MenuStateOptionsNetwork::reloadUI() {
			...
			labelServerPortLabel.setText(lang.getString("ServerPort"));
			labelPublishServerExternalPort.setText(lang.
				getString
				("PublishServerExternalPort"));
			labelEnableFTP.setText(lang.getString("EnableFTP"));
			labelEnableFTPServer.setText(lang.getString("EnableFTPServer"));
			labelFTPServerPortLabel.setText(lang.getString("FTPServerPort"));
			labelFTPServerDataPortsLabel.setText(lang.
				getString("FTPServerDataPort"));
			labelEnableFTPServerInternetTilesetXfer.setText(lang.
				getString
				("EnableFTPServerInternetTilesetXfer"));
			labelEnableFTPServerInternetTechtreeXfer.setText(lang.
		...
		void
			MenuStateOptionsNetwork::mouseClick(int x, int y,
			...
			if (mainMessageBox.getEnabled()) {
				...
				if (listBoxServerPort.mouseClick(x, y)) {
					int
						selectedPort = strToInt(listBoxServerPort.getSelectedItem());
		...
		void
			MenuStateOptionsNetwork::mouseMove(int x, int y, const MouseState * ms) {
			if (mainMessageBox.getEnabled()) {
		...
		void
			MenuStateOptionsNetwork::keyPress(SDL_KeyboardEvent c) {
			//      if(activeInputLabel != NULL) {
			//          //printf("[%d]\n",c); fflush(stdout);
		...
		void
			MenuStateOptionsNetwork::render() {
			...
			if (mainMessageBox.getEnabled()) {
				...
				renderer.
					renderCheckBox(&checkBoxEnableFTPServerInternetTechtreeXfer);
		...
		void
			MenuStateOptionsNetwork::saveConfig() {
		...
		void
			MenuStateOptionsNetwork::setActiveInputLable(GraphicLabel * newLable) {
		}
